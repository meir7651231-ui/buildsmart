...
- **batch-7 (5 מסכים · עיצוב):** cfgRadius ב-`store_profile`(6)·`departments`(6)·`worker_attendance`(5)·`courier_profile`(5)·`ai_hub`(5) = 27 אתרים. `courier_portal_tab`+`persona_portal` הוחזרו (3 אתרים ב-helpers חסרי-context — נדחים). **16 מסכים מאומצים-radius.** נותרו ~80 אתרי-radiusCard ב-lib/screens (רובם helpers חסרי-context או מסכים קטנים) → lib/widgets → CfgText literals. **דפוס מזוקק:** מסכים עם helpers `Widget _x(args)` (בלי context) דורשים context-threading — נדחים ל-pass ייעודי. gate: analyze 0 · config_theme/a11y ירוקים.
