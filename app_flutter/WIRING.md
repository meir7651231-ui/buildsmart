...
- **batch-10 (קטלוג · כפתורי-פעולה):** 4 ליטרלים עם emoji-לגאסי → CfgText (`catalog.detail.dataHeader`·`catalog.action.{buildBom/addToProject/saveVersion}`) + 4 registry rows. **14 תוויות-קטלוג ניתנות-לעריכה.** emoji-לגאסי (אין emoji חדש → gate-64). נותרו: ליטרלי-emoji לא-const (📋/✨/🔌 — בכפתורים עם style ריצה) + interpolated (out-of-v1) + עוד מסכים · radius-tail. gate: gate_118/registry/zero_regression · analyze 0. **deferred round-2 (criticalViolations→publish-sheet warning) — ערך-גבוה, מועמד הבא.**
