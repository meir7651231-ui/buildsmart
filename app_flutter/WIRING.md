...
- **batch-6 (5 מסכים · עיצוב):** cfgRadius ב-`tasks`(8)·`profile`(9)·`smart_project`(6)·`projects`(6)·`courier_dashboard`(7) = 36 אתרים. **לקח:** 2 אתרים ב-helpers חסרי-context (`_kvTile`/`_logDay`) — analyze תפס, הושארו ב-radiusCard (יאומצו עם context-threading). +הוסר import מת. **11 מסכים מאומצים-radius.** נותרו ~110 אתרי-radiusCard ב-lib/screens (store_profile·departments·courier_portal·persona·ai_hub·...) → ואז lib/widgets → ואז CfgText literals. gate: analyze 0 · config_theme/a11y ירוקים.
