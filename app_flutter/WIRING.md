...
- **STILL OPEN (needs a product decision — NOT guessed):** HIGH — the order's recorded `site` is decoupled from the active project (two project-name systems: the store checkout-picker `cartProjectProvider` vs the projects engine `activeProjectProvider`, with different name sets). Reconciling them is a design call (which is canonical / merge the names) — deferred for decision.
Gate: central-verify green — analyze 0 · tests green · build · conformance · required-tests.
