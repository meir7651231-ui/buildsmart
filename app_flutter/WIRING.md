...
- **B (store/JSON hardening):** _clampScale is-guard (fontScale לא-מספרי לא מפיל doc) · monotonic version-id · empty-draft publish guard · _load pristine guard · no-op short-circuit. *(בקומיט נפרד.)*
- **C (perf/scale):** debounce + off-thread `_save`. *(בקומיט נפרד.)*
- **deferred (מתועד, לא סיכון חי):** criticalIds→s26 · reactive canUndo→שלב-undo-UI · hashCode-note · agent-1 #1 (published-survives-flag = product-model, end-users מרנדרים published בלי הדגל — תיקון תיעוד בלבד).
