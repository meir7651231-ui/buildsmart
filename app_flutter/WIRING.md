...
- **batch-9 (קטלוג · תוכן):** עוד 5 ליטרלים → CfgText (`catalog.card.productBadge`·`catalog.templates.label`·`catalog.detail.brandGuide`·`catalog.detail.recentlyViewed`·`catalog.search.clearAll`) + 5 registry rows. **10 תוויות-קטלוג ניתנות-לעריכה.** נותרו: ליטרלים עם emoji-prefix (🔧/📦/...) + interpolated (out-of-v1) + עוד מסכים · radius-tail (context-threading). gate: gate_118/registry/zero_regression · analyze 0.
