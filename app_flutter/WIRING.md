...
- **functions/src/reviewRoleRequest.ts**: `onUserCreatedQueueApproval` הוחזר ל-**NO-OP מתועד** — משתמשים נשארים ממתינים עד אישור-בעלים; אין יותר בקשות-קבלן-אנונימיות שהציפו את התיבה. תפקידים-תפעוליים מוגברים (עובד/שליח/חנות/מנהל) עדיין דרך submitRoleRequest → reviewRoleRequest.
