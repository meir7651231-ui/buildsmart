...
באג-ריצה חי: tap על כרטיס-ההגדרה לא פתח את הגיליון-הפנימי (owner: "עלה לי רק החיצוני לא הפנימי"). **שורש (מאומת בבייטים — 135/144 אריחים):** `prioritizedSchema.familyId` = קטגוריית-קטלוג, אך `_inFamily`/`familyOf` מתאימים רק משפחת-מנוע ⇒ `familyProducts` ריק ⇒ `variantForSelection`=null ⇒ ה-`_onOpenDetails` הישן פגע ב-`if(product==null)return` ⇒ אין גיליון. **תיקון:** (1) `catalog_config_screen.dart:159-176` — `_onOpenDetails` חסין: `variantForSelection ?? family.first ?? _product(schema.sku)` (פותח את מוצר-הטייל עצמו — נתיב ה-94%), לעולם לא no-op שקט; siblings לפייג'ר = המשפחה, אחרת בני-הקטגוריה (`showLipskeyProductSheet` מגן על רשימה-ריקה). (2) `config_card.dart` — כפתור **"📄 פרטים"** (`_detailsButton` :570 · `Key('configOpenDetails')`) מתחת לתמונה/גלגלים ומעל הוסף-לסל/בנה-קו — יעד-tap גדול חסין-גרירה; גם image-tap וגם הכפתור עוברים אותו handler. **🔑 golden** (`config_card_open_details_real_test`): הנתיב **האמיתי** (לא mock) על טייל-variant-null — image-tap + כפתור-פרטים פותחים `LipskeyProductSheet`; RED-לפני / GREEN-אחרי + mutation. 82 catalog_config + 32 רגרסיה ירוקים · analyze נקי.

## #catalog-config-goes-live — 🎛️ הקטלוג-המגדיר עלה לאוויר על הבית (owner "תדליק") — 2026-08-06
מקטע-הבית `catalogConfig` **הופסק להיות מגודר** — `childrenFor` מרנדר את `_CatalogConfigOpen` (⟵ `CatalogConfigScreen`) **ללא-תנאי**, כמו categories/products. `smart_home_screen.dart`: הוסר ה-gate `kCatalogConfig` מ-`childrenFor` + מ-`_CatalogConfigHero` (ה-hero מנווט ישירות ל-`CatalogConfigScreen`) → הוסר ה-import של הדגל (אין `kCatalogConfig` בקובץ). **⚠️ מוותר על byte-identical:** `features/catalog_config/` עכשיו **קומפל ומרונדר בכל build** (לא tree-shaken) — החלטת-בעלים מפורשת. **הכפתור "כמו כל השאר":** הסקשן הוא `HomeSection` מלא → מקבל **הצג/הסתר + הזזה** ב"סידור מסך הבית" חינם. הדגל `kCatalogConfig` נותר default-OFF רק על הראוט-העצמאי + gate_128 (הערתו עודכנה — כבר לא מבטיח byte-identical לכלל-הפיצ'ר). **🔑** כל בדיקות-הבית עוברות (הסקשן נבנה עצל, מחוץ-למסך ב-viewport הבדיקה); +33 ירוק.

## #home-catalog-config-section — 🎛️ מסך-הקטלוג-המגדיר על מסך-הבית (מגודר · off-live) — 2026-08-06
מקטע-בית חדש `HomeSection.catalogConfig` (🎛️ "קטלוג מגדיר") — **מירור מדויק של `superFinder`**: ה-`CatalogConfigScreen` המלא נרנדר **פתוח** בקופסת-560 בתוך ה-ListView של הבית. `home_content_order.dart`: enum + `kHomeSectionMeta`(🎛️) + `kDefaultHomeOrder`. `smart_home_screen.dart`: `_CatalogConfigOpen` (עותק של `_SuperFinderOpen`, child=`CatalogConfigScreen()`) + `_CatalogConfigHero` (טוקן reorder-preview) + case ב-`childrenFor` **מגודר `kCatalogConfig`** (`kCatalogConfig ? [...] : []` — בדיוק כמו `kAxisDive`). tap על תמונת-כרטיס בתוך המסך פותח את הגיליון-הפנימי (החיבור מ-`#catalog-config-internal-link`) — כך "השניים" חיים על הבית. **🔒 keystone:** `kCatalogConfig` OFF ⇒ `_CatalogConfigOpen` + כל `catalog_config/` tree-shaken ⇒ הבית **byte-identical** (אפס-ריפל — כמו superFinder; מאומת ע"י כל בדיקות-הבית העוברות ללא-שינוי). **🔑 golden** (`home_catalog_config_section_test`): catalogConfig ב-kDefaultHomeOrder + מטא-🎛️ · smartHomeSectionFor רנדר בלי-קריסה. ripple: `t3_ghi` (סדר-ברירת-מחדל). תצוגה חיה: `--dart-define=CATALOG_CONFIG=true`.

## #catalog-config-internal-link — 🔗 כרטיס-חיצוני (ConfigCard) → גיליון-פנימי (מגודר · off-live) — 2026-08-06
tap על תמונת ה-`ConfigCard` פותח את הגיליון-הפנימי (`LipskeyProductSheet`) לווריאנט הנבחר — הזוג חיצוני↔פנימי ("תחבר ביניהם"). `config_card.dart`: prop חדש `onOpenDetails` (`ConfigCardAction?` · כמו onAddToCart/onBuildLine) + `onTap` על ה-`_stage()` GestureDetector — **הגרירה ↕/↔ (החלפת-ווריאנט) נשארת; ה-tap תוספתי בלבד; `null`⇒inert**. `catalog_config_screen.dart`: `_onOpenDetails` → `variantForSelection(schema, selection, resolvedCatalogProducts)` → `showLipskeyProductSheet(context, product, familyProducts(...))`, מושחל דרך `_FamilySection`. **אפס פונקציה חדשה ב-`variant_image.dart`** — reuse של `familyProducts` הקיים (המפקח מנע כפילות `familyMembers`). **🔒 keystone:** הכל מתחת `kCatalogConfig` (OFF) ⇒ tree-shaken ⇒ האפליקציה החיה byte-identical; `lipskey_product_sheet.dart` כבר בבנייה-החיה ⇒ הייבוא לא מוסיף קוד-חי. **🔑 golden** (`config_card_open_details_test`): tap→onOpenDetails עם schema+selection+qty · **נושא את הווריאנט הנוכחי אחרי שינוי-גלגל+כמות** · null⇒inert בלי-קריסה. mutation-verified (שבירת ה-onTap → אדום). variant_image = אפס-דיף.

## #fittings-intel-pE3 — 📄 מנוע-אביזרים · E-lite slice 3: ייצוא-מסמך (build-plan export) — יכולת (מגודר) — 2026-08-04
`lib/features/fittings/intel/build_plan_export.dart` (טהור · מחרוזת בלבד · אפס I/O · מגודר · off-live): `exportBuildPlanText(plan, {spacing})` → מסמך-Markdown-lite עברי: כותרת · **רצף-אביזרים ממוספר** (מצרה נושאת od2 `50→32`) · **כתב-חיתוך** פר-מרווח (מרכיב `cutListUniform` · D5). `build_plan_export_screen.dart` (מגודר): `BuildPlanExportScreen` = טקסט **ניתן-לבחירה** (העתקה). שיתוף/שמירה-לקובץ = תלות-עתידית (בעלים) — כאן תצוגה+העתקה בלבד (אפס תלות חדשה). **🔒 keystone:** אף מסך לא מייבא `intel/` ⇒ tree-shaken ⇒ byte-identical. **🔑 golden** (`build_plan_export_test`): כותרת+רצף-ממוספר+כתב-חיתוך · מצרה `50→32` · רצף-ריק → סמני-ריק בלי-קריסה · spacing → כותרת · מסך=SelectableText. **E-lite הושלמה: נפילת-לחץ · תאימות(M5-clean) · ייצוא-מסמך.** fittings 346/346.

## #fittings-intel-pE2 — 🔗 מנוע-אביזרים · E-lite slice 2: תאימות-אביזרים (M5 · ביקורת-יריב) — יכולת (מגודר) — 2026-08-04
`lib/features/fittings/intel/compatibility.dart` (שאילתה טהורה · מגודר `kFittingEngineIntel` · off-live): `pairCompatible(a,b)` · `incompatibilityReason(a,b)` · `compatibleTargets(product, universe)` — **שאילתה מעל גרף-החיבורים המאומת החי** (`canConnect`/`connectionFailReason`, `install_engine`). `compatibility_screen.dart` (מגודר): `CompatibilityScreen` = ✓/✗ פר-מועמד + סיבת-אי-התאמה. **🛡️ M5 — ביקורת-יריב (reliability+safety, זו שתפסה את M2) הורצה על הפרוסה: CLEAN — אפס over-match** · האצלה-נאמנה ל-`canConnect` (הנתיב-המאומת `kVerifiedSpecs.compatibleWith`, M2-fix): אין נתיב-קבלה נוסף · אין היפוך · אין exception-שנבלע-כ"תואם" · זהה-SKU מסונן. הגרוע = false-negative (בטוח). **🔒 keystone:** `install_engine` כבר חי · אף מסך לא מייבא `intel/` ⇒ tree-shaken ⇒ byte-identical. **🔑 golden** (`compatibility_test`): האצלה מדויקת ל-`canConnect` · **זוג-לא-תואם (32↔50) לעולם לא compatible** · self לא-תואם · reason=null↔תואם · `compatibleTargets` רק-canConnect+ללא-עצמו. fittings 342/342. **הבא:** ייצוא-מסמך (E3).

## #fittings-intel-pE1 — 💧 מנוע-אביזרים · E-lite slice 1: נפילת-לחץ (pressure-drop) — יכולת (מגודר) — 2026-08-04
E-lite (יכולות-מנוע בנתיב, מגודר `kFittingEngineIntel` OFF, **אפס תלות חדשה**). `lib/features/fittings/intel/line_pressure.dart` (גשר טהור): `pressureDropForPlan(plan, {flow,length,rise})` = `estimatePressureDrop(plan.products, …)` — מריץ את המעריך-החי הקיים (`lib/logic/pressure_drop.dart`) על מוצרי-תוכנית-הקו (D1). `line_pressure_screen.dart` (מגודר): `LinePressureScreen` = ΔP · צוואר-בקבוק · הצעות-ייעוץ. **אומדן-הנדסי (הצעות=ייעוץ · לא ערך-בטיחות-מחייב) ⇒ לא נדרשת ביקורת-M5.** **🔒 keystone:** `pressure_drop.dart` כבר חי ⇒ ייבוא לא מוסיף קוד-חי · אף מסך לא מייבא `intel/` ⇒ tree-shaken ⇒ byte-identical. **🔑 golden** (`line_pressure_test`): הגשר מאציל ל-`estimatePressureDrop` (ΔP/K זהים) · פרמטרי-זרימה עוברים (זרימה↑⇒ΔP↑) · תוכנית-ריקה לא-קורסת · מסך בונה. fittings 336/336.
...
## #fittings-intel-pD2 — 🧩 מנוע-אביזרים · פאזה D slice 2: מסך "בנה-קו" (יכולת-מאחורי-כפתור · מגודר) — 2026-08-04
`lib/features/fittings/intel/line_builder_screen.dart` (מגודר `kFittingEngineIntel` · off-live) — **יעד-הכפתור** של יכולת-תכנון-החיבור: `LineBuilderScreen` מציג `FittingLinePlan` = **סיכום-רצף** (Chip פר-אביזר: אימוג'י-משפחה + קוטר + od2 למצרה) **+ תצוגת-3D** (`FittingPreview3d` על `plan.route`). `FittingLinePlan.fromRoute(route)` (factory לדמו/preview). מרכיב את D1 (planner) + C6 (3D) למסך-יכולת יחיד. **🔒 keystone:** אף route חי לא מייבא `intel/` ⇒ tree-shaken ⇒ byte-identical. widget-smoke (`line_builder_screen_test`): המסך בונה/מצייר · Chip פר-אביזר · תוכנית-מסופקת מזינה סיכום+3D · תוכנית-ריקה → "רצף ריק" בלי קריסה. **visual-verify נטיב אושר:** כותרת-עברית · צ'יפים · קו-3D מרותך (טי/ברז-פליז/מצרה/ברק). שילוב-סופי (מיקום-הכפתור) = בעלים. **הבא:** "הצג-בתלת-ממד" ככפתור-נפרד / capability הבאה. fittings 302/302.

## #fittings-intel-pD1 — 🧠 מנוע-אביזרים · פאזה D (אינטליגנציה) slice 1: מתכנן-קו — גשר גרף→רצף (מגודר) — 2026-08-04
פאזה D (יכולת-מאחורי-כפתור · מגודר `kFittingEngineIntel` default-OFF · off-live). `lib/features/fittings/intel/line_planner.dart` (טהור) — **הגשר** בין גרף-החיבורים החי (`findShortestPath`/`buildInstallation` → מוצרי-קטלוג) למנוע-האביזרים: `routeFromProducts(products)` ממפה כל מוצר ל-`RunElement` (`familyOf`/`odOf`/`od2Of` · פאזה 0), מדלג צינור/קבוע (M1); `planFittingLine(from,to,{tempC})` → `FittingLinePlan{products, route, complete}` דרך `findShortestPath` (מקורקע). הרצף מזין `assembleRoute` (3D) + `buildableSpecFor` (spec). **🔒 keystone:** `install_engine` כבר בבנייה-החיה ⇒ ייבוא לא מוסיף קוד-חי · אף מסך לא מייבא `intel/` (מאומת: NONE) ⇒ tree-shaken ⇒ הזרימה byte-identical. **🔑 golden** (`line_planner_test`): שרשרת-PPR→משפחות+קטרים · ברך כיוון-ברירת-מחדל · מצרה od2 · צינור מדולג · from==to complete. **הבא:** מסך-preview מגודר לכפתור "בנה-קו" (D2), ואז "הצג-בתלת-ממד". fittings 299/299.

## #fittings-engine-pC10 — ✨ מנוע-קטלוג-3D · renderer-polish 4: ברק ספקולרי פר-חומר (מגודר) — 2026-08-04
ליטוש 4 (דורמנטי · `kFittingEngine3d` OFF · off-live) — מוסיף **ברק-ספקולרי (highlight)** מעל ה-Lambert, כדי שפלסטיק יבריק ופליז/צינור ייראו מתכתיים. פורט-lite מ-**gen3d fragment shader** (`:250-254`): Blinn-Phong `spec = NdotH^sh · (sh+8)/25` (חדות `sh=mix(14,120,1-rough)`) × Fresnel-Schlick (`F0=mix(0.04,1,metal)`), פר-פאה. `render/mesh_projector.dart`: `RenderTriangle.specular` (0..1) + `_shadeParams(mat)` (PP-R metal0/rough.55 · צינור metal.5/rough.4 · פליז metal1/rough.25). `render/route_preview.dart`: הצייר מוסיף `highlight×specular` (דיאלקטרי→לבן · פליז→לבן-חמים) על `base×shade`, מוגבל ל-255. **🔑 golden** (`mesh_projector_test`): כל spec∈[0,1] · צינור-מואר מראה highlight · **פליז (מבריק) שיא חזק מ-PP-R (מחוספס)** · projectMeshes-ברירת-מחדל spec סופי-בתחום. **visual-verify נטיב אושר:** פסי-ברק לבנים על הצינורות (מראה-מתכת) · שיין עדין על ה-PP-R · ידית-פליז חמימה. **🔒 keystone:** אף route חי לא מייבא render/ ⇒ byte-identical. **🛑 צעד 43 = GO-בעלים.** fittings 293/293.
...
- `render/mesh_projector.dart`: `RenderTriangle` נושא כעת `mat` (PartMaterial) · `projectParts(parts, …)` (מקביל ל-`projectMeshes`, משמר-חומר, ממוין far→near) · לוגיקת-ההקרנה משותפת ב-`_emit`.
- `render/route_preview.dart`: `RoutePainter` צובע פר-חומר (`colorForMaterial`: PP-R ירוק · צינור אפור · פליז) + רקע-במה כהה · `FittingPreview3d` בונה `assembleRoute` וממסגר מ-bbox-החלקים.
**🔑 golden** (`route_assembly_test`): צינורות-מעבר בין אביזרים (lead+mid+trail) · ברך-90°↑ מרימה את המורד ל-+Y · פקק סוגר (בלי צינור-סיום) · מתאם נושא חלקי-פליז · מצרה-taper · well-formed. **visual-verify נטיב אושר:** רצף-צנרת רציף · טי עם הסתעפות · ידית-ברז פליז · מדרגת-קוטר. **🔒 keystone:** אף route חי לא מייבא render/geometry-3D ⇒ tree-shaken ⇒ הכרטיס-החי byte-identical. פונט-עברית בכותרת = ליטוש 3. **🛑 צעד 43 = GO-בעלים.** fittings 290/290.

## #fittings-engine-pC7 — 🧱 מנוע-קטלוג-3D · renderer-polish 1: גוף-אביזר אמיתי פר-משפחה (מגודר) — 2026-08-04
ליטוש ה-3D מ-preview ל-מוכן (דורמנטי · `kFittingEngine3d` OFF · off-live). `lib/features/fittings/geometry/element_meshes.dart` (טהור · מגודר) — **פורט 1:1 מ-`gen3d.elemMeshes` (`:280-319`) + `taperGeom` (`:174`), סקאלת-S מוסרת (הכל מ״מ).** מחליף את ה-envelope-tube הזמני בגוף-אמת פר-משפחה: **ברך = 2 זרועות + כדור-פינה** (arm2 מסובב לתוך הכיפוף) · **טי = רַץ + כדור + הסתעפות + stub** · **ברז = גוף + ידית-פליז** · **מתאם = שקע-PP-R + משושה-פליז + תבריג-פליז** · **מצרה = שקע + חרוט-מעבר** · פקק/רוכב/צווארון/מצמד. `ElementMeshes{parts[ElementPart{mesh, mat, branchAxis}], ex, ey, turn, term}` · `PartMaterial{ppr·pipe·brass}` · `taperGeom`/`straightPipe`/`kStubLen`/`kPipeLen` (לצנרת-הריתוך, פרוסה הבאה). טרנספורם-מקומי (rotateZ→translate) מוטבע ברשת. **🔑 golden** (`element_meshes_test`): ספירות-חלקים · חומרים (hex/thread/ידית=brass · stub=pipe) · דלתות `ex/ey/turn`≡turtle · arm2-הברך מגיע ל-+Y · טי-הסתעפות על ציר-B · well-formed לכל המשפחות. **פרוסה additive — שום consumer עדיין לא מייבא** ⇒ אפס-סיכון לנתיב-הקיים · הכרטיס-החי byte-identical. fittings 281/281.
## #courier-profile-card-material — 🛵 שורת-כרטיס אזור-אישי עוטפת Material (ink/lint) — 2026-08-04
`lib/screens/courier_profile_screen.dart` — תיקון כירורגי (ללא refactor): ה-`Column` של `_CourierPersonalAreaCard` (נוכחות/טפסים/תעודות נהג/תלושי שכר) נעטף ב-`Material(type: MaterialType.transparency)`. אותו דפוס כמו worker_profile: 4 ה-`ListTile` ישבו בתוך `Container(decoration:)` בלי `Material` ביניים → אדוות-ההקשה מוסתרת ע"י רקע-הכרטיס (debug assertion · release ink בלתי-נראה). כרטיס-הפעולות (הגדרות/החלפת-תפקיד/יציאה) הוא `Card` ⇒ כבר נותן Material, לא נגוע. **מקור:** `tools/atom/testgen` (List A, הפיילוט האחרון). מאומת (`--dart-define=atomgen=true`): 35 כשלי-crash → **30 pass-מאומת + 5 not-found-כן** (תוכן דיאלוג `_RoleSwitchCodeDialog`, bucket B). 0 No-Material/ink-hidden. אפס force-pass · main-suite ירוק (5601/12/0) · screenshot אומת · `git diff -w` = עטיפה אחת. **List A סגורה (0 ממצאי-crash גלובלית).**
...
- `route_preview.dart` (Flutter · **מגודר · off-live**): `RoutePainter` (CustomPainter דק — מילוי-משולשים בלבד) · `FittingPreview3d` (מסגור-אוטומטי + גרירה-לסיבוב + צביטה-לזום) · `FittingPreviewScreen` (מסך-בדיקה, רצף-דמו). **🔒 keystone:** אף route חי אינו מייבא render/ (מאומת: grep=NONE outside features/fittings) ⇒ `kFittingEngine3d` OFF ⇒ tree-shaken ⇒ הכרטיס-החי byte-identical. נגיש רק ב-`--dart-define=FITTING_ENGINE_3D=true`. widget-smoke (`route_preview_test`): המסך בונה/מצייר · רצף-ריק לא-קורס · shouldRepaint · גרירה-מסובבת. **🛑 צעד 43 (הטמעה על הכרטיס-החי) = GO-בעלים מפורש בלבד.** fittings 269/269 · dormant.

## #fittings-engine-pC5 — 🎥 מנוע-קטלוג-3D · פאזה C (renderer 2): מצלמה/היטל טהורים (מגודר · off-live) — 2026-08-04
`lib/features/fittings/render/camera.dart` (מגודר · טהור · בלי Flutter/canvas · הכרטיס-החי לא-נגוע) — פרוסת-renderer שנייה (option א, off-live · `kFittingEngine3d` default-OFF). **פורט 1:1 מ-gen3d** (`:140-142` persp/look · `:426` eye): `Mat4` **column-major** (בדיוק פריסת-gen3d) + `perspective(fov,aspect,near,far)` + `lookAt(eye,center,up)` + `orbitEye(target,dist,yaw,pitch)` + `projectToScreen(proj,view,world,W,H)→ScreenPoint{x,y,depth,w}` (NDC→פיקסלים · ציר-Y מתהפך · `w≤0`⇒culled). קבועי-ברירת-מחדל של gen3d verbatim (`kDefaultYaw=-0.6 · kDefaultPitch=0.26 · kDefaultFov=0.72 · kFrameDistRatio=2.7`). זו הליבה-הניתנת-לבדיקה של ה-render; שכבת-הציור (CustomPainter) + מסך-preview מגודר off-live = הפרוסה-הבאה (C6), צורכת `projectToScreen` בלבד. **🔑 golden** (`camera_test`): נוסחת-persp מדויקת · look-at ממפה origin→(0,0,−dist) · **מטרת-המצלמה מוקרנת למרכז-המסך** · +Y→מעל-המרכז · נקודה-רחוקה→עומק-גדול (painter far→near) · מאחורי-המצלמה→culled · `Mat4.mul` column-major ≡ proj·(view·v). אפס גזירת-פיזיקה · **step 43 עדיין ממתין ל-GO-בעלים.** fittings 256/256 · dormant/byte-identical.

## #fittings-engine-pC4 — 🧩 מנוע-קטלוג-3D · פאזה C (renderer 1): רשתות-מסלול world-space (מגודר · off-live) — 2026-08-04
`lib/features/fittings/geometry/route_meshes.dart` (מגודר · טהור · הכרטיס-החי לא-נגוע) — פרוסת-renderer ראשונה (option א, native, off-live · `kFittingEngine3d` default-OFF). `buildRoutePreviewMeshes(List<RunElement>)` → `List<Mesh>`: לכל אביזר בפריסה (`layoutRoute`) נבנה **צינור בגודל-ה-envelope** (`tube(0, axialLength, ro, ro·0.6)`) ומונח למרחב-עולם דרך `_placeInFrame` — **טרנספורם וקטורי טהור** (בלי מטריצות): `world = P + F·lx + U·ly + side·lz`, `side = norm(F×U)`; נורמלים מסובבים בלבד. אביזר לא-פריס (envelope==null) מדולג (M1 · fallback, לעולם לא מיקום-שגוי). זהו רצף-הצילינדרים המנותב — שכבת-ה-render (CustomPainter) = הפרוסה-הבאה. **🔑 golden** (`route_meshes_test`): מסגרת-זהות ⇒ הצינור-המונח מרוכז על ציר-X ומשתרע `[0, axialLength]` (אנקור למנוע-envelope) · ברך-90°-up **מרימה** אביזרים-במורד ל-+Y · פקק סוגר · לא-פריס מדולג · כל רשת watertight ו-normals unit. **step 43 (הטמעה על הכרטיס-החי) עדיין ממתין ל-GO-בעלים מפורש.** fittings 246/246 · הכל dormant/byte-identical.
...
## #screen-mgmt-s0 — כיבוי טריגר-עריכה-על-המסך (חובה · ראשון · חי) — 2026-07-27
**דירקטיבה "ניהול-מסכים באשף" (SSOT: `knowledge/DIRECTIVE-screen-management-in-wizard.md`) · פרוסה-0 (מאושר-לדחיפה-מיד · חי).** ל-chrome-העריכה-על-המסך (`studio_overlay.dart` — הטוגל **נווט⇄ערוך** + בורר-הבורדים **🔀**) אין מיקום-טריגר אוניברסלי טוב (מסכי ניהול-אתר בלי לוגו קבוע) ⇒ **הוקפא**: `StudioOverlay.build` מחזיר **תמיד `SizedBox.shrink()`** (mounted-inert · דפוס ConnectionIndicator). **אין כניסה-על-המסך לעריכה כרגע** — אין קורא אחר ל-`enterEdit` בכל הקוד ⇒ edit-mode **לא ניתן-להפעלה מהמסך**. **האשף (`org_setup_wizard`) = דרך-הכניסה היחידה לעריכה**; `STUDIO`/`kOrgConfigFlag` נשארים דלוקים. שער-בעלים **#84** (`studioCanEditProvider`/`enterEdit`) **לא-שונה**. **זהה-בייטים:** off-gate ממילא shrink, ו-`kStudioFlag` const-off ⇒ live tree-shake ⇒ לא-בעלים/פרודקשן לא-מושפעים. `main.dart` **לא-נגע** (עדיין mount `const StudioOverlay()`); re-enable עתידי = revert מ-git (ההיסטוריה שומרת טוגל/בורד/publish). בדיקות: `zero_regression` overlay (off-gate inert + **on-gate-owner עדיין-inert** — נווט/ערוך/בורד findsNothing · edit-mode לא-ניתן-להפעלה) · 192 studio · analyze 0.

## #wizard-studio-s5 — האשף = הסטודיו · פרוסה-5: גרסאות והיסטוריה — 2026-07-27 ✅ הדירקטיבה הושלמה
**דירקטיבה "האשף = הסטודיו" · פרוסה-5 (האחרונה · בלי push עד "תדחוף").** כפתור **"🕘 גרסאות"** (`Key('open-history')`, ב-Row עם מצא-והחלף) משגר מסלול-מלא `_WizardHistoryScreen` המארח את **`HistoryPane` של הסטודיו — verbatim** (רשימת `ConfigVersion` חדש-ראשון · הערה/מפרסם/זמן · "שחזר"=`rollback` קדימה-בלבד מאחורי דיאלוג-אישור · מצב-ריק ידידותי). כל "פרסם לכולם" (מ-✎/slice-2 או מצא-והחלף/slice-4) נרשם כאן — וניתן לשחזר. **אפס-שכפול:** לוגיקת-השחזור בפאנל היחיד (studio-tested); בדיקת-האשף מוכיחה חיווט + מצב-ריק. 19/19 · analyze 0.

> **✅ הדירקטיבה הושלמה — 5 פרוסות:** אקורדיון-Maor (s1) · ✎ מפקח-פר-רכיב חי (s2) · מונחים-שזורים (s3) · מצא-והחלף (s4) · גרסאות-והיסטוריה (s5). כל 5 gate-green, כולן על ה-Studio-store הקיים (מיחזור, לא בנייה-מחדש). **מקומיות — 5 קומיטים ממתינים ל-"תדחוף".** follow-ups מתועדים: align/direction (לא במודל CfgStyle) · וקבולר-מונחים עשיר-יותר (רק ~8 מחווטים ל-termOf).

## #wizard-studio-s4 — האשף = הסטודיו · פרוסה-4: מצא-והחלף (מיחזור verbatim) — 2026-07-27
**דירקטיבה "האשף = הסטודיו" · פרוסה-4 (בלי push עד "תדחוף").** כפתור **"🔎 מצא והחלף בטקסטים"** (`Key('open-find-replace')`, מתחת למונחים) משגר מסלול-מלא `_WizardFindReplaceScreen` המארח את **`FindReplacePane` של הסטודיו — verbatim** (חיפוש-מצע על overrides · תצוגת before→after · `CheckboxListTile` פר-hit · `applyOps` אצווה→טיוטה · אזהרת-רוחב · ליבה read-only). האשף מוסיף רק פעולת-AppBar **"פרסם לכולם (חי)"** → `configStore.publish` — הצעד ההופך טיוטה→published (האשף לא ב-edit-mode ⇒ publish הכרחי, בדיוק כמו ✎/slice-2). **אפס-שכפול:** לוגיקת-ההחלפה נשארת בפאנל היחיד — בדיקת-הפאנל (studio) מכסה אותה; בדיקת-האשף מוכיחה רק את החיווט (launcher→pane+publish). 18/18 · analyze 0. **הערה:** מצא-והחלף פועל על overrides קיימים (`draft⊕published text`) — שימושי אחרי ✎ (slice-2); `labelHe` (שם-הרכיב) לעולם לא נסרק.

## #wizard-studio-s3 — האשף = הסטודיו · פרוסה-3: מונחים-שזורים פר-מודול — 2026-07-27
**דירקטיבה "האשף = הסטודיו" · פרוסה-3 (בלי push עד "תדחוף").** כל סקציית-מודול מציגה **"→ תצוגה" חיה** של מונחי-המודול (`termOf`). מפה `_kModuleTerms` (מפתח-מודול → [(מפתח-מונח · תווית · ברירת-מחדל)]) — **רק מונחי-רישום-V3 המחווטים בפועל** (`nav.home/catalog/updates/store/customers` · `entity.customer` · `brand.club`). כל סקציה מציגה צ׳יפ `🏷️ תווית → ${termOf(_draft,key,def)}` — **חי**: עריכה במקטע "מיתוג ומונחים" → `_draft.terms` → rebuild → הצ׳יפ מתעדכן. **מקור-אמת אחד** (`OrgConfig.terms`), הצ׳יפ read-only ⇒ בלי כפילות-עריכה/סתירה. 17/17 (16 + woven: תצוגה-חיה + עדכון-בעריכה) · analyze 0. **מגבלה כנה:** רק ~8 מונחים מחווטים ל-`termOf`; מונחי-המוקאפ העשירים-יותר (יחיד/הזמנה/פרויקט) אינם מפתחות-מונח חיים באפליקציה.

## #wizard-studio-s2 — האשף = הסטודיו · פרוסה-2: מפקח מלא פר-רכיב (✎) — 2026-07-27
**דירקטיבה "האשף = הסטודיו" · פרוסה-2 (בלי push עד "תדחוף").** ✎ פר-רכיב באקורדיון — **"לא רק הצג/הסתר"**. כל שורת-רכיב עם ציר-תוכן/עיצוב (`editableProps`: text/emoji/style) קיבלה כפתור **✎** (`Key('elem-edit-<id>')`, ליד מתג-ההסתרה שנשמר). ✎ פותח bottom-sheet `_ElementInspectorSheet` contextual לפי `editableProps`: **טקסט · אמוג׳י · צבע · גודל · משקל** (dropdowns מאוצר-המילים של `cfg_text` — 7 צבעים / 4 גדלים / 4 משקלים · null='ברירת מחדל'). כל שינוי → `configStore.applyOps`+**`publish`** ⇒ **חי בכל האפליקציה** (האשף לא ב-edit-mode ⇒ `resolvedNode`=published ⇒ publish הכרחי לליוונס). תצוגה-חיה מקומית ב-`applyCfgTextStyle` (מנוע-הרינדור של האפליקציה עצמה). "אפס לברירת-מחדל" → `CfgStyle` ריק → publish → הרכיב חוזר לבסיס. **מיחזור מלא:** ה-override-store (`CfgNode`/`CfgStyle` · `SetText`/`SetEmoji`/`SetStyle` · `publish`) + `applyCfgTextStyle` כבר-קיימים — ה-✎ רק חושף אותם באשף. **ההסתרה נשארת על מתג-השורה (OrgConfig)** — שני צירים נפרדים, בלי כפילות; `Key('elem-toggle-<id>')` (SwitchListTile) נשמר ⇒ כל בדיקות-הרכיבים הקיימות ירוקות ללא-שינוי. **16/16** (15 + ✎: text-publish-live + style-axes-present) · 192 studio ירוקות · analyze 0. **align/direction = follow-up** — אינם ב-`CfgStyle` (colorToken/fontScale/weightToken/sizeToken/pad) ואינם מוחלים ב-`CfgText`; הוספתם נוגעת ב-value-object הסגור (פרוסה עתידית).

## #wizard-studio-s1 — האשף = הסטודיו · פרוסה-1: אקורדיון-Maor (מודול-קבלן ראשון) — 2026-07-27
**דירקטיבה "האשף = הסטודיו" · פרוסה-1 (רב-פעמי · בלי push עד "תדחוף").** ה-"מודולים"+"רכיבים" הנפרדים אוחדו ל**אקורדיון-Maor אחד** (14 סקציות, 👷 קבלן ראשון — "הפרסונה-הראשית חסרה סקציה, זה החור שהבעלים תפס"). (1) `org_modules.dart` — `kContractorModule` (👷 לוח קבלן) + `kWizardModules=[contractor, ...kOrgModules]` (14) + `moduleForScreen(screen)` (דטרמיניסטי, **כיסוי-מלא** של 123 מסכים→14 מפתחות: מסכי-קבלן מפורשים, השאר מילות-מפתח בשם-המסך, fallback='manager'). **`kOrgModules` נשאר 13** (הסט-הנעול, org_setup_wizard_test:56) — contractor **display-only, בלי שער-פרסונה** ⇒ אין נעילה-עצמית של ה-app-הבסיסי. (2) האשף — כל מודול=סקציה: שער-פרסונה (13 gated=`_moduleTile` SwitchListTile · manager נעול · **קבלן בלי-שער**) + **מונה N/M פעילים** + רצועת-פתיחה-עצלה (lazy — ~896 מתגים לא נבנים סגורים) + **סמן/נקה-הכל** (`_setModuleElementsHidden` — bulk, **מדלג kImmutable**) + מתגי-רכיבים מקובצי-מסך (`screenLabelHe`). מונה גלובלי "X מתוך Y רכיבים פעילים". **חיפוש+צ׳יפים = מסננים** (לא תנאי-הצגה — תיקון באג "התיבה-הריקה": כל 14 נגללים תמיד, חיפוש רק מצמצם+פותח-אוטומטית). **13 ה-SwitchListTile נשמרו** ⇒ renders(13)/pack/self-lock/save/terms/reset/import/element-search כולן ירוקות ללא-שינוי. +3 בדיקות (מיפוי-מלא · קבלן-נגלל+מונה+13-tiles · נקה-הכל-bulk→cart.cta). analyze 0 · **15/15**. הסטודיו לא-נשבר.## #owner-never-pending — הבעלים לא נתקע ב"ממתין לאישור" (bootstrap-gap) — 2026-07-27
**באג-חי (מאובחן 100%, לקח #39):** הבעלים ראה פס "ממתין לאישור" ולא יכול היה לאשר אף אחד (כולל את עצמו) — "הפעולה נכשלה". שורש: ה-bootstrap (19/7) נתן `admin:true`+role אבל **מסמך-המשתמש נשאר `status: pending`**, ו-`permitAction` חוסם **כל** פעולה כשה-pending — **גם לאדמין** (אינווריאנט מכוון · בדיקה `pending blocks even admin`). ⇒ כפתור "אישור" נחסם client-side לפני השרת. **תיקון (מכבד את האינווריאנט — לא נגעתי ב-`permitAction`):** `currentUserProvider` (users_repository) מכריח `status=active` **רק לבעלים** (`isOwnerEmail(user.email)` — email מאומת, לא-ניתן-לזיוף) כשהמסמך pending → הפס נעלם, permitAction עובר, השרת כבר מקבל את ה-admin claim → האישור עובר. אימות: analyze 0 · 147 בדיקות user-system/auth ירוקות (כולל האינווריאנט הנשמר).
...
## #wizard-studio-s0b — עריכה-בחי + ניווט-מלא (מתג נווט⇄ערוך + בורר-בורד) — 2026-07-27
**דירקטיבה "האשף = הסטודיו" · פרוסה-0 המשך (מאושרת-לדחיפה-מיד).** הבאג שהבעלים תפס: edit-mode על המסך-החי לוכד כל tap ⇒ אי-אפשר לפתוח sheet / לעבור בורד ⇒ תקוע במסך-הניהול, לא מגיע לקבלן. **התיקון — שני מפלסי-ניווט ב-`StudioOverlay` (מחזור, לא כפילות ולא מחיקה):** (1) **המעטפת נעשתה קבועה** — מוצגת כש-`studioActiveProvider` דלוק (לא רק ב-editing); off-gate עדיין `SizedBox.shrink` ⇒ **זהה-בייטים** (kStudioFlag const-off → tree-shake). (2) **מתג `נווט⇄ערוך`** (`SegmentedButton`, ברירת-מחדל **נווט**=edit-off ⇒ כל tap ניווט רגיל: sheets/מסלולים/תתי-מסך · **ערוך**=tap עורך רכיב) — ממחזר `enterEdit/exitEdit`, מגודר `studioCanEditProvider` (#84 — non-owner→disabled). (3) **בורר-בורד** — ממחזר `showRolePicker` דרך ה-root-navigator (`bsNavigatorKey`), **מחוץ ל-edit-trap** (מעל ה-Navigator, תמיד לחיץ); מגודר `kStudioFlag` (רק בבניית-preview, שם ה-navigatorKey מחווט). (4) **הרלקסציה הורחבה** — `studioOwnerManagerProvider` בתצוגה מחזיר `true` (במקום `inManager`) ⇒ פתיחת-edit על **כל בורד** (כולל קבלן — הבורד שהבעלים צריך לתקן), עדיין preview-only (`kStudioFlag && kProfileRawShell`); החי (buildsmart · בלי STUDIO) tree-shake, שער-`isOwnerEmail` זהה-בייטים. (5) `main.dart` — `navigatorKey: (kKbGlobal || kStudioFlag) ? bsNavigatorKey : null` (שני compile-consts → החי `null` זהה-בייטים). זרימה: בורר→בורד · נווט→פותח sheet · ערוך→עורך · נווט→הבא. אימות: analyze 0 · zero_regression (off=אין-מעטפת · on=מתג נווט/ערוך · **המתג-מפעיל-edit-ומכבה**) + 304 בדיקות-סטודיו/שער/role-picker ירוקות. הסטודיו הקיים לא-נשבר · מניפסט-הקונפורמנס עודכן (`return true`).

## #p2-w4c — Phase-2 גל-4ג: כפתור-קבלה (משלים את זוג-מסמכי-החיוב) — 2026-07-27
**לולאת-סיום (W4c) — הפרוסה האחרונה.** קבלה (קבלה) היא מסמך-חיוב נפרד מחשבונית (מונפק בתשלום). כפתור "💵 הפק קבלה" בגיליון-פרטי-ההזמנה, מתחת לכפתור-החשבונית, **על אותה רכבת** (`invoiceTitle(order, receipt:true)` → 'קבלה — {id}' · `buildInvoiceRows` · `printDocument`) ותחת **אותו שער `orders.invoicing`** (זוג-מסמכי-חיוב מודלק יחד; מובחן מ-`orders.deliveryNote` שהוא לוגיסטיקה). כבוי כברירת-מחדל ⇒ אין כפתור ⇒ הגיליון זהה-בייטים. בדיקות: הרחבת invoice_gate_test (OFF — חשבונית וקבלה נעדרות · ON — שתיהן נוכחות) · הקרנל invoiceTitle(receipt:true) כבר-מכוסה ב-invoice_test · mutation ×1 (תווית-הקבלה שונתה→אדום, מוכיח נעילת-הכפתור). analyze 0 · guards ×2. **מגבלת-בדיקה כנה:** חיווט ה-receipt:true→כותרת אינו נבדק ברמת-הכפתור (printDocument הוא top-level, לא provider בר-override) — אך הקרנל invoiceTitle(receipt:true) נבדק ישירות. **זוג-מסמכי-החיוב (חשבונית+קבלה) + תעודת-משלוח = שלישיית-מסמכי-ההזמנה שלמה.**
...
## #wizard-studio-s0 — הדלקת WYSIWYG edit-live בתצוגת-preview (מחזור) — 2026-07-27
**דירקטיבה "האשף = הסטודיו" · פרוסה-0 (מאושרת-לדחיפה-מיד).** ה"הקרן-מסך-וערוך" כבר בנוי (edit_mode.dart step-8 + edit_handle.dart step-9 · WYSIWYG: tap על רכיב חי → עורך-inline). מחזור — לא בנייה: (1) `--dart-define=STUDIO=true` בבניית-wizard-preview → `kStudioFlag`→`studioActiveProvider`=true. (2) **רלקסציית-שער-בעלים לתצוגה-בלבד** ב-`studioOwnerManagerProvider`: `if (kStudioFlag && kProfileRawShell) return inManager;` — לתצוגת-clean אין Google-login (isOwnerEmail(null)=false), אז מתירים edit-open לכל manager-context **רק** שם. מגודר בשני compile-consts (`kStudioFlag` — STUDIO=true רק בבניית-preview · `kProfileRawShell` — clean) → **החי (buildsmart · בלי STUDIO) tree-shake-אאוט, שער-הבעלים `isOwnerEmail` עומד זהה-בייטים.** אימות: analyze 0 · studio_gating (פין-שער-הבעלים) + cfg_wrappers + zero_regression ירוקים (ברירת-מחדל demo+ללא-STUDIO ⇒ ה-if false ⇒ else-owner-gate). הסטודיו לא-נשבר. הבעלים לוחץ על רכיב על המסך החי → עורך טקסט/מסתיר, חי.

## #swarm-r2e — סוף כיסוי-ה-composite (26 עיטופים · הזנב האחרון) — 2026-07-27
**נחיל · ריצה-2 phase-E — 3 fixers מקבילים (15 קבצים).** מיצוי הזנב האחרון עד אפס: **26 עטיפות** — courier_reports(3) · courier_forms(1 משותף→4 כפתורי-טפסים) · store_documents(1 משותף→12 שורות-חודש) · consent_modal(3) · coming_soon(2) · signature_pad(5) · reject_reason(2) · smart_project/home(3) · home_content_reorder(2) · site_hub(2) · docs_readiness(2) · stock/finder/tasks_gantt(plain — 0). **`critical:true` על יציאות/CTAs-מונעי-התקעה:** consent-accept · coming_soon-primary-exit · site_hub-back · docs_readiness-exit · smart_project-exit · reorder-back. **החרגה מכוונת:** studio_screen/studio_rules/help_target — כלי-**העורך** (התשתית שעורכת אלמנטים), לא אלמנטי-אפליקציה. זהה-בייטים · analyze 0 · 65 בדיקות-מושפעות ירוקות (כולל pumps חסרי-scope — ה-fallback של CfgVisible). **סיכום כיסוי-ה-composite: ~200 עטיפות ב-~59 קבצים** — כל כפתור/כרטיס/pill/chip באפליקציה נעלם-שלם בהסתרה. דגלים-מינוריים (banners פסיביים: smart_project.t08 · smart_home.workpath_badge · store_documents.t02 — תווית-בלבד; wrap-triviali אם יידרש).

## #swarm-r2d — כיסוי-composites מלא (74 עיטופים · 26 קבצים) + CfgVisible סביל-scope — 2026-07-27
**נחיל · ריצה-2 phase-D — 5 fixers מקבילים (26 קבצים דיסיונקטיים).** מיצוי כיסוי-ה-composite עד הזנב: **74 עטיפות `CfgVisible`** — catalog_screen(23), welcome/login/persona/home_shell(19: **3 CTAs-login `critical:true`**), settings×6(15), worker-sheets×6(12), contractor+profile+notifs(5). כל fixer סרק `const`, דילג על plain/titles/SwitchListTile/empty-states. **תיקון-מנגנון:** `CfgVisible` הפך מ-ConsumerWidget ל-**StatelessWidget עם fallback חסר-scope** (מראה של CfgText) → מתדרדר-בחן ל-child כשאין ProviderScope, מתקן רגרסיית-בדיקה (t6_notif_action pumped בלי scope) **וכל** רגרסיית-scope עתידית של ~175 העטיפות. זהה-בייטים (child verbatim · config ריק) · 133 בדיקות-מסך-מושפעות ירוקות · analyze 0 · gate_118 ירוק. **כיסוי-ה-composite של כל האפליקציה שלם** (~175 עטיפות סה״כ ב-~44 קבצים). דגלים לתיעוד: `login_sheet.t06` id-כפול (מפריד+כפתור) · `welcome.board_existing_login_btn` label-ב-Row (לא-עטיף) — מינוריים.

## #swarm-r2c — סגירת כיסוי-composites (64 עיטופים · 18 קבצים) + בטיחות-יציאה — 2026-07-27
**נחיל · ריצה-2 phase-C — 4 fixers מקבילים (18 קבצים דיסיונקטיים).** השלמת כיסוי-ה-composite: **64 עיטופי `CfgVisible`** — profiles(15: worker/store/courier — save-pills·demo-chips·dialog-actions), worker/courier-ops(21: worker_app 10·attendance·safety·certs), dashboards-2(13: store_dashboard 10·courier 3 — כולל _FlatCard single-content), trade_builder(8)+tasks(7). כל fixer סרק גם `const CfgText`, דילג על תוויות-plain (Padding/Align/Expanded) ועל multi-child cards / ListTile-titles / DropdownMenuItem (edge-cases נכונים). **בטיחות-יציאה:** 7 כפתורי-יציאה/חזרה (worker/store/courier/manager/projects/tasks/profession) קיבלו `critical: true` — org לא יכול להסתירם (מונע התקעת-משתמש · אכיפת הדירקטיבה "הסתרת-ניווט אסורה"). זהה-בייטים (CfgVisible→child verbatim · config ריק). analyze 0 · gate_118 ירוק (כל Cfg-id רשום) · 56 בדיקות-מפתח ירוקות. **כיסוי-ה-composite הגבוה-ערך שלם.** נותרו רק dialogs נמוכי-ערך (כפתורי "ביטול" ב-sheets — org לעיתים נדירות מסתיר) — אופציונלי.

## #swarm-r2b — כיסוי-composites (עטיפת ~36 כפתורים/כרטיסים/pills) — 2026-07-27
**נחיל · ריצה-2 phase-B — 3 fixers מקבילים (קבצים-דיסיונקטיים).** הרחבת הסתרת-ה-composite מהדגלים לכיסוי-רוחב: כל `child:/label: CfgText` שהוא ילד-של-כפתור/צ׳יפ/כרטיס/tappable נעטף ב-`CfgVisible(id, child:…)` → הווידג׳ט השלם נעלם בהסתרה (לא chrome-ריק). **~36 עיטופים חדשים ב-8 קבצים:** store_screen (17 — dialog-actions · ship-to · checkout · cart-actions · orders-hidden · scan), manager_dashboard (4 pills + 2 const: dash.exit · credit_explain), role_requests_inbox (3 .icon), ai_finder · describe_to_cart · projects (5) · profession · contractor_tools (2). תוויות-plain (child של Padding/Align/Center) הושארו — CfgText לבדו מסתירן. **זהה-בייטים** (CfgVisible→child verbatim כשהקונפיג ריק) — 59 בדיקות-מסך-מושפעות ירוקות, analyze 0. **נשאר (2c+):** profiles · worker/courier-ops · dashboards-2 · trade_builder · tasks · dialogs (LOW). אין push עד "תדחוף" (עומד לכל-הלולאה).

## #swarm-r2a — הסתרת-composite (הכפתור/כרטיס השלם, לא chrome-ריק) — 2026-07-27
**נחיל · ריצה-2 phase-A.** ריצה-1 הסתירה **תווית-בלבד** (CfgText→SizedBox) ⇒ composite (כפתור/כרטיס עוטף) נשאר shell-ריק. תיקון: לימדתי את `CfgVisible` (קוד-מת, 0 שימושים ⇒ אפס-שינוי עד עיטוף) את שער-ה-org — `if (!elementVisible(ref,id)) return SizedBox.shrink()` **ראשון, שני-המצבים, בלי ghost** (הגדרת-אשף, לא Studio-inline); critical→child ראשון. עטפתי את הדגלים: **cart.cta** (הבאג-המדווח — `CfgVisible` עוטף את ה-ElevatedButton ב-store_screen) + **`_MetricTile`→ConsumerWidget + שער-אחד על cfgId** (מכסה 5 KPI בלוח-המנהל). זהה-בייטים (config ריק ⇒ child verbatim). בדיקות: 4 רגלי-org ב-cfg_wrappers (כפתור-שלם-נעלם · absent-verbatim · editing-בלי-ghost · critical-מוגן) · mutation ×1 (שער-off → composite נשאר → אדום). **נשאר (ריצות 3+):** ~50 composites נוספים ב-~40 קבצים (batches B-E: dashboards/profiles/worker-courier-ops/dialogs/trade-builder) — עטיפה מכנית `CfgVisible(id, child:…)` דרך fixers מקבילים. אין push עד "תדחוף".

## #swarm-r1 — Studio registry → אשף-טוגלים (ציר הצג/הסתר פר-אלמנט) — 2026-07-27
**נחיל 9×9 · ריצה-1 של המשימה הרב-פעמית** (SSOT: knowledge/DIRECTIVE-studio-registry-to-wizard-toggles.md · origin/claude/nice-volta-BSbVm). ממחזר את `element_registry.dart` (895 descriptors · 889 wired · 5 kImmutable) כמקור-טוגלים — **בלי registry חדש, ה-Studio ממשיך לעבוד**. (1) **מודל** (Option A, אפס-סכימה): `elementShown(OrgConfig,id)` = `features['element.<id>'] != false` — absent=גלוי, רק false-מסתיר; רוכב על ה-codec + carry-through הקיימים (`_rebuild`/`applyVerticalPack`) ⇒ עוקף מוקש-שדה-נמחק, זהה-בייטים. (2) **שער-רינדור מרכזי אחד**: `elementVisible(ref,id)` (org_gates — נושא נעילת-kImmutable דרך `criticalIdsProvider`) ב-`CfgText.build` (המקום-החי היחיד, 868 שימושים) → `SizedBox.shrink` כשמוסתר. absent ⇒ גלוי ⇒ **זהה-בייטים** (37+63 בדיקות-נעולות ירוקות). (3) **אשף — סעיף "רכיבים"** מונחה-חיפוש (`elem-search`) → קבוצות `normalizeScreen(screen)`→area פתוחות, `screenLabelHe` (מפה עברית ~100 מסכים + humanize-fallback, קובץ חדש `screen_labels_he.dart`), טוגל פר-אלמנט (`elem-toggle-<id>`, title=labelHe), **ליבה-נעולה** (kImmutable → onChanged null + 🔒), canonical-minimal (`_setElementHidden` מוחק-מפתח-בהצגה). lazy (רק קבוצות-פתוחות נבנות ⇒ 863 לא-נשפכים). נגיש על APP_PROFILE=clean+ORG_CONFIG=true. בדיקות: config (absent=גלוי-לכל · false-מסתיר · round-trip) · matrix (הסתרה-חיה של home.topbar.brand · kImmutable כפוי-גלוי) · wizard (renders · search · canonical-minimal · locked) · screen_labels_he (5) · mutation ×1 (elementVisible→true → matrix אדום). **מגבלת-composite ידועה (לריצה-הבאה):** CfgText מרוקן תווית — כפתור/כרטיס עוטף נשאר chrome-ריק; הסתרה-מלאה ל-composites דורשת gating ברמת-wrapper. אין push עד "תדחוף".
...
### #reg-approval — אישור-הרשמה בשליטת-בעלים · חיווט מסך-הלקוחות (2026-08-02)
**חוסם-ההשקה (מחר 100+ משתמשים):** USER_SYSTEM דלוק ⇒ כל חשבון חדש נולד `status:pending` וה-pending-gate (rbac `checkoutBlock` + חוק `isActive()` על orders) חוסם אותו בקופה עד אישור. הבעלים רוצה **לשלוט** מי מאושר — לא אישור-אוטומטי-שקט (**מבטל את d04d03e6**) — מ**מסך-הלקוחות הקיים** ("יש מסך כזה בניהול רק צריך חיווט"), שכעת גם מציג את **כל** המשתמשים הרשומים ולא רק מי שהזמין.
- **manager_dashboard_screen.dart** (טאב 👥 לקוחות — **חוּוט, לא נבנה-מחדש**): `_customerViewsProvider` **מאחד** את ספריית-האנשים (`directoryProvider` — כל הרשומים) עם הרשימה-הנגזרת-מהזמנות (מיזוג-לפי-שם: spend/orders זורמים ללקוח-רשום תואם; לא-תואם ⇒ אפס-סטטיסטיקות). directory **ריק כשה-backend כבוי** ⇒ מתקפל לרשימת-היום ⇒ **זהה-בייטים כבוי**. `_ApprovalBadge` פר-כרטיס (⏳ ממתין / ✓ מאושר) — live-only ⇒ נעדר כבוי. `_PendingApprovalPanel` בראש הטאב (גדור `useFirebaseBackend` + persona מנהל) עם 3 המצבים שהבעלים ביקש: **"אשר הכל"** (כל הממתינים, מתעלם מהסימונים) · **"אשר מסומנים"** (רק המסומנים — הנתיב הידני) · **"אשר הכל חוץ מ-"** (השורות מסומנות כברירת-מחדל, מבטלים סימון למוחרגים ואז "אשר מסומנים"). `_CustomerChatButton` פותח צ'אט ישירות על `directUid` (uid מהספרייה) ⇒ משתמש-רשום שלא-הזמין (בלי טלפון) עדיין נגיש.
- **directory.dart**: קבועי `kDirectoryStatusPending/Active` (נעולים מול השרת) + `DirectoryEntry.status` (מ-`directory.status`) + טהורים `pendingDirectoryEntries` / `resolveApproveTargets` — לוגיקת-הצ'ק-ליסט (all / selected / all-except) מכוסה widget-free.
- **role_requests.dart**: `userApproverProvider` (seam מעל ה-callable `approveUsers`, region me-west1) — null כבוי ⇒ הפאנל inert ⇒ זהה-בייטים.
- **functions/src/approveUsers.ts** (חדש): callable **מנהל/אדמין בלבד**, אצווה `users.status` pending⇄active דרך ה-Admin SDK (עוקף את הקפאת-הכתיבה-העצמית של S5 על `status`) + `syncDirectoryEntry` פר-uid. best-effort פר-uid, cap 500/אצווה. טהורים `mayApproveUsers`+`cleanApproveUids` selftested. מיוצא ב-index.ts. נפרס אוטומטית ב-push דרך firebase-deploy.yml (`functions/**`).
- **functions/src/reviewRoleRequest.ts**: `onUserCreatedQueueApproval` (טריגר users/{uid} onCreate) — לא-מפעיל-אוטומטית (משתמשים נשארים ממתינים עד אישור-בעלים; אין בקשות-קבלן-אנונימיות). **חדש (2026-08-03): התראת-push לאישור** — על נרשם-חדש `status=pending` שולח FCM ל**בעלים (`OWNER_EMAIL`→uid) ∪ כל `manager` בספרייה** (deduped, מינוס-הנרשם) דרך `sendToUsers` (מיוצא מ-push.ts). בטוח+אדיטיבי: יורה אחרי-שהמסמך-קיים (throw לא מפיל הרשמה), כל lookup best-effort, ו-`sendToUsers` no-op כשאין `fcmToken`. תפקידים-תפעוליים מוגברים עדיין דרך submitRoleRequest → reviewRoleRequest.
- **מחזור מלא:** משתמש חדש → `users.status=pending` → onUserDocWritten → `directory.status=pending` → מופיע בפאנל + תג ⏳ → אישור → approveUsers → `users.status=active` + directory sync → נושר מהפאנל, תג ✓, הקופה נפתחת.
- **launch-clean (seed-filter):** `_customerViewsProvider` מסנן שורה גזורת-הזמנה **בלי טלפון** כש-`useFirebaseBackend` — לקוחות ה-seed המובנה (משה אברהם … `kOrdersEngineSeed`) צצים דרך ה-born-seed כשאוסף ה-`orders` האמיתי ריק (`firestore_cached_repo._onSnapshot` שומר seed על snapshot-ריק-ראשון לא-scoped); הם **לא חשבונות אמיתיים** ומחיקת-DB לא מסירה אותם. לקוח אמיתי נושא `customerPhone`; משתמש-רשום בספרייה נשמר (שלב 1, ללא-תלות-בטלפון). כבוי ⇒ לא-מסונן ⇒ **זהה-בייטים** (ה-seed הוא תוכן-הדמו שם). לבקשת-הבעלים "מי שאין לו מייל או מספר — תעיף".

gate: functions `tsc` 0 + selftest **100/100** · `flutter analyze` 0 errors · `manager_approval_panel_test` **8/8** (all/selected/all-except + OFF-gate).

### #catalog-config — הקטלוג-המגדיר (הכרטיס-החיפוש) · מודל-קנוני + UI-מלא · מגודר `kCatalogConfig` OFF
> **בעלים-אחד (המיזוג בוצע):** המודל **הקנוני** (`product_config_schema.dart` · `ProductConfigSchema` על `AttributeDef` · `configSchemaFor` · `catalog_config_flags.dart` · gate #128) — בסיס. שכבת-ה-UI/E2E המלאה הורכבה עליו. אפס-קוד-פר-מוצר · בלי 3D · tree-shaken כשכבוי ⇒ הקטלוג-החי **byte-identical**.
- **מודל (קנוני + גזירה-אמיתית שקופלה · פאזה F):** `configSchemaFor(product,{universe})` — קוטר=`odOf` נבדל פר-משפחה · זווית=קבוצת-הברכיים · **יציאות=`dims['יציאות']` נבדל** · **צבע=בחירת-`color` אמיתית (כחול/אדום)** · קוטר-קטן=`od2Of`. **כל הגלגלים ערכים אמיתיים** (נסיגה-לתבנית כשאין אחים · M1).
- **UI (`lib/features/catalog_config/`):** `browse_model` (צלילה catalog_tree→אריחים) · `wheel_picker` (`ListWheelScrollView`+פס-בחירה · kind=`AttributeKind`) · `config_card` (מרנדר `schema.attributes` · selection ב-`attr.id` · תמונת-מוצר+fallback · qty · הוסף-לסל) · `catalog_config_screen` (accordion · `configSchemaFor` · חיווט-סל · `route()` null כשכבוי).
- **סל:** `SmartCartLine.selection` אדיטיבי (JSON זהה-בייטים לשורות ריקות). מחיר stub 0/'לפי ספק' · תמונה per-SKU (החלטות-בעלים). פיילוט=הנגזרים (PPR+ברך+מצמד+מחלק).
- gate: `flutter analyze` **0 errors** · **51/51** (product_config_schema קנוני + gate_128 + browse/wheel/card/cart/schema) · כל גלגל mutation-verified · gating מחוץ-לתיקייה **=0**. **בלי הדלקה-חיה (GO-בעלים).**

### #internal-card — הכרטיס-הפנימי המלא (13 סקציות · "המנוע חשוף") · מגודר `kInternalCard` OFF
> P1 מתוך D1–D18 (`knowledge/internal-card/WIRING-SSOT.md`). ווידג'ט `FullInternalCard` (ConsumerWidget) מרנדר 13 סקציות-נתונים לאביזר, כל סקציה מחווטת לפונקציית-מנוע קיימת ומוצגת רק כשיש נתונים (R8 · אין המצאה). מקטע-בית `HomeSection.internalCard` מגודר `kInternalCard` (כמו `kAxisDive`) → build כבוי tree-shaken → byte-identical; חי דרך `--dart-define=INTERNAL_CARD=true`.
...
משוב-בעלים: "בפנימי שיציג רק מה שיש לפי הכותרת שזה יהיה מעין סינון" (על כרטיס-הקונפיג · הגלגלים). האריח כבר יודע את חומרו (`#catalog-config-material-rail`); כעת גם **הכרטיס** שנפתח מסונן לאותו חומר — ברך-PPR מציג גלגלי-מידה/זווית של PPR בלבד, לא מעורבב עם HDPE/נחושת. **`catalog_taxonomy`:** `_materialOf` הפרטי (שהיה כפול ב-`browse_model`) עלה ל-`materialOf` **ציבורי** — נגזרת-חומר אחת ש-**גם הרכבת מקבצת לפיה וגם הכרטיס מסתנן אליה** (אריח↔כרטיס לא יכולים לחלוק). **`catalog_config_screen`:** `_materialUniverse(product)` = חברי-typeGroup החולקים את חומר-המוצר; `ConfigCard.schema = prioritizedSchema(product, universe: _materialUniverse(product))` — הגלגלים נאגרים מהחומר בלבד. **`config_card`:** `_resolveFamily` מצמצם את `_family` (קבוצת-החלפת-הווריאנט בגרירה) לאותו חומר — גרירה נשארת בתוך החומר, לא קופצת ל-DN של פלסטיק על כרטיס-נחושת.
- gate: `flutter analyze` 0 · catalog_config **109/109** (+1 material-scope: נחושת 77777677 → 3 מידות DN15/20/25, בלי DN110).

### #catalog-config-image-drag-angle — משיכת-תמונה אנכית (זווית) מחליפה וריאנט (2026-08-09)
משוב-בעלים: "משיכה של התמונות לא עובד". רק המשיכה האופקית (קוטר) החליפה וריאנט; האנכית (זווית) לא — **אי-התאמת-טוקן**: גלגל-הזווית נשא canonical `45`/`90` למשפחות-מנוע (`_angleValues`) אך `45°`/`90°` למוצרי-axisChips (`catAxesOf`), בעוד משפחת-הווריאנט (`chipValuesOf`) נשאה `45°`/`90°` — אז טוקן-הבחירה שנבחר במשיכה לא תאם אף חבר-משפחה, `variantByAxes` דילג על הזווית, והתמונה נשארה. **`product_chips`:** (1) `chipValuesOf` מקפל זווית ל-ספרות (`45°`→`45`); (2) `_angleAttribute` חדש (ב-`prioritizedSchema`, סימטרי ל-`_dnAttribute`) מקפל את **כל** גלגלי-הזווית ל-canonical ספרתי (labelHe נשאר `45°`). **`config_card`:** `_lastAxis` — הציר האחרון שזז (גלגל או משיכה) נפתר **ראשון** ב-`variantByAxes`, כך שכל כיוון-משיכה מנצח וגורר החלפת-וריאנט (במקום שהקוטר-קודם יקבע והתמונה לא תזוז).
- gate: `flutter analyze` 0 · catalog_config **112/112** (+3 image-drag). probe: ברך-PPR ↕ `45°→90°` · ↔ `20→63` · 213072 שני הכיוונים מחליפים.

### #catalog-config-elbow-image-mix — תיקון תמונות-שגויות בברכיים + ביטול קפיצת-וריאנט (2026-08-09)
משוב-בעלים: "יש ערבוב בין ברכים מסעפים" (במשיכת התמונה בכרטיס). **שורש (data):** `fitting_image_overrides.dart` מיפה SKU-ים של **ברכיים** לתמונות מ**משפחה שגויה** — `92117042–92117051` ("ברך PPR 90°") → `98417808_3.jpeg` שהיא צילום של **צווארון PPR 75**, ו-`92117109` ("ברך PPR 45°") → `95270720_1.jpeg` שהיא **צינור**. משיכת-תמונה של ברך הציגה את הצילום הזר → נראה כמו סוג-אחר. תיקון-הזווית של הסשן (`309084b2`) הפך את המשיכה-האנכית לפעילה וחשף אותן. **תיקון (data):** הוסרו 11 המיפויים השגויים → כל ברך נופל-חזרה ל-`_pprImageFor` הנכון (`ppr_elbow_90.jpg`/`_45.jpg`). מיפויי-המסעפים (`992213xxx`→`992113139`, מסעף→מסעף) **נכונים ונשמרו**. **תיקון (logic):** בוטל `config_card._lastAxis` (מ-`309084b2`) — הוא גרם למשיכה **לקפוץ** לווריאנט-רחוק (למשל "זווית 105° NTM" מחבר-גינה ענפי) שהגביר את תחושת-"הערבוב"; חזרה למסנן-חמדני קוטר-קודם ⇒ משיכה קוהרנטית (נשאר בסוג). תיקון-הטוקן-של-הזווית (`chipValuesOf`/`_angleAttribute`) **נשמר** — המשיכה-האנכית עדיין עובדת במקרה-הנפוץ (PPR: DN20 קיים ב-45° וב-90°).
- gate: `flutter analyze` 0 · catalog_config **112/112**. probe: ברך-PPR 90° → `ppr_p19_b.jpg` (ברך אמיתי, לא צווארון); משיכה-אנכית ב-material-less לא קופצת ל-NTM.

### #auth-registration-audit — 2 תיקונים בטוחים + ממצא-אמת על מחיקת-חשבון (2026-08-11)
ביקורת "מה קורה עם הרשמה/ניהול-חשבונות" (2 Explore agents + נחיל). המערכת בריאה (162 טסטים, 0 errors). תוקנו 2 באגים בטוחים-וחסומים:
- **הרשמת-אימייל (welcome_screen `_registerViaAuth`):** `userProfileProvider.register(...)` הועבר לאחר ש-`createUserWithEmailPassword` מצליח — כישלון-יצירה (email-already-in-use/weak-password) לא ישאיר עוד פרופיל מקומי `registered:true` בלי חשבון Firebase. מאחורי `kEmailPasswordAuth` (כבוי) — תיקון-נכונות-חבוי, ה-flag לא הודלק.
- **הערת-קוד (`auth_state.usersProfileWriterProvider`):** תוקנה לשקף את המציאות — המירור כותב 6 שדות (displayName/phone/email/profession/address/businessId) וה-rule הוא **deny-list** שמקפיא role/roles/storeUid/orgId/status, לא allow-list של 2 שדות.
- **לא-תוקן (מכוון · ממצא-אמת):** מחיקת-חשבון משאירה נתוני-פרופיל-לוח מקומיים. הנחיל חשף שהתיקון **לא בטוח כעצמאי**: חנויות-הלוח הן `StateNotifier<Map<username,Profile>>` — מפתח-prefs יחיד מכיל את כל שמות-המשתמשים (ran/omer/…), **מנותק מ-uid של Firebase**. ניקוי-עיוור במחיקה ימחק PII של אנשים-אחרים במכשיר משותף. התיקון הנכון תלוי בקשירת-הלוחות-ל-uid (UID_SCOPED_QUERIES) — חלק מעבודת-ה-launch, לא תיקון-נקודתי.
...
### #filtered-auth-BC — סשן ריצתי + AuthGateway פר-מצב-מסונן (שלבים B+C) (2026-08-12)
המשך שלב-A. **דורמנטי** — עדיין לא מחווט לשורש-הספק (שלב E מחליף לפי הדגל הריצתי).
- **`lib/data/edge/filtered_session.dart` (שלב B):** ‏`FilteredSession` — מחזיק idToken/refreshToken/localId/email + זמן-פקיעה מוחלט; `validIdToken()` מרענן אוטומטית כשנותרו <5 דק' (`kRefreshSkew`) דרך `token.`, מנקה-סשן בכשל-רענון; התמדה JSON ב-`EdgeKvStore` מוזרק (‏`kFilteredSessionKey='bs_filtered_session_v1'`, פר-מקור כמו ה-SDK); `restore()` בהפעלה. האחסון+השעון מוזרקים ⇒ טהור, אפס dart:html.
- **`lib/data/edge/filtered_auth_gateway.dart` (שלב C):** ‏`FilteredAuthGateway implements AuthGateway` מעל A+B — signInWithEmailPassword/createUser/signOut/currentUser/authStateChanges; גוגל/SMS/deleteAccount/setRole ⇒ זורקים `unavailable` (מוסתרים ב-UI, שלב E). ‏`validIdToken()` נחשף לשכבת-הנתונים (שלב D). **⚠️ `authStateChanges` = controller ידני עם מנוי-ב-onListen (לא async*/yield*)** — אחרת המנוי ל-broadcast נרשם רק אחרי צריכת ה-yield הראשון, ואירוע-הכניסה שנפלט לפני-כן נבלע (broadcast לא מאגר). ratchet תופס בדיוק את זה (`seen.last?.uid`).
- **בדיקות (`edge_filtered_session_test.dart`, 8 ירוקות):** שלב B — adopt+תוקף · רענון-אוטומטי · כשל-רענון-מנקה · persist+restore. שלב C — signIn⇒currentUser+אירוע-בזרם · שגיאה⇒AuthGatewayException(code) · signOut⇒null · גוגל/SMS⇒unavailable. analyze 0 על edge/+הבדיקה.
- **הבא:** שלב D (Firestore-REST repos · Bearer idToken) · E (UI מתג-מסונן, מייל-בלבד) · F (אימות-שטח).
### #filtered-auth-E-wiring — דגל-מסונן ריצתי + החלפת-גשר-Auth (שלב E · חלק 1) (2026-08-12)
מחווט את גשר-ה-REST (שלבים A–C) לשורש-הספק מאחורי דגל-ריצתי פר-מכשיר. **ברירת-מחדל כבוי ⇒ native/VM/כל-לקוח-רגיל = ביט-זהה להיום** (אותו אינווריאנט של כל דגל).
- **`lib/data/edge/edge_kv.dart` (+`_stub`/`_web`):** בורר-פלטפורמה ל-`EdgeKvStore` (import מותנה כמו `services/geo.dart`) — web ⇒ localStorage אמיתי · native/VM ⇒ מפה-בזיכרון (בלי `package:web` ⇒ מתקמפל על ה-Dart-VM של הבדיקות).
- **`lib/data/edge/edge_http.dart`:** `makeEdgeHttpSend()` — שולח-HTTP אמיתי מעל `package:http` (על web ⇒ fetch לתת-דומיין המתווך; מוזרק ל-EdgeRestAuth שנשאר טהור; לא-נקרא בבדיקות).
- **`lib/data/edge/filtered_mode.dart`:** `kFilteredModeKey='bs_filtered_mode_v1'` (‏'1'=דלוק · חסר=כבוי, canonical-minimal) · `readFilteredMode`/`writeFilteredMode` טהורים · `edgeKvStoreProvider` · `filteredModeProvider` (StateNotifier — נקרא מהאחסון בלידה, מתעדכן חי).
- **`lib/state/auth_state.dart` — `authGatewayProvider`:** ‏`ref.watch(filteredModeProvider)` דלוק ⇒ בונה `FilteredAuthGateway` (apiKey = `Firebase.app().options.apiKey` — ה-web-key הציבורי, אין נגיעה ב-`FirebaseAuth.instance`) במקום `FirebaseAuthGateway`. כבוי ⇒ הנתיב הישן ללא-שינוי. **ריאקטיבי** — החלפת-הדגל בונה-מחדש את הגשר.
- **בדיקות (`edge_filtered_mode_test.dart`, 4 ירוקות):** דגל טהור (ברירת-מחדל/כתיבה/מחיקה) · כבוי⇒גשר-null (Firebase-free) · הדלקה⇒`FilteredAuthGateway` · התמדה (קונטיינר-חדש נולד-דלוק). analyze 0 על edge/+auth_state+הבדיקה. הכשל היחיד בסוויטת-הכניסה (`ink_sparkle.frag`) הוא אי-התאמת-shader של ה-SDK — קדם-קיים, לא-קשור (אומת ב-stash).
- **הבא (E · חלק 2 — UI):** ‏login_sheet ⇒ מצב-מסונן דלוק מציג רק מייל+סיסמה (מסתיר גוגל/SMS) + מתג "אני על אינטרנט מסונן". ‏D (Firestore-REST).
### #filtered-auth-E-ui — מסך-כניסה במצב-מסונן: מייל-בלבד + מתג (שלב E · חלק 2) (2026-08-12)
`lib/screens/login_sheet.dart` — כשמצב-מסונן דלוק המסך מציג **רק מייל+סיסמה**; המתג "אני על אינטרנט מסונן" מדליק/מכבה פר-מכשיר.
- **`_emailAllowed`** = `kEmailPasswordAuth || filteredMode` — שלושת מחזירי-הסיסמה (`_emailLogin`/`_emailCreate`/`_resetPassword`) עברו אליו (LOCK 2 נשמר — עדיין מגן על עצם-הקריאה, לא רק ה-UI).
- **צעד-אפקטיבי:** ‏`step = filtered ? email : _step` — טלפון/קוד לא-נגישים במצב-מסונן; פאנל-המייל נבחר דרך `(kEmailPasswordAuth || filtered)`. **⚠️ מוותר על LOCK 3 (byte-identical) במצב-מסונן בלבד** — הפאנל נכנס לבנדל (הכרעת-בעלים "מייל+סיסמה דרך הקישור שלי", הדלת היחידה לקו-מסונן); החלק ה-const נשמר ⇒ **בנייה-רגילה עדיין ביט-זהה** (לקוח לא-מסונן לא רואה מייל). "חזרה לכניסה עם טלפון" מוסתר במצב-מסונן (הטלפון dead-end שם).
...
- **`lib/data/edge/edge_http.dart`:** נוסף `EdgeHttpRequest` (נושא-מתודה GET/PATCH/POST) + `makeEdgeHttpRequest()` — שכבת-הנתונים צריכה GET/PATCH, לא רק POST של ה-Auth.
- **בדיקות (`edge_firestore_rest_test.dart`, 6 ירוקות):** round-trip כל-הסוגים (כולל מקונן) · int⇒integerValue-מחרוזת · timestamp⇒ISO⇒DateTime · getDoc פונה-ל-fs.-עם-Bearer+מפענח · 404⇒null · setDoc-PATCH-מקודד · שגיאה⇒חריגה. analyze 0.
- **הבא (D · חלקים):** ‏`firestoreRestProvider` (חולק את סשן-הגשר) + החלפת-ספק פר-מאגר לפי מצב-מסונן, החל מהמאגרים שהלקוח פוגש ראשונים אחרי-כניסה (users-doc, קטלוג). ואז שלב F (אימות-שטח).
...
### #system-setup-unified — כניסה מאוחדת "🔌 הקמת המערכת" (מנהל · בונה-ענף→הגדרת-ארגון) (2026-08-13)
מיזוג 2 הכניסות הנפרדות בטאב 🛠️ ניהול (בונה-ענפים · אשף-הקמת-חברה) לכניסה אחת דו-שלבית: קודם מקימים ענף, אז מגדירים ארגון.
- **חדש `lib/screens/trade_builder/system_setup_host_screen.dart`:** host דק ללא-Scaffold, `switch` על `_phase` (לא IndexedStack → פאזה-2 נבנית עצל, קוראת orgConfig טרי). פאזה-0=`TradeBuilderHome(onContinue:→1)`, פאזה-1=`OrgSetupWizard`.
- **`trade_builder_home.dart`:** נוסף `VoidCallback? onContinue` (null-default → route עצמאי byte-identical); כשמסופק, כפתור "המשך להגדרת החברה" ב-bottom-bar.
...
- **`tasks_firebase.dart`:** `tasksRepositoryProvider` ממודר-לפי-תפקיד — עובד(board.uid)→`assignedWorkerUid==uid` · מנהל→god(unscoped) · קבלן(roleProvider null + currentUid)→`employerId==uid` · אחר→null. `..attach()` + onDispose.
- **`tasks_engine.dart`:** `bindRemote` אמיתי (bind-once/re-bind + addListener) · `_refreshFromRemote` (`state=all()`) · `dispose` · **`_commit(next)`** מנתב create/patch: bound→`repo.upsert` else local. `_patch`/createTask/proposeTask/resetToSeed עוברים דרכו. side-effects (rewards/bell/order-fold) נשארים בקוראים. `tasksProvider` watch+bind+re-bind (מראה orders).
- **`firestore.rules`:** `tasks/{taskId}` 6-state cross-party — read: worker/employer/manager · create: קבלן pending-עצמי או עובד proposed-עצמי · update: **עובד מוגבל ל-{active,rejected,review} (אין self-approval ל-done!), assignedWorkerUid נשאר-עצמי, employerId write-once** · קבלן/מנהל: כל-סטטוס + employerId קפוא · delete:false.
- **`deleteAccount.ts`:** scrub cross-party (מראה orders) — worker נמחק→scrub {assignedWorkerUid,photo,note} · קבלן נמחק→scrub {employerId}. ניתוק לא-מחיקה (הלוח משותף).
- **אימות:** analyze 0 · `tasks_realtime_bind_test` (3: DOWN all()→state · UP mutator→upsert · אין self-approval תחת binding) + `tasks_firebase_test` 9 · **52 טסטי-המשימות הקיימים ירוקים (אפס-רגרסיה מקומי)** · stage2 · mutation-verify (ניתוב `_commit`→repo · RED UP → GREEN).
- **⚠️ נותר ל-increment 2:** 2b — השחלת-uid אמיתי + בורר→directory + ~8 משטחי-עובד int→uid · 2c — worker_notifs uid-addressed · 2d — הדלקת `TASKS_SERVER` ב-workflows.
