...
import 'package:buildsmart/genesis/dart-gen-bs/gen_retarget_volunteer_from_fee_sk1fce5c.dart' show VolunteerScreen, VolunteerFacts;
...
import 'package:buildsmart/genesis/dart-gen-bs/gen_retarget_room_from_rm_sk1fce5c.dart' show RoomScreen, RoomFacts;
import 'package:buildsmart/genesis/dart-gen-bs/gen_retarget_family_from_stu_sk1fce5c.dart' show FamilyScreen, FamilyFacts;
import 'package:buildsmart/genesis/dart-gen-bs/gen_retarget_shopitem_from_crs_sk1fce5c.dart' show ShopItemScreen, ShopItemFacts;
...
void main() {
  ...
  testWidgets('KehilaApp · חיפוש-רכזת נגזר: "מתנדבים" ⇒ 1/6 · ג׳יבריש ⇒ EmptyState · ריק ⇒ הכול', (tester) async {
    ...
    expect(find.byType(ForgeHubTile), findsNothing); expect(find.byType(EmptyState), findsOneWidget); expect(find.text('0/6'), findsWidgets);
    await tester.enterText(find.byType(TextField).first, ''); await tester.pump(const Duration(milliseconds: 300));
    expect(find.byType(ForgeHubTile), findsNWidgets(6)); expect(find.byType(EmptyState), findsNothing); expect(tester.takeException(), isNull);
  });
  ...
  testWidgets('KehilaApp · אריח-hero ⇒ חדרים (Room) נפתח על רשומת-ה-hero', (tester) async {
    ...
    if (id != null) { expect(find.byType(BottomSheet), findsOneWidget); expect(find.textContaining('מסונן למדד'), findsOneWidget); expect(find.textContaining('· ${RoomFacts.heroRows(RoomFacts.heroKey).length} מתוך'), findsOneWidget); }
    // ignore: avoid_print
  ...
  testWidgets('KehilaApp · אריח-hero ⇒ משפחה (Family) נפתח על רשומת-ה-hero + מקטע-הגרעין על הרשומה', (tester) async {
    ...
    if (id != null) { expect(find.byType(BottomSheet), findsOneWidget); expect(find.textContaining('מחזור-חיים · רשומה'), findsWidgets); expect(find.textContaining('מסונן למדד'), findsOneWidget); expect(find.textContaining('· ${FamilyFacts.heroRows(FamilyFacts.heroKey).length} מתוך'), findsOneWidget); }
    // ignore: avoid_print
  ...
  testWidgets('KehilaApp · אריח-hero ⇒ פריט (ShopItem) נפתח על רשומת-ה-hero', (tester) async {
    ...
    if (id != null) { expect(find.byType(BottomSheet), findsOneWidget); expect(find.textContaining('מסונן למדד'), findsOneWidget); expect(find.textContaining('· ${ShopItemFacts.heroRows(ShopItemFacts.heroKey).length} מתוך'), findsOneWidget); }
    // ignore: avoid_print
