...
void main() {
  testWidgets('TzedakaApp · בית: 7 אריחים', (tester) async {
    ...
    expect(find.byType(ForgeGridHubCard), findsNWidgets(7)); expect(tester.takeException(), isNull);
  ...
  testWidgets('TzedakaApp · חיפוש-רכזת נגזר: "קופה" ⇒ 1/7 · ג׳יבריש ⇒ EmptyState · ריק ⇒ הכול', (tester) async {
    ...
    expect(find.byType(ForgeGridHubCard), findsNWidgets(1)); expect(find.text('1/7'), findsWidgets); expect(tester.takeException(), isNull);
    await tester.enterText(find.byType(TextField).first, 'zzqqxx'); await tester.pump(const Duration(milliseconds: 300));
    expect(find.byType(ForgeGridHubCard), findsNothing); expect(find.byType(ForgeAnimatedEmpty), findsOneWidget); expect(find.text('0/7'), findsWidgets);
    await tester.enterText(find.byType(TextField).first, ''); await tester.pump(const Duration(milliseconds: 300));
    expect(find.byType(ForgeGridHubCard), findsNWidgets(7)); expect(find.byType(ForgeAnimatedEmpty), findsNothing); expect(tester.takeException(), isNull);
  });
  ...
  testWidgets('TzedakaApp · בית ⇒ קופה (TzBox) מרונדר וחוזר', (tester) async {
    ...
    expect(find.byType(TzBoxScreen), findsOneWidget); expect(tester.takeException(), isNull);
    tester.state<NavigatorState>(find.byType(Navigator).first).pop(); await tester.pump(); await tester.pump(const Duration(milliseconds: 600)); expect(find.byType(ForgeGridHubCard), findsNWidgets(7)); // DsScaffold ללא AppBar ⇒ pop דרך ה-Navigator, לא pageBack
  });
