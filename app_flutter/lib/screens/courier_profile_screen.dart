// #73 — פרופיל שליח ייעודי (וגם גוף טאב "אזור אישי" של לוח השליח, #72 טאב 4).
//
// מציג את ה-session החי מ-[boardAuthProvider] (שם תצוגה, שם משתמש, מצב demo),
// סטטיסטיקת מסירות חיה ממנוע ההזמנות המשותף, ושלוש פעולות:
...
class CourierProfileBody extends ConsumerWidget {
  const CourierProfileBody({this.standalone = false, super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListView(
      ...
      children: [
        ...
        Row(
          children: [
            _PStat(value: '${delivered.length}', label: 'נמסרו ✅'),
            _PStat(value: '$onRoad', label: 'בדרך 🚚'),
            _PStat(value: '$podCount', label: 'POD 📸'),
          ],
        ),
        const SizedBox(height: BsTokens.space2),
        Container(
          ...
          child: Text(
            'סה״כ ערך שנמסר: ${fMoney(deliveredSum)}',
  ...
  Future<void> _logout(BuildContext context, WidgetRef ref) async {
    final ok = await confirmDestructive(
      context,
      title: 'יציאה מהחשבון?',
      message: 'תנותק מלוח השליח ותחזור למסך הרישום.',
      confirmLabel: 'יציאה',
    );
    if (!ok || !context.mounted) return;
    ref.read(boardAuthProvider.notifier).logout();
    showToast(context, 'התנתקת מלוח השליח');
