  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        ...
        children: const [
          _QuickAction(icon: Icons.chat_bubble_outline,   label: 'שיחה'),
          _QuickAction(icon: Icons.calendar_today_outlined, label: 'תזמון'),
          _QuickAction(icon: Icons.dialpad_outlined,      label: 'חיוג'),
          _QuickAction(icon: Icons.favorite_border,       label: 'מועדפים'),
        ],
      ),
    );
  }
...
class _ThreadRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final arrowIcon = switch (thread.direction) {
      _Direction.outgoing => Icons.north_east_rounded,
      _Direction.incoming => Icons.south_west_rounded,
      _Direction.missed   => Icons.south_west_rounded,
    };
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            // Action icon — left side (trailing in RTL)
            Icon(Icons.chat_bubble_outline, color: Colors.white38, size: 22),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Text(
                        thread.time,
                        style: TextStyle(
                          color: missed ? BsTokens.brand : const Color(0xFF888888),
                          fontSize: 12,
                        ),
                      ),
                    ],
            ...
            // Avatar — right side (leading in RTL)
            Container(
              width: 48,
              height: 48,
  ...
  void _openChat(BuildContext context, _Thread thread) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => _ChatPage(thread: thread),
      ),
    );
  }
}
