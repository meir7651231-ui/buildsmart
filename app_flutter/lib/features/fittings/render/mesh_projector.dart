...
/// משולש מוכן-לציור: שלוש נקודות-מסך, עומק-מיון (far→near), וגורם-הצללה ב-[0,1].
@immutable
...
List<RenderTriangle> projectMeshes(
  ...
  for (final m in meshes) {
    for (var t = 0; t < m.indices.length; t += 3) {
      ...
      // נורמל-פאה בעולם (ממוצע נורמלי-הקודקודים) + מרכז לחישוב כיוון-העין.
  ...
  // painter's algorithm: מציירים את הרחוקים תחילה (עומק יורד).
