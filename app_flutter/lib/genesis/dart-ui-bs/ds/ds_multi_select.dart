                    ...
                    children: [
                      for (final id in ids)
                        Container(
                          ...
                          decoration: BoxDecoration(color: lk.accentSoft, borderRadius: BorderRadius.circular(20)),
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            Text(appStore.displayOf(entity, id).isEmpty ? id : appStore.displayOf(entity, id),
                                style: TextStyle(color: lk.accentDark, fontSize: 12.5, fontWeight: FontWeight.w700)),
                            const SizedBox(width: 4),
                            GestureDetector(
                              onTap: () => onChanged((ids.where((x) => x != id)).join(',')),
                              child: Icon(Icons.close, size: 15, color: lk.accentDark),
                            ),
                          ]),
                        ),
                    ],
                ...
                child: remaining.isEmpty
                    ? Padding(
                        ...
                        child: Text(opts.isEmpty ? 'אין $entity עדיין — הוסף כדי לקשר' : 'כל ה$entity נבחרו',
                            style: TextStyle(color: lk.faint, fontSize: 13)),
                      )
                    : DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          isExpanded: true,
                          value: null,
                          hint: Text('הוסף $entity', style: TextStyle(color: lk.faint, fontSize: 14)),
                          icon: Icon(Icons.add, color: lk.faint),
                          items: remaining
