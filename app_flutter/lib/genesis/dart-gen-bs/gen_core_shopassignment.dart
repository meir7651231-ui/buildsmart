...
/// דאטה-הגרעין של ShopAssignment — נגזר, לא מומצא; המצבים בסדר-ההצהרה של domain.ts
class _ShopAssignmentCore {
...
class ShopAssignmentCoreScreen extends StatefulWidget {
  ...
  @override
  State<ShopAssignmentCoreScreen> createState() => _ShopAssignmentCoreScreenState();
}

class _ShopAssignmentCoreScreenState extends State<ShopAssignmentCoreScreen> {
  String _state = _ShopAssignmentCore.states.first;
  @override
  Widget build(BuildContext context) {
    final next = _ShopAssignmentCore.next(_state);
    return DsScaffold(
      title: '🧠 ${_ShopAssignmentCore.term} · גרעין',
      subtitle: '${_ShopAssignmentCore.states.length} מצבים · ${_ShopAssignmentCore.relations.length} יחסים · ${_ShopAssignmentCore.rules.length} חוקים · ${_ShopAssignmentCore.channels.length} ערוצים',
      icon: '🧠',
      children: [
        DsSection(title: 'מחזור-חיים · status', children: [
          Wrap(spacing: 6, runSpacing: 6, children: [for (final s in _ShopAssignmentCore.states) StatusChip(label: s, tone: s == _state ? 1 : 0)]),
        ...
        DsSection(title: 'יחסים', children: [DsTable(labels: const ['שדה', 'יעד', 'איך'], rows: _ShopAssignmentCore.relations)]),
        DsSection(title: 'חוקים', children: [DsTable(labels: const ['סוג', 'שדה', 'פרטים'], rows: _ShopAssignmentCore.rules)]),
