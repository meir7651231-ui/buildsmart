...
class _GenAppPeruk03HomeScreenState extends State<GenAppPeruk03HomeScreen> {
  ...
  List<Map<String, String>> get _open => appStore.records('app_peruk03_ent1').where((r) => appStore.stageOf('app_peruk03_ent1', r[AppStore.idKey] ?? '') < 4).toList();
  ...
  List<_Item> _items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in _open) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { out.add(_Item('${f.label} · $who', gen_app_peruk03_home_c32.replaceAll('{date}', _iso(d)), rid, f.label, d, f.hard, true)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_Item('${f.label} · $who', off == 0 ? gen_app_peruk03_home_c33 : gen_app_peruk03_home_c34.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false)); break; }
        }
      }
    }
  ...
  List<Widget> _proposals(BuildContext context, DateTime today) {
    ...
    for (final r in _open) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk03_home_c35.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: who, okLabel: gen_app_peruk03_home_c36, noLabel: gen_app_peruk03_home_c37, alwaysLabel: gen_app_peruk03_home_c38,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk03_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk03_home_c39.replaceAll('{n}', n.toString()), source: who, okLabel: gen_app_peruk03_home_c40, noLabel: gen_app_peruk03_home_c41, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); _send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
      if (appStore.stageOf('app_peruk03_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk03_home_c42.replaceAll('{next}', gen_app_peruk03_home_c30), source: who, okLabel: gen_app_peruk03_home_c43, noLabel: gen_app_peruk03_home_c44, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk03_home_c45.replaceAll('{next}', gen_app_peruk03_home_c30), entity: 'app_peruk03_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }
  ...
  void _autopilot() {
    ...
    for (final r in _open) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk03_home_c46.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk03_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk03_home_c47, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk03_home_c48, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  void _act(_Item it, int i, DateTime today) {
    final labels = it.overdue ? [gen_app_peruk03_home_c49, gen_app_peruk03_home_c50, gen_app_peruk03_home_c51] : (it.due == today ? [gen_app_peruk03_home_c52] : [gen_app_peruk03_home_c53, gen_app_peruk03_home_c54]);   // P4 · ביום-ההכרעה אין דחייה
    final a = labels[i.clamp(0, labels.length - 1)];
    if (a == gen_app_peruk03_home_c55) { appStore.advance('app_peruk03_ent1', it.rid, 5); }
    else if (a == gen_app_peruk03_home_c56) { final r = appStore.byId('app_peruk03_ent1', it.rid); if (r != null) { final prev = r[it.field] ?? ''; appStore.update('app_peruk03_ent1', it.rid, {it.field: _iso(it.due.add(const Duration(days: 1)))}); appStore.logAction('auto', gen_app_peruk03_home_c57 + ' · ' + it.title, entity: 'app_peruk03_ent1', rid: it.rid, field: it.field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk03_home_c58 : n <= 1 ? gen_app_peruk03_home_c59 : gen_app_peruk03_home_c60.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk03_home_c61, subtitle: lead, icon: gen_app_peruk03_home_c62, children: [
      DsLoadMeter(count: n, label: gen_app_peruk03_home_c63.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk03_home_c64, gen_app_peruk03_home_c65, gen_app_peruk03_home_c66]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk03_home_c67, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: [gen_app_peruk03_home_c68, gen_app_peruk03_home_c69, gen_app_peruk03_home_c70], onAct: (i) => _act(it, i, today))]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk03_home_c71, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.due == today ? [gen_app_peruk03_home_c72] : [gen_app_peruk03_home_c73, gen_app_peruk03_home_c74], onAct: (i) => _act(it, i, today))]),
      for (final r in open) DsSection(title: (r[gen_app_peruk03_home_c0] ?? '') + ' · ' + const [gen_app_peruk03_home_c23, gen_app_peruk03_home_c24, gen_app_peruk03_home_c25, gen_app_peruk03_home_c26, gen_app_peruk03_home_c27][appStore.stageOf('app_peruk03_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], children: [
        ...
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => _send(context, r, r[AppStore.idKey] ?? ''), child: ForgeToneButton(items: [[gen_app_peruk03_home_c19]]))), const SizedBox(width: 8), Flexible(child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk03RootScreen(id: r[AppStore.idKey] ?? ''))), child: ForgeToneButton(items: [[gen_app_peruk03_home_c21]])))])),
      ]),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk03_home_c75 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk03_home_c76 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk03_home_c77, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk03_home_c78 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk03_home_c79 + ' ' + gen_app_peruk03_home_c80, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
