...
class GenAppPeruk03HomeScreenToday {
  ...
  static DsTodayItem _mk(String title, String sub, String rid, String field, DateTime d, bool hard, bool overdue, DateTime today) {
    final acts = overdue ? [gen_app_peruk03_home_c33, gen_app_peruk03_home_c34, gen_app_peruk03_home_c35] : (d == today ? [gen_app_peruk03_home_c36] : [gen_app_peruk03_home_c37, gen_app_peruk03_home_c38]);   // P4 · ביום-ההכרעה אין דחייה
  ...
  static void _act(String rid, String field, DateTime due, List<String> acts, int i) {
    ...
    if (a == gen_app_peruk03_home_c39) { appStore.advance('app_peruk03_ent1', rid, 5); }
    else if (a == gen_app_peruk03_home_c40) { final r = appStore.byId('app_peruk03_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk03_ent1', rid, {field: _iso(due.add(const Duration(days: 1)))}); appStore.logAction('auto', gen_app_peruk03_home_c41 + ' · ' + field, entity: 'app_peruk03_ent1', rid: rid, field: field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { out.add(_mk('${f.label} · $who', gen_app_peruk03_home_c42.replaceAll('{date}', _iso(d)), rid, f.label, d, f.hard, true, today)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk('${f.label} · $who', off == 0 ? gen_app_peruk03_home_c43 : gen_app_peruk03_home_c44.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today)); break; }
        }
      }
    }
  ...
  static List<Widget> proposals(BuildContext context, DateTime today) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk03_home_c45.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk03_home_c46, noLabel: gen_app_peruk03_home_c47, alwaysLabel: gen_app_peruk03_home_c48,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk03_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk03_home_c49.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk03_home_c50, noLabel: gen_app_peruk03_home_c51, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
      if (appStore.stageOf('app_peruk03_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk03_home_c52.replaceAll('{next}', gen_app_peruk03_home_c30), source: module + ' · ' + who, okLabel: gen_app_peruk03_home_c53, noLabel: gen_app_peruk03_home_c54, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk03_home_c55.replaceAll('{next}', gen_app_peruk03_home_c30), entity: 'app_peruk03_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }

  // כרטיס-הרשומה (G30): נוסחים · שלח · פתח — ≤2 הקשות
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: (((r[gen_app_peruk03_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk03_home_c56 : (r[gen_app_peruk03_home_c0] ?? '')) + ' · ' + const [gen_app_peruk03_home_c23, gen_app_peruk03_home_c24, gen_app_peruk03_home_c25, gen_app_peruk03_home_c26, gen_app_peruk03_home_c27][appStore.stageOf('app_peruk03_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], children: [
        ...
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk03_home_c57, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk03_home_c58, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk03RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk03_home_c59.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk03_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk03HomeScreenState extends State<GenAppPeruk03HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk03_home_c60, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk03_home_c61, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk03_home_c62 : n <= 1 ? gen_app_peruk03_home_c63 : gen_app_peruk03_home_c64.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk03_home_c65, subtitle: lead, icon: gen_app_peruk03_home_c66, children: [
      DsLoadMeter(count: n, label: gen_app_peruk03_home_c67.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk03_home_c68, gen_app_peruk03_home_c69, gen_app_peruk03_home_c70]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk03_home_c71, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk03_home_c72, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk03HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk03_home_c73 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk03_home_c74 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk03_home_c75, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk03_home_c76 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk03_home_c77 + ' ' + gen_app_peruk03_home_c78, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
