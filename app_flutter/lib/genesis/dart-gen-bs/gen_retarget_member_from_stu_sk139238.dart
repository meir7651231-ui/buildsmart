  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(
      ...
      children: [
        ...
        if (_metric != null) AlertBanner(glyph: '🎯', tone: 1, message: 'מסונן למדד: ${MemberFacts.metricDefs.firstWhere((d) => d['key'] == _metric, orElse: () => const {'label': ''})['label']} · ${visible.length} מתוך ${visibleAll.length}'),
        ...
        GradientCard(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ConstrainedBox(constraints: const BoxConstraints(maxWidth: 420), child: ForgeStatBlock(fields: ['בני משפחה בסיכון-גבוה — לפעול עכשיו', '${_StuData.highN}', ''])),
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [
          ...
          if (_StuData.exportOk(_role)) SoftButton(label: '⬇ CSV (${visible.length})', tone: 0, onTap: () => _showText(context, 'ייצוא CSV · ${visible.length} בני משפחה (BOM + חסימת-הזרקה)', _StuData.csvOf(visible))),
        ...
        if (_loading)
          ...
          const Padding(padding: EdgeInsets.only(top: 24), child: EmptyState(glyph: '🎓', message: 'אין בני משפחה עדיין — רשום תלמיד ראשון או ייבא'))
        else if (visible.isEmpty)
          const Padding(padding: EdgeInsets.only(top: 24), child: EmptyState(glyph: '🔍', message: 'אין בני משפחה תואמים לחיפוש/סינון'))
        else if (_mode == 1)
          _table(visible)
        else ...[
