...
class _ShopCriterionScreenState extends State<ShopCriterionScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: 'חוגים ומערכת', subtitle: '${live.length} חוגים חיים · ${_ShopCriterionData.teachers.length} מורים · ${_ShopCriterionData.rooms.where((r) => r['active'] == true).length} חדרים', icon: '📚', header: false, children: [ForgeCenteredPageHeader(fields: ['', 'חוגים ומערכת', '${live.length} חוגים חיים · ${_ShopCriterionData.teachers.length} מורים · ${_ShopCriterionData.rooms.where((r) => r['active'] == true).length} חדרים']), ...[
        ...
        if (_metric != null) ForgeSectionPill(fields: ['מסונן למדד: ${ShopCriterionFacts.metricDefs.firstWhere((d) => d['key'] == _metric, orElse: () => const {'label': ''})['label']} · ${visible.length} מתוך ${visibleAll.length}', '']),
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [
          ...
          ForgeIntelPill(fields: ['תפקיד: ${_ShopCriterionData.roleName(_role)}${_ShopCriterionData.myTeacherId(_role) != null ? ' · החוגים-שלי' : _ShopCriterionData.myFamilyId(_role) != null ? ' · המערכת-שלי' : ''}']),
        ]),
        _gap(6),
  ...
  List<Widget> _automations(List<Map<String, dynamic>> live) {
    ...
    return [
      ForgeTitledSection(fields: ['🤖 אוטומציות · ${hl.length + rem.length + bm.length + pr.length + noRoomCs.length + noTeacherCs.length + clashRooms.length + demand.length} אותות', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
        ...
        for (final c in bm)
          ForgeSectionPill(fields: ['${c['name']}: ${_ShopCriterionData.enrolled(c)} רשומים מול מינ׳ ${_ShopCriterionData.minToOpen(c)} · ${_ShopCriterionData.daysToStart(c) >= 0 ? 'מתחיל בעוד ${_ShopCriterionData.daysToStart(c)} ימים' : 'התחיל לפני ${-_ShopCriterionData.daysToStart(c)} ימים'} — לא-כלכלי', '']),
        // הצעת-חדר-חלופי: ללא-חדר / התנגשות-חדר ⇒ חדרים פנויים ב-slot
        for (final c in [...noRoomCs, ...clashRooms])
          ForgeSectionPill(fields: ['${c['name']} — ${_ShopCriterionData.noRoom(c) ? 'ללא-חדר' : 'התנגשות-חדר'} · חדר חלופי: ${_ShopCriterionData.freeRooms(c).isEmpty ? 'אין חדר פנוי ב-slot' : _ShopCriterionData.freeRooms(c).map((r) => '${r['name']} (${r['cap']})').join(' / ')}', '']),
        // הצעת-מורה-מחליף: ללא-מורה ⇒ מורים פנויים ב-slot (התמחות תואמת קודם)
        for (final c in noTeacherCs)
          ForgeSectionPill(fields: ['${c['name']} — ללא-מורה · מורה חלופי: ${_ShopCriterionData.freeTeachers(c).isEmpty ? 'אין מורה פנוי ב-slot' : _ShopCriterionData.freeTeachers(c).map((t) => '${t['name']}${t['specialty'] == c['cat'] ? ' ✓' : ''}').join(' / ')}', '']),
        // המתנה-עם-מקום ⇒ העלאה
        for (final c in pr) withAction(
          ForgeSectionPill(fields: ['${c['name']}: ${_ShopCriterionData.waitlist(c).length} ממתינים ויש ${_ShopCriterionData.capacity(c) - _ShopCriterionData.enrolled(c)} מקומות פנויים', '']),
        ...
        // תזכורות 48h
        if (rem.isNotEmpty) ForgeSectionPill(fields: ['תזכורת ל-48 השעות הקרובות: ${rem.map((r) => '${(r['course'] as Map)['name']} ${_ShopCriterionData._iso(r['dt'] as DateTime)} ${_ShopCriterionData.hm((r['dt'] as DateTime).hour * 60 + (r['dt'] as DateTime).minute)} (${_ShopCriterionData.liveEnrollmentsOf(r['course'] as Map<String, dynamic>).length} משפחות)').join(' · ')} — שלח-הודעה מהפאנל', '']),
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: FittedBox(
          ...
          child: ForgeIntelPill(fields: ['${cancelled ? '✖ ' : ''}${c['name']} ${_ShopCriterionData.enrolled(c)}/${_ShopCriterionData.capacity(c)}']),
        ),
      ),
    );
  }
  ...
  Widget _row(Map<String, dynamic> c) {
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        if (_ShopCriterionData.waitlist(c).isNotEmpty) ...[const SizedBox(width: 6), ForgeIntelPill(fields: ['⏳ ${_ShopCriterionData.waitlist(c).length}'])],
    ...
    return [
      ...
      if (_ShopCriterionData.noRoom(c) || _ShopCriterionData.clashesOf(c).any((k) => k['kind'] == 'room')) ...[
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          if (_ShopCriterionData.freeRooms(c).isEmpty) const ForgeIntelPill(fields: ['אין חדר פנוי']),
      ...
      if (_ShopCriterionData.noTeacher(c) || _ShopCriterionData.clashesOf(c).any((k) => k['kind'] == 'teacher')) ...[
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          if (_ShopCriterionData.freeTeachers(c).isEmpty) const ForgeIntelPill(fields: ['אין מורה פנוי']),
      ...
      if (_ShopCriterionData.belowMin(c)) ForgeSectionPill(fields: ['מתחת-למינימום: ${_ShopCriterionData.enrolled(c)} רשומים מול ${_ShopCriterionData.minToOpen(c)} (${c['minStudents'] == null ? 'נקודת-איזון: שכר-מורה+חדר ÷ מחיר-לשיעור' : 'מינימום-מוגדר'}) — לא-כלכלי', '']),
      _h('📅 השיעורים הבאים'),
