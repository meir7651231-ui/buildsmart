  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: 'Tzedaka', subtitle: '7 מודולים ממשפטים · כל אחד חצוב מהזהב', icon: '🧬', children: [
      ...
      Wrap(spacing: 12, runSpacing: 12, children: [ // KPI-רכזת (G9b): עובדות-אמת בלבד — כמו _Home של הזהב (מסכים-מחוברים + הדחוף של כל מודול)
        ...
        if (vis.contains(0)) SizedBox(width: 168, child: KpiTile(glyph: '🧬', value: TzCoordinatorFacts.hero, label: TzCoordinatorFacts.heroLabel)), // TzCoordinator · אין מדדים ⇒ count
        if (vis.contains(1)) SizedBox(width: 168, child: KpiTile(glyph: '🧬', value: TzCampaignFacts.hero, label: TzCampaignFacts.heroLabel)), // TzCampaign · המדד הראשון שהזהב צובע-סכנה כשאינו-אפס
        if (vis.contains(2)) SizedBox(width: 168, child: KpiTile(glyph: '🧬', value: ShopProductFacts.hero, label: ShopProductFacts.heroLabel)), // ShopProduct · המדד הראשון שהזהב צובע-סכנה כשאינו-אפס
        if (vis.contains(3)) SizedBox(width: 168, child: KpiTile(glyph: '🧬', value: ShopStoreFacts.hero, label: ShopStoreFacts.heroLabel)), // ShopStore · ה-StatHero של הזהב (המטרה המוצהרת)
        if (vis.contains(4)) SizedBox(width: 168, child: KpiTile(glyph: '🧬', value: TeacherFacts.hero, label: TeacherFacts.heroLabel)), // Teacher · ה-StatHero של הזהב (המטרה המוצהרת)
        if (vis.contains(5)) SizedBox(width: 168, child: KpiTile(glyph: '🧬', value: EnrollmentFacts.hero, label: EnrollmentFacts.heroLabel)), // Enrollment · אין מדדים ⇒ count
        if (vis.contains(6)) SizedBox(width: 168, child: KpiTile(glyph: '🧬', value: SupporterFacts.hero, label: SupporterFacts.heroLabel)), // Supporter · אין מדדים ⇒ count
      ]),
      const SizedBox(height: 8),
