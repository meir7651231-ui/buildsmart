  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(
      ...
      children: [
        ...
        if (_metric != null) ForgeSectionPill(items: [['מסונן למדד: ${ShopAssignmentFacts.metricDefs.firstWhere((d) => d['key'] == _metric, orElse: () => const {'label': ''})['label']} · ${visible.length} מתוך ${visibleAll.length}']], variants: const <int>[0]),
        ...
        ForgeTitledSection(fields: ['🧠 מחזור-חיים · ${ShopAssignmentCore.term} (גרעין)', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
          ...
          ForgeSectionPill(items: [['הבא אחרי ${ShopAssignmentCore.states.first}: ${ShopAssignmentCore.next(ShopAssignmentCore.states.first) ?? 'סופי'} · ${ShopAssignmentCore.rules.length} חוקים · ${ShopAssignmentCore.channels.length} ערוצים · ${ShopAssignmentCore.relations.length} יחסים']], variants: const <int>[0]),
        ]])),
        ...
        Row(children: [
          ...
          if (_TeamData.can(_role, 'team.add')) ...[const SizedBox(width: 6), Padding(padding: const EdgeInsets.only(bottom: 12), child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() => _TeamData.addTeacher(_who)), child: ForgeSoftButton(fields: ['➕ שיוך'])))],
          const SizedBox(width: 6),
            ...
            builder: (ctx, scroll) => Padding(
              ...
              child: ForgeStripPanelFrame(fields: ['', ''], child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                  ...
                  Builder(builder: (_) {
                    ...
                    if (!ShopAssignmentCore.states.contains(cur)) return ForgeSectionPill(items: [['מצב הרשומה "$cur" אינו במחזור-החיים החצוב (${ShopAssignmentCore.states.join('→')}) — פער זרע/סכמה, מקום-שמור']], variants: const <int>[0]);
                    ...
                    return ForgeTitledSection(fields: ['🧠 מחזור-חיים · רשומה (גרעין)', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
                      ...
                      ForgeSectionPill(items: [[nx == null ? 'מצב-סופי: $cur' : 'הבא אחרי $cur: $nx']], variants: const <int>[0]),
                      GestureDetector(behavior: HitTestBehavior.opaque, onTap: nx == null ? null : () => act(() => _coreState['${t['id']}'] = nx), child: ForgeSoftButton(fields: [nx == null ? 'אין מעבר' : 'קדם מצב ⇒ $nx'])),
                    ]]));
                  }),
                  Row(children: [
