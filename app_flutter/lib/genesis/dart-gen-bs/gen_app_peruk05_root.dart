...
String _fmtDate(String s) { final t = s.trim(); final d = DateTime.tryParse(t.length == 10 ? '${t}T12:00:00' : t); if (d == null) return t; final now = DateTime.now(); final iso = t.length >= 10 ? t.substring(0, 10) : t; final tIso = now.toIso8601String().substring(0, 10); final n = -cockpitDaysSince(iso, tIso).toInt(); if (n == 0) return gen_app_peruk05_root_c64; if (n == 1) return gen_app_peruk05_root_c65; if (n == -1) return gen_app_peruk05_root_c66; final dm = dayMonthOfIso(iso, d.year != now.year); return n.abs() <= 6 ? gen_app_peruk05_root_c67.replaceAll('{day}', gen_app_peruk05_root_c68.split(',')[weekdayOfIso(iso)]) + ' ' + dm : dm; }   // ב׳-מג · תאריך במילים בתיק · G34 · דבק על חלקיקים (ימים-מאז · יום-בשבוע · יום.חודש)
...
class GenAppPeruk05RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk05_root_c69, subtitle: gen_app_peruk05_root_c70, icon: gen_app_peruk05_root_c71, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk05_ent1', id), subtitle: const [gen_app_peruk05_root_c59, gen_app_peruk05_root_c60, gen_app_peruk05_root_c61, gen_app_peruk05_root_c62, gen_app_peruk05_root_c63][appStore.stageOf('app_peruk05_ent1', id).clamp(0, 4)], icon: gen_app_peruk05_root_c72, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.only(bottom: 10), child: DsQuickAdd(hint: gen_app_peruk05_root_c49, onSubmit: (t) { final v = t.trim(); if (v.isEmpty) return; final prev = r0['__note'] ?? ''; final stamp = DateTime.now().toIso8601String().substring(0, 10); appStore.update('app_peruk05_ent1', id, {'__note': (prev.isEmpty ? '' : prev + '\n') + stamp + ' · ' + v}); appStore.logAction('auto', gen_app_peruk05_root_c50.replaceAll('{who}', appStore.displayOf('app_peruk05_ent1', id)), entity: 'app_peruk05_ent1', rid: id, field: '__note', prev: prev); }))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: (appStore.stageOf('app_peruk05_ent1', id) >= 4 ? const SizedBox.shrink() : Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_peruk05_root_c51, onTap: () { final prev = r0[AppStore.stageKey] ?? '0'; appStore.update('app_peruk05_ent1', id, {AppStore.stageKey: '4'}); appStore.logAction('auto', gen_app_peruk05_root_c52.replaceAll('{who}', appStore.displayOf('app_peruk05_ent1', id)), entity: 'app_peruk05_ent1', rid: id, field: AppStore.stageKey, prev: prev); })])))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_peruk05_root_c53, onTap: () { final snap = Map<String, String>.from(r0); appStore.removeById('app_peruk05_ent1', id); appStore.logAction('del', gen_app_peruk05_root_c54.replaceAll('{who}', snap.values.take(2).join(' · ')), entity: 'app_peruk05_ent1', rid: id, prev: jsonEncode(snap)); Navigator.of(context).pop(); })]))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_peruk05_root_c55, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk05Ent1Screen(editId: id)))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk05_root_c56, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk05Ent1Screen(initial: {for (final e in r0.entries) if (!e.key.startsWith('__') && !const <String>[].contains(e.key)) e.key: e.value}))))]))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__note'] ?? '').trim().isNotEmpty ? DsFold(title: gen_app_peruk05_root_c57, details: [Text(_noteText(r0['__note'] ?? ''), style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, height: 1.5))]) : const SizedBox.shrink())),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk05_root_c58, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
