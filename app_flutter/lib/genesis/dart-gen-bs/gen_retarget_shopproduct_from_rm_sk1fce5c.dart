  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(
      ...
      children: [
        ...
        Align(
          alignment: Alignment.centerRight,
          child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: ForgeSegmentedPillToggleSelection(bare: true, items: [for (final s in [for (final r in _ShopProductData.roleDefs) r['label'] as String]) [s]], selected: {_role}, onSelect: (i) => setState(() => _role = i))),
        ),
        ...
        if (_view == 0) ...[
          ...
          Align(
            ...
            child: SingleChildScrollView(
              ...
              child: ForgeSegmentedPillToggleSelection(bare: true, items: [for (final s in [for (var i = 0; i < letters.length; i++) '${letters[i]} ${_ShopProductData.weekIsos[i].substring(8)}']) [s]], selected: {_dayIdx}, onSelect: (i) => setState(() => _dayIdx = i)),
            ),
          ),
        ],
        const SizedBox(height: 10),
        ...
        if (_loading)
          ...
          ForgeSearchEmptyState(fields: ['אין חדרים — הוסף מוצר ראשון', ''])
  ...
  List<Widget> _tabBody(BuildContext ctx, Map<String, dynamic> r, int tab, void Function(void Function()) act) {
    switch (tab) {
      ...
      default: // אודיט · פנקס-הפעולות של החדר (מי · מה · מתי)
        ...
        return au.isEmpty ? [ForgeSearchEmptyState(fields: ['אין רשומות-אודיט למוצר', ''])] : [_h('אודיט · ${au.length}'), for (final a in au) ForgeNotifRow(items: [['${a['what']}', '${a['when']}']])];
    }
  }
    ...
    showModalBottomSheet<void>(
      ...
      builder: (c2) => Padding(
        ...
        child: ForgeStripPanelFrame(fields: ['', ''], child: ListView(shrinkWrap: true, padding: const EdgeInsets.all(8), children: [
          ...
          _gap(8),
          if (alts.isEmpty)
            ForgeSearchEmptyState(fields: ['אין מוצר חלופי פנוי שמקיים קיבולת+ציוד במשבצת', ''])
          else
            for (final a in alts)
              Row(children: [
  ...
  List<Widget> _automationCenter() {
    ...
    return [
      ForgeTitledSection(fields: ['🚨 דורש-פעולה · ${_ShopProductData.actionItems}', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
        if (_ShopProductData.actionItems == 0) ForgeSearchEmptyState(fields: ['אין כפל-תפיסה · אין הזמנות ממתינות · כל שיעור במוצר עם הציוד הנדרש', '']),
