...
import '../dart-data-bs/auto/gen_app_peruk07_shell_content.dart';
...
import 'gen_app_peruk07_hub.dart';
...
class GenAppPeruk07ShellScreen extends StatefulWidget {
  ...
  @override
  State<GenAppPeruk07ShellScreen> createState() => _GenAppPeruk07ShellScreenState();
}

class _GenAppPeruk07ShellScreenState extends State<GenAppPeruk07ShellScreen> {
  int _t = 0;
  @override
  Widget build(BuildContext context) => CallbackShortcuts(   // G30 · D3/D4: ≤3 מקשים — T היום · I רשימה · A הוספה · Ctrl/Cmd+K פלטה
    bindings: <ShortcutActivator, VoidCallback>{
      ...
      const SingleActivator(LogicalKeyboardKey.keyA): () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk07Ent1Screen())),
      const SingleActivator(LogicalKeyboardKey.keyK, control: true): () => DsPalette.show(context, hint: gen_app_peruk07_shell_c15, items: [DsPaletteItem(label: gen_app_peruk07_shell_c13, sub: gen_app_peruk07_shell_c14, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk07Ent1Screen()))), for (final r in appStore.records('app_peruk07_ent1')) DsPaletteItem(label: (r[gen_app_peruk07_shell_c9] ?? ''), sub: (r[gen_app_peruk07_shell_c10] ?? ''), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk07RootScreen(id: r[AppStore.idKey] ?? ''))))]),
      const SingleActivator(LogicalKeyboardKey.keyK, meta: true): () => DsPalette.show(context, hint: gen_app_peruk07_shell_c16, items: [DsPaletteItem(label: gen_app_peruk07_shell_c13, sub: gen_app_peruk07_shell_c14, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk07Ent1Screen()))), for (final r in appStore.records('app_peruk07_ent1')) DsPaletteItem(label: (r[gen_app_peruk07_shell_c9] ?? ''), sub: (r[gen_app_peruk07_shell_c10] ?? ''), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk07RootScreen(id: r[AppStore.idKey] ?? ''))))]),
    },
    child: Focus(autofocus: true, child: Scaffold(
    backgroundColor: DsLook.of(context).bg,
    body: IndexedStack(index: _t.clamp(0, 2), children: [const GenAppPeruk07HomeScreen(), _RootTab(), const GenAppPeruk07HubScreen()]),
    bottomNavigationBar: SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(12, 6, 12, 10), child: Center(heightFactor: 1.0, child: ForgeMustChip(bare: true, items: [for (final s in [gen_app_peruk07_shell_c0, gen_app_peruk07_shell_c1, gen_app_peruk07_shell_c2]) [s]], selected: {_t}, onSelect: (i) => setState(() => _t = i))))),   // heightFactor: Center ללא-גובה מתפשט לכל הגובה שה-Scaffold מציע ⇒ הגוף נעלם
  )));
}
