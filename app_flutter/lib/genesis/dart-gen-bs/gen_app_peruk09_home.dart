...
import '../dart-data-bs/auto/gen_app_peruk09_home_content.dart';
...
class GenAppPeruk09HomeScreenToday {
  static const module = gen_app_peruk09_home_c13;
  static const _dates = [];
  static const List<String> _times = [];
  static const List<String> _phones = [gen_app_peruk09_home_c11];
  ...
  static String _dayLabel(DateTime d, DateTime today) { final n = -cockpitDaysSince(_iso(d), _iso(today)).toInt(); if (n == 0) return gen_app_peruk09_home_c14; if (n == 1) return gen_app_peruk09_home_c15; if (n == -1) return gen_app_peruk09_home_c16; final dm = dayMonthOfIso(_iso(d), d.year != today.year); return n.abs() <= 6 ? gen_app_peruk09_home_c17.replaceAll('{day}', gen_app_peruk09_home_c18.split(',')[weekdayOfIso(_iso(d))]) + ' ' + dm : dm; }   // G34 · דבק: ימים-מאז · יום-בשבוע · יום.חודש — חלקיקים; כאן רק מונחים
  static String _remKey(String rid, String field) => 'rem:$rid:$field';
  static List<Map<String, String>> open() => appStore.records('app_peruk09_ent1').where((r) => appStore.stageOf('app_peruk09_ent1', r[AppStore.idKey] ?? '') < 4).toList();
  static Future<void> send(BuildContext context, Map<String, String> r0, String id0) async {
    appStore.logAction('send', gen_app_peruk09_home_c12.replaceAll('{who}', appStore.displayOf('app_peruk09_ent1', id0)), entity: 'app_peruk09_ent1', rid: id0, prev: appStore.stageOf('app_peruk09_ent1', id0).toString());
    final text = reportTextGenAppPeruk09Rp1Screen(r0, id0);
    final dynamic url = waLink((r0[gen_app_peruk09_home_c1] ?? ''), text, waDigits);
  ...
  static DsTodayItem _mk(String title, String sub, String rid, String field, DateTime d, bool hard, bool overdue, DateTime today, [String time = '', bool rep = false, String phone = '', String money = '']) {
    final acts = <String>[...(overdue ? [gen_app_peruk09_home_c19, gen_app_peruk09_home_c20, gen_app_peruk09_home_c21, gen_app_peruk09_home_c22] : (d == today ? [gen_app_peruk09_home_c23, gen_app_peruk09_home_c24] : [gen_app_peruk09_home_c25, gen_app_peruk09_home_c26, gen_app_peruk09_home_c27])), if (phone.isNotEmpty) gen_app_peruk09_home_c28];   /* ב׳-לח · תיק עם טלפון ⇒ «התקשר» מהשורה, הקשה אחת */   // P4 · ביום-ההכרעה אין דחייה · «ליומן» = קישור-יומן, אפס-מפתח
  ...
  static void _act(String rid, String field, DateTime due0, List<String> acts, int i, [DateTime? today]) {
    ...
    if (a == gen_app_peruk09_home_c29) {
      final r0 = appStore.byId('app_peruk09_ent1', rid); final rep = (r0 == null ? '' : (r0['__repeat'] ?? '')).trim();
      final prevStage = r0 == null ? '' : (r0[AppStore.stageKey] ?? '0');
      appStore.advance('app_peruk09_ent1', rid, 5);
      appStore.decide('ign:$rid:$field', 'no');   // השורה של התאריך הזה טופלה — לא חוזרת מחר כ«באיחור»
      appStore.logAction('done', gen_app_peruk09_home_c30.replaceAll('{what}', field + ' · ' + appStore.displayOf('app_peruk09_ent1', rid)), entity: 'app_peruk09_ent1', rid: rid, field: field, prev: prevStage);   // «עשיתי» + החזר (השורה חוזרת, השלב חוזר)
      if (r0 != null && rep.isNotEmpty) {   // ↻ רגע חוזר: «סיים» יוצר את הבא לבד (המועד-הבא בשדה שנסגר), עם החזר
        ...
        final nid = appStore.add('app_peruk09_ent1', next);
        appStore.logAction('add', gen_app_peruk09_home_c31.replaceAll('{title}', appStore.displayOf('app_peruk09_ent1', nid) + ' · ' + next[field]!), entity: 'app_peruk09_ent1', rid: nid);
      }
    }
    else if (a == gen_app_peruk09_home_c32) { final r = appStore.byId('app_peruk09_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk09_ent1', rid, {field: _iso(_shift(_plus(due, 1), false))}); /* «דחה למחר» לא נוחת בשבת (אותו _shift של תזכורת-רכה) */ appStore.logAction('auto', gen_app_peruk09_home_c33 + ' · ' + field, entity: 'app_peruk09_ent1', rid: rid, field: field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
    else if (a == gen_app_peruk09_home_c34) { final r = appStore.byId('app_peruk09_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk09_ent1', rid, {field: _iso(_shift(_plus(due, 7), false))}); appStore.logAction('auto', gen_app_peruk09_home_c35 + ' · ' + field, entity: 'app_peruk09_ent1', rid: rid, field: field, prev: prev); } }   /* «דחה לשבוע» — נגיעה-ידנית עם החזר, לא בשבת */
    else if (a == gen_app_peruk09_home_c36) {   // «ליומן»: עם שעה ⇒ אירוע בשעתו (אורך = בלוק-ההגדרה); בלי ⇒ יום-שלם
      final r = appStore.byId('app_peruk09_ent1', rid); final tm = r == null ? '' : _timeOf(r); final d = _iso(due0).replaceAll('-', '');
      ...
      launchUrl(Uri.parse('https://calendar.google.com/calendar/render?action=TEMPLATE&text=' + Uri.encodeComponent(field + ' · ' + appStore.displayOf('app_peruk09_ent1', rid)) + '&dates=' + dates), mode: LaunchMode.externalApplication);
    }
    else if (a == gen_app_peruk09_home_c37) { final r = appStore.byId('app_peruk09_ent1', rid); final ph = r == null ? '' : _phoneOf(r); if (ph.isNotEmpty) launchUrl(Uri.parse('tel:' + ph), mode: LaunchMode.externalApplication); }
    else { appStore.decide('ign:$rid:$field', 'no'); appStore.logAction('decide', gen_app_peruk09_home_c38.replaceAll('{what}', field + ' · ' + appStore.displayOf('app_peruk09_ent1', rid)), entity: 'app_peruk09_ent1', rid: rid, field: 'ign:$rid:$field'); }   /* ב׳-לב · «התעלם» נרשם ביומן ⇒ החזר (kind=decide: field = מפתח-ההכרעה) — שום דבר לא בלתי-הפיך */
  }
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk09_ent1', rid); final tm = _timeOf(r); final rep = (r['__repeat'] ?? '').trim().isNotEmpty; final ph = _phoneOf(r); final mo = _moneyOf(r);
      for (final f in _dates) {
        final d = _parse(r[f.label] ?? ''); if (d == null) continue;
        if (appStore.decision('ign:$rid:${f.label}') == 'no') continue;
        if (dayDelta == 0 && d.isBefore(today)) { final ago = cockpitDaysSince(_iso(d), _iso(today)).toInt(); out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', gen_app_peruk09_home_c39.replaceAll('{date}', _dayLabel(d, today)) + (ago <= 1 ? '' : ' · ' + gen_app_peruk09_home_c40.replaceAll('{n}', ago.toString()))   /* ב׳-עז · «היה אתמול» כבר אומר הכל — בלי «· אתמול» כפול */, rid, f.label, d, f.hard, true, today, tm, rep, ph, mo)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == _plus(today, dayDelta)) { out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', off == 0 ? '' : gen_app_peruk09_home_c41.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep, ph, mo)); break; }
        }
      }
    }
  ...
  static List<DsTodayItem> undated(DateTime today) {
    ...
    for (final r in open()) {
      ...
      final acts = [gen_app_peruk09_home_c42, gen_app_peruk09_home_c43, gen_app_peruk09_home_c44];
  ...
  static void _setDate(String rid, String field, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    if (a == gen_app_peruk09_home_c45) { appStore.decide('undated:$rid', 'no'); appStore.logAction('decide', gen_app_peruk09_home_c46.replaceAll('{what}', appStore.displayOf('app_peruk09_ent1', rid)), entity: 'app_peruk09_ent1', rid: rid, field: 'undated:$rid'); return; }
    final r = appStore.byId('app_peruk09_ent1', rid); if (r == null) return;
    final prev = r[field] ?? '';
    appStore.update('app_peruk09_ent1', rid, {field: _iso(_shift(_plus(today, a == gen_app_peruk09_home_c47 ? 7 : 1), false))});   /* «קבע» לא נוחת בשבת */
    appStore.logAction('auto', a + ' · ' + field, entity: 'app_peruk09_ent1', rid: rid, field: field, prev: prev);
  }
  ...
  static List<DsTodayItem> stale(DateTime today) {
    ...
    for (final r in open()) {
      ...
      final acts = [gen_app_peruk09_home_c48, gen_app_peruk09_home_c49, gen_app_peruk09_home_c50];
      out.add(DsTodayItem(title: appStore.displayOf('app_peruk09_ent1', rid), sub: [gen_app_peruk09_home_c51.replaceAll('{n}', n.toString()), _moneyOf(r)].where((x) => x.isNotEmpty).join(' · '), rid: rid, field: '', due: t, hard: false, overdue: false, module: module, actions: acts, act: (i) => _staleAct(rid, today, acts, i)));
    }
  ...
  static void _staleAct(String rid, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    final r = appStore.byId('app_peruk09_ent1', rid); if (r == null) return;
    if (a == gen_app_peruk09_home_c52) { final f = _dates.firstWhere((x) => x.hard, orElse: () => _dates.first); appStore.decide('ign:$rid:${f.label}', ''); appStore.decide('undated:$rid', ''); _setDate(rid, f.label, today, [a], 0); return; }   /* המועד החדש חוזר ל«היום» — ההתעלמות הישנה נמחקת */
    if (a == gen_app_peruk09_home_c53) { final prev = r[AppStore.stageKey] ?? '0'; appStore.update('app_peruk09_ent1', rid, {AppStore.stageKey: '4'}); appStore.logAction('auto', gen_app_peruk09_home_c54.replaceAll('{who}', appStore.displayOf('app_peruk09_ent1', rid)), entity: 'app_peruk09_ent1', rid: rid, field: AppStore.stageKey, prev: prev); return; }
    appStore.decide('stale:$rid', 'no'); appStore.logAction('decide', gen_app_peruk09_home_c55.replaceAll('{what}', appStore.displayOf('app_peruk09_ent1', rid)), entity: 'app_peruk09_ent1', rid: rid, field: 'stale:$rid');
  }
  ...
  static List<DsTodayItem> remPending(DateTime today) {
    final out = <DsTodayItem>[];
    for (final r in open()) {
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk09_ent1', rid);
      for (final f in _dates) {
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true, bool rem = true}) {   // rem=false: «בלגן» מרכז את התזכורות לכרטיס אחד (ב׳-לז)   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    final out = <Widget>[];
    String days(List<int> os) => os.map((o) => o == 0 ? gen_app_peruk09_home_c56 : o == 1 ? gen_app_peruk09_home_c57 : gen_app_peruk09_home_c58.replaceAll('{n}', o.toString())).join(' · ');   /* ב׳-מב · «3 ימים לפני · יום לפני · ביום» במקום (−3/−1/−0) · ב׳-צא · רק מה שעוד לפנינו */
    for (final r in open()) {
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk09_ent1', rid);
      if (rem) for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk09_home_c59.replaceAll('{field}', f.label).replaceAll('{days}', days(ahead)).replaceAll('{date}', _dayLabel(d, today)), source: module + ' · ' + who, okLabel: gen_app_peruk09_home_c60, noLabel: gen_app_peruk09_home_c61, alwaysLabel: gen_app_peruk09_home_c62,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : cockpitDaysSince(_iso(_day(at)), _iso(today)).toInt(); if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk09_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk09_home_c63.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk09_home_c64, noLabel: gen_app_peruk09_home_c65, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
    
    return out;
  }
  /// רשומות שהגיעו לשלב-האחרון ובלי הכרעת-צעד-הבא — ל«בלגן» (שרשרת חוצת-מודולים)
  static List<Map<String, String>> done() => appStore.records('app_peruk09_ent1').where((r) => appStore.stageOf('app_peruk09_ent1', r[AppStore.idKey] ?? '') >= 4 && appStore.decision('next:${r[AppStore.idKey] ?? ''}').isEmpty).toList();

  // כרטיס-הרשומה (G30): נוסחים · שלח · פתח — ≤2 הקשות
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: module + ' · ' + (((r[gen_app_peruk09_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk09_home_c66 : (r[gen_app_peruk09_home_c0] ?? '')), trailing: Text(const [gen_app_peruk09_home_c6, gen_app_peruk09_home_c7, gen_app_peruk09_home_c8, gen_app_peruk09_home_c9, gen_app_peruk09_home_c10][appStore.stageOf('app_peruk09_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), children: [
        
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk09_home_c67, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk09_home_c68, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk09RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk09_home_c69.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk09_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}

class GenAppPeruk09HomeScreen extends StatefulWidget {
  const GenAppPeruk09HomeScreen({super.key});
  @override
  State<GenAppPeruk09HomeScreen> createState() => _GenAppPeruk09HomeScreenState();
}

class _GenAppPeruk09HomeScreenState extends State<GenAppPeruk09HomeScreen> {
  static DateTime _day(DateTime d) => DateTime(d.year, d.month, d.day);
  static String _iso(DateTime d) => d.toIso8601String().substring(0, 10);
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk09_home_c70, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk09_home_c71, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
      appStore.setSetting('digestShown', key);
    } catch (_) {}
  }

  @override
  void initState() { super.initState(); WidgetsBinding.instance.addPostFrameCallback((_) { GenAppPeruk09HomeScreenToday.autopilot(); }); appStore.addListener(_onStore); }
  void _onStore() { WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) GenAppPeruk09HomeScreenToday.autopilot(); }); }
  @override
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    final today = _day(DateTime.now());
    final open = GenAppPeruk09HomeScreenToday.open();
    final all0 = GenAppPeruk09HomeScreenToday.items(today, dayDelta: 0);
    final overdue = all0.where((x) => x.overdue).toList();
    final todayItems = all0.where((x) => !x.overdue).toList();
    final tomorrow = GenAppPeruk09HomeScreenToday.items(today, dayDelta: 1);
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk09_home_c72 : n <= 1 ? gen_app_peruk09_home_c73 : gen_app_peruk09_home_c74.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk09_home_c75, subtitle: lead, icon: gen_app_peruk09_home_c76, children: [
      DsLoadMeter(count: n, label: gen_app_peruk09_home_c77.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk09_home_c78, gen_app_peruk09_home_c79, gen_app_peruk09_home_c80]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk09_home_c81, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk09_home_c82, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk09HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk09_home_c83 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk09_home_c84 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk09_home_c85, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk09_home_c86 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk09_home_c87 + ' ' + gen_app_peruk09_home_c88, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
