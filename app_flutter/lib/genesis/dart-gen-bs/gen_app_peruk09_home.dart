...
class GenAppPeruk09HomeScreenToday {
  ...
  static String _dayLabel(DateTime d, DateTime today) { final n = _day(d).difference(_day(today)).inDays; if (n == 0) return gen_app_peruk09_home_c14; if (n == 1) return gen_app_peruk09_home_c15; if (n == -1) return gen_app_peruk09_home_c16; final dm = '${d.day}.${d.month}' + (d.year == today.year ? '' : '.${d.year}'); return n.abs() <= 6 ? gen_app_peruk09_home_c17.replaceAll('{day}', gen_app_peruk09_home_c18.split(',')[d.weekday % 7]) + ' ' + dm : dm; }
  ...
  static void _act(String rid, String field, DateTime due0, List<String> acts, int i, [DateTime? today]) {
    ...
    else if (a == gen_app_peruk09_home_c32) { final r = appStore.byId('app_peruk09_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk09_ent1', rid, {field: _iso(_shift(due.add(const Duration(days: 1)), false))}); /* «דחה למחר» לא נוחת בשבת (אותו _shift של תזכורת-רכה) */ appStore.logAction('auto', gen_app_peruk09_home_c33 + ' · ' + field, entity: 'app_peruk09_ent1', rid: rid, field: field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
    else if (a == gen_app_peruk09_home_c34) { final r = appStore.byId('app_peruk09_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk09_ent1', rid, {field: _iso(_shift(due.add(const Duration(days: 7)), false))}); appStore.logAction('auto', gen_app_peruk09_home_c35 + ' · ' + field, entity: 'app_peruk09_ent1', rid: rid, field: field, prev: prev); } }   /* «דחה לשבוע» — נגיעה-ידנית עם החזר, לא בשבת */
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { final ago = today.difference(d).inDays; out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', gen_app_peruk09_home_c39.replaceAll('{date}', _dayLabel(d, today)) + (ago <= 1 ? '' : ' · ' + gen_app_peruk09_home_c40.replaceAll('{n}', ago.toString()))   /* ב׳-עז · «היה אתמול» כבר אומר הכל — בלי «· אתמול» כפול */, rid, f.label, d, f.hard, true, today, tm, rep, ph, mo)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', off == 0 ? '' : gen_app_peruk09_home_c41.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep, ph, mo)); break; }
        }
      }
    }
  ...
  static void _setDate(String rid, String field, DateTime today, List<String> acts, int i) {
    ...
    final prev = r[field] ?? '';
    appStore.update('app_peruk09_ent1', rid, {field: _iso(_shift(today.add(Duration(days: a == gen_app_peruk09_home_c47 ? 7 : 1)), false))});   /* «קבע» לא נוחת בשבת */
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true, bool rem = true}) {   // rem=false: «בלגן» מרכז את התזכורות לכרטיס אחד (ב׳-לז)   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk09_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk09_home_c63.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk09_home_c64, noLabel: gen_app_peruk09_home_c65, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
