...
class GenAppPeruk26HomeScreenToday {
  ...
  static String _dayLabel(DateTime d, DateTime today) { final p = bhDayLabelParts(_iso(d), _iso(today)); switch (p[0]) { case 'today': return gen_app_peruk26_home_c14; case 'tomorrow': return gen_app_peruk26_home_c15; case 'yesterday': return gen_app_peruk26_home_c16; case 'weekday': return gen_app_peruk26_home_c17.replaceAll('{day}', gen_app_peruk26_home_c18.split(',')[int.parse(p[1])]) + ' ' + p[2]; default: return p[2]; } }   // G34ב · שכבת-ההרכבה מחזירה חלקים; כאן רק מונחים
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true, bool rem = true}) {   // rem=false: «בלגן» מרכז את התזכורות לכרטיס אחד (ב׳-לז)   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : bhDaysSince(_iso(_day(at)), _iso(today)); if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk26_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk26_home_c63.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk26_home_c64, noLabel: gen_app_peruk26_home_c65, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
