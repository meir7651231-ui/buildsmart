...
class GenAppPeruk26HomeScreenToday {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', off == 0 ? '' : gen_app_peruk26_home_c41.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep, ph, mo)); break; }
        }
      }
    }
  ...
  static void _setDate(String rid, String field, DateTime today, List<String> acts, int i) {
    ...
    final prev = r[field] ?? '';
    appStore.update('app_peruk26_ent1', rid, {field: _iso(_shift(today.add(Duration(days: a == gen_app_peruk26_home_c47 ? 7 : 1)), false))});   /* «קבע» לא נוחת בשבת */
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true, bool rem = true}) {   // rem=false: «בלגן» מרכז את התזכורות לכרטיס אחד (ב׳-לז)   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk26_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk26_home_c63.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk26_home_c64, noLabel: gen_app_peruk26_home_c65, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
