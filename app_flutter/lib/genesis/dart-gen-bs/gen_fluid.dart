// 🌊 נוזל-אמת אינטראקטיבי (Navier-Stokes, Stam) — חיווט של פעולת-ממוצע קיימת על-פני רשת.
// תובנת-הבעלים: "ערבב-שכנים = ממוצע". הממוצע פעולה-קיימת (cf. supAvgDon / סכומים / reduce);
// איסוף-4-השכנים (field[i±1], field[i±W]) = גישה-למערך (חיווט); הלולאה על הרשת = איטרציה (חיווט);
// החזקת-השדה בין-פריימים = מצב (חיווט). אפס אלגוריתם-מומצא — כל צעד הוא ממוצע-שכנים-שנאספו:
//   • diffuse/project = ממצע כל תא עם 4 שכניו, N סבבים.
//   • advect         = צעד-אחורה (כמו confetti pos−=v·dt) + ממוצע-משוקלל של 4 הפינות (bilinear).
// אצבע מזריקה צפיפות+מהירות; הצבע זורם, מתפזר ומסתחרר — נוזל משמר-מסה.
import 'dart:async';
...
class _GenFluidState extends State<GenFluid> {
  static const int N = 64, W = N + 2;
  static const double dt = 0.12, diff = 0.00005, visc = 0.00003, fade = 0.985;
  static const int iters = 16;

  late List<double> u, v, u0, v0, dens, dens0; // ← השדה, מוחזק בין-פריימים (מצב=חיווט)
  Timer? _loop;
  Offset? _prev;
  int _frames = 0, _fps = 0;
  late DateTime _fpsMark;

  int ix(int i, int j) => i + W * j; // אינדקס-רשת (חיווט טריוויאלי)
  ...
  // ── ליבת-הנוזל: ממצע כל תא עם 4 שכניו (הפעולה הקיימת), N סבבים ──
  void _linSolve(int b, List<double> x, List<double> x0, double a, double c) {
    for (var k = 0; k < iters; k++) {
      for (var j = 1; j <= N; j++) {
        for (var i = 1; i <= N; i++) {
          // ממוצע-משוקלל של 4 השכנים (שנאספו באינדקס) + המקור
          x[ix(i, j)] = (x0[ix(i, j)] +
  ...
  // advect: צעד-אחורה (כמו confetti) + ממוצע-משוקלל של 4 הפינות (bilinear = ממוצע)
  ...
  void _project(List<double> uu, List<double> vv, List<double> p, List<double> div) {
    ...
    _setBnd(0, div);
    _setBnd(0, p);
    _linSolve(0, p, div, 1, 4); // אותו ממוצע-שכנים
    for (var j = 1; j <= N; j++) {
      for (var i = 1; i <= N; i++) {
        ...
        child: Column(
          children: [
            Padding(
              ...
              child: BareStat(
                value: '$_fps',
                label: '🌊 נוזל-אמת · ${N}² · fps · גרור לצייר',
                inkColor: const Color(0xFF66D9E8),
