...
class GenAppCalendarRootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_calendar_root_c29, subtitle: gen_app_calendar_root_c30, icon: gen_app_calendar_root_c31, children: const []);
    return DsScaffold(title: appStore.displayOf('app_calendar_ent1', id), subtitle: const [gen_app_calendar_root_c27, gen_app_calendar_root_c28][appStore.stageOf('app_calendar_ent1', id).clamp(0, 1)], icon: gen_app_calendar_root_c32, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_calendar_root_c26, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
