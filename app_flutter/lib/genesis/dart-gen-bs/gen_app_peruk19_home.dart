...
class GenAppPeruk19HomeScreenToday {
  ...
  static void _act(String rid, String field, DateTime due, List<String> acts, int i) {
    ...
    else if (a == gen_app_peruk19_home_c28) { final d = _iso(due).replaceAll('-', ''); launchUrl(Uri.parse('https://calendar.google.com/calendar/render?action=TEMPLATE&text=' + Uri.encodeComponent(field + ' · ' + appStore.displayOf('app_peruk19_ent1', rid)) + '&dates=' + d + '/' + d), mode: LaunchMode.externalApplication); }
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { out.add(_mk('${f.label} · $who', gen_app_peruk19_home_c29.replaceAll('{date}', _iso(d)), rid, f.label, d, f.hard, true, today, tm, rep)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk('${f.label} · $who', off == 0 ? gen_app_peruk19_home_c30 : gen_app_peruk19_home_c31.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep)); break; }
        }
      }
    }
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true}) {   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk19_home_c32.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk19_home_c33, noLabel: gen_app_peruk19_home_c34, alwaysLabel: gen_app_peruk19_home_c35,
    ...
    if (chain) for (final r in appStore.records('app_peruk19_ent1')) {   // P14 · הצעד-הבא: רשומה שהגיעה לשלב-האחרון (סגורה — לא ב-open) ובלי הכרעה
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk19_ent1', rid);
      if (appStore.stageOf('app_peruk19_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk19_home_c39.replaceAll('{next}', gen_app_peruk19_home_c12), source: module + ' · ' + who, okLabel: gen_app_peruk19_home_c40, noLabel: gen_app_peruk19_home_c41, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk19_home_c42.replaceAll('{next}', gen_app_peruk19_home_c12), entity: 'app_peruk19_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }
  ...
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: module + ' · ' + (((r[gen_app_peruk19_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk19_home_c43 : (r[gen_app_peruk19_home_c0] ?? '')), trailing: Text(const [gen_app_peruk19_home_c6, gen_app_peruk19_home_c7, gen_app_peruk19_home_c8, gen_app_peruk19_home_c9, gen_app_peruk19_home_c10][appStore.stageOf('app_peruk19_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), children: [
        
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk19_home_c44, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk19_home_c45, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk19RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk19_home_c46.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk19_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk19HomeScreenState extends State<GenAppPeruk19HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk19_home_c47, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk19_home_c48, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk19_home_c49 : n <= 1 ? gen_app_peruk19_home_c50 : gen_app_peruk19_home_c51.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk19_home_c52, subtitle: lead, icon: gen_app_peruk19_home_c53, children: [
      DsLoadMeter(count: n, label: gen_app_peruk19_home_c54.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk19_home_c55, gen_app_peruk19_home_c56, gen_app_peruk19_home_c57]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk19_home_c58, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk19_home_c59, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk19HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk19_home_c60 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk19_home_c61 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk19_home_c62, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk19_home_c63 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk19_home_c64 + ' ' + gen_app_peruk19_home_c65, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
