...
class GenAppPeruk23HomeScreenToday {
  ...
  static void _act(String rid, String field, DateTime due, List<String> acts, int i) {
    ...
    else if (a == gen_app_peruk23_home_c25) { final r = appStore.byId('app_peruk23_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk23_ent1', rid, {field: _iso(due.add(const Duration(days: 1)))}); appStore.logAction('auto', gen_app_peruk23_home_c26 + ' · ' + field, entity: 'app_peruk23_ent1', rid: rid, field: field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', off == 0 ? gen_app_peruk23_home_c31 : gen_app_peruk23_home_c32.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep)); break; }
        }
      }
    }
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true}) {   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk23_home_c33.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk23_home_c34, noLabel: gen_app_peruk23_home_c35, alwaysLabel: gen_app_peruk23_home_c36,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk23_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk23_home_c37.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk23_home_c38, noLabel: gen_app_peruk23_home_c39, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
    if (chain) for (final r in appStore.records('app_peruk23_ent1')) {   // P14 · הצעד-הבא: רשומה שהגיעה לשלב-האחרון (סגורה — לא ב-open) ובלי הכרעה
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk23_ent1', rid);
      if (appStore.stageOf('app_peruk23_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk23_home_c40.replaceAll('{next}', gen_app_peruk23_home_c11), source: module + ' · ' + who, okLabel: gen_app_peruk23_home_c41, noLabel: gen_app_peruk23_home_c42, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk23_home_c43.replaceAll('{next}', gen_app_peruk23_home_c11), entity: 'app_peruk23_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }
  ...
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: module + ' · ' + (((r[gen_app_peruk23_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk23_home_c44 : (r[gen_app_peruk23_home_c0] ?? '')), trailing: Text(const [gen_app_peruk23_home_c6, gen_app_peruk23_home_c7, gen_app_peruk23_home_c8, gen_app_peruk23_home_c9, gen_app_peruk23_home_c10][appStore.stageOf('app_peruk23_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), children: [
        
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk23_home_c45, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk23_home_c46, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk23RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk23_home_c47.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk23_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk23HomeScreenState extends State<GenAppPeruk23HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk23_home_c48, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk23_home_c49, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk23_home_c50 : n <= 1 ? gen_app_peruk23_home_c51 : gen_app_peruk23_home_c52.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk23_home_c53, subtitle: lead, icon: gen_app_peruk23_home_c54, children: [
      DsLoadMeter(count: n, label: gen_app_peruk23_home_c55.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk23_home_c56, gen_app_peruk23_home_c57, gen_app_peruk23_home_c58]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk23_home_c59, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk23_home_c60, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk23HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk23_home_c61 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk23_home_c62 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk23_home_c63, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk23_home_c64 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk23_home_c65 + ' ' + gen_app_peruk23_home_c66, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
