// 🎯 AyinCaseScreen — retarget של schoolos_fees.dart לישות AyinCase (GENMAX·G5c/G5d · הכרעה-24) · מחולל דטרמיניסטי: retarget.mjs --module schoolos_fees.dart --entity AyinCase
//   זרע-ראשי: families (מועמדים: families(36/38) charges(9/10) charges(7/8) charges(7/8) charges(7/8) charges(7/8) charges(7/8) incoming(7/8) charges(6/7) hist(5/5) criteria(3/3) calls(3/3) payments(3/4) payments(3/4) payments(3/4) payments(3/4) calls(3/3) payments(3/4)) · מיפוי שם 1 · ערוץ 0 · טיפוס-יחיד 1 · מקום-שמור 16 · חוזה-מנוע (לא משתנה) 20
//   note⇒note(name) · id⇒∅(engine-contract) · name⇒∅(engine-contract) · phone⇒∅(engine-contract) · email⇒∅(engine-contract) · idNum⇒∅(engine-contract) · date⇒∅(engine-contract) · amount⇒∅(engine-contract) · cur⇒∅(engine-contract) · cat⇒∅(engine-contract) · hok⇒∅(engine-contract) · day⇒∅(engine-contract) · active⇒∅(engine-contract) · carryBalance⇒∅(engine-contract) · kevaId⇒∅(engine-contract) · hist⇒∅(engine-contract) · d⇒∅(engine-contract) · a⇒∅(engine-contract) · c⇒∅(engine-contract) · clearer⇒∅(engine-contract) · nextDate⇒∅(engine-contract) · payer⇒answeredNote(unique) · members⇒∅(reserved(6 מועמדים)) · grade⇒∅(reserved) · first⇒∅(reserved) · charges⇒∅(reserved(6 מועמדים)) · method⇒∅(reserved) · memberId⇒∅(reserved) · payments⇒∅(reserved(6 מועמדים)) · rid⇒∅(reserved) · startedAt⇒∅(reserved(2 מועמדים)) · criteria⇒∅(reserved(6 מועמדים)) · calls⇒∅(reserved) · at⇒∅(reserved(2 מועמדים)) · outcome⇒∅(reserved) · nextNote⇒∅(reserved) · installmentOf⇒∅(reserved) · cancelledAt⇒∅(reserved(2 מועמדים))
//   עור-forge (G12c/e): BareStat⇒ForgeStatPlain ×0 (ב-Wrap) · ×21 (ב-Row, Expanded) · פנימיים: button×22 statusChip×16 banner×16 emptyState×9 mediaRow×9 · StatHero⇒ForgeStatPlain ×0 · KpiTile⇒— · DsNavTile⇒— — fields לפי תפקידי-חריצים; צבעי-מצב-DS לא מועברים
//   תפר-עובדות (G9b): AyinCaseFacts · count=families.length (static-const) · מדדים 0 · hero=count · שורות-מדד (G10a) ∅ · תפר-כניסה initialPanelId · תפר-סינון-מדד ∅ · תפר-הזרקה ∅
//   שדות-AyinCase בלי מקור (מקום-שמור, יאירו כשיוזרם נתון): stage, paid, answerPushed, nextTalk, nextTalkTime, lastTouch, names, answers, log, time, mat, kit, boardEventIds · תוויות: מונחי Supporter (תורם/—) ⇒ AyinCase (תיק/—) · 0 החלפות · הזרע = זרע-הצבה של המקור, לא ערך-אמת של AyinCase
...
// G17c · מטרה (צעד 1, אדם): כל חוזה שנשלח הוא תיק עם שלב: התקבל, שולם, בבדיקה, נמסר, תיקון — ולראות מה דחוף להיום · פעולות-יסוד (צעד 2, אדם): יתרה · ייצוא ⇒ חלקיקים (המנוע): fee.balance · fee.export
...
import 'gen_core_ayincase.dart'; // G6c · הגרעין-מהסכמה של AyinCase (מצבים · מעבר · חוקים · ערוצים)
...
// ═══════════════════════════════════════════════════════════════════════════════════════
// 🧮 _AyinCaseData — לוגיקה-טהורה + חוזה-דאטה (אפס-DOM). מקורות-אמת (§20-ג · אפס-זיוף):
...
class _AyinCaseData {
  ...
  static const families = <Map<String, dynamic>>[
    {
      'id': 'f1', 'name': 'משפחת כהן', 'answeredNote': 'דוד כהן', 'phone': '050-1111111', 'email': 'cohen@family', 'idNum': '012345678',
    ...
    {
      'id': 'f2', 'name': 'משפחת לוי', 'answeredNote': 'רחל לוי', 'phone': '052-2222222', 'email': 'levi@family', 'idNum': '023456789',
    ...
    {
      'id': 'f3', 'name': 'משפחת מזרחי', 'answeredNote': 'יוסי מזרחי', 'phone': '054-3333333', 'email': 'mizrahi@family', 'idNum': '034567890',
    ...
    {
      'id': 'f4', 'name': 'משפחת אברהם', 'answeredNote': 'מרים אברהם', 'phone': '053-4444444', 'email': 'avraham@family', 'idNum': '045678901',
    ...
    {
      'id': 'f5', 'name': 'משפחת פרץ', 'answeredNote': 'אבי פרץ', 'phone': '058-5555555', 'email': 'peretz@family', 'idNum': '056789012',
    ...
    {
      'id': 'f6', 'name': 'משפחת שמעוני', 'answeredNote': 'גלית שמעוני', 'phone': '050-6666666', 'email': 'shimoni@family', 'idNum': '067890123',
    ...
    {
      'id': 'f7', 'name': 'משפחת ביטון', 'answeredNote': 'שלומי ביטון', 'phone': '052-7777777', 'email': 'biton@family', 'idNum': '078901234',
    ...
    {
      'id': 'f8', 'name': 'משפחת נחום', 'answeredNote': 'הדס נחום', 'phone': '054-8888888', 'email': 'nahum@family', 'idNum': '089012345',
  ...
  static String? payLinkOf(Map<String, dynamic> f) => payLink(payUrl, balance(f), f['answeredNote'] as String, _safeHttps);
  ...
  static List<String> _termsOf(Map<String, dynamic> f) =>
      ['${f['name']}', '${f['answeredNote']}', '${f['phone']}', ...(f['members'] as List).map((m) => '${(m as Map)['first']}'), gradesOf(f)];
  ...
  static List<List<Object?>> csvRows(List<Map<String, dynamic>> fs) => [
        csvHeader,
        for (final f in fs)
          [f['name'], studentsN(f), gradesOf(f), charged(f), paid(f), balance(f), agingDays(f), lastPaymentDate(f), lastMethod(f), hokFlag(f) ? (hokActive(f) ? 'פעילה' : 'נכשלה') : '', discountLabel(f), remindersSent(f).length, riskLabel(risk(f)), statusOf(f), f['answeredNote'], f['phone']],
      ];
...
// ═══════════════════════════════════════════════════════════════════════════════════════
// 💰 AyinCaseScreen — המסך (const · ללא main). המנהל מחבר לניווט-הבית.
// ═══════════════════════════════════════════════════════════════════════════════════════
class AyinCaseScreen extends StatefulWidget {
  const AyinCaseScreen({this.initialPanelId, super.key});
  ...
  static void resetLedger() => _AyinCaseData.reset();
  @override
  State<AyinCaseScreen> createState() => _AyinCaseScreenState();
}

class _AyinCaseScreenState extends State<AyinCaseScreen> {
  @override
  void initState() {
    super.initState();
    final p0 = widget.initialPanelId == null ? null : AyinCaseFacts.byId(widget.initialPanelId!); // G10a
  ...
  String get _roleName => _AyinCaseData.roleName(_role);
  bool get _amounts => _AyinCaseData.amounts(_role);
  ...
  Widget _row(Map<String, dynamic> f) {
    final charged = _AyinCaseData.charged(f), paid = _AyinCaseData.paid(f), bal = _AyinCaseData.balance(f);
    final band = _AyinCaseData.agingBand(f);
    final act = _AyinCaseData.rightAction(f);
    ...
    return Padding(
      ...
      child: ForgeStripPanelFrame(fields: ['', ''], child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Row(children: [
            Expanded(child: ForgeContactTile(fields: [f['name'] as String, '${_AyinCaseData.studentsOf(f)} · ${_AyinCaseData.gradesOf(f)}'])),
          ...
          if (_amounts) ...[
            ...
            Row(children: [
              ...
              Expanded(child: ForgeStatPlain(fields: [bal > 0 ? '= יתרה · ${_AyinCaseData.agingDays(f)} י׳' : '= יתרה', shekel(bal)])),
            ]),
          ] else
            Row(children: [
  ...
  List<Widget> _facts(Map<String, dynamic> f) {
    final r = _AyinCaseData.risk(f);
    return [
      ForgeStatusChip(items: [[_AyinCaseData.statusOf(f)]], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.statusOf(f) == 'תקין' || _AyinCaseData.statusOf(f) == 'מלגה-מלאה' ? 1 : _AyinCaseData.statusOf(f).contains('פיגור') ? 2 : 3) % 4]]),
      if (_AyinCaseData.hasHok(f)) ForgeStatusChip(items: [[_AyinCaseData.hokFlag(f) ? (_AyinCaseData.hokActive(f) ? '💳 הו״ק ${_amounts ? shekel((f['hok'] as Map)['amount'] as num) : ''} · יום ${(f['hok'] as Map)['day']}' : '⚠️ הו״ק נכשלה') : '⏸ הו״ק מופסקת']], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.hokFailed(f) ? 2 : _AyinCaseData.hokFlag(f) ? 1 : 0) % 4]]),
      if (_AyinCaseData.discountLabel(f).isNotEmpty) ForgeStatusChip(items: [['🎓 ${_AyinCaseData.discountLabel(f)}']], variants: const <int>[0]),
      if (_AyinCaseData.remindersSent(f).isNotEmpty) ForgeStatusChip(items: [['🔔 ${_AyinCaseData.remindersSent(f).length} תזכורות']], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.remindersSent(f).length > 2 ? 3 : 0) % 4]]),
      if (_AyinCaseData.balance(f) > 0) ForgeStatusChip(items: [['סיכון ${_AyinCaseData.riskLabel(r)} · ${_AyinCaseData.tierLabel(f)}']], variants: [const <int>[0, 1, 3, 2][(r == 2 ? 2 : r == 1 ? 3 : 1) % 4]]),
      if (_AyinCaseData.lastMethod(f).isNotEmpty) ForgeStatusChip(items: [['${_AyinCaseData.lastMethod(f)} · ${fmtDate(_AyinCaseData.lastPaymentDate(f))}']], variants: const <int>[0]),
  ...
  Widget _hokView(List<Map<String, dynamic>> all, List<Map<String, dynamic>> due) {
    final withHok = all.where(_AyinCaseData.hasHok).toList();
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ForgeTitledSection(fields: ['💳 הו״ק לרישום החודש · ${due.length} · ${_m(_AyinCaseData.hokExpected(all))}', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
        if (due.isEmpty)
          ForgeAnimatedEmpty(fields: ['כל ההו״ק הפעילות נרשמו החודש', ''])
        else ...[
          for (final f in due)
            ForgeNotifRow(items: [['${f['name']} · יום ${(f['hok'] as Map)['day']}', _AyinCaseData.hokMethod(f)]]),
          if (_AyinCaseData.can(_role, 'fees.hok')) ...[
            _gap(8),
            ...
            if (!_hokArmed)
              ...
              ForgeToneBanner(items: [['שלב 2/2: יירשמו ${due.length} תשלומי-הו״ק בסך ${_m(_AyinCaseData.hokExpected(all))} (${due.map((f) => f['name']).join(' · ')}). לאשר?']], variants: const <int>[2]),
              _wrap([
                GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() { _AyinCaseData.runHokBatch(all, _roleName); _hokArmed = false; }), child: ForgeToneButton(items: [['✅ אשר ורשום ${due.length}']], variants: const <int>[3])),
      ...
      ForgeTitledSection(fields: ['📇 כל הוראות-הקבע · ${withHok.length}', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
        for (final f in withHok)
          Padding(
            ...
            child: Row(children: [
              StatusDot(tone: _AyinCaseData.hokFailed(f) ? 2 : _AyinCaseData.hokFlag(f) ? 1 : 3),
              const SizedBox(width: 10),
              Expanded(child: ForgeContactTile(fields: [f['name'] as String, '${_m((f['hok'] as Map)['amount'] as num)} · יום ${(f['hok'] as Map)['day']} · ${_AyinCaseData.hokMethod(f)} · מ-${fmtDate((f['hok'] as Map)['startedAt'] as String?)}'])),
              Flexible(child: ForgeStatusChip(items: [[_AyinCaseData.hokFailed(f) ? 'נכשלה' : _AyinCaseData.hokFlag(f) ? (_AyinCaseData.hokRecorded(f) ? 'נרשמה החודש' : 'ממתינה') : 'מופסקת']], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.hokFailed(f) ? 2 : _AyinCaseData.hokFlag(f) ? 1 : 0) % 4]])),
              if (_AyinCaseData.can(_role, 'fees.hok')) ...[
                const SizedBox(width: 8),
                GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() => _AyinCaseData.toggleHok(f, _roleName)), child: ForgeToneButton(items: [[_AyinCaseData.hokFlag(f) ? '⏸ הפסק' : '▶ הפעל']], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.hokFlag(f) ? 2 : 1) % 4]])),
              ],
            ]),
          ),
  ...
  Widget _reminderCard(Map<String, dynamic> f) {
    final nr = _AyinCaseData.nextReminder(f)!;
    final grade = nr['grade'] as String;
    final plan = _AyinCaseData.reminderPlan(f);
    return Padding(
      padding: const EdgeInsets.only(bottom: 10),
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ForgeContactTile(fields: ['${f['name']} · דרגה: $grade', 'יתרה ${_m(_AyinCaseData.balance(f))} · ותק ${_AyinCaseData.agingDays(f)} י׳ · נשלחו ${_AyinCaseData.remindersSent(f).length}']),
        ...
        if (_amounts) ...[
          ...
          Container(
            ...
            child: SelectableText(_AyinCaseData.reminderText(f, grade), style: const TextStyle(color: _ink, fontSize: 12.5, height: 1.5)),
          ),
        ],
        if (_AyinCaseData.can(_role, 'fees.remind'))
          _wrap([GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() => _AyinCaseData.sendReminder(f, _roleName, grade)), child: ForgeToneButton(items: [['📨 שלח תזכורת $grade (פרטי)']], variants: const <int>[2]))], top: 8),
      ]),
    );
  }
  ...
  Widget _reportsView(List<Map<String, dynamic>> all, Map<String, dynamic> trend, int thisMonth) {
    final counts = _AyinCaseData.statusCounts(all);
    final t = DateTime.parse('${_AyinCaseData.today}T12:00:00');
    final wa = DateTime(t.year, t.month, t.day - 7); // חשבון-תאריך אמיתי (חוצה-חודש) — לא clamp בתוך החודש
    final weekAgo = '${_AyinCaseData.ymOf(wa)}-${wa.day.toString().padLeft(2, '0')}';
    final weekPaid = grandTotal([for (final f in all) for (final p in _AyinCaseData.paymentsOf(f)) if (dateInRange(p['date'] as String, weekAgo, _AyinCaseData.today)) p['amount']], (x) => x as num).toInt();
    final weekRem = grandTotal([for (final f in all) for (final c in _AyinCaseData.remindersSent(f)) if (dateInRange(c['at'] as String, weekAgo, _AyinCaseData.today)) 1], (x) => x as num).toInt();
    final byType = <String, int>{};
    for (final f in all) {
      _AyinCaseData.byType(f).forEach((k, v) => byType[k] = (byType[k] ?? 0) + v);
    }
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
      ...
      _gap(10),
      ForgeTitledSection(fields: ['🗓 דוח-גזבר שבועי · מ-${fmtDate(weekAgo)} עד ${fmtDate(_AyinCaseData.today)}', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
        Row(children: [
          Expanded(child: ForgeStatPlain(fields: ['נגבה השבוע', _m(weekPaid)])),
          Expanded(child: ForgeStatPlain(fields: ['תזכורות השבוע', '$weekRem'])),
          Expanded(child: ForgeStatPlain(fields: ['הו״ק נכשלו', '${all.where(_AyinCaseData.hokFailed).length}'])),
          Expanded(child: ForgeStatPlain(fields: ['חוב-ותיק', '${all.where(_AyinCaseData.oldDebt).length}'])),
        ]),
      ]])),
      ForgeTitledSection(fields: ['🏁 סוף-שנה · סגירת-חשבונות', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
        Row(children: [
          Expanded(child: ForgeStatPlain(fields: ['משפחות סגורות', '${all.where((f) => _AyinCaseData.balance(f) <= 0).length}/${all.length}'])),
          Expanded(child: ForgeStatPlain(fields: ['יתרה להעברה (carryBalance)', _m(_AyinCaseData.kOpen(all))])),
          Expanded(child: ForgeStatPlain(fields: ['זכויות להחזר', _m(grandTotal(all, (f) => _AyinCaseData.credit(f as Map<String, dynamic>)))])),
        ]),
      ...
      ForgeTitledSection(fields: ['🔗 התאמת-תשלומים-נכנסים לחיוב (שער-חיצוני · מקום-שמור)', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
        for (final inc in _AyinCaseData.incoming)
          () {
            final m = _AyinCaseData.matchIncoming(inc, all);
  ...
  void _openPanel(Map<String, dynamic> f) {
      ...
      showModalBottomSheet<void>(
        ...
        builder: (ctx) => Padding(
          ...
          child: ForgeStripPanelFrame(fields: ['', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              // ═══ הגרעין על הרשומה (G6d): מצב-הרשומה ⊕ AyinCaseCore.next ⊕ פנקס-overlay — מצב שאינו במחזור-החיים החצוב מדווח כפער, לא מתוקן בשקט ═══
              Builder(builder: (_) {
                final cur = _coreState['${f['id']}'] ?? '${f['status'] ?? AyinCaseCore.states.first}';
                if (!AyinCaseCore.states.contains(cur)) return ForgeToneBanner(items: [['מצב הרשומה "$cur" אינו במחזור-החיים החצוב (${AyinCaseCore.states.join('→')}) — פער זרע/סכמה, מקום-שמור']], variants: const <int>[2]);
                final nx = AyinCaseCore.next(cur);
                return ForgeTitledSection(fields: ['🧠 מחזור-חיים · רשומה (גרעין)', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
                  Wrap(spacing: 6, runSpacing: 6, children: [for (final st in AyinCaseCore.states) ForgeStatusChip(items: [[st]], variants: [const <int>[0, 1, 3, 2][(st == cur ? 1 : 0) % 4]])]),
                  ForgeToneBanner(items: [[nx == null ? 'מצב-סופי: $cur' : 'הבא אחרי $cur: $nx']], variants: const <int>[0]),
                  GestureDetector(behavior: HitTestBehavior.opaque, onTap: nx == null ? null : () => setState(() => _coreState['${f['id']}'] = nx), child: ForgeToneButton(items: [[nx == null ? 'אין מעבר' : 'קדם מצב ⇒ $nx']])),
                ]]));
              }),
              ForgeContactTile(fields: [f['name'] as String, '${_AyinCaseData.studentsOf(f)} · ${_AyinCaseData.gradesOf(f)}']),
              _gap(10),
              ForgeToneBanner(items: [[_AyinCaseData.balance(f) > 0 ? 'דגל-חוב — פרטים וסכומים בגזברות בלבד. לא לפנות לתלמיד/ה (מגן-כבוד).' : 'תקין — אין דגל-חוב']], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.balance(f) > 0 ? 3 : 1) % 4]]),
            ])),
        ),
      );
      return;
    ...
    showModalBottomSheet<void>(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setSheet) {
        ...
        final charged = _AyinCaseData.charged(f), paid = _AyinCaseData.paid(f), bal = _AyinCaseData.balance(f), band = _AyinCaseData.agingBand(f);
        final balColor = band == 3 ? _danger : band == 2 ? _warning : band == 1 ? _acc : _ok;
        final actn = _AyinCaseData.rightAction(f);
        return DraggableScrollableSheet(
          ...
          builder: (ctx, scroll) => Padding(
            ...
            child: ForgeStripPanelFrame(fields: ['', ''], child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                ForgeContactTile(fields: [f['name'] as String, '${f['answeredNote']} · ${f['phone']} · ${_AyinCaseData.studentsOf(f)} (${_AyinCaseData.gradesOf(f)})']),
                _gap(10),
                Row(children: [
                  Expanded(child: ForgeStatPlain(fields: [bal > 0 ? 'יתרה · ותק ${_AyinCaseData.agingDays(f)} י׳' : 'יתרה', _m(bal)])),
                  Expanded(child: ForgeStatPlain(fields: ['חיובים', _m(charged)])),
                  Expanded(child: ForgeStatPlain(fields: ['שולם', _m(paid)])),
                  if (_AyinCaseData.credit(f) > 0) Expanded(child: ForgeStatPlain(fields: ['זכות', _m(_AyinCaseData.credit(f))])),
                ]),
                _gap(8),
                ...
                Builder(builder: (_) {
                  final acts = <Widget>[
                    if (_AyinCaseData.can(_role, 'fees.charge')) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => _openChargeForm(f, [f], onDone: () => setSheet(() {})), child: ForgeToneButton(items: [['➕ חיוב']], variants: const <int>[0])),
                    if (_AyinCaseData.can(_role, 'fees.pay')) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => _openPaymentForm(f, [f], onDone: () => setSheet(() {})), child: ForgeToneButton(items: [['💳 רשום תשלום']], variants: const <int>[1])),
                    if (_AyinCaseData.can(_role, 'fees.pay') && bal > 0) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.addPayment(f, _roleName, amount: (bal / 2).ceil(), method: 'מזומן', date: _AyinCaseData.today, note: 'תשלום-חלקי')), child: ForgeToneButton(items: [['➗ תשלום-חלקי (½)']], variants: const <int>[1])),
                    if (_AyinCaseData.can(_role, 'fees.remind') && _AyinCaseData.nextReminder(f) != null) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.sendReminder(f, _roleName, _AyinCaseData.nextReminder(f)!['grade'] as String)), child: ForgeToneButton(items: [['📨 תזכורת ${_AyinCaseData.nextReminder(f)!['grade']}']], variants: const <int>[2])),
                    if (_AyinCaseData.can(_role, 'fees.arrangement') && bal > 0 && !_AyinCaseData.hasArrangement(f)) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.setArrangement(f, _roleName, 3)), child: ForgeToneButton(items: [['📆 הסדר 3 תשלומים']], variants: const <int>[0])),
                    if (_AyinCaseData.can(_role, 'fees.hok') && _AyinCaseData.hasHok(f)) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.toggleHok(f, _roleName)), child: ForgeToneButton(items: [[_AyinCaseData.hokFlag(f) ? '⏸ הפסק הו״ק' : '▶ הפעל הו״ק']], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.hokFlag(f) ? 2 : 1) % 4]])),
                    if (_AyinCaseData.can(_role, 'fees.writeoff') && bal > 0 && _AyinCaseData.oldDebt(f)) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.writeOff(f, _roleName)), child: ForgeToneButton(items: [['🗂 סמן חוב-אבוד']], variants: const <int>[3])),
                    if (_AyinCaseData.can(_role, 'fees.refund') && _AyinCaseData.credit(f) > 0) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.refund(f, _roleName)), child: ForgeToneButton(items: [['💸 החזר-זכות ${_m(_AyinCaseData.credit(f))}']], variants: const <int>[1])),
                    if ((_AyinCaseData.can(_role, 'fees.pay') || _AyinCaseData.can(_role, 'fees.self')) && bal > 0)
                      // שער-חיצוני (מקום-שמור): payLink מחזיר null כש-payUrl ריק ⇒ הכפתור שמור, לא מזייף קישור
                      GestureDetector(behavior: HitTestBehavior.opaque, onTap: _AyinCaseData.payLinkOf(f) == null ? null : () {}, child: ForgeToneButton(items: [[_AyinCaseData.payLinkOf(f) == null ? '🔗 קישור-תשלום (שער לא-מוגדר)' : '🔗 שלח קישור-תשלום']], variants: const <int>[0])),
  ...
  List<Widget> _tabOverview(Map<String, dynamic> f, int charged, int paid, int bal) => [
        if (_amounts) ...[
          ...
          _gap(8),
          if (_AyinCaseData.byType(f).isNotEmpty) ForgeBarChart(fields: ['פירוק-חיובים לפי-סוג', ''], values: (() { final _vs = [for (final v in _AyinCaseData.byType(f).values) v.toDouble()]; final _m = _vs.fold<double>(0.0, (a, b) => a > b ? a : b); return [for (final v in _vs) _m == 0 ? 0.0 : v / _m]; })()),
        ],
        _wrap([for (final m in f['members'] as List) ForgeStatusChip(items: [['🎓 ${(m as Map)['first']} · ${m['grade']}']], variants: const <int>[0])]),
        _wrap(_facts(f)),
        _gap(8),
        Row(children: [
          Expanded(child: ForgeStatPlain(fields: ['דפוס-תשלום (RFM) · ${_AyinCaseData.tierLabel(f)}', '${_AyinCaseData.rfm(f)}'])),
          Expanded(child: ForgeStatPlain(fields: ['מגמת-תשלומים (6 חודשים)', '${_AyinCaseData.trend(f)['dir'] == 'up' ? '↑' : _AyinCaseData.trend(f)['dir'] == 'down' ? '↓' : '→'} ${_AyinCaseData.trend(f)['pct']}%'])),
          Expanded(child: ForgeStatPlain(fields: ['סיכון-גבייה (ותק · דפוס · מגמה)', _AyinCaseData.riskLabel(_AyinCaseData.risk(f))])),
        ]),
      ];
  ...
  List<Widget> _tabCharges(Map<String, dynamic> f, void Function(void Function()) act) {
    final cs = _AyinCaseData.chargesOf(f);
    final dups = _AyinCaseData.duplicateCharges(f).map((c) => c['id']).toSet();
    return [
      if (cs.isEmpty) ForgeAnimatedEmpty(fields: ['אין חיובים למשפחה — לרשום חיוב-שנה', '']),
      for (final c in cs)
        () {
          final cancelled = c['cancelledAt'] != null || _AyinCaseData.cancelledIds.contains(c['id']);
          final net = _AyinCaseData.netOf(f, c), gross = _AyinCaseData.grossOf(f, c);
          return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ForgeNotifRow(items: [['${cancelled ? '🚫 ' : dups.contains(c['id']) ? '👯 ' : ''}${c['cat']} · ${c['memberId']}${c['installmentOf'] != null ? ' · הסדר' : ''}', fmtDate(c['date'] as String?)]]),
            if (!cancelled && _AyinCaseData.can(_role, 'fees.writeoff'))
              _wrap([GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.cancelCharge(f, _roleName, c, dups.contains(c['id']) ? 'חיוב-כפול' : 'ביטול-ידני')), child: ForgeToneButton(items: [['✖ בטל חיוב']], variants: const <int>[3]))], top: 2),
          ]);
        }(),
    ];
  }
  ...
  List<Widget> _tabPayments(Map<String, dynamic> f) {
    final ps = [..._AyinCaseData.paymentsOf(f)]..sort((a, b) => '${b['date']}'.compareTo('${a['date']}'));
    return [
  ...
  List<Widget> _tabHok(Map<String, dynamic> f, void Function(void Function()) act) {
    if (!_AyinCaseData.hasHok(f)) return [ForgeAnimatedEmpty(fields: ['אין הוראת-קבע למשפחה (מקום-שמור: תוגדר בשער-הסליקה)', ''])];
    final h = f['hok'] as Map;
    ...
    return [
      Row(children: [
        StatusDot(tone: _AyinCaseData.hokFailed(f) ? 2 : _AyinCaseData.hokFlag(f) ? 1 : 3),
        const SizedBox(width: 10),
        Expanded(child: ForgeContactTile(fields: ['${_m(h['amount'] as num)} · יום ${h['day']} בחודש', '${_AyinCaseData.hokMethod(f)} · מ-${fmtDate(h['startedAt'] as String?)}${h['kevaId'] != null ? ' · סליקה ${h['kevaId']}' : ' · ידנית'}'])),
      ]),
      _wrap([
        ForgeStatusChip(items: [[_AyinCaseData.hokFlag(f) ? 'מסומנת פעילה' : 'מופסקת']], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.hokFlag(f) ? 1 : 0) % 4]]),
        ForgeStatusChip(items: [[_AyinCaseData.hokActive(f) ? 'סליקה חיה' : 'סליקה פסקה >2 חודשים']], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.hokActive(f) ? 1 : 2) % 4]]),
        ForgeStatusChip(items: [[_AyinCaseData.hokRecorded(f) ? 'נרשמה החודש ✅' : 'טרם נרשמה החודש']], variants: [const <int>[0, 1, 3, 2][(_AyinCaseData.hokRecorded(f) ? 1 : 3) % 4]]),
      ]),
      if (_AyinCaseData.hokFailed(f)) ...[
        _gap(6),
  ...
  List<Widget> _tabReminders(Map<String, dynamic> f, void Function(void Function()) act) {
    final plan = _AyinCaseData.reminderPlan(f), sent = _AyinCaseData.remindersSent(f), nr = _AyinCaseData.nextReminder(f);
    return [
      if (_AyinCaseData.fullScholarship(f)) ForgeToneBanner(items: [['מלגה מלאה — אפס-תזכורות (מגן-כבוד)']], variants: const <int>[1]) else if (plan.isEmpty) ForgeAnimatedEmpty(fields: ['אין חוב פתוח — אין לוח-תזכורות', '']) else ...[
        Text('לוח מדורג (מהחיוב-הפתוח-הוותיק ${fmtDate(_AyinCaseData.oldestOpenDate(f))})', style: const TextStyle(color: _muted, fontSize: 13, fontWeight: FontWeight.w800)),
        _wrap([for (var i = 0; i < plan.length; i++) ForgeStatusChip(items: [['${i < sent.length ? '✅' : '${plan[i]['date']}'.compareTo(_AyinCaseData.today) <= 0 ? '⏰' : '⏳'} ${plan[i]['grade']} · ${fmtDate(plan[i]['date'] as String?)}']], variants: [const <int>[0, 1, 3, 2][(i < sent.length ? 1 : nr != null && nr['grade'] == plan[i]['grade'] ? 3 : 0) % 4]])]),
        if (nr != null && _AyinCaseData.can(_role, 'fees.remind')) _wrap([GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.sendReminder(f, _roleName, nr['grade'] as String)), child: ForgeToneButton(items: [['📨 שלח תזכורת ${nr['grade']} (פרטי)']], variants: const <int>[2]))], top: 8),
      ],
      _gap(8),
  ...
  List<Widget> _tabDiscounts(Map<String, dynamic> f, void Function(void Function()) act) {
    final ids = _AyinCaseData.effectiveCriteria(f);
    return [
      Row(children: [
        Expanded(child: ForgeStatPlain(fields: ['הנחה אפקטיבית (הגבוהה מנצחת)', '${_AyinCaseData.discountPct(f)}%'])),
        Expanded(child: ForgeStatPlain(fields: ['שווי-ההנחה השנה', _m(_AyinCaseData.scholarshipOf(f))])),
        Expanded(child: ForgeStatPlain(fields: ['אחים (הנחת-אחים אוטו)', '${_AyinCaseData.studentsN(f)}'])),
      ]),
      _wrap([for (final c in _AyinCaseData.criteria) ForgeStatusChip(items: [['${ids.contains(c['id']) ? '✅ ' : ''}${c['label']} ${c['discountPct']}%']], variants: [const <int>[0, 1, 3, 2][(ids.contains(c['id']) ? 1 : 0) % 4]])]),
      if (_AyinCaseData.can(_role, 'fees.scholarship'))
        _wrap([for (final c in _AyinCaseData.criteria) if (!ids.contains(c['id'])) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.grantDiscount(f, _roleName, c['id'] as String)), child: ForgeToneButton(items: [['🎓 הענק ${c['label']}']], variants: const <int>[0]))], top: 8)
      else
  ...
  List<Widget> _tabArrangement(Map<String, dynamic> f, void Function(void Function()) act) {
    final ins = _AyinCaseData.installments(f);
    return [
      if (ins.isEmpty) ...[
        ForgeAnimatedEmpty(fields: ['אין הסדר-תשלומים', '']),
        if (_AyinCaseData.can(_role, 'fees.arrangement') && _AyinCaseData.balance(f) > 0)
          _wrap([for (final n in const [2, 3, 6]) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _AyinCaseData.setArrangement(f, _roleName, n)), child: ForgeToneButton(items: [['📆 פריסה ל-$n']], variants: const <int>[0]))], top: 4),
      ] else ...[
        if (_AyinCaseData.arrangementLate(f)) ForgeToneBanner(items: [['הסדר בפיגור — תשלום שמועדו עבר לא כוסה']], variants: const <int>[2]),
        for (final c in ins)
  ...
  List<Widget> _tabStatement(Map<String, dynamic> f) {
    final lines = <String>[
      'מצב-חשבון · ${f['name']} · ${_AyinCaseData.orgName} · ${fmtDate(_AyinCaseData.today)}',
      'הורה-משלם: ${f['answeredNote']} · ${f['phone']}',
      'תלמידים: ${_AyinCaseData.studentsOf(f)} (${_AyinCaseData.gradesOf(f)})',
      '',
      'חיובים:',
      for (final c in _AyinCaseData.liveCharges(f)) '  ${fmtDate(c['date'] as String?)}  ${c['cat']}  ${c['memberId']}  ${shekel(_AyinCaseData.netOf(f, c))}',
      'תשלומים:',
      for (final p in _AyinCaseData.paymentsOf(f)) '  ${fmtDate(p['date'] as String?)}  ${p['method']}  ${shekel(p['amount'] as num)}',
      '',
      'סך-חיובים ${shekel(_AyinCaseData.charged(f))} · שולם ${shekel(_AyinCaseData.paid(f))} · יתרה ${shekel(_AyinCaseData.balance(f))}',
    ];
    return [
  ...
  List<Widget> _tabAudit(Map<String, dynamic> f) {
    final rows = _AyinCaseData.audit.where((a) => a['family'] == f['id']).toList();
    return [
  ...
  void _openChargeForm(Map<String, dynamic>? fixed, List<Map<String, dynamic>> pool, {VoidCallback? onDone}) {
    ...
    var cat = _AyinCaseData.chargeTypes.first, amount = '', date = _AyinCaseData.today, note = '', member = '', bulk = '';
    showModalBottomSheet<void>(
      context: context, backgroundColor: Colors.transparent, isScrollControlled: true,
      builder: (ctx) => StatefulBuilder(builder: (ctx, setSheet) {
        final f = _AyinCaseData.families.firstWhere((x) => x['id'] == fam, orElse: () => _AyinCaseData.families.first);
        final members = [for (final m in f['members'] as List) '${(m as Map)['first']}'];
        if (!members.contains(member)) member = members.first;
        final grades = _AyinCaseData.grades(_AyinCaseData.families);
        return Padding(
          ...
          child: DraggableScrollableSheet(
            ...
            builder: (ctx, scroll) => Padding(
              ...
              child: ForgeStripPanelFrame(fields: ['', ''], child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                  ...
                  _gap(8),
                  ForgeDsEnumField(fields: ['משפחה'], control: DsEnumField(label: 'משפחה', options: [for (final x in _AyinCaseData.families) '${x['name']}'], value: '${f['name']}', onChanged: (v) => setSheet(() => fam = _AyinCaseData.families.firstWhere((x) => x['name'] == v)['id'] as String), bare: true)),
                  ...
                  ForgeDsEnumField(fields: ['סוג-חיוב'], control: DsEnumField(label: 'סוג-חיוב', options: _AyinCaseData.chargeTypes, value: cat, onChanged: (v) => setSheet(() => cat = v), bare: true)),
                  ...
                  GestureDetector(behavior: HitTestBehavior.opaque, onTap: () {
                    ...
                    if (a <= 0) return;
                    if (bulk.isEmpty) {
                      _AyinCaseData.addCharge(f, _roleName, cat: cat, amount: a, date: date, memberId: member, note: note);
                    } else {
                      for (final x in _AyinCaseData.families) {
                        for (final m in x['members'] as List) {
                          if ('${(m as Map)['grade']}'.startsWith(bulk)) _AyinCaseData.addCharge(x, _roleName, cat: cat, amount: a, date: date, memberId: '${m['first']}', note: note.isEmpty ? 'חיוב-מרוכז $bulk' : note);
                        }
                      }
                    }
  ...
  void _openPaymentForm(Map<String, dynamic>? fixed, List<Map<String, dynamic>> pool, {VoidCallback? onDone}) {
    var fam = fixed?['id'] as String? ?? (pool.isNotEmpty ? pool.first['id'] as String : '');
    var method = 'אשראי', amount = '', date = _AyinCaseData.today, note = '';
    showModalBottomSheet<void>(
      context: context, backgroundColor: Colors.transparent, isScrollControlled: true,
      builder: (ctx) => StatefulBuilder(builder: (ctx, setSheet) {
        final f = _AyinCaseData.families.firstWhere((x) => x['id'] == fam, orElse: () => _AyinCaseData.families.first);
        final bal = _AyinCaseData.balance(f);
        return Padding(
          padding: EdgeInsets.only(bottom: MediaQuery.of(ctx).viewInsets.bottom),
          child: DraggableScrollableSheet(
            ...
            builder: (ctx, scroll) => Padding(
              ...
              child: ForgeStripPanelFrame(fields: ['', ''], child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                  ...
                  _gap(8),
                  ForgeDsEnumField(fields: ['משפחה'], control: DsEnumField(label: 'משפחה', options: [for (final x in _AyinCaseData.families) '${x['name']}'], value: '${f['name']}', onChanged: (v) => setSheet(() => fam = _AyinCaseData.families.firstWhere((x) => x['name'] == v)['id'] as String), bare: true)),
                  ForgeDsEnumField(fields: ['אמצעי'], control: DsEnumField(label: 'אמצעי', options: _AyinCaseData.payMethodsSchool, value: method, onChanged: (v) => setSheet(() => method = v), bare: true)),
                  ...
                  GestureDetector(behavior: HitTestBehavior.opaque, onTap: () {
                    ...
                    if (a <= 0) return;
                    _AyinCaseData.addPayment(f, _roleName, amount: a, method: method, date: date, note: note);
                    Navigator.of(ctx).pop();
                    setState(() {});
                    onDone?.call();
  ...
  void _openExport(List<Map<String, dynamic>> fs) {
    final csv = _AyinCaseData.csvOf(fs);
    showModalBottomSheet<void>(
      context: context, backgroundColor: Colors.transparent, isScrollControlled: true,
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx, scroll) => Padding(
          padding: const EdgeInsets.all(12),
          child: ForgeStripPanelFrame(fields: ['', ''], child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
              ForgeContactTile(fields: ['ייצוא CSV', '${fs.length} משפחות · ${_AyinCaseData.csvHeader.length} עמודות (PDF = שער-חיצוני, מקום-שמור)']),
              _gap(10),
              Container(
                padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(color: const Color(0xFF0C0D1E), borderRadius: BorderRadius.circular(10)),
  ...
  Widget build(BuildContext context) {
    final all = _AyinCaseData.visibleFor(_role);
    // דירוג: סיכון-יורד ⇒ ותק-יורד ⇒ יתרה-יורדת (המפרט: חוב · ותק-חוב · סיכון)
    final ranked = [...all]..sort((a, b) {
        final r = _AyinCaseData.risk(b).compareTo(_AyinCaseData.risk(a));
        if (r != 0) return r;
        final ag = _AyinCaseData.agingDays(b).compareTo(_AyinCaseData.agingDays(a));
        return ag != 0 ? ag : _AyinCaseData.balance(b).compareTo(_AyinCaseData.balance(a));
      });
    final visible = _AyinCaseData.filter(_AyinCaseData.search(ranked, _q), _locks,
        {'grade': _grade, 'type': _type, 'course': _course, 'method': _method, 'year': _year});
    // KPI-10 על כל-המשפחות-הנראות-לתפקיד (הורה ⇒ משפחתו בלבד)
    final kCharged = _AyinCaseData.kCharged(all), kPaid = _AyinCaseData.kPaid(all), kOpen = _AyinCaseData.kOpen(all), kPct = _AyinCaseData.kPct(all);
    final inDebt = _AyinCaseData.kInDebt(all), oldN = all.where(_AyinCaseData.oldDebt).length, kOld = _AyinCaseData.kOld(all);
    final kExp = _AyinCaseData.kExpected(all), kHok = _AyinCaseData.kHokActive(all), kSch = _AyinCaseData.kScholar(all), kRem = _AyinCaseData.kReminders(all);
    final failed = all.where(_AyinCaseData.hokFailed).toList();
    final late = all.where(_AyinCaseData.arrangementLate).toList();
    final dups = [for (final f in all) if (_AyinCaseData.duplicateCharges(f).isNotEmpty) f];
    final hokDue = _AyinCaseData.hokDueList(all);
    final follow = _AyinCaseData.followUps(all);
    final tripDebt = all.where((f) => _AyinCaseData.balance(f) > 0 && _AyinCaseData.liveCharges(f).any((c) => c['cat'] == 'טיול')).toList();
    final buckets = <int, List<Map<String, dynamic>>>{2: [], 1: [], 0: [], -1: []};
    for (final f in visible) {
      // דגל-בלבד (מחנך): שני דליים — דגל/תקין; אין דירוג-סיכון גלוי
      buckets[_AyinCaseData.balance(f) <= 0 ? -1 : _amounts ? _AyinCaseData.risk(f) : 0]!.add(f);
    }
    final secTitle = {2: '🔴 סיכון-גבוה / חוב-ותיק', 1: '🟠 בפיגור / בינוני', 0: _amounts ? '🟢 חוב-טרי' : '🚩 דגל-חוב', -1: '✅ ללא-חוב'};
    const secTone = {2: 2, 1: 3, 0: 0, -1: 1};
    final trend = _AyinCaseData.collectionTrend(all);
    final thisMonth = _AyinCaseData.collectedInMonth(all, monthKey(_AyinCaseData.today));

    return DsScaffold(title: 'AyinCaseScreen', subtitle: 'AyinCaseScreen · מודול-משנה מחולל · 3 בונים מחווטים-לשקעי-הזהב', icon: '🧬', header: false, children: [ForgeCenteredPageHeader(fields: ['', 'AyinCaseScreen', 'AyinCaseScreen · מודול-משנה מחולל · 3 בונים מחווטים-לשקעי-הזהב']), ...[
      _hokView(all, hokDue),
...
// ═══ תפר-עובדות ציבורי (G9b · לרכזת-האפליקציה): AyinCaseFacts — נגזרות-אמת של דאטה-המודול; כל ערך = ביטוי חי על הזרע/המנועים (§20-ג), אפס ליטרל-מומצא. מחולל: retarget.mjs ═══
class AyinCaseFacts {
  static const String entity = 'AyinCase';
  static const String label = 'תיק'; // מונח-הישות מ-entity-terms (דאטה)
  static int get count => _AyinCaseData.families.length; // רשומות הזרע-הראשי "families" (static-const)
  ...
  static List<Map<String, dynamic>> get rows => _AyinCaseData.families; // כל רשומות הזרע-הראשי (static-const)
