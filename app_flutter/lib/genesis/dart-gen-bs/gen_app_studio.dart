...
import 'gen_retarget_course_from_crs_skeda887.dart' show CourseScreen, CourseFacts; // רק התפר הציבורי (מסך+עובדות) — מחלקות-ציבוריות אחרות של הזהב (DashInput) לא מתנגשות
import 'gen_retarget_member_from_stu_skeda887.dart' show MemberScreen, MemberFacts; // רק התפר הציבורי (מסך+עובדות) — מחלקות-ציבוריות אחרות של הזהב (DashInput) לא מתנגשות
...
import 'gen_retarget_shopassignment_from_tch_skeda887.dart' show ShopAssignmentScreen, ShopAssignmentFacts; // רק התפר הציבורי (מסך+עובדות) — מחלקות-ציבוריות אחרות של הזהב (DashInput) לא מתנגשות
import 'gen_retarget_shopcriterion_from_crs_skeda887.dart' show ShopCriterionScreen, ShopCriterionFacts; // רק התפר הציבורי (מסך+עובדות) — מחלקות-ציבוריות אחרות של הזהב (DashInput) לא מתנגשות
import 'gen_retarget_donation_from_fee_skeda887.dart' show DonationScreen, DonationFacts; // רק התפר הציבורי (מסך+עובדות) — מחלקות-ציבוריות אחרות של הזהב (DashInput) לא מתנגשות
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: 'Studio', subtitle: '6 מודולים ממשפטים · כל אחד חצוב מהזהב', icon: '🧬', children: [
      ...
      Wrap(spacing: 12, runSpacing: 12, children: [ // KPI-רכזת (G9b): עובדות-אמת בלבד — כמו _Home של הזהב (מסכים-מחוברים + הדחוף של כל מודול)
        ...
        if (vis.contains(0)) GestureDetector(key: const ValueKey('hero-Course'), onTap: () { final id = CourseFacts.heroFirstId; _go(context, id == null ? const CourseScreen() : CourseScreen(initialPanelId: id, initialMetric: CourseFacts.heroKey)); }, child: SizedBox(width: 168, child: ForgeStatPlain(fields: [CourseFacts.heroLabel, CourseFacts.hero]))), // Course · המדד הראשון שהזהב צובע-סכנה כשאינו-אפס · טאפ ⇒ המודול פתוח על רשומת-ה-hero הראשונה (G10a)
      ...
      if (vis.isEmpty) ForgeAnimatedEmpty(fields: ['אין מודול שתואם לחיפוש', 'נסה מילה אחרת']) else DsSection(title: 'כלים · ${vis.length}', children: [
        if (vis.contains(0)) GestureDetector(key: const ValueKey('nav-Course'), behavior: HitTestBehavior.opaque, onTap: () => _go(context, const CourseScreen()), child: ForgeGridHubCard(fields: ['חוג', '${CourseFacts.count} ${CourseFacts.label} · חוגים ותפוסה'])), // אריח-ניווט forge (G12c)
        if (vis.contains(1)) GestureDetector(key: const ValueKey('nav-Member'), behavior: HitTestBehavior.opaque, onTap: () => _go(context, const MemberScreen()), child: ForgeGridHubCard(fields: ['בני משפחה', '${MemberFacts.count} ${MemberFacts.label} · בני משפחה לפי גיל'])), // אריח-ניווט forge (G12c)
        if (vis.contains(2)) GestureDetector(key: const ValueKey('nav-Teacher'), behavior: HitTestBehavior.opaque, onTap: () => _go(context, const TeacherScreen()), child: ForgeGridHubCard(fields: ['מורה', '${TeacherFacts.count} ${TeacherFacts.label} · מדריכה עם שעות'])), // אריח-ניווט forge (G12c)
        if (vis.contains(3)) GestureDetector(key: const ValueKey('nav-ShopAssignment'), behavior: HitTestBehavior.opaque, onTap: () => _go(context, const ShopAssignmentScreen()), child: ForgeGridHubCard(fields: ['שיוך', '${ShopAssignmentFacts.count} ${ShopAssignmentFacts.label} · שיוכים פתוחים'])), // אריח-ניווט forge (G12c)
        if (vis.contains(4)) GestureDetector(key: const ValueKey('nav-ShopCriterion'), behavior: HitTestBehavior.opaque, onTap: () => _go(context, const ShopCriterionScreen()), child: ForgeGridHubCard(fields: ['קריטריון', '${ShopCriterionFacts.count} ${ShopCriterionFacts.label} · קריטריון זכאות להנחה'])), // אריח-ניווט forge (G12c)
        if (vis.contains(5)) GestureDetector(key: const ValueKey('nav-Donation'), behavior: HitTestBehavior.opaque, onTap: () => _go(context, const DonationScreen()), child: ForgeGridHubCard(fields: ['תרומות', '${DonationFacts.count} ${DonationFacts.label} · נדבות לפי חודש'])), // אריח-ניווט forge (G12c)
      ]),
    ]);
  }
}
