  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: 'גבייה ותשלומים', subtitle: '${all.length} משפחות · ${_FeesData.year} · $_roleName', icon: '💰', header: false, children: [ForgeCenteredPageHeader(fields: ['', 'גבייה ותשלומים', '${all.length} משפחות · ${_FeesData.year} · $_roleName']), ...[
        ...
        if (dups.isNotEmpty && _amounts) ...[
          ForgeToneBanner(items: [['חיוב-כפול-חשוד (אותו סוג·סכום·תאריך·תלמיד): ${dups.map((f) => '${f['name']} (${_FeesData.duplicateCharges(f).map((c) => c['cat']).join(',')})').join(' · ')}']], variants: const <int>[2]),
          _gap(8),
        ],
        ...
        if (hokDue.isNotEmpty && _FeesData.can(_role, 'fees.hok')) ...[
          ForgeToneBanner(items: [['${hokDue.length} הו״ק לרישום החודש (${_m(_FeesData.hokExpected(all))}) — ראה מבט הו״ק']], variants: const <int>[0]),
          _gap(8),
        ],
        ...
        if (due.isEmpty)
          ...
          if (_FeesData.can(_role, 'fees.hok')) ...[
            ...
            if (!_hokArmed)
              ...
              ForgeToneBanner(items: [['שלב 2/2: יירשמו ${due.length} תשלומי-הו״ק בסך ${_m(_FeesData.hokExpected(all))} (${due.map((f) => f['name']).join(' · ')}). לאשר?']], variants: const <int>[2]),
              _wrap([
                GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() { _FeesData.runHokBatch(all, _roleName); _hokArmed = false; }), child: ForgeToneButton(items: [['✅ אשר ורשום ${due.length}']], variants: const <int>[3])),
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              if (_FeesData.can(_role, 'fees.hok')) ...[
                ...
                GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() => _FeesData.toggleHok(f, _roleName)), child: ForgeToneButton(items: [[_FeesData.hokFlag(f) ? '⏸ הפסק' : '▶ הפעל']], variants: [const <int>[0, 1, 3, 2][(_FeesData.hokFlag(f) ? 2 : 1) % 4]])),
              ],
            ]),
          ),
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        if (_FeesData.can(_role, 'fees.remind'))
          _wrap([GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() => _FeesData.sendReminder(f, _roleName, grade)), child: ForgeToneButton(items: [['📨 שלח תזכורת $grade (פרטי)']], variants: const <int>[2]))], top: 8),
      ]),
    );
  }
      ...
      showModalBottomSheet<void>(
        ...
        builder: (ctx) => Padding(
          ...
          child: ForgeStripPanelFrame(fields: ['', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              ...
              _gap(10),
              ForgeToneBanner(items: [[_FeesData.balance(f) > 0 ? 'דגל-חוב — פרטים וסכומים בגזברות בלבד. לא לפנות לתלמיד/ה (מגן-כבוד).' : 'תקין — אין דגל-חוב']], variants: [const <int>[0, 1, 3, 2][(_FeesData.balance(f) > 0 ? 3 : 1) % 4]]),
            ])),
        ),
      );
      return;
        ...
        return DraggableScrollableSheet(
          ...
          builder: (ctx, scroll) => Padding(
            ...
            child: ForgeStripPanelFrame(fields: ['', ''], child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                ...
                Builder(builder: (_) {
                  final acts = <Widget>[
                    if (_FeesData.can(_role, 'fees.charge')) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => _openChargeForm(f, [f], onDone: () => setSheet(() {})), child: ForgeToneButton(items: [['➕ חיוב']], variants: const <int>[0])),
                    if (_FeesData.can(_role, 'fees.pay')) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => _openPaymentForm(f, [f], onDone: () => setSheet(() {})), child: ForgeToneButton(items: [['💳 רשום תשלום']], variants: const <int>[1])),
                    if (_FeesData.can(_role, 'fees.pay') && bal > 0) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _FeesData.addPayment(f, _roleName, amount: (bal / 2).ceil(), method: 'מזומן', date: _FeesData.today, note: 'תשלום-חלקי')), child: ForgeToneButton(items: [['➗ תשלום-חלקי (½)']], variants: const <int>[1])),
                    if (_FeesData.can(_role, 'fees.remind') && _FeesData.nextReminder(f) != null) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _FeesData.sendReminder(f, _roleName, _FeesData.nextReminder(f)!['grade'] as String)), child: ForgeToneButton(items: [['📨 תזכורת ${_FeesData.nextReminder(f)!['grade']}']], variants: const <int>[2])),
                    if (_FeesData.can(_role, 'fees.arrangement') && bal > 0 && !_FeesData.hasArrangement(f)) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _FeesData.setArrangement(f, _roleName, 3)), child: ForgeToneButton(items: [['📆 הסדר 3 תשלומים']], variants: const <int>[0])),
                    if (_FeesData.can(_role, 'fees.hok') && _FeesData.hasHok(f)) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _FeesData.toggleHok(f, _roleName)), child: ForgeToneButton(items: [[_FeesData.hokFlag(f) ? '⏸ הפסק הו״ק' : '▶ הפעל הו״ק']], variants: [const <int>[0, 1, 3, 2][(_FeesData.hokFlag(f) ? 2 : 1) % 4]])),
                    if (_FeesData.can(_role, 'fees.writeoff') && bal > 0 && _FeesData.oldDebt(f)) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _FeesData.writeOff(f, _roleName)), child: ForgeToneButton(items: [['🗂 סמן חוב-אבוד']], variants: const <int>[3])),
                    if (_FeesData.can(_role, 'fees.refund') && _FeesData.credit(f) > 0) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _FeesData.refund(f, _roleName)), child: ForgeToneButton(items: [['💸 החזר-זכות ${_m(_FeesData.credit(f))}']], variants: const <int>[1])),
                    if ((_FeesData.can(_role, 'fees.pay') || _FeesData.can(_role, 'fees.self')) && bal > 0)
                      ...
                      GestureDetector(behavior: HitTestBehavior.opaque, onTap: _FeesData.payLinkOf(f) == null ? null : () {}, child: ForgeToneButton(items: [[_FeesData.payLinkOf(f) == null ? '🔗 קישור-תשלום (שער לא-מוגדר)' : '🔗 שלח קישור-תשלום']], variants: const <int>[0])),
        ...
        () {
          ...
          return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            if (!cancelled && _FeesData.can(_role, 'fees.writeoff'))
              _wrap([GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _FeesData.cancelCharge(f, _roleName, c, dups.contains(c['id']) ? 'חיוב-כפול' : 'ביטול-ידני')), child: ForgeToneButton(items: [['✖ בטל חיוב']], variants: const <int>[3]))], top: 2),
          ]);
        }(),
    ];
  }
  ...
  List<Widget> _tabDiscounts(Map<String, dynamic> f, void Function(void Function()) act) {
    ...
    return [
      ...
      if (_FeesData.can(_role, 'fees.scholarship'))
        _wrap([for (final c in _FeesData.criteria) if (!ids.contains(c['id'])) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => act(() => _FeesData.grantDiscount(f, _roleName, c['id'] as String)), child: ForgeToneButton(items: [['🎓 הענק ${c['label']}']], variants: const <int>[0]))], top: 8)
      else
  ...
  List<Widget> _tabArrangement(Map<String, dynamic> f, void Function(void Function()) act) {
    ...
    return [
      if (ins.isEmpty) ...[
        ...
        if (_FeesData.arrangementLate(f)) ForgeToneBanner(items: [['הסדר בפיגור — תשלום שמועדו עבר לא כוסה']], variants: const <int>[2]),
        for (final c in ins)
