...
class GenAppPeruk21BehaviorScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) => DsScaffold(title: gen_app_peruk21_behavior_c0, subtitle: gen_app_peruk21_behavior_c1, icon: gen_app_peruk21_behavior_c2, children: [
    ...
    ForgeDsField(state: (appStore.setting('dayStart', '9')).toString().trim().isEmpty ? ForgeDsFieldState.empty : ForgeDsFieldState.filled, fields: [gen_app_peruk21_behavior_c5, ''], control: DsField(label: gen_app_peruk21_behavior_c5, hint: '9', value: appStore.setting('dayStart', '9'), onChanged: (v) => appStore.setSetting('dayStart', v), bare: true)),
    ForgeDsField(state: (appStore.setting('blockMin', '30')).toString().trim().isEmpty ? ForgeDsFieldState.empty : ForgeDsFieldState.filled, fields: [gen_app_peruk21_behavior_c6, ''], control: DsField(label: gen_app_peruk21_behavior_c6, hint: '30', value: appStore.setting('blockMin', '30'), onChanged: (v) => appStore.setSetting('blockMin', v), bare: true)),
    DsToggleTile(label: gen_app_peruk21_behavior_c7, value: appStore.setting('always:rem') == '1' ? 'true' : 'false', onChanged: (v) => appStore.setSetting('always:rem', v == 'true' ? '1' : '')),
    DsSection(title: gen_app_peruk21_behavior_c8, children: [for (final e in appStore.log) DsLogRow(text: e['what'] ?? '', sub: _short(e['at'] ?? ''), undoLabel: e['undone'] == '1' ? '' : gen_app_peruk21_behavior_c9, onUndo: e['undone'] == '1' ? null : () => appStore.undo(e['id'] ?? ''))]),
  ]));
}
