...
String _fmtDate(String s) { final t = s.trim(); final d = DateTime.tryParse(t.length == 10 ? '${t}T12:00:00' : t); if (d == null) return t; final now = DateTime.now(); final iso = t.length >= 10 ? t.substring(0, 10) : t; final tIso = now.toIso8601String().substring(0, 10); final n = -cockpitDaysSince(iso, tIso).toInt(); if (n == 0) return gen_app_tasks_root_c36; if (n == 1) return gen_app_tasks_root_c37; if (n == -1) return gen_app_tasks_root_c38; final dm = dayMonthOfIso(iso, d.year != now.year); return n.abs() <= 6 ? gen_app_tasks_root_c39.replaceAll('{day}', gen_app_tasks_root_c40.split(',')[weekdayOfIso(iso)]) + ' ' + dm : dm; }   // ב׳-מג · תאריך במילים בתיק · G34 · דבק על חלקיקים (ימים-מאז · יום-בשבוע · יום.חודש)
