...
String _fmtDate(String s) { final t = s.trim(); final d = DateTime.tryParse(t.length == 10 ? '${t}T12:00:00' : t); if (d == null) return t; final now = DateTime.now(); final n = DateTime(d.year, d.month, d.day).difference(DateTime(now.year, now.month, now.day)).inDays; final dm = '${d.day}.${d.month}'; if (n == 0) return gen_app_tasks_root_c36 + ' (' + dm + ')'; if (n == 1) return gen_app_tasks_root_c37 + ' (' + dm + ')'; if (n == -1) return gen_app_tasks_root_c38 + ' (' + dm + ')'; if (n.abs() <= 6) return gen_app_tasks_root_c39.replaceAll('{day}', gen_app_tasks_root_c40.split(',')[d.weekday % 7]) + ' ' + dm; return dm + '.${d.year}'; }
...
class GenAppTasksRootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    return DsScaffold(title: appStore.displayOf('app_tasks_ent1', id), subtitle: const [gen_app_tasks_root_c34, gen_app_tasks_root_c35][appStore.stageOf('app_tasks_ent1', id).clamp(0, 1)], icon: gen_app_tasks_root_c44, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_tasks_root_c30, onTap: () { final lines = <String>[appStore.displayOf('app_tasks_ent1', id)]; for (final e in r0.entries) { if (e.key.startsWith('__') || e.value.trim().isEmpty) continue; lines.add(e.key + ': ' + (const <String>[gen_app_tasks_root_c31].contains(e.key) ? _fmtDate(e.value) : e.value.trim())); }   /* ב׳-עו · שיתוף עם תאריכים במילים */ launchUrl(Uri.parse('https://wa.me/?text=' + Uri.encodeComponent(lines.join('\n'))), mode: LaunchMode.externalApplication); })]))),
