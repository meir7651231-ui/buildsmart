...
class _GenBalaganTopicsScreenState extends State<GenBalaganTopicsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    for (final rg in [balaganRangeOf(_q, DateTime.now())]) if (rg.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [DsChipButton(label: gen_balagan_topics_c12.replaceAll('{from}', bhDayMonth(rg[0], false)).replaceAll('{to}', bhDayMonth(rg[1], false)).replaceAll('{n}', balaganRangeItems(rg[0], rg[1]).length.toString()), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => BalaganRange(from: rg[0], to: rg[1]))))])),   // ב׳-קל · G42 · «בין 1.9 ל-15.9»
    for (final wd in [balaganWeekOf(_q)]) if (wd != null) for (final rg in [bhWeekRange(bhIso(DateTime.now()), wd)]) Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [DsChipButton(label: gen_balagan_topics_c13.replaceAll('{week}', balaganWeekName(wd)).replaceAll('{n}', balaganRangeItems(rg[0], rg[1]).length.toString()), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => BalaganWeek(delta: wd))))])),   // ב׳-קכב · G40
    for (final af in [balaganAmountFilter(_q)]) if (af.isNotEmpty) for (final items in [balaganAmountItems(af[0], af[1])]) DsSection(title: (af[0] == '>' ? gen_balagan_topics_c14 : gen_balagan_topics_c15).replaceAll('{n}', balaganFmtMoney(double.tryParse(af[1]) ?? 0)).replaceAll('{m}', items.length.toString()), children: [for (final it in items.take(30)) DsNavTile(glyph: '', title: (_titleOf[it[0]] ?? it[0]) + ' · ' + appStore.displayOf(it[0], it[1]), sub: '₪ ' + it[2], onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => _open(it[0], it[1]))))]),   // ב׳-קכג · G40 · «מעל 5000»
    for (final mk in [balaganMonthOf(_q, DateTime.now())]) if (mk.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [DsChipButton(label: gen_balagan_topics_c16.replaceAll('{month}', balaganMonthName(mk)).replaceAll('{n}', balaganMonthItems(mk).length.toString()), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => BalaganMonth(monthKey: mk))))])),   // ב׳-קכא · G39 · «ספטמבר» ⇒ מסך-החודש
    for (final p in [balaganPersonFor(_q)]) if (p != null) DsSection(title: p.name, children: [
      Text(gen_balagan_topics_c17.replaceAll('{n}', p.files.toString()).replaceAll('{open}', p.open.toString()) + (p.money > 0 ? ' · ' + gen_balagan_topics_c18.replaceAll('{n}', balaganFmtMoney(p.money)) : '') + (p.last.isNotEmpty ? ' · ' + gen_balagan_topics_c19.replaceAll('{d}', (() { final d = DateTime.tryParse(p.last); return d == null ? p.last : balaganDayLabel(d, DateTime.now()); })()) : ''), style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)),
      if (p.open > 0) for (final t in [balaganPersonOpenText(p.name, DateTime.now())]) if (t.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [DsChipButton(label: gen_balagan_topics_c20, onTap: () { appStore.logAction('send', gen_balagan_topics_c21 + ' · ' + p.name); Clipboard.setData(ClipboardData(text: t)); launchUrl(Uri.parse('https://wa.me/' + (p.phones.isEmpty ? '' : balaganIntl(p.phones.first)) + '?text=' + Uri.encodeComponent(t)), mode: LaunchMode.externalApplication); })])),   // ב׳-צ · «שלח לו את הפתוחים» — ההודעה מוכנה מהתיקים, ללוח + לוואטסאפ שלו
      if (p.phones.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 8), child: Wrap(spacing: 8, runSpacing: 8, children: [for (final ph in p.phones.take(2)) ...[DsChipButton(label: gen_balagan_topics_c22 + ' ' + ph, onTap: () => launchUrl(Uri.parse('tel:' + ph), mode: LaunchMode.externalApplication)), DsChipButton(label: gen_balagan_topics_c23, onTap: () => launchUrl(Uri.parse('https://wa.me/' + balaganIntl(ph)), mode: LaunchMode.externalApplication))]])),
      for (final nAll in [appStore.log.where((e) => e['undone'] != '1' && p.rids.contains(e['rid'] ?? '')).length]) if (nAll > 3) Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [DsChipButton(label: gen_balagan_topics_c24.replaceAll('{n}', nAll.toString()), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => BalaganPersonLog(name: p.name))))])),   // ב׳-קלח · G44
      for (final recent in [appStore.log.where((e) => e['undone'] != '1' && p.rids.contains(e['rid'] ?? '')).take(3).toList()]) if (recent.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(gen_balagan_topics_c25, style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), for (final e in recent) DsLogRow(text: e['what'] ?? '', sub: (() { final at = DateTime.tryParse(e['at'] ?? ''); return at == null ? '' : balaganAgo(at, DateTime.now()); })(), undoLabel: '', onUndo: null)])),   // ב׳-פג · מה עשיתי איתו לאחרונה
      Padding(padding: const EdgeInsets.only(top: 8), child: DsQuickAdd(hint: gen_balagan_topics_c26.replaceAll('{who}', p.name), autofocus: false, onSubmit: (s0) { final s = s0.trim(); if (s.isEmpty) return; final hits = balaganIdentify(s); if (hits.isEmpty) return; final m = hits.first.module; final facts = balaganFacts(s, m); if (m.personFields.isNotEmpty && !m.personFields.any((f) => (facts[f] ?? '').trim().isNotEmpty)) facts[m.personFields.first] = p.name; Navigator.of(context).push<bool>(MaterialPageRoute<bool>(builder: (_) => GenBalaganConfirmScreen(module: m, facts: facts, alternatives: hits.skip(1).map((h) => h.module).toList(), text: s))); })),   // ב׳-נח · רגע עם האדם הזה: השם כבר בטופס
    ]),   // ב׳-לה · כרטיס-אדם: השם בחיפוש = אדם מהתיקים ⇒ סיכום + התקשר/וואטסאפ מעל התוצאות
    if (_q.trim().length >= 2 && hits.isEmpty) Padding(padding: const EdgeInsets.only(top: 8), child: DsNote(message: gen_balagan_topics_c27, label: '', tone: 0)),
    if (_q.trim().length >= 2 && hits.isEmpty) for (final sug in [bhClosest(_q, [...balaganPersonNames(), ..._titleOf.values])]) if (sug.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [DsChipButton(label: gen_balagan_topics_c28.replaceAll('{s}', sug), onTap: () => setState(() => _q = sug))])),   // ב׳-קז · «התכוונת ל…?» — הקרוב ביותר במרחק-עריכה מבין האנשים והמודולים
    if (_q.trim().isEmpty) ...(() { final counts = <String, int>{}; for (final m in kBalaganModules) { if (m.personFields.isEmpty) continue; for (final r in appStore.records(m.rootSlug)) { for (final f in m.personFields) { final v = (r[f] ?? '').trim(); if (v.length >= 2) counts[v] = (counts[v] ?? 0) + 1; } } } final names = counts.keys.toList()..sort((a, b) => counts[b]!.compareTo(counts[a]!)); final covered = <String>{}; final rows = <List<String>>[]; for (final n in names) { if (covered.contains(n.toLowerCase())) continue; final al = balaganAliases(n); covered.addAll(al); final others = [for (final x in names) if (x != n && al.contains(x.toLowerCase())) x]; rows.add([n, others.join(' · ')]); }   /* ב׳-קו · אותו אדם בכמה שמות = צ׳יפ אחד */ return rows.isEmpty ? <Widget>[] : [DsSection(title: gen_balagan_topics_c29, children: [Wrap(spacing: 8, runSpacing: 8, children: [for (final row in rows.take(12)) DsChipButton(label: [row[0], for (final p in [balaganPerson(row[0])]) if (p != null) p.files.toString(), for (final p in [balaganPerson(row[0])]) if (p != null && p.money > 0) '₪ ' + balaganFmtMoney(p.money), for (final p in [balaganPerson(row[0])]) if (p != null && p.open > 0 && p.last.length >= 10 && bhDaysSince(p.last, bhIso(DateTime.now())) >= 14) gen_balagan_topics_c30.replaceAll('{n}', bhDaysSince(p.last, bhIso(DateTime.now())).toString()), /* ב׳-קכ · G39 · תיק פתוח ואין נגיעה 14 יום */ if (row[1].isNotEmpty) gen_balagan_topics_c31.replaceAll('{names}', row[1])].join(' · '), onTap: () => setState(() => _q = row[0]))])])]; })(),   // «אנשים»: מי מופיע בתיקים (שדות-אדם מכל המודולים) ⇒ הקשה = חיפוש לפי השם
    if (_q.trim().isEmpty) for (final top in [balaganTopDescs()]) if (top.isNotEmpty) DsSection(title: gen_balagan_topics_c32, children: [Wrap(spacing: 8, runSpacing: 8, children: [for (final t in top.take(8)) DsChipButton(label: (t[0] as String) + ' · ' + t[1].toString(), onTap: () => setState(() => _q = t[0] as String))])]),   // ב׳-קלו · G43 · מה חוזר הכי הרבה בתיקים ⇒ הקשה = חיפוש
    if (_q.trim().isEmpty) ...(() { final seen = <String>{}; final rows = <Widget>[]; for (final e in appStore.log) { if (rows.length >= 5) break; final ent = e['entity'] ?? '', rid = e['rid'] ?? ''; if (ent.isEmpty || rid.isEmpty || e['undone'] == '1' || !_titleOf.containsKey(ent) || !seen.add(ent + '|' + rid) || appStore.byId(ent, rid) == null) continue; rows.add(DsNavTile(glyph: '', title: (_titleOf[ent] ?? ent) + ' · ' + appStore.displayOf(ent, rid), sub: [(() { final at = DateTime.tryParse(e['at'] ?? ''); return at == null ? '' : balaganAgo(at, DateTime.now()); })(), e['what'] ?? ''].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => _open(ent, rid))))); } return rows.isEmpty ? <Widget>[] : [DsSection(title: gen_balagan_topics_c33, children: rows)]; })(),   // «איפה הייתי»: התיקים שנגעת בהם לאחרונה, מהיומן
    if (hits.isNotEmpty) DsSection(title: gen_balagan_topics_c34.replaceAll('{n}', hits.length.toString()), children: [for (final h in hits.take(30)) DsNavTile(glyph: '', title: (_titleOf[h[0]] ?? h[0]) + ' · ' + appStore.displayOf(h[0], h[1]), sub: (() { final ms = kBalaganModules.where((m) => m.rootSlug == h[0]); final r = appStore.byId(h[0], h[1]); final ctx = ms.isEmpty || r == null ? '' : balaganDupSub(ms.first, r, DateTime.now()); final t = h[2].length > 60 ? h[2].substring(0, 60) + '…' : h[2]; return [ctx, t].where((x) => x.isNotEmpty).join(' · '); })(), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => _open(h[0], h[1]))))]),
    for (final to in [balaganTopicOpen('משימות ויומן')]) DsSection(title: gen_balagan_topics_c35 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c36.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c37.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c38, sub: [gen_balagan_topics_c39, balaganOpenCount('app_calendar_ent1', 2)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppCalendarEnt1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c40, sub: [gen_balagan_topics_c41, balaganOpenCount('app_tasks_ent1', 2)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppTasksEnt1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('דירה')]) DsSection(title: gen_balagan_topics_c42 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c43.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c44.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c45, sub: [gen_balagan_topics_c46, balaganOpenCount('app_peruk01_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk01Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c47, sub: [gen_balagan_topics_c48, balaganOpenCount('app_peruk02_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk02Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c49, sub: [gen_balagan_topics_c50, balaganOpenCount('app_peruk03_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk03Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c51, sub: [gen_balagan_topics_c52, balaganOpenCount('app_peruk04_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk04Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c53, sub: [gen_balagan_topics_c54, balaganOpenCount('app_peruk05_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk05Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('בית ספר')]) DsSection(title: gen_balagan_topics_c55 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c56.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c57.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c58, sub: [gen_balagan_topics_c59, balaganOpenCount('app_peruk21_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk21Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c60, sub: [gen_balagan_topics_c61, balaganOpenCount('app_peruk22_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk22Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c62, sub: [gen_balagan_topics_c63, balaganOpenCount('app_peruk23_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk23Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('רכב')]) DsSection(title: gen_balagan_topics_c64 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c65.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c66.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c67, sub: [gen_balagan_topics_c68, balaganOpenCount('app_peruk11_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk11Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c69, sub: [gen_balagan_topics_c70, balaganOpenCount('app_peruk12_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk12Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('בריאות')]) DsSection(title: gen_balagan_topics_c71 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c72.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c73.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c74, sub: [gen_balagan_topics_c75, balaganOpenCount('app_peruk13_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk13Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c76, sub: [gen_balagan_topics_c77, balaganOpenCount('app_peruk14_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk14Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c78, sub: [gen_balagan_topics_c79, balaganOpenCount('app_peruk15_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk15Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c80, sub: [gen_balagan_topics_c81, balaganOpenCount('app_peruk16_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk16Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('כסף')]) DsSection(title: gen_balagan_topics_c82 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c83.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c84.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c85, sub: [gen_balagan_topics_c86, balaganOpenCount('app_peruk06_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk06Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c87, sub: [gen_balagan_topics_c88, balaganOpenCount('app_peruk07_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk07Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c89, sub: [gen_balagan_topics_c90, balaganOpenCount('app_peruk08_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk08Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c91, sub: [gen_balagan_topics_c92, balaganOpenCount('app_peruk10_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk10Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c93, sub: [gen_balagan_topics_c94, balaganOpenCount('app_peruk17_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk17Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c95, sub: [gen_balagan_topics_c96, balaganOpenCount('app_peruk18_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk18Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c97, sub: [gen_balagan_topics_c98, balaganOpenCount('app_peruk19_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk19Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c99, sub: [gen_balagan_topics_c100, balaganOpenCount('app_peruk20_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk20Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c101, sub: [gen_balagan_topics_c102, balaganOpenCount('app_peruk26_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk26Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('הורים')]) DsSection(title: gen_balagan_topics_c103 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c104.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c105.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c106, sub: [gen_balagan_topics_c107, balaganOpenCount('app_peruk24_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk24Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('עבודה')]) DsSection(title: gen_balagan_topics_c108 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c109.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c110.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c111, sub: [gen_balagan_topics_c112, balaganOpenCount('app_peruk09_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk09Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c113, sub: [gen_balagan_topics_c114, balaganOpenCount('app_peruk25_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk25Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c115, sub: [gen_balagan_topics_c116, balaganOpenCount('app_peruk28_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk28Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('פרידה')]) DsSection(title: gen_balagan_topics_c117 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c118.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c119.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c120, sub: [gen_balagan_topics_c121, balaganOpenCount('app_peruk27_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk27Ent1Screen()))),
    ]),
    DsSection(title: gen_balagan_topics_c122, children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c123, sub: gen_balagan_topics_c124, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenBalaganKeysScreen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c125, sub: gen_balagan_topics_c126, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenBalaganBehaviorScreen()))),
    ]),
  ]);
  }
}
