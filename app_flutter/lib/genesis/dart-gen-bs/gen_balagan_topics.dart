...
class _GenBalaganTopicsScreenState extends State<GenBalaganTopicsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_q.trim().isEmpty) ...(() { final counts = <String, int>{}; for (final m in kBalaganModules) { if (m.personFields.isEmpty) continue; for (final r in appStore.records(m.rootSlug)) { for (final f in m.personFields) { final v = (r[f] ?? '').trim(); if (v.length >= 2) counts[v] = (counts[v] ?? 0) + 1; } } } final names = counts.keys.toList()..sort((a, b) => counts[b]!.compareTo(counts[a]!)); final covered = <String>{}; final rows = <List<String>>[]; for (final n in names) { if (covered.contains(n.toLowerCase())) continue; final al = balaganAliases(n); covered.addAll(al); final others = [for (final x in names) if (x != n && al.contains(x.toLowerCase())) x]; rows.add([n, others.join(' · ')]); }   /* ב׳-קו · אותו אדם בכמה שמות = צ׳יפ אחד */ return rows.isEmpty ? <Widget>[] : [DsSection(title: gen_balagan_topics_c32, children: [Wrap(spacing: 8, runSpacing: 8, children: [for (final row in rows.take(12)) DsChipButton(label: [row[0], for (final p in [balaganPerson(row[0])]) if (p != null) p.files.toString(), for (final p in [balaganPerson(row[0])]) if (p != null && p.money > 0) '₪ ' + balaganFmtMoney(p.money), for (final p in [balaganPerson(row[0])]) if (p != null && p.open > 0 && p.last.length >= 10 && bhDaysSince(p.last, bhIso(DateTime.now())) >= 14) gen_balagan_topics_c33.replaceAll('{n}', bhDaysSince(p.last, bhIso(DateTime.now())).toString()), /* ב׳-קכ · G39 · תיק פתוח ואין נגיעה 14 יום */ if (row[1].isNotEmpty) gen_balagan_topics_c34.replaceAll('{names}', row[1])].join(' · '), onTap: () => setState(() => _q = row[0]))])])]; })(),   // «אנשים»: מי מופיע בתיקים (שדות-אדם מכל המודולים) ⇒ הקשה = חיפוש לפי השם
    if (_q.trim().isEmpty) for (final top in [balaganTopDescs()]) if (top.isNotEmpty) DsSection(title: gen_balagan_topics_c35, children: [Wrap(spacing: 8, runSpacing: 8, children: [for (final t in top.take(8)) DsChipButton(label: (t[0] as String) + ' · ' + t[1].toString(), onTap: () => setState(() => _q = t[0] as String))])]),   // ב׳-קלו · G43 · מה חוזר הכי הרבה בתיקים ⇒ הקשה = חיפוש
    if (_q.trim().isEmpty) ...(() { final seen = <String>{}; final rows = <Widget>[]; for (final e in appStore.log) { if (rows.length >= 5) break; final ent = e['entity'] ?? '', rid = e['rid'] ?? ''; if (ent.isEmpty || rid.isEmpty || e['undone'] == '1' || !_titleOf.containsKey(ent) || !seen.add(ent + '|' + rid) || appStore.byId(ent, rid) == null) continue; rows.add(DsNavTile(glyph: '', title: (_titleOf[ent] ?? ent) + ' · ' + appStore.displayOf(ent, rid), sub: [(() { final at = DateTime.tryParse(e['at'] ?? ''); return at == null ? '' : balaganAgo(at, DateTime.now()); })(), e['what'] ?? ''].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => _open(ent, rid))))); } return rows.isEmpty ? <Widget>[] : [DsSection(title: gen_balagan_topics_c36, children: rows)]; })(),   // «איפה הייתי»: התיקים שנגעת בהם לאחרונה, מהיומן
    if (hits.isNotEmpty) DsSection(title: gen_balagan_topics_c37.replaceAll('{n}', hits.length.toString()), children: [for (final h in hits.take(30)) DsNavTile(glyph: '', title: (_titleOf[h[0]] ?? h[0]) + ' · ' + appStore.displayOf(h[0], h[1]), sub: (() { final ms = kBalaganModules.where((m) => m.rootSlug == h[0]); final r = appStore.byId(h[0], h[1]); final ctx = ms.isEmpty || r == null ? '' : balaganDupSub(ms.first, r, DateTime.now()); final t = h[2].length > 60 ? h[2].substring(0, 60) + '…' : h[2]; return [ctx, t].where((x) => x.isNotEmpty).join(' · '); })(), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => _open(h[0], h[1]))))]),
    for (final to in [balaganTopicOpen('משימות ויומן')]) DsSection(title: gen_balagan_topics_c38 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c39.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c40.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c41, sub: [gen_balagan_topics_c42, balaganOpenCount('app_calendar_ent1', 2)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppCalendarEnt1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c43, sub: [gen_balagan_topics_c44, balaganOpenCount('app_tasks_ent1', 2)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppTasksEnt1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('דירה')]) DsSection(title: gen_balagan_topics_c45 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c46.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c47.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c48, sub: [gen_balagan_topics_c49, balaganOpenCount('app_peruk01_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk01Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c50, sub: [gen_balagan_topics_c51, balaganOpenCount('app_peruk02_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk02Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c52, sub: [gen_balagan_topics_c53, balaganOpenCount('app_peruk03_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk03Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c54, sub: [gen_balagan_topics_c55, balaganOpenCount('app_peruk04_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk04Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c56, sub: [gen_balagan_topics_c57, balaganOpenCount('app_peruk05_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk05Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('בית ספר')]) DsSection(title: gen_balagan_topics_c58 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c59.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c60.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c61, sub: [gen_balagan_topics_c62, balaganOpenCount('app_peruk21_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk21Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c63, sub: [gen_balagan_topics_c64, balaganOpenCount('app_peruk22_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk22Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c65, sub: [gen_balagan_topics_c66, balaganOpenCount('app_peruk23_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk23Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('רכב')]) DsSection(title: gen_balagan_topics_c67 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c68.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c69.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c70, sub: [gen_balagan_topics_c71, balaganOpenCount('app_peruk11_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk11Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c72, sub: [gen_balagan_topics_c73, balaganOpenCount('app_peruk12_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk12Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('בריאות')]) DsSection(title: gen_balagan_topics_c74 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c75.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c76.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c77, sub: [gen_balagan_topics_c78, balaganOpenCount('app_peruk13_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk13Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c79, sub: [gen_balagan_topics_c80, balaganOpenCount('app_peruk14_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk14Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c81, sub: [gen_balagan_topics_c82, balaganOpenCount('app_peruk15_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk15Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c83, sub: [gen_balagan_topics_c84, balaganOpenCount('app_peruk16_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk16Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('כסף')]) DsSection(title: gen_balagan_topics_c85 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c86.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c87.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c88, sub: [gen_balagan_topics_c89, balaganOpenCount('app_peruk06_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk06Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c90, sub: [gen_balagan_topics_c91, balaganOpenCount('app_peruk07_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk07Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c92, sub: [gen_balagan_topics_c93, balaganOpenCount('app_peruk08_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk08Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c94, sub: [gen_balagan_topics_c95, balaganOpenCount('app_peruk10_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk10Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c96, sub: [gen_balagan_topics_c97, balaganOpenCount('app_peruk17_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk17Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c98, sub: [gen_balagan_topics_c99, balaganOpenCount('app_peruk18_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk18Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c100, sub: [gen_balagan_topics_c101, balaganOpenCount('app_peruk19_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk19Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c102, sub: [gen_balagan_topics_c103, balaganOpenCount('app_peruk20_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk20Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c104, sub: [gen_balagan_topics_c105, balaganOpenCount('app_peruk26_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk26Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('הורים')]) DsSection(title: gen_balagan_topics_c106 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c107.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c108.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c109, sub: [gen_balagan_topics_c110, balaganOpenCount('app_peruk24_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk24Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('עבודה')]) DsSection(title: gen_balagan_topics_c111 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c112.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c113.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c114, sub: [gen_balagan_topics_c115, balaganOpenCount('app_peruk09_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk09Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c116, sub: [gen_balagan_topics_c117, balaganOpenCount('app_peruk25_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk25Ent1Screen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c118, sub: [gen_balagan_topics_c119, balaganOpenCount('app_peruk28_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk28Ent1Screen()))),
    ]),
    for (final to in [balaganTopicOpen('פרידה')]) DsSection(title: gen_balagan_topics_c120 + ((to[0] as int) > 0 ? ' · ' + gen_balagan_topics_c121.replaceAll('{n}', to[0].toString()) + (to[1] > 0 ? ' · ' + gen_balagan_topics_c122.replaceAll('{n}', balaganFmtMoney(to[1].toDouble())) : '') : ''), children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c123, sub: [gen_balagan_topics_c124, balaganOpenCount('app_peruk27_ent1', 5)].where((x) => x.isNotEmpty).join(' · '), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenAppPeruk27Ent1Screen()))),
    ]),
    DsSection(title: gen_balagan_topics_c125, children: [
      DsNavTile(glyph: '', title: gen_balagan_topics_c126, sub: gen_balagan_topics_c127, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenBalaganKeysScreen()))),
      DsNavTile(glyph: '', title: gen_balagan_topics_c128, sub: gen_balagan_topics_c129, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const GenBalaganBehaviorScreen()))),
    ]),
  ]);
  }
}
