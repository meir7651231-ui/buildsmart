...
String balaganOpenCount(String slug, int stages) { var n = 0; for (final e in countBy(appStore.records(slug), (r) => stages == 0 || (int.tryParse(((r as Map)['__stage'] ?? '0').toString()) ?? 0) < stages - 1 ? 'open' : 'closed')) { if (e[0] == 'open') n = e[1] as int; } return n == 0 ? '' : gen_balagan_topics_c0.replaceAll('{n}', n.toString()); }   // G34 · דבק על חלקיק countBy
