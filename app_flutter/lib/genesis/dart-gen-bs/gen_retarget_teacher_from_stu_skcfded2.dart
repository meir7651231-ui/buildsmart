    ...
    return DsScaffold(
      ...
      children: [
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [
          ...
          if (_StuData.exportOk(_role)) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => _showText(context, 'ייצוא CSV · ${visible.length} תלמידים (BOM + חסימת-הזרקה)', _StuData.csvOf(visible)), child: ForgeSoftButton(fields: ['⬇ CSV (${visible.length})'])),
    ...
    switch (tab) {
      ...
      case 6: // מסמכים: Family.docs · תיק-רפואי (Member.health מוגן) · תעודות (מקום-שמור)
        ...
        return [
          ...
          if (docs.isEmpty) const ForgeSearchEmptyState(fields: ['אין מסמכים', '']) else for (final d in docs) TimelineItem(title: '📎 ${d['name']}', time: _StuData.fmt(d['startDate'] as String?)),
          _h('תיק-רפואי · 🔒 מוגן'),
