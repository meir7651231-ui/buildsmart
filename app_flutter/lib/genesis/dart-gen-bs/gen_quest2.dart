...
// 🧬 בקשה: עולם משחק שלם - חדר 2: · הירו 🎯 עולם משחק שלם של מסע בריחה - חדרים שמובילים זה אל זה חידות חיות שמגיבות לשחקן ושערים נסתרים בדרך אל היציאה | מטרה אחת קיבלתי - את המבנה האטומים והחיבורים בחרתי לבד · כותרת בורר המנועים - הבחירה מחליפה את המנוע החי המוצג · בחירה פרש ימים או שם חודש או כותב כי: פרש ימים / שם חודש / כותב כי · חישוב פרש ימים קלנדרי בין שני תאריכים חסין בונה חצות משני (daysBetweenDst) · חישוב שם חודש עברי (hebMonthHe) · חישוב כותב כי וא מבוסס (isoLocal) · מתג 🔓 מתג השער - פתיחתו חושפת מנוע חבוי · חישוב מגיעים כפרמטרים אין טהור דטרמיניסטי (validDateRange) · ניווט quest3 השער אל החדר הבא | המסע ממשיך · באנר מטרה אחת קיבלתי מהבעלים - את כל השאר תכננתי בחרתי וחיווטתי לבד מהמדף החי
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          IndexedStack(index: _n1, children: [Row(children: [RStat(value: daysBetweenDst(DateTime.now(), DateTime.now()).toString(), label: gen_quest2_stat_label)]), Row(children: [RStat(value: hebMonthHe(DateTime.now()), label: gen_quest2_stat_label2)]), Row(children: [RStat(value: isoLocal(DateTime.now()), label: gen_quest2_stat_label3)])]),
          SwitchRow(label: gen_quest2_switch_label, value: _v2, onChanged: (v) => setState(() => _v2 = v)),
          if (_v2) Row(children: [RStat(value: validDateRange(DateTime.now(), DateTime.now()).toString(), label: gen_quest2_stat_label4)]),
          HeroCard(glyph: gen_quest2_card_glyph2, title: gen_quest2_card_title2, sub: gen_quest2_card_sub2, onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const GenQuest3Screen())), cardColor: BsTokens.cardLight, inkColor: BsTokens.inkLight, mutedColor: BsTokens.mutedLight, borderColor: BsTokens.divider, radius: 12),
