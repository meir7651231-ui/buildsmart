...
import '../dart-data-bs/auto/gen_app_tasks_ent1_content.dart';
...
class GenAppTasksEnt1Screen extends StatefulWidget {
  const GenAppTasksEnt1Screen({this.scopeField, this.scopeId, this.initial, this.editId, super.key});
  ...
  @override
  State<GenAppTasksEnt1Screen> createState() => _GenAppTasksEnt1ScreenState();
}
...
class _GenAppTasksEnt1ScreenState extends State<GenAppTasksEnt1Screen> {
  ...
  @override
  void initState() { super.initState(); _prefill(); if (widget.editId != null) { final r = appStore.byId('app_tasks_ent1', widget.editId!); if (r != null) WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) _edit(r); }); } }
  ...
  void _save() {
    ...
    final miss = <String>[];
      if ((_v[0] ?? '').trim().isEmpty) miss.add('חסר ' + gen_app_tasks_ent1_c9);
    ...
    if (_editId != null) {
      ...
      appStore.add('app_tasks_ent1', <String, String>{...map, '__stage': '0'});
    }
  ...
  Widget build(BuildContext context) {
    final lk = DsLook.of(context);
    return DsScaffold(
      title: gen_app_tasks_ent1_c0,
      subtitle: gen_app_tasks_ent1_c1,
      icon: gen_app_tasks_ent1_c2,
      ...
      children: [
        ...
        DsSection(title: gen_app_tasks_ent1_c6, trailing: Row(mainAxisSize: MainAxisSize.min, children: [_viewBar(context), const SizedBox(width: 8), _csvBtn(context)]), children: [
          AnimatedBuilder(
            animation: appStore,
            builder: (context, _) {
              final all = (widget.scopeId == null ? appStore.records('app_tasks_ent1') : appStore.records('app_tasks_ent1').where((r) => (r[widget.scopeField ?? ''] ?? '') == widget.scopeId).toList());
