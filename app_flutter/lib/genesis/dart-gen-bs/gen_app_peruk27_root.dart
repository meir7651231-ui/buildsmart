...
String _fmtDate(String s) { final t = s.trim(); final d = DateTime.tryParse(t.length == 10 ? '${t}T12:00:00' : t); if (d == null) return t; final now = DateTime.now(); final n = DateTime(d.year, d.month, d.day).difference(DateTime(now.year, now.month, now.day)).inDays; final dm = '${d.day}.${d.month}'; if (n == 0) return gen_app_peruk27_root_c40 + ' (' + dm + ')'; if (n == 1) return gen_app_peruk27_root_c41 + ' (' + dm + ')'; if (n == -1) return gen_app_peruk27_root_c42 + ' (' + dm + ')'; if (n.abs() <= 6) return gen_app_peruk27_root_c43.replaceAll('{day}', gen_app_peruk27_root_c44.split(',')[d.weekday % 7]) + ' ' + dm; return dm + '.${d.year}'; }
...
class GenAppPeruk27RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk27_root_c45, subtitle: gen_app_peruk27_root_c46, icon: gen_app_peruk27_root_c47, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk27_ent1', id), subtitle: const [gen_app_peruk27_root_c35, gen_app_peruk27_root_c36, gen_app_peruk27_root_c37, gen_app_peruk27_root_c38, gen_app_peruk27_root_c39][appStore.stageOf('app_peruk27_ent1', id).clamp(0, 4)], icon: gen_app_peruk27_root_c48, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_peruk27_root_c32, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk27Ent1Screen(editId: id))))]))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__note'] ?? '').trim().isNotEmpty ? DsFold(title: gen_app_peruk27_root_c33, details: [Text(_noteText(r0['__note'] ?? ''), style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, height: 1.5))]) : const SizedBox.shrink())),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk27_root_c34, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
