...
String _fmtDate(String s) { final t = s.trim(); final d = DateTime.tryParse(t.length == 10 ? '${t}T12:00:00' : t); if (d == null) return t; final now = DateTime.now(); final n = DateTime(d.year, d.month, d.day).difference(DateTime(now.year, now.month, now.day)).inDays; final dm = '${d.day}.${d.month}'; if (n == 0) return gen_app_peruk27_root_c41 + ' (' + dm + ')'; if (n == 1) return gen_app_peruk27_root_c42 + ' (' + dm + ')'; if (n == -1) return gen_app_peruk27_root_c43 + ' (' + dm + ')'; if (n.abs() <= 6) return gen_app_peruk27_root_c44.replaceAll('{day}', gen_app_peruk27_root_c45.split(',')[d.weekday % 7]) + ' ' + dm; return dm + '.${d.year}'; }
