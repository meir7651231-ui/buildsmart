...
class GenAppPeruk10HomeScreenToday {
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true}) {   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk10_home_c34.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk10_home_c35, noLabel: gen_app_peruk10_home_c36, alwaysLabel: gen_app_peruk10_home_c37,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk10_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk10_home_c38.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk10_home_c39, noLabel: gen_app_peruk10_home_c40, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
  ...
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: module + ' · ' + (((r[gen_app_peruk10_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk10_home_c41 : (r[gen_app_peruk10_home_c0] ?? '')), trailing: Text(const [gen_app_peruk10_home_c6, gen_app_peruk10_home_c7, gen_app_peruk10_home_c8, gen_app_peruk10_home_c9, gen_app_peruk10_home_c10][appStore.stageOf('app_peruk10_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), children: [
        
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk10_home_c42, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk10_home_c43, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk10RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk10_home_c44.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk10_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk10HomeScreenState extends State<GenAppPeruk10HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk10_home_c45, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk10_home_c46, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk10_home_c47 : n <= 1 ? gen_app_peruk10_home_c48 : gen_app_peruk10_home_c49.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk10_home_c50, subtitle: lead, icon: gen_app_peruk10_home_c51, children: [
      DsLoadMeter(count: n, label: gen_app_peruk10_home_c52.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk10_home_c53, gen_app_peruk10_home_c54, gen_app_peruk10_home_c55]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk10_home_c56, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk10_home_c57, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk10HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk10_home_c58 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk10_home_c59 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk10_home_c60, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk10_home_c61 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk10_home_c62 + ' ' + gen_app_peruk10_home_c63, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
