...
class GenAppPeruk10HomeScreenToday {
  ...
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: (r[gen_app_peruk10_home_c0] ?? '') + ' · ' + const [gen_app_peruk10_home_c6, gen_app_peruk10_home_c7, gen_app_peruk10_home_c8, gen_app_peruk10_home_c9, gen_app_peruk10_home_c10][appStore.stageOf('app_peruk10_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], children: [
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk10_home_c32.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk10_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk10HomeScreenState extends State<GenAppPeruk10HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk10_home_c33, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk10_home_c34, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk10_home_c35 : n <= 1 ? gen_app_peruk10_home_c36 : gen_app_peruk10_home_c37.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk10_home_c38, subtitle: lead, icon: gen_app_peruk10_home_c39, children: [
      DsLoadMeter(count: n, label: gen_app_peruk10_home_c40.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk10_home_c41, gen_app_peruk10_home_c42, gen_app_peruk10_home_c43]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk10_home_c44, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk10_home_c45, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk10HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk10_home_c46 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk10_home_c47 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk10_home_c48, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk10_home_c49 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk10_home_c50 + ' ' + gen_app_peruk10_home_c51, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
