...
class GenAppPeruk10HomeScreenToday {
  static const module = gen_app_peruk10_home_c13;
  static const _dates = [];
  ...
  static Future<void> send(BuildContext context, Map<String, String> r0, String id0) async {
    appStore.logAction('send', gen_app_peruk10_home_c12.replaceAll('{who}', appStore.displayOf('app_peruk10_ent1', id0)), entity: 'app_peruk10_ent1', rid: id0, prev: appStore.stageOf('app_peruk10_ent1', id0).toString());
  ...
  static DsTodayItem _mk(String title, String sub, String rid, String field, DateTime d, bool hard, bool overdue, DateTime today, [String time = '', bool rep = false, String phone = '']) {
    final acts = <String>[...(overdue ? [gen_app_peruk10_home_c14, gen_app_peruk10_home_c15, gen_app_peruk10_home_c16, gen_app_peruk10_home_c17] : (d == today ? [gen_app_peruk10_home_c18, gen_app_peruk10_home_c19] : [gen_app_peruk10_home_c20, gen_app_peruk10_home_c21, gen_app_peruk10_home_c22])), if (phone.isNotEmpty) gen_app_peruk10_home_c23];   /* ב׳-לח · תיק עם טלפון ⇒ «התקשר» מהשורה, הקשה אחת */   // P4 · ביום-ההכרעה אין דחייה · «ליומן» = קישור-יומן, אפס-מפתח
  ...
  static void _act(String rid, String field, DateTime due, List<String> acts, int i) {
    ...
    if (a == gen_app_peruk10_home_c24) {
      ...
      appStore.logAction('done', gen_app_peruk10_home_c25.replaceAll('{what}', field + ' · ' + appStore.displayOf('app_peruk10_ent1', rid)), entity: 'app_peruk10_ent1', rid: rid, field: field, prev: prevStage);   // «עשיתי» + החזר (השורה חוזרת, השלב חוזר)
      if (r0 != null && rep.isNotEmpty) {   // ↻ רגע חוזר: «סיים» יוצר את הבא לבד (המועד-הבא בשדה שנסגר), עם החזר
        ...
        appStore.logAction('add', gen_app_peruk10_home_c26.replaceAll('{title}', appStore.displayOf('app_peruk10_ent1', nid) + ' · ' + next[field]!), entity: 'app_peruk10_ent1', rid: nid);
      }
    }
    else if (a == gen_app_peruk10_home_c27) { final r = appStore.byId('app_peruk10_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk10_ent1', rid, {field: _iso(_shift(due.add(const Duration(days: 1)), false))}); /* «דחה למחר» לא נוחת בשבת (אותו _shift של תזכורת-רכה) */ appStore.logAction('auto', gen_app_peruk10_home_c28 + ' · ' + field, entity: 'app_peruk10_ent1', rid: rid, field: field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
    else if (a == gen_app_peruk10_home_c29) { final r = appStore.byId('app_peruk10_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk10_ent1', rid, {field: _iso(_shift(due.add(const Duration(days: 7)), false))}); appStore.logAction('auto', gen_app_peruk10_home_c30 + ' · ' + field, entity: 'app_peruk10_ent1', rid: rid, field: field, prev: prev); } }   /* «דחה לשבוע» — נגיעה-ידנית עם החזר, לא בשבת */
    else if (a == gen_app_peruk10_home_c31) {   // «ליומן»: עם שעה ⇒ אירוע בשעתו (אורך = בלוק-ההגדרה); בלי ⇒ יום-שלם
    ...
    else if (a == gen_app_peruk10_home_c32) { final r = appStore.byId('app_peruk10_ent1', rid); final ph = r == null ? '' : _phoneOf(r); if (ph.isNotEmpty) launchUrl(Uri.parse('tel:' + ph), mode: LaunchMode.externalApplication); }
    else { appStore.decide('ign:$rid:$field', 'no'); appStore.logAction('decide', gen_app_peruk10_home_c33.replaceAll('{what}', field + ' · ' + appStore.displayOf('app_peruk10_ent1', rid)), entity: 'app_peruk10_ent1', rid: rid, field: 'ign:$rid:$field'); }   /* ב׳-לב · «התעלם» נרשם ביומן ⇒ החזר (kind=decide: field = מפתח-ההכרעה) — שום דבר לא בלתי-הפיך */
  }
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk10_ent1', rid); final tm = _timeOf(r); final rep = (r['__repeat'] ?? '').trim().isNotEmpty; final ph = _phoneOf(r);
      for (final f in _dates) {
        final d = _parse(r[f.label] ?? ''); if (d == null) continue;
        if (appStore.decision('ign:$rid:${f.label}') == 'no') continue;
        if (dayDelta == 0 && d.isBefore(today)) { final ago = today.difference(d).inDays; out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', gen_app_peruk10_home_c34.replaceAll('{date}', _iso(d)) + ' · ' + (ago == 1 ? gen_app_peruk10_home_c35 : gen_app_peruk10_home_c36.replaceAll('{n}', ago.toString())), rid, f.label, d, f.hard, true, today, tm, rep, ph)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', off == 0 ? '' : gen_app_peruk10_home_c37.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep, ph)); break; }
        }
      }
    }
  ...
  static List<DsTodayItem> undated(DateTime today) {
    ...
    for (final r in open()) {
      ...
      final acts = [gen_app_peruk10_home_c38, gen_app_peruk10_home_c39, gen_app_peruk10_home_c40];
  ...
  static void _setDate(String rid, String field, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    if (a == gen_app_peruk10_home_c41) { appStore.decide('undated:$rid', 'no'); appStore.logAction('decide', gen_app_peruk10_home_c42.replaceAll('{what}', appStore.displayOf('app_peruk10_ent1', rid)), entity: 'app_peruk10_ent1', rid: rid, field: 'undated:$rid'); return; }
    final r = appStore.byId('app_peruk10_ent1', rid); if (r == null) return;
    final prev = r[field] ?? '';
    appStore.update('app_peruk10_ent1', rid, {field: _iso(_shift(today.add(Duration(days: a == gen_app_peruk10_home_c43 ? 7 : 1)), false))});   /* «קבע» לא נוחת בשבת */
  ...
  static List<DsTodayItem> stale(DateTime today) {
    ...
    for (final r in open()) {
      ...
      out.add(DsTodayItem(title: appStore.displayOf('app_peruk10_ent1', rid), sub: gen_app_peruk10_home_c47.replaceAll('{n}', n.toString()), rid: rid, field: '', due: t, hard: false, overdue: false, module: module, actions: acts, act: (i) => _staleAct(rid, today, acts, i)));
    }
  ...
  static void _staleAct(String rid, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    final r = appStore.byId('app_peruk10_ent1', rid); if (r == null) return;
    if (a == gen_app_peruk10_home_c48) { final f = _dates.firstWhere((x) => x.hard, orElse: () => _dates.first); appStore.decide('ign:$rid:${f.label}', ''); appStore.decide('undated:$rid', ''); _setDate(rid, f.label, today, [a], 0); return; }   /* המועד החדש חוזר ל«היום» — ההתעלמות הישנה נמחקת */
    if (a == gen_app_peruk10_home_c49) { final prev = r[AppStore.stageKey] ?? '0'; appStore.update('app_peruk10_ent1', rid, {AppStore.stageKey: '4'}); appStore.logAction('auto', gen_app_peruk10_home_c50.replaceAll('{who}', appStore.displayOf('app_peruk10_ent1', rid)), entity: 'app_peruk10_ent1', rid: rid, field: AppStore.stageKey, prev: prev); return; }
    appStore.decide('stale:$rid', 'no'); appStore.logAction('decide', gen_app_peruk10_home_c51.replaceAll('{what}', appStore.displayOf('app_peruk10_ent1', rid)), entity: 'app_peruk10_ent1', rid: rid, field: 'stale:$rid');
  }
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true, bool rem = true}) {   // rem=false: «בלגן» מרכז את התזכורות לכרטיס אחד (ב׳-לז)   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      if (rem) for (final f in _dates) {
        ...
        if (appStore.decision(_remKey(rid, f.label)).isNotEmpty) continue;
        out.add(DsApproveCard(question: gen_app_peruk10_home_c52.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk10_home_c53, noLabel: gen_app_peruk10_home_c54, alwaysLabel: gen_app_peruk10_home_c55,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk10_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk10_home_c56.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk10_home_c57, noLabel: gen_app_peruk10_home_c58, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
  ...
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: module + ' · ' + (((r[gen_app_peruk10_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk10_home_c59 : (r[gen_app_peruk10_home_c0] ?? '')), trailing: Text(const [gen_app_peruk10_home_c6, gen_app_peruk10_home_c7, gen_app_peruk10_home_c8, gen_app_peruk10_home_c9, gen_app_peruk10_home_c10][appStore.stageOf('app_peruk10_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), children: [
        
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk10_home_c60, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk10_home_c61, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk10RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk10_home_c62.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk10_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk10HomeScreenState extends State<GenAppPeruk10HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk10_home_c63, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk10_home_c64, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk10_home_c65 : n <= 1 ? gen_app_peruk10_home_c66 : gen_app_peruk10_home_c67.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk10_home_c68, subtitle: lead, icon: gen_app_peruk10_home_c69, children: [
      DsLoadMeter(count: n, label: gen_app_peruk10_home_c70.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk10_home_c71, gen_app_peruk10_home_c72, gen_app_peruk10_home_c73]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk10_home_c74, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk10_home_c75, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk10HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk10_home_c76 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk10_home_c77 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk10_home_c78, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk10_home_c79 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk10_home_c80 + ' ' + gen_app_peruk10_home_c81, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
