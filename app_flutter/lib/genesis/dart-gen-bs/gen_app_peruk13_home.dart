...
class _GenAppPeruk13HomeScreenState extends State<GenAppPeruk13HomeScreen> {
  ...
  List<Map<String, String>> get _open => appStore.records('app_peruk13_ent1').where((r) => appStore.stageOf('app_peruk13_ent1', r[AppStore.idKey] ?? '') < 4).toList();
  ...
  List<_Item> _items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in _open) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { out.add(_Item('${f.label} · $who', gen_app_peruk13_home_c12.replaceAll('{date}', _iso(d)), rid, f.label, d, f.hard, true)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_Item('${f.label} · $who', off == 0 ? gen_app_peruk13_home_c13 : gen_app_peruk13_home_c14.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false)); break; }
        }
      }
    }
  ...
  List<Widget> _proposals(BuildContext context, DateTime today) {
    ...
    for (final r in _open) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk13_home_c15.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: who, okLabel: gen_app_peruk13_home_c16, noLabel: gen_app_peruk13_home_c17, alwaysLabel: gen_app_peruk13_home_c18,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk13_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk13_home_c19.replaceAll('{n}', n.toString()), source: who, okLabel: gen_app_peruk13_home_c20, noLabel: gen_app_peruk13_home_c21, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); _send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
  ...
  void _autopilot() {
    ...
    for (final r in _open) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk13_home_c22.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk13_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk13_home_c23, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk13_home_c24, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  void _act(_Item it, int i, DateTime today) {
    final labels = it.overdue ? [gen_app_peruk13_home_c25, gen_app_peruk13_home_c26, gen_app_peruk13_home_c27] : (it.due == today ? [gen_app_peruk13_home_c28] : [gen_app_peruk13_home_c29, gen_app_peruk13_home_c30]);   // P4 · ביום-ההכרעה אין דחייה
    final a = labels[i.clamp(0, labels.length - 1)];
    if (a == gen_app_peruk13_home_c31) { appStore.advance('app_peruk13_ent1', it.rid, 5); }
    else if (a == gen_app_peruk13_home_c32) { final r = appStore.byId('app_peruk13_ent1', it.rid); if (r != null) { final prev = r[it.field] ?? ''; appStore.update('app_peruk13_ent1', it.rid, {it.field: _iso(it.due.add(const Duration(days: 1)))}); appStore.logAction('auto', gen_app_peruk13_home_c33 + ' · ' + it.title, entity: 'app_peruk13_ent1', rid: it.rid, field: it.field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk13_home_c34 : n <= 1 ? gen_app_peruk13_home_c35 : gen_app_peruk13_home_c36.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk13_home_c37, subtitle: lead, icon: gen_app_peruk13_home_c38, children: [
      DsLoadMeter(count: n, label: gen_app_peruk13_home_c39.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk13_home_c40, gen_app_peruk13_home_c41, gen_app_peruk13_home_c42]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk13_home_c43, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: [gen_app_peruk13_home_c44, gen_app_peruk13_home_c45, gen_app_peruk13_home_c46], onAct: (i) => _act(it, i, today))]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk13_home_c47, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.due == today ? [gen_app_peruk13_home_c48] : [gen_app_peruk13_home_c49, gen_app_peruk13_home_c50], onAct: (i) => _act(it, i, today))]),
      for (final r in open) DsSection(title: (r[gen_app_peruk13_home_c0] ?? '') + ' · ' + const [gen_app_peruk13_home_c6, gen_app_peruk13_home_c7, gen_app_peruk13_home_c8, gen_app_peruk13_home_c9, gen_app_peruk13_home_c10][appStore.stageOf('app_peruk13_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], children: [
      ...
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk13_home_c51 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk13_home_c52 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk13_home_c53, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk13_home_c54 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk13_home_c55 + ' ' + gen_app_peruk13_home_c56, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
