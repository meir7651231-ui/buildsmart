...
class GenAppPeruk04RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk04_root_c93, subtitle: gen_app_peruk04_root_c94, icon: gen_app_peruk04_root_c95, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk04_ent1', id), subtitle: const [gen_app_peruk04_root_c88, gen_app_peruk04_root_c89, gen_app_peruk04_root_c90, gen_app_peruk04_root_c91, gen_app_peruk04_root_c92][appStore.stageOf('app_peruk04_ent1', id).clamp(0, 4)], icon: gen_app_peruk04_root_c96, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__note'] ?? '').trim().isNotEmpty ? DsFold(title: gen_app_peruk04_root_c86, details: [Text(r0['__note'] ?? '', style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, height: 1.5))]) : const SizedBox.shrink())),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk04_root_c87, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
