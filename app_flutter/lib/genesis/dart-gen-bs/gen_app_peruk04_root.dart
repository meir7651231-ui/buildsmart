...
class GenAppPeruk04RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk04_root_c96, subtitle: gen_app_peruk04_root_c97, icon: gen_app_peruk04_root_c98, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk04_ent1', id), subtitle: const [gen_app_peruk04_root_c91, gen_app_peruk04_root_c92, gen_app_peruk04_root_c93, gen_app_peruk04_root_c94, gen_app_peruk04_root_c95][appStore.stageOf('app_peruk04_ent1', id).clamp(0, 4)], icon: gen_app_peruk04_root_c99, children: [
      Padding(padding: const EdgeInsets.only(bottom: 12), child: DsSection(title: gen_app_peruk04_root_c82 + ' · ' + appStore.referencing('app_peruk04_ent2', gen_app_peruk04_root_c75, id).length.toString(), children: [for (final r in appStore.referencing('app_peruk04_ent2', gen_app_peruk04_root_c75, id)) DsNavTile(glyph: gen_app_peruk04_root_c81, title: (r[gen_app_peruk04_root_c76] ?? ''), sub: (r[gen_app_peruk04_root_c77] ?? ''), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk04Ent2Screen(scopeField: gen_app_peruk04_root_c78, scopeId: id)))), DsChipButton(label: gen_app_peruk04_root_c79, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk04Ent2Screen(scopeField: gen_app_peruk04_root_c78, scopeId: id))))], tone: 0)),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: DsChipButton(label: gen_app_peruk04_root_c83, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk04Rp1Screen(initialId: id))))),
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: (() { final ph = [(r0[gen_app_peruk04_root_c86] ?? '')].map((x) => x.replaceAll(RegExp(r'[^0-9+]'), '')).firstWhere((x) => x.length >= 9, orElse: () => ''); if (ph.isEmpty) return const SizedBox.shrink(); final intl = ph.startsWith('+') ? ph.substring(1) : (ph.startsWith('0') ? '972' + ph.substring(1) : ph); return Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_peruk04_root_c87, onTap: () => launchUrl(Uri.parse('tel:' + ph), mode: LaunchMode.externalApplication)), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk04_root_c88, onTap: () => launchUrl(Uri.parse('https://wa.me/' + intl), mode: LaunchMode.externalApplication))])); })()),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__note'] ?? '').trim().isNotEmpty ? DsFold(title: gen_app_peruk04_root_c89, details: [Text(r0['__note'] ?? '', style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, height: 1.5))]) : const SizedBox.shrink())),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk04_root_c90, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
