...
class _GenBalaganConfirmScreenState extends State<GenBalaganConfirmScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: m.title, subtitle: gen_balagan_confirm_c5, icon: gen_balagan_confirm_c6, children: [
      ...
      if ((_v['__repeat'] ?? '').isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 8), child: DsNote(message: gen_balagan_confirm_c10.replaceAll('{every}', balaganRepeatLabel(_v['__repeat']!)), label: '', tone: 0)),   // ב׳-לד · גם מצ׳יפ
      ...
      for (final f in shown) ...[_field(f), if (f.label == dateF && (_v[dateF] ?? '').trim().isEmpty) Padding(padding: const EdgeInsets.only(bottom: 10), child: Wrap(spacing: 8, runSpacing: 8, children: [for (final c in balaganDateChips(DateTime.now())) DsChipButton(label: c[0], onTap: () => setState(() => _v[dateF] = c[1]))])),   // ב׳-לא · «מתי?» — הקשה אחת
        ...
        if (m.numFields.isNotEmpty && f.type == 'num' && !m.percentFields.contains(f.label) && f.label == m.numFields.firstWhere((x) => !m.percentFields.contains(x), orElse: () => '') && (_v[f.label] ?? '').trim().isEmpty) for (final a in [balaganLastAmount(m, m.personFields.isEmpty ? '' : (_v[m.personFields.first] ?? ''))]) if (a.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 10), child: Wrap(spacing: 8, runSpacing: 8, children: [DsChipButton(label: gen_balagan_confirm_c26.replaceAll('{n}', a), onTap: () => setState(() => _v[f.label] = a))]))],   // ב׳-פט · «₪ 1,500 כמו בפעם הקודמת» — הסכום מהתיק האחרון (אותו אדם אם יש), מהנתונים
      if (rest.isNotEmpty) DsFold(title: gen_balagan_confirm_c27.replaceAll('{n}', rest.length.toString()), details: [for (final f in rest) _field(f)]),
      Padding(padding: const EdgeInsets.only(top: 10), child: Row(children: [DsChipButton(label: gen_balagan_confirm_c28, onTap: () => _save(again: true))])),   // ב׳-עח
      Padding(padding: const EdgeInsets.only(top: 14), child: DsPrimaryButton(label: (() { if (m.stages > 0 && balaganIsPast(widget.text)) return gen_balagan_confirm_c29; /* ב׳-קא */ if (dateF.isEmpty) return gen_balagan_confirm_c30; final d = DateTime.tryParse((_v[dateF] ?? '').trim()); if (d == null) return gen_balagan_confirm_c31; final t0 = DateTime.now(); final n = -bhDaysSince(bhIso(d), bhIso(t0)); return n == 0 ? gen_balagan_confirm_c32 : n < 0 ? gen_balagan_confirm_c33 : gen_balagan_confirm_c34.replaceAll('{day}', balaganDayLabel(d, t0)); })(), onTap: () => _save())   /* ב׳-סא · «יופיע במחר» — האדם יודע לאן זה הולך */),
    ]);
  }
}
