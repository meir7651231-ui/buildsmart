...
class GenBalaganConfirmScreen extends StatefulWidget {
  const GenBalaganConfirmScreen({required this.module, required this.facts, super.key});
...
class _GenBalaganConfirmScreenState extends State<GenBalaganConfirmScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: m.title, subtitle: gen_balagan_confirm_c0, icon: gen_balagan_confirm_c1, children: [
      ...
      if (rest.isNotEmpty) DsFold(title: gen_balagan_confirm_c2.replaceAll('{n}', rest.length.toString()), details: [for (final f in rest) _field(f)]),
      Padding(padding: const EdgeInsets.only(top: 14), child: DsPrimaryButton(label: gen_balagan_confirm_c3, onTap: _save)),
    ]);
  }
}
