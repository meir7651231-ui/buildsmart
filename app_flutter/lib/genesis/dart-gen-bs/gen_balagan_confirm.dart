...
class _GenBalaganConfirmScreenState extends State<GenBalaganConfirmScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: m.title, subtitle: gen_balagan_confirm_c5, icon: gen_balagan_confirm_c6, children: [
      ...
      if ((_v['__repeat'] ?? '').isEmpty) for (final code in [balaganRecurHint(m, _v)]) if (code.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(children: [Expanded(child: DsNote(message: gen_balagan_confirm_c11.replaceAll('{every}', balaganRepeatLabel(code)), label: '', tone: 0)), const SizedBox(width: 8), DsChipButton(label: gen_balagan_confirm_c12.replaceAll('{every}', balaganRepeatLabel(code)), onTap: () => setState(() => _v['__repeat'] = code))])),   // ב׳-קיא · G37 · אותו מתאר חזר במרווחים קבועים ⇒ הצעה לסמן ↻
      if (widget.queue.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 8), child: DsNote(message: gen_balagan_confirm_c13.replaceAll('{n}', widget.queue.length.toString()), label: '', tone: 0)),
      if (!_forceNew) for (final d in balaganPastMatches(m, widget.text).take(1)) DsApproveCard(question: gen_balagan_confirm_c14.replaceAll('{who}', appStore.displayOf(m.rootSlug, d['__id'] ?? '')), source: balaganDupSub(m, d, DateTime.now()), okLabel: gen_balagan_confirm_c15, noLabel: gen_balagan_confirm_c16, onOk: () { balaganCloseFile(m, d['__id'] ?? ''); Navigator.of(context).pop(true); }, onNo: () => setState(() => _forceNew = true)),   // ב׳-צט · «שילמתי ארנונה» + תיק פתוח «לשלם ארנונה» ⇒ «סיימת אותו?» — הקשה אחת סוגרת, עם החזר
      if (!_forceNew) for (final d in balaganDuplicates(m, _v).where((d) => !balaganPastMatches(m, widget.text).contains(d)).take(1)) DsApproveCard(question: gen_balagan_confirm_c17.replaceAll('{who}', appStore.displayOf(m.rootSlug, d['__id'] ?? '')), source: balaganDupSub(m, d, DateTime.now()), okLabel: gen_balagan_confirm_c18, noLabel: gen_balagan_confirm_c19, onOk: () { final id = d['__id'] ?? ''; final n = balaganMerge(m, id, {for (final e in _v.entries) if (e.value.trim().isNotEmpty) e.key: e.value, if (widget.doc.isNotEmpty) '__doc': widget.doc}, gen_balagan_confirm_c20.replaceAll('{who}', appStore.displayOf(m.rootSlug, id))); Navigator.of(context).pushReplacement<bool, bool>(MaterialPageRoute<bool>(builder: (_) => _openRoot(m.rootSlug, id))); if (n == 0) return; }, onNo: () => setState(() => _forceNew = true)),   // «פתח את הקיים» = המידע החדש נכנס לתיק הקיים (שדות ריקים + «מה כתבת» נצבר), עם החזר   // תיק כפול: «זה אותו עניין?» לפני שנפתח תיק שני
      for (final f in shown) ...[_field(f), if (f.label == dateF && (_v[dateF] ?? '').trim().isEmpty) Padding(padding: const EdgeInsets.only(bottom: 10), child: Wrap(spacing: 8, runSpacing: 8, children: [for (final c in balaganDateChips(DateTime.now())) DsChipButton(label: c[0], onTap: () => setState(() => _v[dateF] = c[1]))])),   // ב׳-לא · «מתי?» — הקשה אחת
        ...
        if (f.label == dateF && (_v[dateF] ?? '').trim().isNotEmpty && (_v['__repeat'] ?? '').trim().isEmpty) Padding(padding: const EdgeInsets.only(bottom: 10), child: Wrap(spacing: 8, runSpacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: [Text(gen_balagan_confirm_c21, style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), for (final c in balaganRepeatChips()) DsChipButton(label: c[0], onTap: () => setState(() => _v['__repeat'] = c[1]))])),   // ב׳-לד · «חוזר?» — אחרי שיש מועד
        ...
        if (m.numFields.isNotEmpty && f.type == 'num' && !m.percentFields.contains(f.label) && f.label == m.numFields.firstWhere((x) => !m.percentFields.contains(x), orElse: () => '') && (_v[f.label] ?? '').trim().isEmpty) for (final a in [balaganLastAmount(m, m.personFields.isEmpty ? '' : (_v[m.personFields.first] ?? ''))]) if (a.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 10), child: Wrap(spacing: 8, runSpacing: 8, children: [DsChipButton(label: gen_balagan_confirm_c22.replaceAll('{n}', a), onTap: () => setState(() => _v[f.label] = a))]))],   // ב׳-פט · «₪ 1,500 כמו בפעם הקודמת» — הסכום מהתיק האחרון (אותו אדם אם יש), מהנתונים
      if (rest.isNotEmpty) DsFold(title: gen_balagan_confirm_c23.replaceAll('{n}', rest.length.toString()), details: [for (final f in rest) _field(f)]),
      Padding(padding: const EdgeInsets.only(top: 10), child: Row(children: [DsChipButton(label: gen_balagan_confirm_c24, onTap: () => _save(again: true))])),   // ב׳-עח
      Padding(padding: const EdgeInsets.only(top: 14), child: DsPrimaryButton(label: (() { if (m.stages > 0 && balaganIsPast(widget.text)) return gen_balagan_confirm_c25; /* ב׳-קא */ if (dateF.isEmpty) return gen_balagan_confirm_c26; final d = DateTime.tryParse((_v[dateF] ?? '').trim()); if (d == null) return gen_balagan_confirm_c27; final t0 = DateTime.now(); final n = -bhDaysSince(bhIso(d), bhIso(t0)); return n == 0 ? gen_balagan_confirm_c28 : n < 0 ? gen_balagan_confirm_c29 : gen_balagan_confirm_c30.replaceAll('{day}', balaganDayLabel(d, t0)); })(), onTap: () => _save())   /* ב׳-סא · «יופיע במחר» — האדם יודע לאן זה הולך */),
    ]);
  }
}
