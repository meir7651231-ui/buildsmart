...
class _GenBalaganConfirmScreenState extends State<GenBalaganConfirmScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: m.title, subtitle: gen_balagan_confirm_c5, icon: gen_balagan_confirm_c6, children: [
      ...
      if (widget.queue.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 8), child: DsNote(message: gen_balagan_confirm_c10.replaceAll('{n}', widget.queue.length.toString()), label: '', tone: 0)),
      if (!_forceNew) for (final d in balaganPastMatches(m, widget.text).take(1)) DsApproveCard(question: gen_balagan_confirm_c11.replaceAll('{who}', appStore.displayOf(m.rootSlug, d['__id'] ?? '')), source: balaganDupSub(m, d, DateTime.now()), okLabel: gen_balagan_confirm_c12, noLabel: gen_balagan_confirm_c13, onOk: () { balaganCloseFile(m, d['__id'] ?? ''); Navigator.of(context).pop(true); }, onNo: () => setState(() => _forceNew = true)),   // ב׳-צט · «שילמתי ארנונה» + תיק פתוח «לשלם ארנונה» ⇒ «סיימת אותו?» — הקשה אחת סוגרת, עם החזר
      if (!_forceNew) for (final d in balaganDuplicates(m, _v).where((d) => !balaganPastMatches(m, widget.text).contains(d)).take(1)) DsApproveCard(question: gen_balagan_confirm_c14.replaceAll('{who}', appStore.displayOf(m.rootSlug, d['__id'] ?? '')), source: balaganDupSub(m, d, DateTime.now()), okLabel: gen_balagan_confirm_c15, noLabel: gen_balagan_confirm_c16, onOk: () { final id = d['__id'] ?? ''; final n = balaganMerge(m, id, {for (final e in _v.entries) if (e.value.trim().isNotEmpty) e.key: e.value, if (widget.doc.isNotEmpty) '__doc': widget.doc}, gen_balagan_confirm_c17.replaceAll('{who}', appStore.displayOf(m.rootSlug, id))); Navigator.of(context).pushReplacement<bool, bool>(MaterialPageRoute<bool>(builder: (_) => _openRoot(m.rootSlug, id))); if (n == 0) return; }, onNo: () => setState(() => _forceNew = true)),   // «פתח את הקיים» = המידע החדש נכנס לתיק הקיים (שדות ריקים + «מה כתבת» נצבר), עם החזר   // תיק כפול: «זה אותו עניין?» לפני שנפתח תיק שני
      for (final f in shown) ...[_field(f), if (f.label == dateF && (_v[dateF] ?? '').trim().isEmpty) Padding(padding: const EdgeInsets.only(bottom: 10), child: Wrap(spacing: 8, runSpacing: 8, children: [for (final c in balaganDateChips(DateTime.now())) DsChipButton(label: c[0], onTap: () => setState(() => _v[dateF] = c[1]))])),   // ב׳-לא · «מתי?» — הקשה אחת
        ...
        if (f.label == dateF && (_v[dateF] ?? '').trim().isNotEmpty && (_v['__repeat'] ?? '').trim().isEmpty) Padding(padding: const EdgeInsets.only(bottom: 10), child: Wrap(spacing: 8, runSpacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: [Text(gen_balagan_confirm_c18, style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), for (final c in balaganRepeatChips()) DsChipButton(label: c[0], onTap: () => setState(() => _v['__repeat'] = c[1]))])),   // ב׳-לד · «חוזר?» — אחרי שיש מועד
