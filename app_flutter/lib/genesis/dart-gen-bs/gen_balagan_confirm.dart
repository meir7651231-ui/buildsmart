  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: m.title, subtitle: gen_balagan_confirm_c4, icon: gen_balagan_confirm_c5, children: [
      ...
      for (final f in shown) ...[_field(f), if (f.label == dateF && (_v[dateF] ?? '').trim().isEmpty) Padding(padding: const EdgeInsets.only(bottom: 10), child: Wrap(spacing: 8, runSpacing: 8, children: [for (final c in balaganDateChips(DateTime.now())) DsChipButton(label: c[0], onTap: () => setState(() => _v[dateF] = c[1]))])),   // ב׳-לא · «מתי?» — הקשה אחת
        ...
        if (m.numFields.isNotEmpty && f.type == 'num' && !m.percentFields.contains(f.label) && f.label == m.numFields.firstWhere((x) => !m.percentFields.contains(x), orElse: () => '') && (_v[f.label] ?? '').trim().isEmpty) for (final a in [balaganLastAmount(m, m.personFields.isEmpty ? '' : (_v[m.personFields.first] ?? ''))]) if (a.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 10), child: Wrap(spacing: 8, runSpacing: 8, children: [DsChipButton(label: gen_balagan_confirm_c15.replaceAll('{n}', a), onTap: () => setState(() => _v[f.label] = a))]))],   // ב׳-פט · «₪ 1,500 כמו בפעם הקודמת» — הסכום מהתיק האחרון (אותו אדם אם יש), מהנתונים
      if (rest.isNotEmpty) DsFold(title: gen_balagan_confirm_c16.replaceAll('{n}', rest.length.toString()), details: [for (final f in rest) _field(f)]),
      Padding(padding: const EdgeInsets.only(top: 10), child: Row(children: [DsChipButton(label: gen_balagan_confirm_c17, onTap: () => _save(again: true))])),   // ב׳-עח
      Padding(padding: const EdgeInsets.only(top: 14), child: DsPrimaryButton(label: (() { if (dateF.isEmpty) return gen_balagan_confirm_c18; final d = DateTime.tryParse((_v[dateF] ?? '').trim()); if (d == null) return gen_balagan_confirm_c19; final t0 = DateTime.now(); final n = DateTime(d.year, d.month, d.day).difference(DateTime(t0.year, t0.month, t0.day)).inDays; return n == 0 ? gen_balagan_confirm_c20 : gen_balagan_confirm_c21.replaceAll('{day}', balaganDayLabel(d, t0)); })(), onTap: () => _save())   /* ב׳-סא · «יופיע במחר» — האדם יודע לאן זה הולך */),
    ]);
  }
}
