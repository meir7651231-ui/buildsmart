...
class _GenBalaganHomeScreenState extends State<GenBalaganHomeScreen> {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    return DsScaffold(title: gen_balagan_home_c61 + (bhHebDate(_isoD(today)).isEmpty ? '' : ' · ' + bhHebDate(_isoD(today))), subtitle: empty ? gen_balagan_home_c62 : lead, icon: gen_balagan_home_c63, children: [
      ...
      if (money > 0 || moneyTm > 0 || monthSum[1] > 0 || balaganOpenSummary()[1] > 0) Padding(padding: const EdgeInsets.only(top: 6), child: Text([if (money > 0) gen_balagan_home_c76.replaceAll('{n}', balaganFmtMoney(money)), if (moneyTm > 0) gen_balagan_home_c77.replaceAll('{n}', balaganFmtMoney(moneyTm)), if (monthSum[1] > 0) gen_balagan_home_c78.replaceAll('{n}', balaganFmtMoney(monthSum[1].toDouble())).replaceAll('{m}', monthSum[0].toString()), for (final os in [balaganOpenSummary()]) if (os[1] > 0 && os[1] != monthSum[1]) gen_balagan_home_c79.replaceAll('{n}', balaganFmtMoney(os[1].toDouble())).replaceAll('{m}', os[0].toString()) /* ב׳-קכט · G41 */].join(' · '), style: TextStyle(color: lk.ink, fontSize: 15, fontWeight: FontWeight.w600))),   // ב׳-כט · כסף-במבט
      if (showUndo) Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsNote(message: gen_balagan_home_c80.replaceAll('{what}', lastAct['what'] ?? ''), label: '', tone: 0)), const SizedBox(width: 8), DsChipButton(label: gen_balagan_home_c81, onTap: () => appStore.undo(lastAct['id'] ?? ''))])),
      if (backupDue) Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsNote(message: bAge < 0 ? gen_balagan_home_c82 : gen_balagan_home_c83.replaceAll('{n}', bAge.toString()), label: '', tone: 1)), const SizedBox(width: 8), DsChipButton(label: gen_balagan_home_c84, onTap: () async { final t = appStore.exportJson(); await Clipboard.setData(ClipboardData(text: t)); appStore.setSetting('backupAt', _iso(today)); setState(() => _mailNote = gen_balagan_home_c85.replaceAll('{n}', t.length.toString())); })])),   // ב׳-לו · גיבוי: מקומי-בלבד ⇒ תזכורת מעולם/30 יום, העתקה בהקשה
      for (final bd in [bhMoney(appStore.setting('budget'))]) if (bd > 0 && monthSum[1] > 0) Padding(padding: const EdgeInsets.only(top: 6), child: DsNote(message: gen_balagan_home_c86.replaceAll('{n}', balaganFmtMoney(monthSum[1].toDouble())).replaceAll('{b}', balaganFmtMoney(bd)).replaceAll('{p}', (monthSum[1] * 100 / bd).round().toString()) + (monthSum[1] > bd ? ' · ' + gen_balagan_home_c87 : ''), label: '', tone: monthSum[1] > bd ? 2 : 0)),   // ב׳-קלב · G42 · תקציב-חודשי (הגדרה ב«חיבורים») מול «החודש»
      for (final nx in [(() { final now = DateTime.now(); final nowT = _isoT(now); List<dynamic>? best; for (final it in todayItems) { if (it.time.isEmpty) continue; final mn = bhMinutesUntil(nowT, _isoD(today), it.time); if (mn >= 0 && mn <= 90 && (best == null || mn < (best[0] as int))) best = [mn, it]; } return best; })()]) if (nx != null) Padding(padding: const EdgeInsets.only(top: 6), child: DsNote(message: gen_balagan_home_c88.replaceAll('{n}', (nx[0] as int).toString()).replaceAll('{title}', (nx[1] as DsTodayItem).title), label: '', tone: 1)),   // ב׳-קיח · G39 · הדבר הבא עם שעה, כשהוא קרוב (≤90 דק׳)
      if (monthTopics.length >= 2) DsFold(title: gen_balagan_home_c89.replaceAll('{n}', monthTopics.length.toString()), details: [for (final r in monthTopics) DsActionRow(title: gen_balagan_home_c90.replaceAll('{topic}', r[0] as String).replaceAll('{n}', balaganFmtMoney(r[2] as double)).replaceAll('{m}', r[1].toString()))]),   // ב׳-קיד · G38 · לאן הכסף הולך החודש
      if (!empty) Padding(padding: const EdgeInsets.only(top: 6), child: Row(children: [DsChipButton(label: gen_balagan_home_c91, onTap: () => _shareDay(overdue, todayItems, plan.length, evening ? tomorrow : const [], undated.length, stale.length))])),   // היום כטקסט: ללוח + וואטסאפ (לעצמו / לבן-הזוג) — אפס-שרת
      ...
      if (first != null) Padding(padding: const EdgeInsets.only(bottom: 12), child: Text([if (first.overdue) gen_balagan_home_c92, first.sub, first.module, lead2].where((x) => x.isNotEmpty).join(' · '), style: TextStyle(color: lk.muted, fontSize: 14))),
      if (overdue.isNotEmpty) DsSection(title: gen_balagan_home_c93 + ' · ' + overdue.length.toString() + (balaganMoney(overdue) > 0 ? ' · ' + gen_balagan_home_c94.replaceAll('{n}', balaganFmtMoney(balaganMoney(overdue))) : ''), tone: 2, trailing: overdue.length < 2 ? null : Row(mainAxisSize: MainAxisSize.min, children: [DsChipButton(label: gen_balagan_home_c95, onTap: () => appStore.grouped(() { for (final it in overdue) { final i = it.actions.indexOf(gen_balagan_home_c96); if (i >= 0) it.act(i); } })), const SizedBox(width: 8), DsChipButton(label: gen_balagan_home_c97, onTap: () => _snoozeAll(overdue, today))]), children: [for (final it in overdue) DsActionRow(title: it.title, sub: [it.sub, it.module, snz(it)].where((x) => x.isNotEmpty).join(' · '), tone: 2, onOpen: () => _openItem(context, it), actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון · ב׳-פז · «דחה הכל למחר» = הקשה אחת, החזר אחד
      if (todayItems.isNotEmpty) DsSection(title: gen_balagan_home_c98 + ' · ' + todayItems.length.toString() + (balaganMoney(todayItems) > 0 ? ' · ' + gen_balagan_home_c99.replaceAll('{n}', balaganFmtMoney(balaganMoney(todayItems))) : ''), trailing: !evening || todayItems.where((x) => x.field.isNotEmpty).isEmpty ? null : DsChipButton(label: gen_balagan_home_c100, onTap: () => _snoozeAll(todayItems, today)), /* ב׳-קה · בערב: מה שנשאר עובר למחר בהקשה אחת, החזר אחד */ children: [for (final it in todayItems) DsActionRow(title: it.title, sub: [it.sub, it.module, snz(it)].where((x) => x.isNotEmpty).join(' · '), onOpen: () => _openItem(context, it), actions: it.actions, onAct: it.act)]),
      if (plan.isNotEmpty) DsFold(title: gen_balagan_home_c101.replaceAll('{n}', plan.length.toString()), details: plan),   // תזמון-אוטומטי: מקופל — הוא מסתכל כשהוא רוצה
      ...cards.take(3),   // 3 למעלה
      if (cards.length > 3) DsFold(title: gen_balagan_home_c102.replaceAll('{n}', (cards.length - 3).toString()), details: [for (final x in cardRows.skip(3)) for (final ms in [kBalaganModules.where((mm) => mm.title == x[3])]) DsActionRow(title: ms.isEmpty ? (x[3] as String) : appStore.displayOf(ms.first.rootSlug, x[4] as String), sub: [x[3] as String, if ((x[0] as DateTime?) != null) balaganDayLabel(x[0] as DateTime, today), if (ms.isNotEmpty) for (final nf in ms.first.numFields.where((f) => !ms.first.percentFields.contains(f)).take(1)) for (final v in [double.tryParse(((appStore.byId(ms.first.rootSlug, x[4] as String) ?? const <String, String>{})[nf] ?? '').replaceAll(',', '').trim())]) if (v != null && v > 0) '₪ ' + balaganFmtMoney(v)].join(' · '),   /* ב׳-ע · ₪ גם בשורות «עוד» */ onOpen: () { if (ms.isNotEmpty) Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => balaganOpenRoot(ms.first.rootSlug, x[4] as String))); })]),   // ב׳-מו · «עוד» = שורה לכל תיק (שם · מודול · המועד הקרוב), הקשה ⇒ התיק — לא כרטיס של 200px
      if (_mailNote.isNotEmpty) DsNote(message: _mailNote, label: '', tone: 0),
      if (pending.isNotEmpty) DsSection(title: gen_balagan_home_c103 + ' · ' + pending.length.toString(), children: pending),   // D5 · הגיע (מייל) · הצעד-הבא (שרשרת) · תזכורות
      if (silent.isNotEmpty) DsSection(title: gen_balagan_home_c104 + ' · ' + silent.length.toString(), tone: 1, children: [for (final s in silent.take(3)) DsActionRow(title: appStore.displayOf(s[0], s[1]), sub: gen_balagan_home_c105.replaceAll('{n}', s[2]), tone: 1, onOpen: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => balaganOpenRoot(s[0], s[1]))), actions: [gen_balagan_home_c106, gen_balagan_home_c107], onAct: (i) { if (i == 0) { Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => balaganOpenRoot(s[0], s[1]))); return; } appStore.decide('silent:' + s[1] + ':' + s[4], 'ign'); appStore.logAction('decide', gen_balagan_home_c108 + ' · ' + appStore.displayOf(s[0], s[1]), entity: s[0], rid: s[1], field: 'silent:' + s[1] + ':' + s[4]); })]),   // ב׳-קיב · G37 · שלחת ולא נענה ≥3 ימים — פתח / התעלם (עם החזר דרך היומן)
      if (did.isNotEmpty) DsSection(title: gen_balagan_home_c109 + ' · ' + didRows.length.toString(), children: [for (final e in didRows) DsLogRow(text: ((gCount[e['group'] ?? ''] ?? 1) > 1 && (e['group'] ?? '').isNotEmpty ? gen_balagan_home_c110.replaceAll('{n}', (gCount[e['group']] ?? 1).toString()) + ' · ' : '') + (e['what'] ?? ''), sub: (() { final at = DateTime.tryParse(e['at'] ?? ''); final ago = at == null ? '' : balaganAgo(at, DateTime.now()); final ca = e['kind'] == 'done' ? balaganClosedAfter(e) : -1; return [ago, if (ca >= 1) gen_balagan_home_c111.replaceAll('{n}', ca.toString())].where((x) => x.isNotEmpty).join(' · '); })(),   /* ב׳-קיז · G38 */ undoLabel: gen_balagan_home_c112, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (wk.isNotEmpty) DsFold(title: gen_balagan_home_c113.replaceAll('{n}', wk.length.toString()).replaceAll('{m}', wSaved.toString()) + (streak >= 2 ? ' · ' + gen_balagan_home_c114.replaceAll('{n}', streak.toString()) : ''), details: [Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(children: [DsChipButton(label: gen_balagan_home_c115, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const BalaganDay(delta: -1))))])), /* ב׳-פ */ if (wAdd > 0) DsActionRow(title: gen_balagan_home_c116.replaceAll('{n}', wAdd.toString())), if (wSend > 0) DsActionRow(title: gen_balagan_home_c117.replaceAll('{n}', wSend.toString())), if (wAuto > 0) DsActionRow(title: gen_balagan_home_c118.replaceAll('{n}', wAuto.toString())), if (cnt('done') > 0) DsActionRow(title: gen_balagan_home_c119.replaceAll('{n}', cnt('done').toString()), sub: (() { var mm = 0.0; final seen = <String>{}; for (final e in wk) { if (e['kind'] != 'done') continue; final ent = e['entity'] ?? '', rid = e['rid'] ?? ''; if (!seen.add(ent + '|' + rid)) continue; final ms = kBalaganModules.where((m) => m.rootSlug == ent); if (ms.isEmpty) continue; final fs = ms.first.numFields.where((f) => !ms.first.percentFields.contains(f)).toList(); final r = appStore.byId(ent, rid); if (fs.isEmpty || r == null) continue; mm += bhMoney(r[fs.first]); } return mm > 0 ? gen_balagan_home_c120.replaceAll('{n}', seen.length.toString()).replaceAll('{m}', balaganFmtMoney(mm)) : ''; })()),   /* ב׳-קלג · G42 · כמה כסף נסגר השבוע */   /* ב׳-סז · מה סיימת השבוע */ DsNote(message: gen_balagan_home_c121, label: '', tone: 0)]),   // שמירת-זמן: מקופל, מוכח מהיומן
      if (soon.isNotEmpty) DsFold(title: gen_balagan_home_c122.replaceAll('{n}', soon.length.toString()) + (moneyWk > 0 ? ' · ' + gen_balagan_home_c123.replaceAll('{n}', balaganFmtMoney(moneyWk)) : ''), details: [for (final g in [bhGroupRows([for (final x in soon) {'d': (x[0] as int).toString()}], 'd')]) if (g.isNotEmpty && (g.first[1] as int) >= 3) Padding(padding: const EdgeInsets.only(bottom: 6), child: DsNote(message: gen_balagan_home_c124.replaceAll('{day}', balaganDayLabel(_dayPlus(today, int.parse(g.first[0] as String)), today)).replaceAll('{n}', g.first[1].toString()), label: '', tone: 0)),   /* ב׳-קיט · G39 · היום העמוס בשבוע (bhGroupRows ⇒ count.by, ממוין-יורד) */ Padding(padding: const EdgeInsets.only(bottom: 8), child: Wrap(spacing: 8, runSpacing: 8, children: [for (final d in soon.map((x) => x[0] as int).toSet().toList()) DsChipButton(label: balaganDayLabel(_dayPlus(today, d), today) + ' · ' + soon.where((x) => x[0] == d).length.toString(), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => BalaganDay(delta: d))))])), /* ב׳-מה · יום ⇒ מסך-היום */ for (final x in soon) DsActionRow(title: balaganDayLabel(_dayPlus(today, x[0] as int), today) + ' · ' + (x[1] as DsTodayItem).title, sub: [(x[1] as DsTodayItem).sub, (x[1] as DsTodayItem).module].where((x) => x.isNotEmpty).join(' · '), onOpen: () => _openItem(context, x[1] as DsTodayItem), actions: (x[1] as DsTodayItem).actions, onAct: (x[1] as DsTodayItem).act)]),   // ב׳-לד · גם השבוע עם פעולות
      if (tomorrow.isNotEmpty) DsFold(open: evening, title: gen_balagan_home_c125 + ' (' + tomorrow.length.toString() + ')' + (moneyTm > 0 ? ' · ' + gen_balagan_home_c126.replaceAll('{n}', balaganFmtMoney(moneyTm)) : ''), details: [Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(children: [DsChipButton(label: gen_balagan_home_c127.replaceAll('{day}', gen_balagan_home_c128), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const BalaganDay(delta: 1))))])), /* ב׳-סה · מהקיפול למסך-היום של מחר */ for (final it in tomorrow) DsActionRow(title: it.title, sub: [it.sub, it.module].where((x) => x.isNotEmpty).join(' · '), onOpen: () => _openItem(context, it), actions: it.actions, onAct: it.act)]),   // D8
      if (undated.isNotEmpty) DsFold(title: gen_balagan_home_c129.replaceAll('{n}', undated.length.toString()), details: [DsNote(message: gen_balagan_home_c130, label: '', tone: 0), for (final it in undated) DsActionRow(title: it.title, sub: [it.sub, it.module].where((x) => x.isNotEmpty).join(' · '), onOpen: () => _openItem(context, it), actions: it.actions, onAct: it.act)]),   // ב׳-כח · בלי תאריך
      if (stale.isNotEmpty) DsFold(title: gen_balagan_home_c131.replaceAll('{n}', stale.length.toString()), details: [DsNote(message: gen_balagan_home_c132, label: '', tone: 0), for (final it in stale) DsActionRow(title: it.title, sub: [it.sub, it.module].where((x) => x.isNotEmpty).join(' · '), onOpen: () => _openItem(context, it), actions: it.actions, onAct: it.act)]),   // ב׳-ל · נשכחים
      if (!empty && overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_balagan_home_c133, style: TextStyle(color: lk.muted, fontSize: 14))),
      if (empty) DsNote(message: gen_balagan_home_c134, label: '', tone: 0),
      if (empty) Padding(padding: const EdgeInsets.only(top: 14), child: Text(gen_balagan_home_c135, style: TextStyle(color: lk.muted, fontSize: 13))),
      if (empty) Padding(padding: const EdgeInsets.only(top: 6), child: Wrap(spacing: 8, runSpacing: 8, children: [for (final ex in gen_balagan_home_c136.split('|')) DsChipButton(label: ex, onTap: () { final hits = balaganIdentify(ex); if (hits.isEmpty) return; Navigator.of(context).push<bool>(MaterialPageRoute<bool>(builder: (_) => GenBalaganConfirmScreen(module: hits.first.module, facts: balaganFacts(ex, hits.first.module), alternatives: hits.skip(1).map((h) => h.module).toList(), text: ex))); })])),   // מסך ריק = הדרך בהקשה אחת
    ]);
  });
}
...
class BalaganMonth extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: gen_balagan_home_c137.replaceAll('{month}', balaganMonthName(monthKey)).replaceAll('{n}', items.length.toString()) + (balaganItemsMoney(items) > 0 ? ' · ' + gen_balagan_home_c138.replaceAll('{n}', balaganFmtMoney(balaganItemsMoney(items))) : ''), subtitle: gen_balagan_home_c139, icon: gen_balagan_home_c140, children: [
      if (items.isEmpty) DsNote(message: gen_balagan_home_c141, label: '', tone: 0),
...
class BalaganRange extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: gen_balagan_home_c142.replaceAll('{from}', bhDayMonth(from, false)).replaceAll('{to}', bhDayMonth(to, false)).replaceAll('{n}', items.length.toString()) + (balaganItemsMoney(items) > 0 ? ' · ' + gen_balagan_home_c143.replaceAll('{n}', balaganFmtMoney(balaganItemsMoney(items))) : ''), subtitle: gen_balagan_home_c144, icon: gen_balagan_home_c145, children: [
      if (items.isEmpty) DsNote(message: gen_balagan_home_c146, label: '', tone: 0),
      for (final it in items) DsNavTile(glyph: '', title: balaganDayLabel(DateTime.parse(it[0] + 'T12:00:00'), t0) + ' · ' + appStore.displayOf(it[1], it[2]), sub: it[3], onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => balaganOpenRoot(it[1], it[2])))),
    ]);
  }
}
...
class BalaganWeek extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: gen_balagan_home_c147.replaceAll('{week}', balaganWeekName(delta)).replaceAll('{n}', items.length.toString()) + (balaganItemsMoney(items) > 0 ? ' · ' + gen_balagan_home_c148.replaceAll('{n}', balaganFmtMoney(balaganItemsMoney(items))) : ''), subtitle: gen_balagan_home_c149.replaceAll('{from}', bhDayMonth(rg[0], false)).replaceAll('{to}', bhDayMonth(rg[1], false)), icon: gen_balagan_home_c150, children: [
      if (items.isEmpty) DsNote(message: gen_balagan_home_c151, label: '', tone: 0),
      for (final it in items) DsNavTile(glyph: '', title: balaganDayLabel(DateTime.parse(it[0] + 'T12:00:00'), t0) + ' · ' + appStore.displayOf(it[1], it[2]), sub: it[3], onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => balaganOpenRoot(it[1], it[2])))),
    ]);
  }
}
class BalaganDay extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    return DsScaffold(title: balaganDayLabel(_dayPlus(t0, delta), t0) + (items.isEmpty ? '' : ' · ' + items.length.toString()), subtitle: [bhHebDate(_isoD(_dayPlus(t0, delta))), gen_balagan_home_c152].where((x) => x.isNotEmpty).join(' · '),   /* ב׳-קד · התאריך העברי של היום הזה */ icon: gen_balagan_home_c153, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(children: [Expanded(child: DsQuickAdd(hint: gen_balagan_home_c154, autofocus: false, onSubmit: (s0) { final s = s0.trim(); if (s.isEmpty) return; final hits = balaganIdentify(s); if (hits.isEmpty) return; final m = hits.first.module; final facts = balaganFacts(s, m); if (m.dateFields.isNotEmpty && !m.dateFields.any((f) => (facts[f] ?? '').trim().isNotEmpty)) { final hard = m.fields.where((f) => f.type == 'date' && f.required); facts[hard.isNotEmpty ? hard.first.label : m.dateFields.first] = _GenBalaganHomeScreenState._iso(_dayPlus(t0, delta)); } Navigator.of(context).push<bool>(MaterialPageRoute<bool>(builder: (_) => GenBalaganConfirmScreen(module: m, facts: facts, alternatives: hits.skip(1).map((h) => h.module).toList(), text: s))); })), const SizedBox(width: 8), DsChipButton(label: gen_balagan_home_c155, onTap: () { final t = balaganDayText(const [], items, _dayPlus(t0, delta), money: money); Clipboard.setData(ClipboardData(text: t)); launchUrl(Uri.parse('https://wa.me/?text=' + Uri.encodeComponent(t)), mode: LaunchMode.externalApplication); })])),   // ב׳-מט · רגע ליום הזה: המועד כבר מוכן · ב׳-נא · «שתף» את היום ההוא
      if (delta < 0) for (final past in [appStore.log.where((e) => e['undone'] != '1' && (e['at'] ?? '').startsWith(_GenBalaganHomeScreenState._iso(_dayPlus(t0, delta)))).toList()]) if (past.isNotEmpty) DsSection(title: gen_balagan_home_c156 + ' · ' + past.length.toString(), children: [for (final e in past) DsLogRow(text: e['what'] ?? '', sub: (e['at'] ?? '').length >= 16 ? e['at']!.substring(11, 16) : '', undoLabel: '', onUndo: null)]),   // ב׳-פא · «מה עשיתי אתמול?» — היומן של אותו יום
      if (items.isEmpty && delta >= 0) DsNote(message: gen_balagan_home_c157, label: '', tone: 0),
      if (money > 0) Padding(padding: const EdgeInsets.only(bottom: 8), child: Text(gen_balagan_home_c158.replaceAll('{n}', balaganFmtMoney(money)), style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, fontWeight: FontWeight.w600))),
