...
class _GenBalaganHomeScreenState extends State<GenBalaganHomeScreen> {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    return DsScaffold(title: gen_balagan_home_c26, subtitle: empty ? gen_balagan_home_c27 : lead, icon: gen_balagan_home_c28, children: [
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_balagan_home_c37, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub + ' · ' + it.module, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_balagan_home_c38, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub + ' · ' + it.module, actions: it.actions, onAct: it.act)]),
      ...
      if (soon.isNotEmpty) DsFold(title: gen_balagan_home_c49.replaceAll('{n}', soon.length.toString()), details: [for (final x in soon) DsActionRow(title: dayNames[today.add(Duration(days: x[0] as int)).weekday % 7] + ' · ' + (x[1] as DsTodayItem).title, sub: (x[1] as DsTodayItem).sub + ' · ' + (x[1] as DsTodayItem).module)]),
      if (tomorrow.isNotEmpty) DsFold(title: gen_balagan_home_c50 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub + ' · ' + it.module)]),   // D8
