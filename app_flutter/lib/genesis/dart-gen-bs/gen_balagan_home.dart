...
class _GenBalaganHomeScreenState extends State<GenBalaganHomeScreen> {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    return DsScaffold(title: gen_balagan_home_c61 + (bhHebDate(_isoD(today)).isEmpty ? '' : ' · ' + bhHebDate(_isoD(today))), subtitle: empty ? gen_balagan_home_c62 : lead, icon: gen_balagan_home_c63, children: [
      ...
      for (final np in [balaganNoPhone()]) if (np.isNotEmpty) DsFold(title: gen_balagan_home_c134.replaceAll('{n}', np.length.toString()), details: [DsNote(message: gen_balagan_home_c135, label: '', tone: 0), for (final e in np.take(10)) DsNavTile(glyph: '', title: appStore.displayOf(e[0], e[1]), sub: (kBalaganModules.where((m) => m.rootSlug == e[0]).firstOrNull?.title ?? ''), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => balaganOpenRoot(e[0], e[1]))))]),   // ב׳-קמה · G45
      for (final ex in [balaganExpiring(today)]) if (ex.isNotEmpty) DsFold(title: gen_balagan_home_c136.replaceAll('{n}', ex.length.toString()), details: [for (final e in ex) DsNavTile(glyph: '', title: appStore.displayOf(e[1], e[2]), sub: gen_balagan_home_c137.replaceAll('{field}', e[3]).replaceAll('{day}', balaganDayLabel(DateTime.parse(e[0] + 'T12:00:00'), today)), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => balaganOpenRoot(e[1], e[2]))))]),   // ב׳-קמא · G44 · תוקף/פקיעה/חידוש ב-30 הימים הבאים
      if (undated.isNotEmpty) DsFold(title: gen_balagan_home_c138.replaceAll('{n}', undated.length.toString()), details: [DsNote(message: gen_balagan_home_c139, label: '', tone: 0), for (final it in undated) DsActionRow(title: it.title, sub: [it.sub, it.module].where((x) => x.isNotEmpty).join(' · '), onOpen: () => _openItem(context, it), actions: [...it.actions, if (freeD > 0 && it.field.isNotEmpty) gen_balagan_home_c140.replaceAll('{day}', balaganDayLabel(_dayPlus(today, freeD), today))], onAct: (i) { if (i < it.actions.length) { it.act(i); return; } final ms = kBalaganModules.where((mm) => mm.title == it.module); if (ms.isEmpty) return; final slug = ms.first.rootSlug; final r = appStore.byId(slug, it.rid); if (r == null) return; final iso = _isoD(_dayPlus(today, freeD)); appStore.logAction('auto', gen_balagan_home_c141.replaceAll('{day}', balaganDayLabel(_dayPlus(today, freeD), today)) + ' · ' + it.title, entity: slug, rid: it.rid, field: it.field, prev: r[it.field] ?? ''); appStore.update(slug, it.rid, {it.field: iso}); })]),   // ב׳-כח · בלי תאריך · ב׳-קלז · G43 · «קבע ליום פנוי» עם החזר
      if (stale.isNotEmpty) DsFold(title: gen_balagan_home_c142.replaceAll('{n}', stale.length.toString()), details: [DsNote(message: gen_balagan_home_c143, label: '', tone: 0), for (final it in stale) DsActionRow(title: it.title, sub: [it.sub, it.module].where((x) => x.isNotEmpty).join(' · '), onOpen: () => _openItem(context, it), actions: it.actions, onAct: it.act)]),   // ב׳-ל · נשכחים
      if (!empty && overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_balagan_home_c144, style: TextStyle(color: lk.muted, fontSize: 14))),
      if (empty) DsNote(message: gen_balagan_home_c145, label: '', tone: 0),
      if (empty) Padding(padding: const EdgeInsets.only(top: 14), child: Text(gen_balagan_home_c146, style: TextStyle(color: lk.muted, fontSize: 13))),
      if (empty) Padding(padding: const EdgeInsets.only(top: 6), child: Wrap(spacing: 8, runSpacing: 8, children: [for (final ex in gen_balagan_home_c147.split('|')) DsChipButton(label: ex, onTap: () { final hits = balaganIdentify(ex); if (hits.isEmpty) return; Navigator.of(context).push<bool>(MaterialPageRoute<bool>(builder: (_) => GenBalaganConfirmScreen(module: hits.first.module, facts: balaganFacts(ex, hits.first.module), alternatives: hits.skip(1).map((h) => h.module).toList(), text: ex))); })])),   // מסך ריק = הדרך בהקשה אחת
    ]);
  });
}
...
class BalaganMonth extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: gen_balagan_home_c148.replaceAll('{month}', balaganMonthName(monthKey)).replaceAll('{n}', items.length.toString()) + (balaganItemsMoney(items) > 0 ? ' · ' + gen_balagan_home_c149.replaceAll('{n}', balaganFmtMoney(balaganItemsMoney(items))) : ''), subtitle: gen_balagan_home_c150, icon: gen_balagan_home_c151, children: [
      if (items.isEmpty) DsNote(message: gen_balagan_home_c152, label: '', tone: 0),
      if (items.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 8), child: Wrap(spacing: 8, runSpacing: 8, children: [DsChipButton(label: gen_balagan_home_c153, onTap: () async { await Clipboard.setData(ClipboardData(text: balaganIcsOf(items, balaganMonthName(monthKey), DateTime.now()))); if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(gen_balagan_home_c154))); }), DsChipButton(label: gen_balagan_home_c155, onTap: () async { await Clipboard.setData(ClipboardData(text: balaganCsvOf(items))); if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(gen_balagan_home_c156.replaceAll('{n}', items.length.toString())))); }) /* ב׳-קמז · G46 */, DsChipButton(label: gen_balagan_home_c157, onTap: () { final t = balaganMonthName(monthKey) + ' · ' + items.length.toString() + (balaganItemsMoney(items) > 0 ? ' · ' + gen_balagan_home_c158.replaceAll('{n}', balaganFmtMoney(balaganItemsMoney(items))) : '') + '\n' + [for (final it in items) '• ' + bhDayMonth(it[0], false) + ' ' + appStore.displayOf(it[1], it[2]) + ' (' + it[3] + ')'].join('\n'); Clipboard.setData(ClipboardData(text: t)); launchUrl(Uri.parse('https://wa.me/?text=' + Uri.encodeComponent(t)), mode: LaunchMode.externalApplication); })])),   // ב׳-קמב/קמג · G45 · ליומן (ICS דרך buildIcs) · שתף את החודש
...
class BalaganRange extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: gen_balagan_home_c159.replaceAll('{from}', bhDayMonth(from, false)).replaceAll('{to}', bhDayMonth(to, false)).replaceAll('{n}', items.length.toString()) + (balaganItemsMoney(items) > 0 ? ' · ' + gen_balagan_home_c160.replaceAll('{n}', balaganFmtMoney(balaganItemsMoney(items))) : ''), subtitle: gen_balagan_home_c161, icon: gen_balagan_home_c162, children: [
      if (items.isEmpty) DsNote(message: gen_balagan_home_c163, label: '', tone: 0),
      if (items.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(children: [DsChipButton(label: gen_balagan_home_c164, onTap: () async { await Clipboard.setData(ClipboardData(text: balaganIcsOf(items, bhDayMonth(from, false) + '–' + bhDayMonth(to, false), DateTime.now()))); if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(gen_balagan_home_c165))); }), DsChipButton(label: gen_balagan_home_c166, onTap: () async { await Clipboard.setData(ClipboardData(text: balaganCsvOf(items))); if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(gen_balagan_home_c167.replaceAll('{n}', items.length.toString())))); }) /* ב׳-קמז · G46 */])),   // ב׳-קמב · G45
      for (final it in items) DsNavTile(glyph: '', title: balaganDayLabel(DateTime.parse(it[0] + 'T12:00:00'), t0) + ' · ' + appStore.displayOf(it[1], it[2]), sub: it[3], onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => balaganOpenRoot(it[1], it[2])))),
    ]);
  }
}
...
class BalaganDay extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    return DsScaffold(title: balaganDayLabel(_dayPlus(t0, delta), t0) + (items.isEmpty ? '' : ' · ' + items.length.toString()), subtitle: [bhHebDate(_isoD(_dayPlus(t0, delta))), gen_balagan_home_c177].where((x) => x.isNotEmpty).join(' · '),   /* ב׳-קד · התאריך העברי של היום הזה */ icon: gen_balagan_home_c178, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(children: [Expanded(child: DsQuickAdd(hint: gen_balagan_home_c179, autofocus: false, onSubmit: (s0) { final s = s0.trim(); if (s.isEmpty) return; final hits = balaganIdentify(s); if (hits.isEmpty) return; final m = hits.first.module; final facts = balaganFacts(s, m); if (m.dateFields.isNotEmpty && !m.dateFields.any((f) => (facts[f] ?? '').trim().isNotEmpty)) { final hard = m.fields.where((f) => f.type == 'date' && f.required); facts[hard.isNotEmpty ? hard.first.label : m.dateFields.first] = _GenBalaganHomeScreenState._iso(_dayPlus(t0, delta)); } Navigator.of(context).push<bool>(MaterialPageRoute<bool>(builder: (_) => GenBalaganConfirmScreen(module: m, facts: facts, alternatives: hits.skip(1).map((h) => h.module).toList(), text: s))); })), const SizedBox(width: 8), DsChipButton(label: gen_balagan_home_c180, onTap: () { final t = balaganDayText(const [], items, _dayPlus(t0, delta), money: money); Clipboard.setData(ClipboardData(text: t)); launchUrl(Uri.parse('https://wa.me/?text=' + Uri.encodeComponent(t)), mode: LaunchMode.externalApplication); })])),   // ב׳-מט · רגע ליום הזה: המועד כבר מוכן · ב׳-נא · «שתף» את היום ההוא
      if (delta < 0) for (final past in [appStore.log.where((e) => e['undone'] != '1' && (e['at'] ?? '').startsWith(_GenBalaganHomeScreenState._iso(_dayPlus(t0, delta)))).toList()]) if (past.isNotEmpty) DsSection(title: gen_balagan_home_c181 + ' · ' + past.length.toString(), children: [for (final e in past) DsLogRow(text: e['what'] ?? '', sub: (e['at'] ?? '').length >= 16 ? e['at']!.substring(11, 16) : '', undoLabel: '', onUndo: null)]),   // ב׳-פא · «מה עשיתי אתמול?» — היומן של אותו יום
      if (items.isEmpty && delta >= 0) DsNote(message: gen_balagan_home_c182, label: '', tone: 0),
      if (money > 0) Padding(padding: const EdgeInsets.only(bottom: 8), child: Text(gen_balagan_home_c183.replaceAll('{n}', balaganFmtMoney(money)), style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, fontWeight: FontWeight.w600))),
