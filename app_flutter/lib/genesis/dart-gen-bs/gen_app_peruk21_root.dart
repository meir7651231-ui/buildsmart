...
String _fmtDate(String s) { final t = s.trim(); if (t.length < 10 || DateTime.tryParse(t.length == 10 ? '${t}T12:00:00' : t) == null) return t; final p = bhDayLabelParts(t.substring(0, 10), bhIso(DateTime.now())); switch (p[0]) { case 'today': return gen_app_peruk21_root_c52; case 'tomorrow': return gen_app_peruk21_root_c53; case 'yesterday': return gen_app_peruk21_root_c54; case 'weekday': return gen_app_peruk21_root_c55.replaceAll('{day}', gen_app_peruk21_root_c56.split(',')[int.parse(p[1])]) + ' ' + p[2]; default: return p[2]; } }   // ב׳-מג · תאריך במילים בתיק · G34ב · שכבת-ההרכבה
