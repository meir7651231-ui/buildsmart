...
class GenAppPeruk06RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk06_root_c67, subtitle: gen_app_peruk06_root_c68, icon: gen_app_peruk06_root_c69, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk06_ent1', id), subtitle: const [gen_app_peruk06_root_c62, gen_app_peruk06_root_c63, gen_app_peruk06_root_c64, gen_app_peruk06_root_c65, gen_app_peruk06_root_c66][appStore.stageOf('app_peruk06_ent1', id).clamp(0, 4)], icon: gen_app_peruk06_root_c70, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_peruk06_root_c59, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk06Ent1Screen(editId: id))))]))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__note'] ?? '').trim().isNotEmpty ? DsFold(title: gen_app_peruk06_root_c60, details: [Text(r0['__note'] ?? '', style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, height: 1.5))]) : const SizedBox.shrink())),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk06_root_c61, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
