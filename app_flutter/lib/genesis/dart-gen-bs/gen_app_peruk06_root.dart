...
class GenAppPeruk06RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk06_root_c62, subtitle: gen_app_peruk06_root_c63, icon: gen_app_peruk06_root_c64, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk06_ent1', id), subtitle: const [gen_app_peruk06_root_c57, gen_app_peruk06_root_c58, gen_app_peruk06_root_c59, gen_app_peruk06_root_c60, gen_app_peruk06_root_c61][appStore.stageOf('app_peruk06_ent1', id).clamp(0, 4)], icon: gen_app_peruk06_root_c65, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk06_root_c56, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
