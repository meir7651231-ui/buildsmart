...
String _fmtDate(String s) { final t = s.trim(); final d = DateTime.tryParse(t.length == 10 ? '${t}T12:00:00' : t); if (d == null) return t; final now = DateTime.now(); final n = DateTime(d.year, d.month, d.day).difference(DateTime(now.year, now.month, now.day)).inDays; final dm = '${d.day}.${d.month}'; if (n == 0) return gen_app_peruk24_root_c56 + ' (' + dm + ')'; if (n == 1) return gen_app_peruk24_root_c57 + ' (' + dm + ')'; if (n == -1) return gen_app_peruk24_root_c58 + ' (' + dm + ')'; if (n.abs() <= 6) return gen_app_peruk24_root_c59.replaceAll('{day}', gen_app_peruk24_root_c60.split(',')[d.weekday % 7]) + ' ' + dm; return dm + '.${d.year}'; }
...
class GenAppPeruk24RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk24_root_c61, subtitle: gen_app_peruk24_root_c62, icon: gen_app_peruk24_root_c63, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk24_ent1', id), subtitle: const [gen_app_peruk24_root_c51, gen_app_peruk24_root_c52, gen_app_peruk24_root_c53, gen_app_peruk24_root_c54, gen_app_peruk24_root_c55][appStore.stageOf('app_peruk24_ent1', id).clamp(0, 4)], icon: gen_app_peruk24_root_c64, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.only(bottom: 10), child: DsQuickAdd(hint: gen_app_peruk24_root_c41, onSubmit: (t) { final v = t.trim(); if (v.isEmpty) return; final prev = r0['__note'] ?? ''; final stamp = DateTime.now().toIso8601String().substring(0, 10); appStore.update('app_peruk24_ent1', id, {'__note': (prev.isEmpty ? '' : prev + '\n') + stamp + ' · ' + v}); appStore.logAction('auto', gen_app_peruk24_root_c42.replaceAll('{who}', appStore.displayOf('app_peruk24_ent1', id)), entity: 'app_peruk24_ent1', rid: id, field: '__note', prev: prev); }))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: (appStore.stageOf('app_peruk24_ent1', id) >= 4 ? const SizedBox.shrink() : Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_peruk24_root_c43, onTap: () { final prev = r0[AppStore.stageKey] ?? '0'; appStore.update('app_peruk24_ent1', id, {AppStore.stageKey: '4'}); appStore.logAction('auto', gen_app_peruk24_root_c44.replaceAll('{who}', appStore.displayOf('app_peruk24_ent1', id)), entity: 'app_peruk24_ent1', rid: id, field: AppStore.stageKey, prev: prev); })])))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_peruk24_root_c45, onTap: () { final snap = Map<String, String>.from(r0); appStore.removeById('app_peruk24_ent1', id); appStore.logAction('del', gen_app_peruk24_root_c46.replaceAll('{who}', snap.values.take(2).join(' · ')), entity: 'app_peruk24_ent1', rid: id, prev: jsonEncode(snap)); Navigator.of(context).pop(); })]))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [DsChipButton(label: gen_app_peruk24_root_c47, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk24Ent1Screen(editId: id)))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk24_root_c48, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk24Ent1Screen(initial: {for (final e in r0.entries) if (!e.key.startsWith('__') && !const <String>[].contains(e.key)) e.key: e.value}))))]))),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__note'] ?? '').trim().isNotEmpty ? DsFold(title: gen_app_peruk24_root_c49, details: [Text(_noteText(r0['__note'] ?? ''), style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, height: 1.5))]) : const SizedBox.shrink())),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk24_root_c50, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
