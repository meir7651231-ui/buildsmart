...
class GenAppCalendarHomeScreenToday {
  ...
  static String _dayLabel(DateTime d, DateTime today) { final p = bhDayLabelParts(_iso(d), _iso(today)); switch (p[0]) { case 'today': return gen_app_calendar_home_c8; case 'tomorrow': return gen_app_calendar_home_c9; case 'yesterday': return gen_app_calendar_home_c10; case 'weekday': return gen_app_calendar_home_c11.replaceAll('{day}', gen_app_calendar_home_c12.split(',')[int.parse(p[1])]) + ' ' + p[2]; default: return p[2]; } }   // G34ב · שכבת-ההרכבה מחזירה חלקים; כאן רק מונחים
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { final ago = bhDaysSince(_iso(d), _iso(today)); out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', gen_app_calendar_home_c33.replaceAll('{date}', _dayLabel(d, today)) + (ago <= 1 ? '' : ' · ' + gen_app_calendar_home_c34.replaceAll('{n}', ago.toString()))   /* ב׳-עז · «היה אתמול» כבר אומר הכל — בלי «· אתמול» כפול */, rid, f.label, d, f.hard, true, today, tm, rep, ph, mo)); continue; }
