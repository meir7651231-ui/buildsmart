...
class GenAppCalendarHomeScreenToday {
  ...
  static List<DsTodayItem> undated(DateTime today) {
    ...
    for (final r in open()) {
      ...
      final acts = [gen_app_calendar_home_c29, gen_app_calendar_home_c30, gen_app_calendar_home_c31];
  ...
  static void _setDate(String rid, String field, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    if (a == gen_app_calendar_home_c32) { appStore.decide('undated:$rid', 'no'); return; }
    final r = appStore.byId('app_calendar_ent1', rid); if (r == null) return;
    final prev = r[field] ?? '';
    appStore.update('app_calendar_ent1', rid, {field: _iso(_shift(today.add(Duration(days: a == gen_app_calendar_home_c33 ? 7 : 1)), false))});   /* «קבע» לא נוחת בשבת */
