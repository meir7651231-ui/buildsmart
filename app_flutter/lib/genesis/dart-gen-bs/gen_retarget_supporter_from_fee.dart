...
class _SupporterScreenState extends State<SupporterScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(
      title: 'גבייה ותשלומים', subtitle: '${all.length} משפחות · ${_SupporterData.year} · $_roleName', icon: '💰',
      children: [
        // בורר-תפקיד (חוק-6 · זהות-מוזרקת) — מדגים גידור-הרשאות (roleOf⊕canGrantedAction)
        SingleChildScrollView(
          scrollDirection: Axis.horizontal,
          child: SegmentedSwitch(items: [for (final r in _SupporterData.roleDefs) r['label'] as String], selected: _role, onSelect: (i) => setState(() { _role = i; _hokArmed = false; })),
        ),
        _gap(10),
        if (!_amounts) ...[
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [
          ...
          if (_amounts) ...[
          _fchip(2, '⏰ ותק>${_SupporterData.oldDebtDays} · $oldN'),
          _fchip(3, '💳 הו״ק · ${all.where(_SupporterData.hokFlag).length}'),
          _fchip(4, '🎓 מלגה/הנחה · ${all.where((f) => _SupporterData.discountPct(f) > 0).length}'),
          _fchip(5, '🔕 ללא-תזכורת · ${all.where((f) => _SupporterData.balance(f) > 0 && _SupporterData.remindersSent(f).isEmpty).length}'),
          _fchip(6, '🔔 תזכורת>2 · ${all.where((f) => _SupporterData.remindersSent(f).length > 2).length}'),
          _fchip(7, '📆 הסדר · ${all.where(_SupporterData.hasArrangement).length}'),
          _fchip(8, '⚠️ בסיכון · ${all.where((f) => _SupporterData.risk(f) >= 1).length}'),
          FilterChipPill(
        ...
        GradientCard(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              BareStat(value: _m(kOld), label: '⏰ חוב-ותיק >${_SupporterData.oldDebtDays}י׳ · $oldN', inkColor: kOld > 0 ? _danger : _ok, mutedColor: _muted),
        ...
        if (dups.isNotEmpty && _amounts) ...[
          AlertBanner(glyph: '👯', tone: 3, message: 'חיוב-כפול-חשוד (אותו סוג·סכום·תאריך·תלמיד): ${dups.map((f) => '${f['name']} (${_SupporterData.duplicateCharges(f).map((c) => c['cat']).join(',')})').join(' · ')}'),
          _gap(8),
        ],
        ...
        if (hokDue.isNotEmpty && _SupporterData.can(_role, 'fees.hok')) ...[
          AlertBanner(glyph: '💳', tone: 0, message: '${hokDue.length} הו״ק לרישום החודש (${_m(_SupporterData.hokExpected(all))}) — ראה מבט הו״ק'),
          _gap(8),
        ],
    ...
    return Padding(
      ...
      child: GradientCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Row(children: [
            Expanded(child: MediaRow(glyph: '👨‍👩‍👧', title: f['name'] as String, subtitle: '${_SupporterData.studentsOf(f)} · ${_SupporterData.gradesOf(f)}')),
          ...
          if (_amounts) ...[
            ...
            Row(children: [
              ...
              BareStat(value: shekel(bal), label: bal > 0 ? '= יתרה · ${_SupporterData.agingDays(f)} י׳' : '= יתרה', inkColor: balColor, mutedColor: _muted),
            ]),
          ] else
            Row(children: [
  ...
  List<Widget> _facts(Map<String, dynamic> f) {
    ...
    return [
      StatusChip(label: _SupporterData.statusOf(f), tone: _SupporterData.statusOf(f) == 'תקין' || _SupporterData.statusOf(f) == 'מלגה-מלאה' ? 1 : _SupporterData.statusOf(f).contains('פיגור') ? 2 : 3),
      if (_SupporterData.hasHok(f)) StatusChip(label: _SupporterData.hokFlag(f) ? (_SupporterData.hokActive(f) ? '💳 הו״ק ${_amounts ? shekel((f['hok'] as Map)['amount'] as num) : ''} · יום ${(f['hok'] as Map)['day']}' : '⚠️ הו״ק נכשלה') : '⏸ הו״ק מופסקת', tone: _SupporterData.hokFailed(f) ? 2 : _SupporterData.hokFlag(f) ? 1 : 0),
      if (_SupporterData.discountLabel(f).isNotEmpty) StatusChip(label: '🎓 ${_SupporterData.discountLabel(f)}', tone: 0),
      if (_SupporterData.remindersSent(f).isNotEmpty) StatusChip(label: '🔔 ${_SupporterData.remindersSent(f).length} תזכורות', tone: _SupporterData.remindersSent(f).length > 2 ? 3 : 0),
      if (_SupporterData.balance(f) > 0) StatusChip(label: 'סיכון ${_SupporterData.riskLabel(r)} · ${_SupporterData.tierLabel(f)}', tone: r == 2 ? 2 : r == 1 ? 3 : 1),
      if (_SupporterData.lastMethod(f).isNotEmpty) StatusChip(label: '${_SupporterData.lastMethod(f)} · ${fmtDate(_SupporterData.lastPaymentDate(f))}', tone: 0),
  ...
  Widget _hokView(List<Map<String, dynamic>> all, List<Map<String, dynamic>> due) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      DsSection(title: '💳 הו״ק לרישום החודש · ${due.length} · ${_m(_SupporterData.hokExpected(all))}', tone: 0, children: [
        if (due.isEmpty)
          const EmptyState(glyph: '✅', message: 'כל ההו״ק הפעילות נרשמו החודש')
        else ...[
          for (final f in due)
            TimelineItem(title: '${f['name']} · יום ${(f['hok'] as Map)['day']}', time: _SupporterData.hokMethod(f), body: '${_m((f['hok'] as Map)['amount'] as num)} — טרם נרשם החודש'),
          if (_SupporterData.can(_role, 'fees.hok')) ...[
            ...
            if (!_hokArmed)
              ...
              AlertBanner(glyph: '⚠️', tone: 3, message: 'שלב 2/2: יירשמו ${due.length} תשלומי-הו״ק בסך ${_m(_SupporterData.hokExpected(all))} (${due.map((f) => f['name']).join(' · ')}). לאשר?'),
              _wrap([
                SoftButton(label: '✅ אשר ורשום ${due.length}', tone: 2, onTap: () => setState(() { _SupporterData.runHokBatch(all, _roleName); _hokArmed = false; })),
        ...
        for (final f in withHok)
          Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: MediaRow(glyph: '💳', title: f['name'] as String, subtitle: '${_m((f['hok'] as Map)['amount'] as num)} · יום ${(f['hok'] as Map)['day']} · ${_SupporterData.hokMethod(f)} · מ-${fmtDate((f['hok'] as Map)['last'] as String?)}')),
              StatusChip(label: _SupporterData.hokFailed(f) ? 'נכשלה' : _SupporterData.hokFlag(f) ? (_SupporterData.hokRecorded(f) ? 'נרשמה החודש' : 'ממתינה') : 'מופסקת', tone: _SupporterData.hokFailed(f) ? 2 : _SupporterData.hokFlag(f) ? 1 : 0),
              if (_SupporterData.can(_role, 'fees.hok')) ...[
                const SizedBox(width: 8),
                SoftButton(label: _SupporterData.hokFlag(f) ? '⏸ הפסק' : '▶ הפעל', tone: _SupporterData.hokFlag(f) ? 2 : 1, onTap: () => setState(() => _SupporterData.toggleHok(f, _roleName))),
              ],
            ]),
          ),
  ...
  Widget _reminderCard(Map<String, dynamic> f) {
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        MediaRow(glyph: '🔔', title: '${f['name']} · דרגה: $grade', subtitle: 'יתרה ${_m(_SupporterData.balance(f))} · ותק ${_SupporterData.agingDays(f)} י׳ · נשלחו ${_SupporterData.remindersSent(f).length}'),
        ...
        if (_SupporterData.can(_role, 'fees.remind'))
          _wrap([SoftButton(label: '📨 שלח תזכורת $grade (פרטי)', tone: 3, onTap: () => setState(() => _SupporterData.sendReminder(f, _roleName, grade)))], top: 8),
      ]),
    );
  }
  ...
  Widget _reportsView(List<Map<String, dynamic>> all, Map<String, dynamic> trend, int thisMonth) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      _gap(10),
      DsSection(title: '🗓 דוח-גזבר שבועי · מ-${fmtDate(weekAgo)} עד ${fmtDate(_SupporterData.today)}', children: [
        Row(children: [
          ...
          BareStat(value: '${all.where(_SupporterData.hokFailed).length}', label: 'הו״ק נכשלו', inkColor: all.any(_SupporterData.hokFailed) ? _danger : _ok, mutedColor: _muted),
          BareStat(value: '${all.where(_SupporterData.oldDebt).length}', label: 'חוב-ותיק', inkColor: all.any(_SupporterData.oldDebt) ? _danger : _ok, mutedColor: _muted),
        ]),
      ]),
      DsSection(title: '🏁 סוף-שנה · סגירת-חשבונות', children: [
        Row(children: [
          BareStat(value: '${all.where((f) => _SupporterData.balance(f) <= 0).length}/${all.length}', label: 'משפחות סגורות', inkColor: _ink, mutedColor: _muted),
          BareStat(value: _m(_SupporterData.kOpen(all)), label: 'יתרה להעברה (carryBalance)', inkColor: _SupporterData.kOpen(all) > 0 ? _warning : _ok, mutedColor: _muted),
          BareStat(value: _m(grandTotal(all, (f) => _SupporterData.credit(f as Map<String, dynamic>))), label: 'זכויות להחזר', inkColor: _acc, mutedColor: _muted),
        ]),
  ...
  Widget _auditView() => DsSection(title: '🧾 יומן-אודיט · ${_SupporterData.audit.length}', children: [
        if (_SupporterData.audit.isEmpty)
          ...
          for (final a in _SupporterData.audit)
            TimelineItem(title: '${a['role']} · ${a['family'] == '*' ? 'כלל-המערכת' : _SupporterData.families.firstWhere((f) => f['id'] == a['family'], orElse: () => const {'name': '?'})['name']}', time: fmtDate(a['date'] as String?), body: a['what'] as String),
      ]);
        ...
        builder: (ctx) => Padding(
          ...
          child: GlassCard(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              MediaRow(glyph: '👨‍👩‍👧', title: f['name'] as String, subtitle: '${_SupporterData.studentsOf(f)} · ${_SupporterData.gradesOf(f)}'),
              _gap(10),
              AlertBanner(glyph: _SupporterData.balance(f) > 0 ? '🚩' : '✅', tone: _SupporterData.balance(f) > 0 ? 3 : 1, message: _SupporterData.balance(f) > 0 ? 'דגל-חוב — פרטים וסכומים בגזברות בלבד. לא לפנות לתלמיד/ה (מגן-כבוד).' : 'תקין — אין דגל-חוב'),
            ]),
          ),
        ),
      );
          ...
          builder: (ctx, scroll) => Padding(
            ...
            child: GlassCard(
              child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                MediaRow(glyph: '👨‍👩‍👧', title: f['name'] as String, subtitle: '${f['payer']} · ${f['phone']} · ${_SupporterData.studentsOf(f)} (${_SupporterData.gradesOf(f)})'),
                _gap(10),
                Row(children: [
                  BareStat(value: _m(bal), label: bal > 0 ? 'יתרה · ותק ${_SupporterData.agingDays(f)} י׳' : 'יתרה', inkColor: balColor, mutedColor: _muted),
                  ...
                  if (_SupporterData.credit(f) > 0) BareStat(value: _m(_SupporterData.credit(f)), label: 'זכות', inkColor: _acc, mutedColor: _muted),
                ]),
                _gap(8),
                ...
                Builder(builder: (_) {
                  final acts = <Widget>[
                    if (_SupporterData.can(_role, 'fees.charge')) SoftButton(label: '➕ חיוב', tone: 0, onTap: () => _openChargeForm(f, [f], onDone: () => setSheet(() {}))),
                    if (_SupporterData.can(_role, 'fees.pay')) SoftButton(label: '💳 רשום תשלום', tone: 1, onTap: () => _openPaymentForm(f, [f], onDone: () => setSheet(() {}))),
                    if (_SupporterData.can(_role, 'fees.pay') && bal > 0) SoftButton(label: '➗ תשלום-חלקי (½)', tone: 1, onTap: () => act(() => _SupporterData.addPayment(f, _roleName, amount: (bal / 2).ceil(), method: 'מזומן', date: _SupporterData.today, note: 'תשלום-חלקי'))),
                    if (_SupporterData.can(_role, 'fees.remind') && _SupporterData.nextReminder(f) != null) SoftButton(label: '📨 תזכורת ${_SupporterData.nextReminder(f)!['grade']}', tone: 3, onTap: () => act(() => _SupporterData.sendReminder(f, _roleName, _SupporterData.nextReminder(f)!['grade'] as String))),
                    if (_SupporterData.can(_role, 'fees.arrangement') && bal > 0 && !_SupporterData.hasArrangement(f)) SoftButton(label: '📆 הסדר 3 תשלומים', tone: 0, onTap: () => act(() => _SupporterData.setArrangement(f, _roleName, 3))),
                    if (_SupporterData.can(_role, 'fees.hok') && _SupporterData.hasHok(f)) SoftButton(label: _SupporterData.hokFlag(f) ? '⏸ הפסק הו״ק' : '▶ הפעל הו״ק', tone: _SupporterData.hokFlag(f) ? 2 : 1, onTap: () => act(() => _SupporterData.toggleHok(f, _roleName))),
                    if (_SupporterData.can(_role, 'fees.writeoff') && bal > 0 && _SupporterData.oldDebt(f)) SoftButton(label: '🗂 סמן חוב-אבוד', tone: 2, onTap: () => act(() => _SupporterData.writeOff(f, _roleName))),
                    if (_SupporterData.can(_role, 'fees.refund') && _SupporterData.credit(f) > 0) SoftButton(label: '💸 החזר-זכות ${_m(_SupporterData.credit(f))}', tone: 1, onTap: () => act(() => _SupporterData.refund(f, _roleName))),
                    if ((_SupporterData.can(_role, 'fees.pay') || _SupporterData.can(_role, 'fees.self')) && bal > 0)
                      ...
                      SoftButton(label: _SupporterData.payLinkOf(f) == null ? '🔗 קישור-תשלום (שער לא-מוגדר)' : '🔗 שלח קישור-תשלום', tone: 0, onTap: _SupporterData.payLinkOf(f) == null ? null : () {}),
  ...
  List<Widget> _tabOverview(Map<String, dynamic> f, int charged, int paid, int bal) => [
        if (_amounts) ...[
          NeonBars(labels: const ['חיובים', 'שולם', 'יתרה'], values: [charged.toDouble(), paid.toDouble(), bal.toDouble()], tone: bal > 0 ? (_SupporterData.agingBand(f) == 3 ? 2 : 3) : 1),
          _gap(8),
          StatRow(label: 'שולם מתוך חיובים', value: '${shekel(paid)} / ${shekel(charged)}', fraction: charged == 0 ? 0 : paid / charged),
          _gap(8),
          if (_SupporterData.byType(f).isNotEmpty) DsBars(title: 'פירוק-חיובים לפי-סוג', labels: _SupporterData.byType(f).keys.toList(), values: [for (final v in _SupporterData.byType(f).values) v.toDouble()]),
        ],
        _wrap([for (final m in f['donations'] as List) StatusChip(label: '🎓 ${(m as Map)['first']} · ${m['grade']}', tone: 0)]),
        _wrap(_facts(f)),
        _gap(8),
        Row(children: [
          BareStat(value: '${_SupporterData.rfm(f)}', label: 'דפוס-תשלום (RFM) · ${_SupporterData.tierLabel(f)}', inkColor: _SupporterData.tierKey(f) == 'red' ? _danger : _ink, mutedColor: _muted),
          BareStat(value: '${_SupporterData.trend(f)['dir'] == 'up' ? '↑' : _SupporterData.trend(f)['dir'] == 'down' ? '↓' : '→'} ${_SupporterData.trend(f)['pct']}%', label: 'מגמת-תשלומים (6 חודשים)', inkColor: _SupporterData.trend(f)['dir'] == 'down' ? _danger : _ok, mutedColor: _muted),
          BareStat(value: _SupporterData.riskLabel(_SupporterData.risk(f)), label: 'סיכון-גבייה (ותק · דפוס · מגמה)', inkColor: _SupporterData.risk(f) == 2 ? _danger : _SupporterData.risk(f) == 1 ? _warning : _ok, mutedColor: _muted),
        ]),
      ];
        ...
        () {
          ...
          return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            TimelineItem(
              ...
              body: '${cancelled ? 'בוטל: ${_SupporterData.cancelReason[c['id']] ?? c['note'] ?? ''}' : _amounts ? (net != gross ? '${shekel(net)} (ברוטו ${shekel(gross)}, הנחה)' : shekel(net)) : '🔒'}${c['note'] != null && !cancelled ? ' · ${c['note']}' : ''}',
            ),
            if (!cancelled && _SupporterData.can(_role, 'fees.writeoff'))
              _wrap([SoftButton(label: '✖ בטל חיוב', tone: 2, onTap: () => act(() => _SupporterData.cancelCharge(f, _roleName, c, dups.contains(c['id']) ? 'חיוב-כפול' : 'ביטול-ידני')))], top: 2),
          ]);
        }(),
    ];
  }
    ...
    return [
      ...
      for (final p in ps)
        TimelineItem(
          ...
          body: '${_m(p['amount'] as num)}${[for (final m in _SupporterData.paymentMeta) if (p[m['key']] != null) ' · ${m['prefix']}${p[m['key']]}${m['suffix']}'].join()}',
        ),
  ...
  List<Widget> _tabHok(Map<String, dynamic> f, void Function(void Function()) act) {
    if (!_SupporterData.hasHok(f)) return [const EmptyState(glyph: '💳', message: 'אין הוראת-קבע למשפחה (מקום-שמור: תוגדר בשער-הסליקה)')];
    final h = f['hok'] as Map;
    ...
    return [
      Row(children: [
        StatusDot(tone: _SupporterData.hokFailed(f) ? 2 : _SupporterData.hokFlag(f) ? 1 : 3),
        const SizedBox(width: 10),
        Expanded(child: MediaRow(glyph: '💳', title: '${_m(h['amount'] as num)} · יום ${h['day']} בחודש', subtitle: '${_SupporterData.hokMethod(f)} · מ-${fmtDate(h['last'] as String?)}${h['kevaId'] != null ? ' · סליקה ${h['kevaId']}' : ' · ידנית'}')),
      ]),
      _wrap([
        StatusChip(label: _SupporterData.hokFlag(f) ? 'מסומנת פעילה' : 'מופסקת', tone: _SupporterData.hokFlag(f) ? 1 : 0),
        StatusChip(label: _SupporterData.hokActive(f) ? 'סליקה חיה' : 'סליקה פסקה >2 חודשים', tone: _SupporterData.hokActive(f) ? 1 : 2),
        StatusChip(label: _SupporterData.hokRecorded(f) ? 'נרשמה החודש ✅' : 'טרם נרשמה החודש', tone: _SupporterData.hokRecorded(f) ? 1 : 3),
      ]),
  ...
  List<Widget> _tabReminders(Map<String, dynamic> f, void Function(void Function()) act) {
    ...
    return [
      if (_SupporterData.fullScholarship(f)) const AlertBanner(glyph: '🎓', tone: 1, message: 'מלגה מלאה — אפס-תזכורות (מגן-כבוד)') else if (plan.isEmpty) const EmptyState(glyph: '🕊', message: 'אין חוב פתוח — אין לוח-תזכורות') else ...[
        ...
        _wrap([for (var i = 0; i < plan.length; i++) StatusChip(label: '${i < sent.length ? '✅' : '${plan[i]['date']}'.compareTo(_SupporterData.today) <= 0 ? '⏰' : '⏳'} ${plan[i]['grade']} · ${fmtDate(plan[i]['date'] as String?)}', tone: i < sent.length ? 1 : nr != null && nr['grade'] == plan[i]['grade'] ? 3 : 0)]),
        if (nr != null && _SupporterData.can(_role, 'fees.remind')) _wrap([SoftButton(label: '📨 שלח תזכורת ${nr['grade']} (פרטי)', tone: 3, onTap: () => act(() => _SupporterData.sendReminder(f, _roleName, nr['grade'] as String)))], top: 8),
      ],
      _gap(8),
  ...
  List<Widget> _tabDiscounts(Map<String, dynamic> f, void Function(void Function()) act) {
    ...
    return [
      Row(children: [
        BareStat(value: '${_SupporterData.discountPct(f)}%', label: 'הנחה אפקטיבית (הגבוהה מנצחת)', inkColor: _acc, mutedColor: _muted),
        BareStat(value: _m(_SupporterData.scholarshipOf(f)), label: 'שווי-ההנחה השנה', inkColor: _ink, mutedColor: _muted),
        BareStat(value: '${_SupporterData.studentsN(f)}', label: 'אחים (הנחת-אחים אוטו)', inkColor: _ink, mutedColor: _muted),
      ]),
      _wrap([for (final c in _SupporterData.criteria) StatusChip(label: '${ids.contains(c['id']) ? '✅ ' : ''}${c['label']} ${c['discountPct']}%', tone: ids.contains(c['id']) ? 1 : 0)]),
      if (_SupporterData.can(_role, 'fees.scholarship'))
        _wrap([for (final c in _SupporterData.criteria) if (!ids.contains(c['id'])) SoftButton(label: '🎓 הענק ${c['label']}', tone: 0, onTap: () => act(() => _SupporterData.grantDiscount(f, _roleName, c['id'] as String)))], top: 8)
      else
    ...
    return [
      if (ins.isEmpty) ...[
        ...
        if (_SupporterData.can(_role, 'fees.arrangement') && _SupporterData.balance(f) > 0)
          _wrap([for (final n in const [2, 3, 6]) SoftButton(label: '📆 פריסה ל-$n', tone: 0, onTap: () => act(() => _SupporterData.setArrangement(f, _roleName, n)))], top: 4),
      ] else ...[
        if (_SupporterData.arrangementLate(f)) const AlertBanner(glyph: '📆', tone: 3, message: 'הסדר בפיגור — תשלום שמועדו עבר לא כוסה'),
        for (final c in ins)
          TimelineItem(title: '${c['note'] ?? 'תשלום-הסדר'}', time: fmtDate(c['date'] as String?), body: '${_m(_SupporterData.netOf(f, c))} · ${'${c['date']}'.compareTo(_SupporterData.today) <= 0 ? (_SupporterData.oldestOpenDate(f) != null && '${c['date']}'.compareTo(_SupporterData.oldestOpenDate(f)!) >= 0 ? 'פתוח' : 'כוסה') : 'עתידי'}'),
      ],
    ];
  }
            ...
            builder: (ctx, scroll) => Padding(
              ...
              child: GlassCard(
                child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                  ...
                  _gap(8),
                  DsEnumField(label: 'משפחה', options: [for (final x in _SupporterData.families) '${x['name']}'], value: '${f['name']}', onChanged: (v) => setSheet(() => fam = _SupporterData.families.firstWhere((x) => x['name'] == v)['id'] as String)),
                  DsEnumField(label: 'עבור-מי', options: members, value: member, onChanged: (v) => setSheet(() => member = v)),
                  DsEnumField(label: 'סוג-חיוב', options: _SupporterData.chargeTypes, value: cat, onChanged: (v) => setSheet(() => cat = v)),
            ...
            builder: (ctx, scroll) => Padding(
              ...
              child: GlassCard(
                child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                  ...
                  _gap(8),
                  DsEnumField(label: 'משפחה', options: [for (final x in _SupporterData.families) '${x['name']}'], value: '${f['name']}', onChanged: (v) => setSheet(() => fam = _SupporterData.families.firstWhere((x) => x['name'] == v)['id'] as String)),
                  DsEnumField(label: 'אמצעי', options: _SupporterData.payMethodsSchool, value: method, onChanged: (v) => setSheet(() => method = v)),
    ...
    showModalBottomSheet<void>(
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx, scroll) => Padding(
          ...
          child: GlassCard(
            child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
              MediaRow(glyph: '⬇', title: 'ייצוא CSV', subtitle: '${fs.length} משפחות · ${_SupporterData.csvHeader.length} עמודות (PDF = שער-חיצוני, מקום-שמור)'),
              _gap(10),
              Container(
                padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(color: const Color(0xFF0C0D1E), borderRadius: BorderRadius.circular(10)),
