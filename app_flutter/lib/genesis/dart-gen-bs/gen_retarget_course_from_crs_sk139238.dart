...
class _CourseScreenState extends State<CourseScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(
      ...
      children: [
        ...
        if (_metric != null) AlertBanner(glyph: '🎯', tone: 1, message: 'מסונן למדד: ${CourseFacts.metricDefs.firstWhere((d) => d['key'] == _metric, orElse: () => const {'label': ''})['label']} · ${visible.length} מתוך ${visibleAll.length}'),
        ...
        Align(
          ...
          child: FittedBox(fit: BoxFit.scaleDown, child: SegmentedSwitch(items: [for (final r in _CourseData.roleDefs) r['label'] as String], selected: _role, onSelect: (i) => setState(() { _role = i; _locks.clear(); }))),
        ),
        _gap(8),
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [
          ...
          if (_can('crs.new')) SoftButton(label: '➕ חוג-חדש', tone: 1, onTap: () => _act(() => _CourseData.newCourse(_who), 'נוצר חוג-חדש (ללא-מורה/ללא-חדר — שבץ בפאנל)')),
          ...
          if (_CourseData.exportOk(_role)) SoftButton(label: '⬇ ייצוא', tone: 0, onTap: () => _openExport(visible)),
          StatusChip(label: 'תפקיד: ${_CourseData.roleName(_role)}${_CourseData.myTeacherId(_role) != null ? ' · החוגים-שלי' : _CourseData.myFamilyId(_role) != null ? ' · המערכת-שלי' : ''}', tone: 0),
        ]),
        _gap(6),
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [
          ...
          for (final st in _CourseData.stateChips) _fchip('state', st[0], '${st[1]} · ${_CourseData.countState(st[0] == 'ended' ? _CourseData.bySemester(_CourseData.allCourses, _sem) : live, st[0])}'),
        ]),
        if (_adv) ...[
          _gap(8),
          ...
          Wrap(spacing: 8, runSpacing: 6, children: [
            ...
            for (final h in _CourseData.gridHours(_CourseData.liveCourses)) _fchip('hour', '${h ~/ 60}', '🕐 ${_CourseData.hm(h)}'),
          ]),
          _gap(6),
        ...
        if (_loading)
          ...
          EmptyState(glyph: '📚', message: _CourseData.myTeacherId(_role) != null ? 'אין חוגים משובצים למורה זה' : _CourseData.myFamilyId(_role) != null ? 'אין חוגים למשפחה — הירשמו מהקטלוג' : 'אין חוגים — צור חוג-חדש או שכפל סמסטר')
        else if (visible.isEmpty)
          const Padding(padding: EdgeInsets.only(top: 24), child: EmptyState(glyph: '🔍', message: 'אין חוגים תואמים לחיפוש/סינון'))
        else if (_view == 1)
          DsSection(title: '📋 רשימת-חוגים · ${visible.length} · ${_CourseData.columnDefs.where((c) => _CourseData.colShown(c, visible)).length} עמודות', children: [_table(ranked)])
        else if (_view == 2)
          ..._byTeacher(visible)
        else if (_view == 3)
          ..._byRoom(visible)
        else ...[
          DsSection(title: '📅 מערכת-שעות · ${_week == 0 ? 'השבוע' : 'שבוע הבא'} (${_CourseData.isoOfDay(0, _week)} – ${_CourseData.isoOfDay(5, _week)})', children: [_grid(visible)]),
    ...
    return [
      DsSection(title: '🤖 אוטומציות · ${hl.length + rem.length + bm.length + pr.length + noRoomCs.length + noTeacherCs.length + clashRooms.length + demand.length} אותות', children: [
        ...
        if (hl.isNotEmpty) withAction(
          ...
          _can('crs.cancel') ? SoftButton(label: '✖ בטל-אוטו', tone: 3, onTap: () => _act(() => _CourseData.autoCancelHolidays('אוטומציה'), '${hl.length} שיעורי-חג בוטלו אוטומטית — שלח הודעה למשפחות מהפאנל')) : null),
        // מתחת-מינימום X ימים לפני/אחרי פתיחה — התרעה כלכלית
        for (final c in bm)
          AlertBanner(glyph: '📉', tone: 2, message: '${c['name']}: ${_CourseData.enrolled(c)} רשומים מול מינ׳ ${_CourseData.minToOpen(c)} · ${_CourseData.daysToStart(c) >= 0 ? 'מתחיל בעוד ${_CourseData.daysToStart(c)} ימים' : 'התחיל לפני ${-_CourseData.daysToStart(c)} ימים'} — לא-כלכלי'),
        // הצעת-חדר-חלופי: ללא-חדר / התנגשות-חדר ⇒ חדרים פנויים ב-slot
        for (final c in [...noRoomCs, ...clashRooms])
          AlertBanner(glyph: '🚪', tone: 3, message: '${c['name']} — ${_CourseData.noRoom(c) ? 'ללא-חדר' : 'התנגשות-חדר'} · חדר חלופי: ${_CourseData.freeRooms(c).isEmpty ? 'אין חדר פנוי ב-slot' : _CourseData.freeRooms(c).map((r) => '${r['name']} (${r['cap']})').join(' / ')}'),
        // הצעת-מורה-מחליף: ללא-מורה ⇒ מורים פנויים ב-slot (התמחות תואמת קודם)
        for (final c in noTeacherCs)
          AlertBanner(glyph: '👩‍🏫', tone: 3, message: '${c['name']} — ללא-מורה · מורה חלופי: ${_CourseData.freeTeachers(c).isEmpty ? 'אין מורה פנוי ב-slot' : _CourseData.freeTeachers(c).map((t) => '${t['name']}${t['specialty'] == c['cat'] ? ' ✓' : ''}').join(' / ')}'),
        // המתנה-עם-מקום ⇒ העלאה
        for (final c in pr) withAction(
          AlertBanner(glyph: '⏳', tone: 3, message: '${c['name']}: ${_CourseData.waitlist(c).length} ממתינים ויש ${_CourseData.capacity(c) - _CourseData.enrolled(c)} מקומות פנויים'),
          _can('crs.waitlist') ? SoftButton(label: '⬆ העלה', tone: 1, onTap: () => _act(() { while (_CourseData.promoteNext(c, 'אוטומציה') != null) {} }, 'הממתינים הועלו')) : null),
        // תזכורות 48h
        if (rem.isNotEmpty) AlertBanner(glyph: '🔔', tone: 0, message: 'תזכורת ל-48 השעות הקרובות: ${rem.map((r) => '${(r['course'] as Map)['name']} ${_CourseData._iso(r['dt'] as DateTime)} ${_CourseData.hm((r['dt'] as DateTime).hour * 60 + (r['dt'] as DateTime).minute)} (${_CourseData.liveEnrollmentsOf(r['course'] as Map<String, dynamic>).length} משפחות)').join(' · ')} — שלח-הודעה מהפאנל'),
        // תחזית-ביקוש (אות-נוכחי; היסטוריה רב-סמסטרית = מקום-שמור)
        if (demand.isNotEmpty) AlertBanner(glyph: '📈', tone: 1, message: 'ביקוש לסמסטר-הבא: ${demand.map((c) => '${c['name']} (${_CourseData.isFull(c) ? 'מלא' : ''}${_CourseData.waitlist(c).isNotEmpty ? ' +${_CourseData.waitlist(c).length} ממתינים' : ''}${_CourseData.trend(c)['dir'] == 'up' ? ' ↑' : ''})').join(' · ')} ⇒ שקול קבוצה נוספת'),
      ...
      DsSection(title: '🛒 הרשמה-עצמית · ${open.length} חוגים פתוחים (רכז/ת מאשר/ת)', children: [
        ...
        for (final c in open)
          Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(children: [
            Expanded(child: MediaRow(glyph: '📚', title: '${c['name']}', subtitle: '${_CourseData.sessionsLabel(c)} · ${_CourseData.enrolled(c)}/${_CourseData.capacity(c)} · שכבות ${c['gradeMin']}–${c['gradeMax']}')),
            for (final m in (fam['members'] as List))
              Padding(padding: const EdgeInsets.only(left: 4), child: SoftButton(label: '➕ ${m['first']}', tone: _CourseData.fitReason(c, _CourseData.memberOf(m['id'])!) == null ? 1 : 3,
                onTap: () => setState(() => _result(_CourseData.selfEnroll(c, m['id'], _who), 'נרשם/ה — ממתין לאישור')))),
          ])),
      ]),
    ];
  }
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: FittedBox(
          ...
          child: StatusChip(label: '${cancelled ? '✖ ' : ''}${c['name']} ${_CourseData.enrolled(c)}/${_CourseData.capacity(c)}', tone: tone),
        ),
      ),
    );
  }
      ...
      for (final t in _CourseData.teachers)
        () {
          ...
          return DsSection(
            ...
            trailing: StatusChip(label: '${cs.length} חוגים · ${_CourseData.weeklyOf(cs)} מפגשים/שבוע', tone: cs.isEmpty ? 0 : 1),
        ...
        for (final r in _CourseData.rooms)
          () {
            ...
            return DsSection(
              ...
              children: [
                StatRow(label: 'ניצולת שבועית', value: '$weekly מתוך $cap משבצות', fraction: _CourseData.roomUtil(r)),
                _gap(6),
  ...
  Widget _row(Map<String, dynamic> c) {
    ...
    return Padding(
      ...
      child: Row(children: [
        Expanded(child: MediaRow(glyph: '📚', title: '${c['name']}', subtitle: '${t?['name'] ?? '—'} · ${r?['name'] ?? '—'} · ${_CourseData.sessionsLabel(c)} · ${_CourseData.enrolled(c)}/${_CourseData.capacity(c)}')),
        StatusChip(label: _CourseData.statusLabel(c), tone: tone),
        if (_CourseData.waitlist(c).isNotEmpty) ...[const SizedBox(width: 6), StatusChip(label: '⏳ ${_CourseData.waitlist(c).length}', tone: 0)],
            ...
            child: Directionality(
              ...
              child: GlassCard(
                child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                  Row(children: [
                    Expanded(child: MediaRow(glyph: '📚', title: '${c['name']}', subtitle: '${_CourseData.teacherOf(c)?['name'] ?? 'ללא-מורה'} · ${_CourseData.roomOf(c)?['name'] ?? 'ללא-חדר'} · ${_CourseData.sessionsLabel(c)}')),
                    StatusChip(label: _CourseData.statusLabel(c), tone: _CourseData.sev(c) >= 2 ? 2 : _CourseData.sev(c) == 1 ? 3 : 1),
                  ]),
                  _gap(8),
    ...
    return [
      ...
      if (_CourseData.noRoom(c) || _CourseData.clashesOf(c).any((k) => k['kind'] == 'room')) ...[
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          if (_CourseData.freeRooms(c).isEmpty) const StatusChip(label: 'אין חדר פנוי', tone: 2),
          for (final r in _CourseData.freeRooms(c))
            if (_can('crs.assignRoom')) SoftButton(label: '🚪 ${r['name']} (${r['cap']})', tone: 1, onTap: () => both(() => _result(_CourseData.assignRoom(c, r['id'], _who), '${r['name']} הוקצה'))) else _chip('🚪 ${r['name']}', 0),
        ]),
      ],
      if (_CourseData.noTeacher(c) || _CourseData.clashesOf(c).any((k) => k['kind'] == 'teacher')) ...[
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          if (_CourseData.freeTeachers(c).isEmpty) const StatusChip(label: 'אין מורה פנוי', tone: 2),
          for (final t in _CourseData.freeTeachers(c))
            if (_can('crs.assignTeacher')) SoftButton(label: '${t['name']}${t['specialty'] == c['cat'] ? ' ✓' : ''}', tone: t['specialty'] == c['cat'] ? 1 : 0, onTap: () => both(() => _result(_CourseData.assignTeacher(c, t['id'], _who), '${t['name']} הוקצה/תה'))) else _chip('${t['name']}', 0),
        ]),
      ],
      if (_CourseData.belowMin(c)) AlertBanner(glyph: '📉', tone: 3, message: 'מתחת-למינימום: ${_CourseData.enrolled(c)} רשומים מול ${_CourseData.minToOpen(c)} (${c['minStudents'] == null ? 'נקודת-איזון: שכר-מורה+חדר ÷ מחיר-לשיעור' : 'מינימום-מוגדר'}) — לא-כלכלי'),
      _h('📅 השיעורים הבאים'),
      ...
      Builder(builder: (_) {
        final acts = <Widget>[
          ...
          if (_can('crs.assignTeacher')) SoftButton(label: '👩‍🏫 הקצה-מורה', tone: _CourseData.noTeacher(c) ? 2 : 0, onTap: () => both(() => _pick = _pick == 'teacher' ? null : 'teacher')),
          if (_can('crs.assignTeacher')) SoftButton(label: '🔄 מורה-מחליף (חד-פעמי)', tone: 0, onTap: () => both(() => _pick = _pick == 'sub' ? null : 'sub')),
          if (_can('crs.assignRoom')) SoftButton(label: '🚪 הקצה-חדר', tone: _CourseData.noRoom(c) ? 2 : 0, onTap: () => both(() => _pick = _pick == 'room' ? null : 'room')),
          if (_can('crs.edit')) SoftButton(label: '✏️ ערוך', tone: 0, onTap: () => both(() => _edit = !_edit)),
          if (_can('crs.duplicate')) SoftButton(label: '📄 שכפל-חוג', tone: 0, onTap: () => both(() { final cp = _CourseData.duplicate(c, _who); _flash('נוצר ${cp['name']} — יורש slot ⇒ בדוק התנגשות והקצה מחדש', 3); })),
          if (_can('crs.message')) SoftButton(label: '💬 שלח-הודעה', tone: 0, onTap: () => both(() => _pick = _pick == 'message' ? null : 'message')),
          if (_can('crs.end')) SoftButton(label: '🏁 סיים-חוג', tone: 2, onTap: () => both(() { _CourseData.endCourse(c, _who); _flash('${c['name']} הסתיים — ההרשמות נסגרו', 3); })),
          if (_can('crs.end')) SoftButton(label: '⛔ בטל-חוג', tone: 2, onTap: () => both(() { _CourseData.cancelCourse(c, _who); _flash('${c['name']} בוטל', 3); })),
        ];
        return acts.isEmpty
      ...
      return [
        ...
        if (cands.isEmpty) const EmptyState(glyph: '🎓', message: 'כל התלמידים כבר רשומים/ממתינים')
          ...
          for (final m in cands)
            SoftButton(label: '${m['first']} ${m['famName']} · ${m['grade']}', tone: _CourseData.fitReason(c, m) == null ? 0 : 3,
              onTap: () => both(() { _pick = null; _result(p == 'enroll' ? _CourseData.enroll(c, m['id'], _who) : _CourseData.invite(c, m['id'], _who), '${m['first']} שובץ/ה ל-${c['name']}'); })),
        ]),
      ];
    }
    if (p == 'teacher' || p == 'sub') {
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          for (final t in _CourseData.teachers)
            SoftButton(label: '${t['name']} · ${t['specialty']}', tone: t['id'] == c['teacherId'] ? 1 : 0,
              onTap: () => both(() { _pick = null; if (p == 'teacher') { _result(_CourseData.assignTeacher(c, t['id'], _who), '${t['name']} הוקצה/תה ל-${c['name']}'); } else { _CourseData.substitute(c, iso, t['id'], _who); _flash('${t['name']} מחליף/ה ב-$iso', 1); } })),
        ]),
      ];
    }
    if (p == 'room') {
      return [
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          for (final r in _CourseData.rooms.where((r) => r['active'] == true))
            SoftButton(label: '${r['name']} · ${r['cap']} · ${r['slot']} דק׳', tone: r['id'] == c['roomId'] ? 1 : (r['cap'] as int) < _CourseData.capacity(c) ? 3 : 0,
              onTap: () => both(() { _pick = null; _result(_CourseData.assignRoom(c, r['id'], _who), '${r['name']} הוקצה ל-${c['name']}'); })),
        ]),
      ];
    }
    if (p == 'message') {
      ...
      return [
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          for (final t in _CourseData.liveCourses.where((t) => t['id'] != c['id']))
            SoftButton(label: '${t['name']} ${_CourseData.enrolled(t)}/${_CourseData.capacity(t)}', tone: _CourseData.isFull(t) ? 3 : 0,
              onTap: () => both(() { _pick = null; _result(_CourseData.move(e, t, _who), 'הועבר/ה ל-${t['name']}'); })),
        ]),
      ];
    }
    return const [];
  ...
  Widget _lessonTile(Map<String, dynamic> c, DateTime dt, void Function(void Function()) both) {
    ...
    return Row(children: [
      Expanded(child: TimelineItem(
        title: '${cancelled ? '✖ מבוטל · ' : ''}${dayNames[dt.weekday % 7]} ${_CourseData.hm(dt.hour * 60 + dt.minute)}${_CourseData.holidayName(dt) != null ? ' · 🕎 ${_CourseData.holidayName(dt)}' : ''}',
        time: iso,
        body: '${subName != null ? '🔄 $subName (מחליף/ה)' : _CourseData.teacherOf(c)?['name'] ?? 'ללא-מורה'} · ${_CourseData.roomOf(c)?['name'] ?? 'ללא-חדר'}',
      )),
      if (_can('crs.cancel')) SoftButton(label: cancelled ? '↩ שחזר' : '✖ בטל', tone: cancelled ? 1 : 2, onTap: () => both(() { _CourseData.cancelSession(c, iso, _who); _flash(cancelled ? 'השיעור $iso שוחזר' : 'השיעור $iso בוטל', cancelled ? 1 : 3); })),
    ]);
  }
  ...
  List<Widget> _tabEnrolled(Map<String, dynamic> c, void Function(void Function()) both) {
    ...
    return [
      ...
      for (final e in es) ...[
        Row(children: [
          Expanded(child: MediaRow(glyph: '🎓', title: _CourseData.memberName(e['memberId']), subtitle: '${_CourseData.enrollStatusLabel(e)} · ${_CourseData.paidLabel(e)}${_CourseData.debtOf(e) > 0 ? ' · חוב ${shekel(_CourseData.debtOf(e).toInt())}' : ''} · נרשם/ה ${e['enrolledAt']}')),
          if (_can('crs.move')) SoftButton(label: '🔁', tone: 0, onTap: () => both(() => _pick = _pick == 'move:${e['id']}' ? null : 'move:${e['id']}')),
          const SizedBox(width: 4),
          if (_can('crs.remove')) SoftButton(label: '➖ הסר', tone: 2, onTap: () => both(() => _result(_CourseData.remove(e, _who), '${_CourseData.memberName(e['memberId'])} הוסר/ה'))),
        ]),
  ...
  List<Widget> _tabWaitlist(Map<String, dynamic> c, void Function(void Function()) both) {
    ...
    return [
      ...
      for (var i = 0; i < w.length; i++)
        Row(children: [
          Expanded(child: MediaRow(glyph: '${i + 1}', title: _CourseData.memberName(w[i]['memberId']), subtitle: 'ממתין/ה מ-${w[i]['enrolledAt']}${_CourseData.clashReason(c, w[i]['memberId']) != null ? ' · ⚠️ התנגשות' : ''}')),
          if (_can('crs.waitlist')) SoftButton(label: '⬆ העלה', tone: _CourseData.isFull(c) ? 3 : 1, onTap: () => both(() => _result(_CourseData.promote(w[i], _who), '${_CourseData.memberName(w[i]['memberId'])} הועלה/תה מההמתנה'))),
          const SizedBox(width: 4),
          if (_can('crs.waitlist')) SoftButton(label: '➖', tone: 2, onTap: () => both(() => _result(_CourseData.remove(w[i], _who), 'הוסר/ה מההמתנה'))),
        ]),
      _gap(6),
      Row(children: [
        ...
        SegmentedSwitch(items: const ['פועל', 'כבוי'], selected: _CourseData.autoPromote ? 0 : 1, onSelect: (i) => both(() => _CourseData.autoPromote = i == 0)),
      ]),
    ];
  }
  ...
  List<Widget> _tabSchedule(Map<String, dynamic> c, void Function(void Function()) both) {
    ...
    return [
      ...
      if (!_CourseData.hasSessions(c)) const EmptyState(glyph: '🗓', message: 'אין מפגשים קבועים — הגדר יום+שעה (מקום-שמור: עורך-מפגשים)'),
      for (final s in ss) if (s['day'] is int) TimelineItem(title: '${dayNames[s['day'] as int]} ${s['time']}', time: '${(s['label'] ?? '') == '' ? 'קבוצה יחידה' : s['label']}', body: _CourseData.roomLabel(c)),
      ...
      for (final e in es)
        () {
          ...
          return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            StatRow(label: _CourseData.memberName(e['memberId']), value: '${sm['presents']}/$tot · החודש ${_CourseData.presentsThisMonth(e)}', fraction: tot == 0 ? 0 : (sm['presents'] as int) / tot),
  ...
  List<Widget> _tabFees(Map<String, dynamic> c) {
    ...
    return [
      ...
      _gap(8),
      for (final e in es)
        MediaRow(glyph: _CourseData.paidStatus(e) == 'paid' ? '✅' : _CourseData.paidStatus(e) == 'partial' ? '🟠' : '🔴', title: _CourseData.memberName(e['memberId']),
          subtitle: '${_CourseData.paidLabel(e)} · שולם ${shekel(paidOf(e).toInt())} מתוך ${shekel(((e['totalDue'] as num?) ?? 0).toInt())}${_CourseData.debtOf(e) > 0 ? ' · חוב ${shekel(_CourseData.debtOf(e).toInt())}' : ''}${'${e['dueDate'] ?? ''}'.isNotEmpty ? ' · לתשלום עד ${e['dueDate']}' : ''}'),
    ];
  }
        ...
        return DraggableScrollableSheet(
          ...
          builder: (ctx, scroll) => Padding(
            ...
            child: Directionality(textDirection: TextDirection.rtl, child: GlassCard(
              child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                MediaRow(glyph: '⬇', title: 'ייצוא', subtitle: '${cs.length} חוגים · ${_CourseData.csvRows(cs).first.length} עמודות · שער-ייצוא פתוח'),
                _gap(8),
