  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk04_home_c61.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk04_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk04HomeScreenState extends State<GenAppPeruk04HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk04_home_c62, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk04_home_c63, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk04_home_c64 : n <= 1 ? gen_app_peruk04_home_c65 : gen_app_peruk04_home_c66.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk04_home_c67, subtitle: lead, icon: gen_app_peruk04_home_c68, children: [
      DsLoadMeter(count: n, label: gen_app_peruk04_home_c69.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk04_home_c70, gen_app_peruk04_home_c71, gen_app_peruk04_home_c72]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk04_home_c73, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk04_home_c74, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk04HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk04_home_c75 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk04_home_c76 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk04_home_c77, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk04_home_c78 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk04_home_c79 + ' ' + gen_app_peruk04_home_c80, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
