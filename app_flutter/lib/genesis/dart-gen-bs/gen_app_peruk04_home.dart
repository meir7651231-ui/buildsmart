...
class GenAppPeruk04HomeScreenToday {
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true}) {   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk04_home_c61.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk04_home_c62, noLabel: gen_app_peruk04_home_c63, alwaysLabel: gen_app_peruk04_home_c64,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk04_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk04_home_c65.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk04_home_c66, noLabel: gen_app_peruk04_home_c67, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
    if (chain) for (final r in appStore.records('app_peruk04_ent1')) {   // P14 · הצעד-הבא: רשומה שהגיעה לשלב-האחרון (סגורה — לא ב-open) ובלי הכרעה
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk04_ent1', rid);
      if (appStore.stageOf('app_peruk04_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk04_home_c68.replaceAll('{next}', gen_app_peruk04_home_c37), source: module + ' · ' + who, okLabel: gen_app_peruk04_home_c69, noLabel: gen_app_peruk04_home_c70, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk04_home_c71.replaceAll('{next}', gen_app_peruk04_home_c37), entity: 'app_peruk04_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }
  ...
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: module + ' · ' + (((r[gen_app_peruk04_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk04_home_c72 : (r[gen_app_peruk04_home_c0] ?? '')), trailing: Text(const [gen_app_peruk04_home_c30, gen_app_peruk04_home_c31, gen_app_peruk04_home_c32, gen_app_peruk04_home_c33, gen_app_peruk04_home_c34][appStore.stageOf('app_peruk04_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), children: [
        ...
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk04_home_c73, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk04_home_c74, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk04RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk04_home_c75.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk04_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk04HomeScreenState extends State<GenAppPeruk04HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk04_home_c76, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk04_home_c77, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk04_home_c78 : n <= 1 ? gen_app_peruk04_home_c79 : gen_app_peruk04_home_c80.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk04_home_c81, subtitle: lead, icon: gen_app_peruk04_home_c82, children: [
      DsLoadMeter(count: n, label: gen_app_peruk04_home_c83.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk04_home_c84, gen_app_peruk04_home_c85, gen_app_peruk04_home_c86]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk04_home_c87, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk04_home_c88, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk04HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk04_home_c89 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk04_home_c90 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk04_home_c91, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk04_home_c92 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk04_home_c93 + ' ' + gen_app_peruk04_home_c94, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
