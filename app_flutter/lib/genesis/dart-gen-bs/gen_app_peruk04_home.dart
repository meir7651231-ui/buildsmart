...
class GenAppPeruk04HomeScreenToday {
  ...
  static DsTodayItem _mk(String title, String sub, String rid, String field, DateTime d, bool hard, bool overdue, DateTime today) {
    final acts = overdue ? [gen_app_peruk04_home_c38, gen_app_peruk04_home_c39, gen_app_peruk04_home_c40] : (d == today ? [gen_app_peruk04_home_c41] : [gen_app_peruk04_home_c42, gen_app_peruk04_home_c43]);   // P4 · ביום-ההכרעה אין דחייה
  ...
  static void _act(String rid, String field, DateTime due, List<String> acts, int i) {
    ...
    if (a == gen_app_peruk04_home_c44) { appStore.advance('app_peruk04_ent1', rid, 5); }
    else if (a == gen_app_peruk04_home_c45) { final r = appStore.byId('app_peruk04_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk04_ent1', rid, {field: _iso(due.add(const Duration(days: 1)))}); appStore.logAction('auto', gen_app_peruk04_home_c46 + ' · ' + field, entity: 'app_peruk04_ent1', rid: rid, field: field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { out.add(_mk('${f.label} · $who', gen_app_peruk04_home_c47.replaceAll('{date}', _iso(d)), rid, f.label, d, f.hard, true, today)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk('${f.label} · $who', off == 0 ? gen_app_peruk04_home_c48 : gen_app_peruk04_home_c49.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today)); break; }
        }
      }
    }
  ...
  static List<Widget> proposals(BuildContext context, DateTime today) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk04_home_c50.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk04_home_c51, noLabel: gen_app_peruk04_home_c52, alwaysLabel: gen_app_peruk04_home_c53,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk04_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk04_home_c54.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk04_home_c55, noLabel: gen_app_peruk04_home_c56, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
      if (appStore.stageOf('app_peruk04_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk04_home_c57.replaceAll('{next}', gen_app_peruk04_home_c35), source: module + ' · ' + who, okLabel: gen_app_peruk04_home_c58, noLabel: gen_app_peruk04_home_c59, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk04_home_c60.replaceAll('{next}', gen_app_peruk04_home_c35), entity: 'app_peruk04_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }

  // כרטיס-הרשומה (G30): נוסחים · שלח · פתח — ≤2 הקשות
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: (((r[gen_app_peruk04_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk04_home_c61 : (r[gen_app_peruk04_home_c0] ?? '')) + ' · ' + const [gen_app_peruk04_home_c28, gen_app_peruk04_home_c29, gen_app_peruk04_home_c30, gen_app_peruk04_home_c31, gen_app_peruk04_home_c32][appStore.stageOf('app_peruk04_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], children: [
        ...
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk04_home_c62, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk04_home_c63, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk04RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk04_home_c64.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk04_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk04HomeScreenState extends State<GenAppPeruk04HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk04_home_c65, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk04_home_c66, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk04_home_c67 : n <= 1 ? gen_app_peruk04_home_c68 : gen_app_peruk04_home_c69.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk04_home_c70, subtitle: lead, icon: gen_app_peruk04_home_c71, children: [
      DsLoadMeter(count: n, label: gen_app_peruk04_home_c72.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk04_home_c73, gen_app_peruk04_home_c74, gen_app_peruk04_home_c75]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk04_home_c76, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk04_home_c77, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk04HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk04_home_c78 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk04_home_c79 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk04_home_c80, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk04_home_c81 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk04_home_c82 + ' ' + gen_app_peruk04_home_c83, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
