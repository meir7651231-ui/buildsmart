...
class GenAppPeruk17HomeScreenToday {
  ...
  static void _act(String rid, String field, DateTime due, List<String> acts, int i) {
    ...
    else { appStore.decide('ign:$rid:$field', 'no'); appStore.logAction('decide', gen_app_peruk17_home_c31.replaceAll('{what}', field + ' · ' + appStore.displayOf('app_peruk17_ent1', rid)), entity: 'app_peruk17_ent1', rid: rid, field: 'ign:$rid:$field'); }   /* ב׳-לב · «התעלם» נרשם ביומן ⇒ החזר (kind=decide: field = מפתח-ההכרעה) — שום דבר לא בלתי-הפיך */
  }
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { final ago = today.difference(d).inDays; out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', gen_app_peruk17_home_c32.replaceAll('{date}', _iso(d)) + ' · ' + (ago == 1 ? gen_app_peruk17_home_c33 : gen_app_peruk17_home_c34.replaceAll('{n}', ago.toString())), rid, f.label, d, f.hard, true, today, tm, rep)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', off == 0 ? '' : gen_app_peruk17_home_c35.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep)); break; }
        }
      }
    }
  ...
  static List<DsTodayItem> undated(DateTime today) {
    ...
    for (final r in open()) {
      ...
      final acts = [gen_app_peruk17_home_c36, gen_app_peruk17_home_c37, gen_app_peruk17_home_c38];
  ...
  static void _setDate(String rid, String field, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    if (a == gen_app_peruk17_home_c39) { appStore.decide('undated:$rid', 'no'); appStore.logAction('decide', gen_app_peruk17_home_c40.replaceAll('{what}', appStore.displayOf('app_peruk17_ent1', rid)), entity: 'app_peruk17_ent1', rid: rid, field: 'undated:$rid'); return; }
    final r = appStore.byId('app_peruk17_ent1', rid); if (r == null) return;
    final prev = r[field] ?? '';
    appStore.update('app_peruk17_ent1', rid, {field: _iso(_shift(today.add(Duration(days: a == gen_app_peruk17_home_c41 ? 7 : 1)), false))});   /* «קבע» לא נוחת בשבת */
  ...
  static List<DsTodayItem> stale(DateTime today) {
    ...
    for (final r in open()) {
      ...
      final acts = [gen_app_peruk17_home_c42, gen_app_peruk17_home_c43, gen_app_peruk17_home_c44];
      out.add(DsTodayItem(title: appStore.displayOf('app_peruk17_ent1', rid), sub: gen_app_peruk17_home_c45.replaceAll('{n}', n.toString()), rid: rid, field: '', due: t, hard: false, overdue: false, module: module, actions: acts, act: (i) => _staleAct(rid, today, acts, i)));
    }
  ...
  static void _staleAct(String rid, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    final r = appStore.byId('app_peruk17_ent1', rid); if (r == null) return;
    if (a == gen_app_peruk17_home_c46) { final f = _dates.firstWhere((x) => x.hard, orElse: () => _dates.first); appStore.decide('ign:$rid:${f.label}', ''); appStore.decide('undated:$rid', ''); _setDate(rid, f.label, today, [a], 0); return; }   /* המועד החדש חוזר ל«היום» — ההתעלמות הישנה נמחקת */
    if (a == gen_app_peruk17_home_c47) { final prev = r[AppStore.stageKey] ?? '0'; appStore.update('app_peruk17_ent1', rid, {AppStore.stageKey: '4'}); appStore.logAction('auto', gen_app_peruk17_home_c48.replaceAll('{who}', appStore.displayOf('app_peruk17_ent1', rid)), entity: 'app_peruk17_ent1', rid: rid, field: AppStore.stageKey, prev: prev); return; }
    appStore.decide('stale:$rid', 'no'); appStore.logAction('decide', gen_app_peruk17_home_c49.replaceAll('{what}', appStore.displayOf('app_peruk17_ent1', rid)), entity: 'app_peruk17_ent1', rid: rid, field: 'stale:$rid');
  }
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true, bool rem = true}) {   // rem=false: «בלגן» מרכז את התזכורות לכרטיס אחד (ב׳-לז)   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      if (rem) for (final f in _dates) {
        ...
        if (appStore.decision(_remKey(rid, f.label)).isNotEmpty) continue;
        out.add(DsApproveCard(question: gen_app_peruk17_home_c50.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk17_home_c51, noLabel: gen_app_peruk17_home_c52, alwaysLabel: gen_app_peruk17_home_c53,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk17_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk17_home_c54.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk17_home_c55, noLabel: gen_app_peruk17_home_c56, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
    if (chain) for (final r in appStore.records('app_peruk17_ent1')) {   // P14 · הצעד-הבא: רשומה שהגיעה לשלב-האחרון (סגורה — לא ב-open) ובלי הכרעה
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk17_ent1', rid);
      if (appStore.stageOf('app_peruk17_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk17_home_c57.replaceAll('{next}', gen_app_peruk17_home_c11), source: module + ' · ' + who, okLabel: gen_app_peruk17_home_c58, noLabel: gen_app_peruk17_home_c59, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk17_home_c60.replaceAll('{next}', gen_app_peruk17_home_c11), entity: 'app_peruk17_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk17_home_c64.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk17_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk17HomeScreenState extends State<GenAppPeruk17HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk17_home_c65, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk17_home_c66, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk17_home_c67 : n <= 1 ? gen_app_peruk17_home_c68 : gen_app_peruk17_home_c69.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk17_home_c70, subtitle: lead, icon: gen_app_peruk17_home_c71, children: [
      DsLoadMeter(count: n, label: gen_app_peruk17_home_c72.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk17_home_c73, gen_app_peruk17_home_c74, gen_app_peruk17_home_c75]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk17_home_c76, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk17_home_c77, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk17HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk17_home_c78 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk17_home_c79 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk17_home_c80, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk17_home_c81 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk17_home_c82 + ' ' + gen_app_peruk17_home_c83, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
