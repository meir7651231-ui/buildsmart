...
class GenAppPeruk17HomeScreenToday {
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk17_ent1', rid); final tm = _timeOf(r);
      for (final f in _dates) {
        final d = _parse(r[f.label] ?? ''); if (d == null) continue;
        if (appStore.decision('ign:$rid:${f.label}') == 'no') continue;
        if (dayDelta == 0 && d.isBefore(today)) { out.add(_mk('${f.label} · $who', gen_app_peruk17_home_c26.replaceAll('{date}', _iso(d)), rid, f.label, d, f.hard, true, today, tm)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk('${f.label} · $who', off == 0 ? gen_app_peruk17_home_c27 : gen_app_peruk17_home_c28.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '')); break; }
        }
      }
    }
