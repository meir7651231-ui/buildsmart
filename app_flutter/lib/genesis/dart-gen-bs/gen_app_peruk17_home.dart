  ...
  static void _act(String rid, String field, DateTime due, List<String> acts, int i) {
    ...
    if (a == gen_app_peruk17_home_c22) {
      ...
      if (r0 != null && rep.isNotEmpty) {   // ↻ רגע חוזר: «סיים» יוצר את הבא לבד (המועד-הבא בשדה שנסגר), עם החזר
        ...
        appStore.logAction('add', gen_app_peruk17_home_c23.replaceAll('{title}', appStore.displayOf('app_peruk17_ent1', nid) + ' · ' + next[field]!), entity: 'app_peruk17_ent1', rid: nid);
      }
    }
    else if (a == gen_app_peruk17_home_c24) { final r = appStore.byId('app_peruk17_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_peruk17_ent1', rid, {field: _iso(due.add(const Duration(days: 1)))}); appStore.logAction('auto', gen_app_peruk17_home_c25 + ' · ' + field, entity: 'app_peruk17_ent1', rid: rid, field: field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
    else if (a == gen_app_peruk17_home_c26) { final d = _iso(due).replaceAll('-', ''); launchUrl(Uri.parse('https://calendar.google.com/calendar/render?action=TEMPLATE&text=' + Uri.encodeComponent(field + ' · ' + appStore.displayOf('app_peruk17_ent1', rid)) + '&dates=' + d + '/' + d), mode: LaunchMode.externalApplication); }
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { out.add(_mk('${f.label} · $who', gen_app_peruk17_home_c27.replaceAll('{date}', _iso(d)), rid, f.label, d, f.hard, true, today, tm, rep)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk('${f.label} · $who', off == 0 ? gen_app_peruk17_home_c28 : gen_app_peruk17_home_c29.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep)); break; }
        }
      }
    }
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true}) {   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk17_home_c30.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk17_home_c31, noLabel: gen_app_peruk17_home_c32, alwaysLabel: gen_app_peruk17_home_c33,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk17_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk17_home_c34.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk17_home_c35, noLabel: gen_app_peruk17_home_c36, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
    if (chain) for (final r in appStore.records('app_peruk17_ent1')) {   // P14 · הצעד-הבא: רשומה שהגיעה לשלב-האחרון (סגורה — לא ב-open) ובלי הכרעה
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk17_ent1', rid);
      if (appStore.stageOf('app_peruk17_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk17_home_c37.replaceAll('{next}', gen_app_peruk17_home_c11), source: module + ' · ' + who, okLabel: gen_app_peruk17_home_c38, noLabel: gen_app_peruk17_home_c39, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk17_home_c40.replaceAll('{next}', gen_app_peruk17_home_c11), entity: 'app_peruk17_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk17_home_c44.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk17_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk17HomeScreenState extends State<GenAppPeruk17HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk17_home_c45, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk17_home_c46, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk17_home_c47 : n <= 1 ? gen_app_peruk17_home_c48 : gen_app_peruk17_home_c49.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk17_home_c50, subtitle: lead, icon: gen_app_peruk17_home_c51, children: [
      DsLoadMeter(count: n, label: gen_app_peruk17_home_c52.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk17_home_c53, gen_app_peruk17_home_c54, gen_app_peruk17_home_c55]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk17_home_c56, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk17_home_c57, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk17HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk17_home_c58 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk17_home_c59 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk17_home_c60, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk17_home_c61 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk17_home_c62 + ' ' + gen_app_peruk17_home_c63, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
