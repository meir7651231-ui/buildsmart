    ...
    return [
      for (final t in _CourseData.teachers)
        () {
          ...
          return ForgeTitledSection(fields: ['👩‍🏫 ${t['name']} · ${t['specialty']}', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [Align(alignment: Alignment.centerLeft, child: ForgeIntelPill(fields: ['${cs.length} חוגים · ${_CourseData.weeklyOf(cs)} מפגשים/שבוע'])), ...cs.isEmpty ? [const ForgeSearchEmptyState(fields: ['אין חוגים למורה זה', ''])] : [for (final c in cs) _row(c)]]));
        }(),
