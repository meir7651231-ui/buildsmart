...
class GenAppPeruk03RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk03_root_c65, subtitle: gen_app_peruk03_root_c66, icon: gen_app_peruk03_root_c67, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk03_ent1', id), subtitle: const [gen_app_peruk03_root_c60, gen_app_peruk03_root_c61, gen_app_peruk03_root_c62, gen_app_peruk03_root_c63, gen_app_peruk03_root_c64][appStore.stageOf('app_peruk03_ent1', id).clamp(0, 4)], icon: gen_app_peruk03_root_c68, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__note'] ?? '').trim().isNotEmpty ? DsFold(title: gen_app_peruk03_root_c58, details: [Text(r0['__note'] ?? '', style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, height: 1.5))]) : const SizedBox.shrink())),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk03_root_c59, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
