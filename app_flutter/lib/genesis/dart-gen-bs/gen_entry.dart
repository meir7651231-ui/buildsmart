...
// 🧬 שם: המחולל
// 🧬 בקשה: המחולל: · הירו 🧬 המחולל של המחצב | עכשיו עם חיבורים חיים בין אטומים · כותרת גשר הלוגיקה פועם · חישוב החודש העברי כרגע לפי מנוע מאור · חישוב קוד הזמנה דטרמיניסטי "genesis" · כותרת הבחירה כאן מחליפה את האטום שמתחתיה · בחירה מה בונים היום: הגדרות / ניהול / דוחות · מתג 🔔 התראות למסך ההגדרות · שדה שם מסך הניהול · נתון 📊 3 דוחות מוכנים להפקה · באנר הבורר למעלה מחובר לאטומים שמתחתיו — בחירה מחליפה אטום · כפתור ✨ צרו מסך חדש
// 🧬 אטומים שנבחרו: HeroCard · CaSubTitle · RStat · RStat · CaSubTitle · UnitSegmentToggle · CoinBanner · ActionRow · SwitchRow · InlineTextRow · RStat
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 12), child: Row(children: [RStat(value: hebMonthHe(DateTime.now()), label: gen_entry_stat_label), RStat(value: genJoinCode('genesis'), label: gen_entry_stat_label2)])),
          ...
          IndexedStack(index: _n3, children: [SwitchRow(label: gen_entry_switch_label, value: _v1, onChanged: (v) => setState(() => _v1 = v)), InlineTextRow(label: gen_entry_textfield_label, hint: gen_entry_textfield_hint, value: _t2, onChanged: (v) => setState(() => _t2 = v)), Row(children: [RStat(value: gen_entry_stat_value, label: gen_entry_stat_label3)])]),
