...
class GenAppPeruk23RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk23_root_c25, subtitle: gen_app_peruk23_root_c26, icon: gen_app_peruk23_root_c27, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk23_ent1', id), subtitle: const [gen_app_peruk23_root_c20, gen_app_peruk23_root_c21, gen_app_peruk23_root_c22, gen_app_peruk23_root_c23, gen_app_peruk23_root_c24][appStore.stageOf('app_peruk23_ent1', id).clamp(0, 4)], icon: gen_app_peruk23_root_c28, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__note'] ?? '').trim().isNotEmpty ? DsFold(title: gen_app_peruk23_root_c18, details: [Text(r0['__note'] ?? '', style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, height: 1.5))]) : const SizedBox.shrink())),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk23_root_c19, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
