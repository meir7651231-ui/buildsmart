...
class CallEntryCoreScreen extends StatefulWidget {
  ...
  @override
  State<CallEntryCoreScreen> createState() => CallEntryCoreScreenState();
}

class CallEntryCoreScreenState extends State<CallEntryCoreScreen> {
  String _state = CallEntryCore.states.first;
  @override
  Widget build(BuildContext context) {
    final next = CallEntryCore.next(_state);
    return DsScaffold(
      title: '🧠 ${CallEntryCore.term} · גרעין',
      subtitle: '${CallEntryCore.states.length} מצבים · ${CallEntryCore.relations.length} יחסים · ${CallEntryCore.rules.length} חוקים · ${CallEntryCore.channels.length} ערוצים',
      icon: '🧠',
      children: [
        DsSection(title: 'מחזור-חיים · outcome', children: [
          Wrap(spacing: 6, runSpacing: 6, children: [for (final s in CallEntryCore.states) StatusChip(label: s, tone: s == _state ? 1 : 0)]),
        ...
        DsSection(title: 'חוקים', children: [DsTable(labels: const ['סוג', 'שדה', 'פרטים'], rows: CallEntryCore.rules)]),
        DsSection(title: 'אירועי-מחזור-חיים', children: [DsTable(labels: const ['שדה', 'אירוע'], rows: CallEntryCore.events)]),
