...
class GenAppPeruk28RootScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    if (r0 == null) return DsScaffold(title: gen_app_peruk28_root_c50, subtitle: gen_app_peruk28_root_c51, icon: gen_app_peruk28_root_c52, children: const []);
    return DsScaffold(title: appStore.displayOf('app_peruk28_ent1', id), subtitle: const [gen_app_peruk28_root_c45, gen_app_peruk28_root_c46, gen_app_peruk28_root_c47, gen_app_peruk28_root_c48, gen_app_peruk28_root_c49][appStore.stageOf('app_peruk28_ent1', id).clamp(0, 4)], icon: gen_app_peruk28_root_c53, children: [
      ...
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__note'] ?? '').trim().isNotEmpty ? DsFold(title: gen_app_peruk28_root_c43, details: [Text(r0['__note'] ?? '', style: TextStyle(color: DsLook.of(context).ink, fontSize: 15, height: 1.5))]) : const SizedBox.shrink())),
      Padding(padding: const EdgeInsets.only(bottom: 12), child: ((r0['__doc'] ?? '').startsWith('data:image') ? DsFold(title: gen_app_peruk28_root_c44, details: [ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.memory(base64Decode((r0['__doc'] ?? '').split(',').last), fit: BoxFit.fitWidth))]) : const SizedBox.shrink())),
    ]);
  });
}
