...
import '../dart-data-bs/auto/gen_app_sechirut_rec1_content.dart';

class GenAppSechirutRec1Screen extends StatefulWidget {
  const GenAppSechirutRec1Screen({this.initialId, super.key});

  final String? initialId;   // רשומה-פתיחה מניווט (הקלקה על שורה); null ⇒ הראשונה.

  @override
  State<GenAppSechirutRec1Screen> createState() => _GenAppSechirutRec1ScreenState();
}

class _GenAppSechirutRec1ScreenState extends State<GenAppSechirutRec1Screen> {
                ...
                child: DropdownButton<int>(
                  ...
                  items: [
                    for (var j = 0; j < recs.length; j++)
                      DropdownMenuItem(value: j, child: Text((recs[j][gen_app_sechirut_rec1_c0] ?? '').isEmpty ? '#' + (j + 1).toString() : (recs[j][gen_app_sechirut_rec1_c0] ?? ''))),
                  ],
                  onChanged: (v) { if (v != null) setState(() => _sel = v); },
                ),
              ),
              const SizedBox(height: 8),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), child: Callout(value: r[gen_app_sechirut_rec1_c1] ?? '', label: gen_app_sechirut_rec1_c2)),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), child: Callout(value: r[gen_app_sechirut_rec1_c3] ?? '', label: gen_app_sechirut_rec1_c4)),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), child: Callout(value: r[gen_app_sechirut_rec1_c5] ?? '', label: gen_app_sechirut_rec1_c6)),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), child: Callout(value: r[gen_app_sechirut_rec1_c7] ?? '', label: gen_app_sechirut_rec1_c8)),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), child: Callout(value: r[gen_app_sechirut_rec1_c9] ?? '', label: gen_app_sechirut_rec1_c10)),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), child: Callout(value: r[gen_app_sechirut_rec1_c11] ?? '', label: gen_app_sechirut_rec1_c12)),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), child: Callout(value: r[gen_app_sechirut_rec1_c13] ?? '', label: gen_app_sechirut_rec1_c14)),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), child: Callout(value: r[gen_app_sechirut_rec1_c15] ?? '', label: gen_app_sechirut_rec1_c16)),
