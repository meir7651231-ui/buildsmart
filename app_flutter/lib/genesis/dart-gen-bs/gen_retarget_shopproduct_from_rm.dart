...
//   תפר-עובדות (G9b): ShopProductFacts · count=rooms.length (static-const) · מדדים 5 · hero=unavailableN
...
class _ShopProductScreenState extends State<ShopProductScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(
      ...
      children: [
        ...
        GradientCard(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              BareStat(value: '${_ShopProductData.busyNowN}', label: '🔴 תפוסים-עכשיו', inkColor: _ink, mutedColor: _muted),
              BareStat(value: '${_ShopProductData.freeNowN}', label: '🟢 פנויים-עכשיו', inkColor: _ok, mutedColor: _muted),
              BareStat(value: '${_ShopProductData.utilAvgPct}%', label: '📊 ניצולת-שבוע', inkColor: _acc, mutedColor: _muted),
            ...
            Row(children: [
              ...
              BareStat(value: '${_ShopProductData.unavailableN}', label: '⛔ לא-זמינים', inkColor: _ShopProductData.unavailableN > 0 ? _danger : _ok, mutedColor: _muted),
              BareStat(value: '${_ShopProductData.pendingApprovals.length}', label: '⏳ ממתינות-אישור', inkColor: _warning, mutedColor: _muted),
              BareStat(value: '${_ShopProductData.brokenEqN}', label: '🧰 ציוד-חסר/תקול', inkColor: _ShopProductData.brokenEqN > 0 ? _warning : _ok, mutedColor: _muted),
        ...
        Align(
          alignment: Alignment.centerRight,
          child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: SegmentedSwitch(items: [for (final r in _ShopProductData.roleDefs) r['label'] as String], selected: _role, onSelect: (i) => setState(() => _role = i))),
        ),
        ...
        Row(children: [
          ...
          const SizedBox(width: 6),
          if (_can('rooms.book')) Padding(padding: const EdgeInsets.only(bottom: 12), child: SoftButton(label: '📌 הזמן-מוצר', tone: 1, onTap: () => _pickRoom(visible, (r) => _openBook(context, r, (f) { f(); setState(() {}); })))),
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [
          for (final b in _ShopProductData.buildings) _fchip('building', b, '🏢 $b'),
          for (final f in _ShopProductData.floors) _fchip('floor', f, '🪜 $f'),
          for (final t in _ShopProductData.types) _fchip('type', t, '🏷 $t'),
          for (final n in _ShopProductData.capSteps) FilterChipPill(label: '👥 ≥$n', selected: _locks['cap$n'] == '1', onTap: () => _setCap(_locks['cap$n'] == '1' ? null : n), activeFillColor: _acc, surfaceColor: const Color(0xFF14162E), activeTextColor: const Color(0xFF0B0B15), inkColor: _ink, outlineColor: const Color(0xFF2A2D4A), pillRadius: 999),
          for (final k in _ShopProductData.eqKeys) _bchip('eq:$k', '🧰 יש $k'),
          ...
          _bchip('under', '🪑 ניצולת<${_ShopProductData.utilFloor}%'),
          _bchip('access', '♿ נגיש'),
        ...
        if (_view == 0) ...[
          ...
          Align(
            ...
            child: SingleChildScrollView(
              ...
              child: SegmentedSwitch(items: [for (var i = 0; i < letters.length; i++) '${letters[i]} ${_ShopProductData.weekIsos[i].substring(8)}'], selected: _dayIdx, onSelect: (i) => setState(() => _dayIdx = i)),
            ),
          ),
        ],
        const SizedBox(height: 10),
        ...
        if (_loading)
          ...
          const EmptyState(glyph: '🏫', message: 'אין חדרים — הוסף מוצר ראשון')
          ...
          DsSection(title: '🗓 שבוע-הבניין · ${_ShopProductData.weekIsos.first} → ${_ShopProductData.weekIsos.last}', children: [_weekGrid(visible, names)])
        else ...[
          ...
          builder: (ctx, scroll) => Padding(
            ...
            child: GlassCard(
              child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                MediaRow(glyph: _ShopProductData.activeOf(r) ? '🏫' : '⛔', title: '${r['name']}', subtitle: '${r['location']} · ${_ShopProductData.roomInfo(r)}'),
                _gap(8),
                _wrap([
                  StatusChip(label: _ShopProductData.statusOf(r), tone: switch (_ShopProductData.statusOf(r)) { 'זמין' => 1, 'תפוס' => 0, 'חסום' => 3, _ => 2 }),
                  if (r['access'] == true) const StatusChip(label: '♿ נגיש', tone: 1),
                  if ('${r['notes']}'.isNotEmpty) StatusChip(label: '📝 ${r['notes']}', tone: 0),
                  if (_ShopProductData.underused(r)) StatusChip(label: '🪑 לא-מנוצל (<${_ShopProductData.utilFloor}%)', tone: 3),
                ]),
                _gap(12),
                Row(children: [
                  BareStat(value: '${r['cap']}', label: 'קיבולת', inkColor: _ink, mutedColor: _muted),
                  BareStat(value: '${_ShopProductData.utilPct(r)}%', label: 'ניצולת-שבוע', inkColor: _acc, mutedColor: _muted),
                ...
                _gap(8),
                _wrap([for (final f in _ShopProductData.presentFields(r)) StatusChip(label: '${f['glyph']} ${f['label']}: ${r[f['key']]}', tone: 1)]),
    ...
    switch (tab) {
      case 0: // היום · ציר-שעות (buildSlots ⊕ TimelineItem) — תפוס/פנוי/חסום/ניקיון
        ...
        return [
          ...
          for (final x in sl)
            TimelineItem(title: '${_kindGlyph('${x['kind']}')} ${x['label']}', time: '${x['time']}', body: x['kind'] == 'course' ? '${_ShopProductData.teacherName((x['course'] as Map)['teacherId'])}' : x['kind'] == 'event' ? '${_ShopProductData.teacherName((x['event'] as Map)['requestedBy'])} · ${(x['event'] as Map)['status'] == 'proposed' ? 'ממתין-אישור' : 'מאושר'}' : null),
        ];
      case 1: // שבוע · מיני-גריד (dayCell × 6)
        ...
        return [
          ...
          _wrap([for (final u in _ShopProductData.topUsers(r)) StatusChip(label: '${u[0]}: ${u[1]}', tone: 0)]),
        ];
      ...
      case 4: // תקלות · פתוחות (TimelineItem) + סגור-תקלה
        ...
        return [
          ...
          for (final f in fs)
            Row(children: [
              Expanded(child: TimelineItem(title: '🔧 ${f['name']} · ${f['severity']}', time: '${f['date']}', body: '${f['status']} · דווח: ${_ShopProductData.teacherName(f['createdBy'])}${'${f['detail']}'.isNotEmpty ? ' · ציוד: ${f['detail']}' : ''}')),
              if (_can('rooms.faultClose')) SoftButton(label: '✔ סגור', tone: 1, onTap: () => act(() => _ShopProductData.closeFault(_who, f))),
            ]),
        ];
      case 5: // אחזקה · זמינות (שיפוץ/סגור) · בדיקה-תקופתית (lastCheck = מקום-שמור) · חסימות-תאריך
        return [
          _h('אחזקה'),
          _wrap([
            StatusChip(label: _ShopProductData.activeOf(r) ? 'זמין לתפיסה' : 'לא-זמין / שיפוץ', tone: _ShopProductData.activeOf(r) ? 1 : 2),
          ...
          if (_ShopProductData.blockedDates.isNotEmpty) ...[
            ...
            _wrap([for (final d in _ShopProductData.blockedDates) SoftButton(label: '⛔ $d · בטל', tone: 2, onTap: () => act(() => _ShopProductData.unblockDate(_who, d)))]),
          ],
        ];
      case 6: // היסטוריה · תקלות-שנסגרו ⊕ אירועים-שבוצעו/נדחו (TimelineItem)
        final hist = <Widget>[
          for (final f in _ShopProductData.faultsOf(r, openOnly: false)) if (f['status'] == 'done') TimelineItem(title: '✔ תקלה נסגרה · ${f['name']}', time: '${f['date']}', body: '${f['severity']}'),
          for (final e in _ShopProductData.liveEvents) if (e['roomId'] == r['id'] && e['done'] == true) TimelineItem(title: '${e['status'] == 'rejected' ? '✖ נדחה' : e['status'] == 'cancelled' ? '↩ בוטל' : '✔ בוצע'} · ${e['title']}', time: '${e['date']} ${e['time']}', body: '${_ShopProductData.teacherName(e['requestedBy'])}'),
        ];
      ...
      default: // אודיט · פנקס-הפעולות של החדר (מי · מה · מתי)
        ...
        return au.isEmpty ? [const EmptyState(glyph: '🧾', message: 'אין רשומות-אודיט למוצר')] : [_h('אודיט · ${au.length}'), for (final a in au) TimelineItem(title: '${a['what']}', time: '${a['when']}', body: '${a['who']} · ${a['target']}')];
    }
  }
  ...
  Widget _occRow(BuildContext ctx, Map<String, dynamic> r, Map<String, dynamic> o, void Function(void Function()) act) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      TimelineItem(title: '${isEv ? '📌' : '🎓'} ${o['name']}', time: '${dayNames()[o['dayIdx'] as int]} ${o['iso']} ${_ShopProductData._m2hm(o['start'])}', body: '${o['who']} · ${st == 'proposed' ? 'ממתין-אישור' : st == 'pending' ? 'מאושר' : st}'),
      _wrap([
        if (isEv && st == 'proposed' && _can('rooms.approve')) ...[
          SoftButton(label: '✔ אשר', tone: 1, onTap: () => act(() => _ShopProductData.setEventStatus(_who, _ShopProductData.eventById(o['id'])!, 'pending'))),
          SoftButton(label: '✖ דחה', tone: 2, onTap: () => act(() => _ShopProductData.setEventStatus(_who, _ShopProductData.eventById(o['id'])!, 'rejected'))),
        ],
        if (isEv && _can('rooms.cancel')) SoftButton(label: '↩ בטל-תפיסה', tone: 2, onTap: () => act(() => _ShopProductData.setEventStatus(_who, _ShopProductData.eventById(o['id'])!, 'cancelled'))),
        if (_can('rooms.move')) SoftButton(label: '➡ העבר למוצר-אחר', tone: 0, onTap: () => _openMove(ctx, r, o, act)),
      ]),
      _gap(8),
    ]);
  }
    ...
    showModalBottomSheet<void>(
      ...
      builder: (c2) => Padding(
        ...
        child: GlassCard(child: ListView(shrinkWrap: true, padding: const EdgeInsets.all(8), children: [
          MediaRow(glyph: '➡', title: 'העבר: ${o['name']}', subtitle: '${o['iso']} ${_ShopProductData._m2hm(o['start'])} · נדרש ≥$need${needsEq.isNotEmpty ? ' · ציוד: ${needsEq.join(', ')}' : ''}'),
          _gap(8),
          if (alts.isEmpty)
            const EmptyState(glyph: '🚫', message: 'אין מוצר חלופי פנוי שמקיים קיבולת+ציוד במשבצת')
          else
            for (final a in alts)
              Row(children: [
                ...
                SoftButton(label: 'בחר', tone: 1, onTap: () { act(() => isEv ? _ShopProductData.moveEvent(_who, ev!, a) : _ShopProductData.moveCourse(_who, c!, a)); Navigator.of(c2).pop(); }),
              ]),
        ])),
      ),
    );
    ...
    showModalBottomSheet<void>(
      ...
      builder: (c2) => StatefulBuilder(builder: (c2, setB) {
        ...
        return Padding(
          padding: const EdgeInsets.all(12),
          child: GlassCard(child: ListView(shrinkWrap: true, padding: const EdgeInsets.all(8), children: [
            MediaRow(glyph: weekly ? '🔁' : '📌', title: weekly ? 'הזמנה-חוזרת (שבועית) · ${r['name']}' : 'הזמן-מוצר · ${r['name']}', subtitle: 'בחר יום ומשבצת פנויה · ${_ShopProductData.roomInfo(r)}'),
            _gap(8),
            DsField(label: 'כותרת', hint: 'למה המוצר נדרש', value: title, onChanged: (v) => title = v),
            ...
            if (blocked != null)
              ...
              _wrap([for (final f in free) SoftButton(label: '🟢 ${f['time']}', tone: 1, onTap: () { act(() => weekly ? _ShopProductData.bookWeekly(_who, r, day, '${f['time']}', title) : _ShopProductData.book(_who, r, iso, '${f['time']}', title, approved: _ShopProductData.autoApprove(_role))); Navigator.of(c2).pop(); })]),
            _gap(8),
          ])),
        );
      }),
  ...
  void _openFault(BuildContext ctx, Map<String, dynamic> r, void Function(void Function()) act) {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (c2) => StatefulBuilder(builder: (c2, setB) => Padding(
        padding: const EdgeInsets.all(12),
        child: GlassCard(child: ListView(shrinkWrap: true, padding: const EdgeInsets.all(8), children: [
          MediaRow(glyph: '🔧', title: 'דווח-תקלה · ${r['name']}', subtitle: 'תקלה-חמורה ⇒ המוצר לא-זמין + העברת-שיעורים'),
          _gap(8),
          ...
          _wrap([for (final k in _ShopProductData.eqOf(r).keys) SoftButton(label: detail == k ? '✔ $k' : k, tone: detail == k ? 1 : 0, onTap: () => setB(() => detail = detail == k ? '' : k))]),
          _gap(10),
          SoftButton(label: '📨 שלח דיווח', tone: 2, onTap: () { act(() => _ShopProductData.reportFault(_who, r, name, sevs[sev], detail: detail)); Navigator.of(c2).pop(); }),
          _gap(8),
        ])),
      )),
    );
  ...
  List<Widget> _actions(BuildContext ctx, Map<String, dynamic> r, void Function(void Function()) act) {
    ...
    final acts = <Widget>[
      if (active && _can('rooms.book')) SoftButton(label: _ShopProductData.autoApprove(_role) ? '📌 הזמן-מוצר' : '📌 הזמן-מוצר (לאישור)', tone: 1, onTap: () => _openBook(ctx, r, act)),
      ...
      if (_can('rooms.availability')) SoftButton(label: active ? '⛔ סמן לא-זמין/שיפוץ' : '✅ החזר לזמינות', tone: active ? 2 : 1, onTap: () => act(() => _ShopProductData.setActive(_who, r, !active))),
      if (_can('rooms.eq')) SoftButton(label: '➕ הוסף-ציוד: מקרן', tone: 0, onTap: () => act(() => _ShopProductData.setEq(_who, r, 'מקרן', true))),
      if (_can('rooms.eq')) SoftButton(label: '📉 דווח ציוד-חסר: מקרן', tone: 2, onTap: () => act(() => _ShopProductData.setEq(_who, r, 'מקרן', false))),
      if (_can('rooms.block')) SoftButton(label: '📅 חסום-תאריך: $_iso', tone: 2, onTap: () => act(() => _ShopProductData.blockDate(_who, _iso))),
      if (_can('rooms.notify')) SoftButton(label: '✉ הודעה למשתמשי-המוצר (${_ShopProductData.usersOf(r).length})', tone: 0, onTap: () => act(() => _ShopProductData.notifyUsers(_who, r, 'שינוי במוצר ${r['name']} — בדקו את היומן'))),
      ...
      if (!active || _ShopProductData.faulty(r)) if (_can('rooms.move') && _ShopProductData.affectedByFault(r).isNotEmpty) SoftButton(label: '🚚 העבר-אוטו ${_ShopProductData.affectedByFault(r).length} תפיסות + הודעה', tone: 2, onTap: () => act(() => _ShopProductData.autoRelocate(_who, r))),
    ];
  ...
  List<Widget> _automationCenter() {
    ...
    return [
      DsSection(title: '🚨 דורש-פעולה · ${_ShopProductData.actionItems}', tone: _ShopProductData.actionItems > 0 ? 2 : 1, children: [
        if (_ShopProductData.actionItems == 0) const EmptyState(glyph: '✅', message: 'אין כפל-תפיסה · אין הזמנות ממתינות · כל שיעור במוצר עם הציוד הנדרש'),
        ...
        for (final c in conf)
          Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(children: [
            ...
            if (_can('rooms.move')) ...[const SizedBox(width: 6), SoftButton(label: '➡ פתור', tone: 2, onTap: () => _openMove(context, c['room'] as Map<String, dynamic>, {...(c['b'] as Map<String, dynamic>), 'iso': c['iso'], 'dayIdx': _ShopProductData.dow('${c['iso']}')}, act))],
          ])),
        // 9 · הזמנות-ממתינות (מחוץ-למדיניות) ⇒ אשר/דחה (rooms.approve)
        for (final e in pend)
          Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(children: [
            Expanded(child: AlertBanner(glyph: '⏳', tone: 3, message: 'ממתין-אישור: ${e['title']} · ${_ShopProductData._room('${e['roomId']}')['name']} · ${e['date']} ${e['time']} · ${_ShopProductData.teacherName(e['requestedBy'])} · ${e['attendees']} משתתפים')),
            if (_can('rooms.approve')) ...[
              const SizedBox(width: 6), SoftButton(label: '✔', tone: 1, onTap: () => act(() => _ShopProductData.setEventStatus(_who, e, 'pending'))),
              const SizedBox(width: 4), SoftButton(label: '✖', tone: 2, onTap: () => act(() => _ShopProductData.setEventStatus(_who, e, 'rejected'))),
            ],
          ])),
        // 7 · ציוד-נדרש-לשיעור חסר/תקול בחדר (התרעה) ⇒ העבר לחדר עם הציוד (altRooms needsEq)
        for (final c in miss)
          Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(children: [
            Expanded(child: AlertBanner(glyph: '🧰', tone: 3, message: 'ציוד חסר לשיעור: ${c['name']} ב-${_ShopProductData._room(_ShopProductData.roomOfCourse(c))['name']} — ${_ShopProductData.missingEqFor(c).join(', ')}')),
        ...
        for (final o in orphan)
          Padding(padding: const EdgeInsets.only(bottom: 6), child: AlertBanner(glyph: '🏚', tone: 2, message: 'שיעור בלי מוצר-פעיל: ${(o['course'] as Map)['name']} (${o['roomName']})')),
        // 4 · חדר-תקול (תקלה-חמורה פתוחה) ⇒ העברת-שיעורים-אוטו + הודעה
        for (final r in faultyRooms)
          Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(children: [
            Expanded(child: AlertBanner(glyph: '🔧', tone: 2, message: 'מוצר תקול (תקלה חמורה): ${r['name']} — ${_ShopProductData.affectedByFault(r).length} תפיסות השבוע מושפעות')),
            if (_can('rooms.move') && _ShopProductData.affectedByFault(r).isNotEmpty) ...[const SizedBox(width: 6), SoftButton(label: '🚚 העבר-אוטו', tone: 2, onTap: () => act(() => _ShopProductData.autoRelocate(_who, r)))],
          ])),
      ]),
      // 6 · חדר-לא-מנוצל · 3 · סנכרון-לוח (חגים קרובים ⇒ חסימה-אוטו) · 5 · בדיקה-תקופתית (מקום-שמור)
      if (under.isNotEmpty) ...[AlertBanner(glyph: '🪑', tone: 3, message: '${under.length} חדרים מתחת לסף-ניצולת ${_ShopProductData.utilFloor}%: ${under.map((r) => '${r['name']} ${_ShopProductData.utilPct(r)}%').join(' · ')}'), _gap(8)],
      ...
      if (dueCheck.isNotEmpty) ...[AlertBanner(glyph: '🗓', tone: 3, message: 'בדיקה-תקופתית באיחור (>${_ShopProductData.checkEveryDays} יום): ${dueCheck.map((r) => r['name']).join(' · ')}'), _gap(8)],
      ...
      if (_ShopProductData.roleName(_role) == 'admin') ...[
        DsSection(title: '📊 ניצולת-שבועית להנהלה · ממוצע ${_ShopProductData.utilAvgPct}%', children: [
          NeonBars(labels: [for (final r in _ShopProductData.utilRanked) '${r['name']}'], values: [for (final r in _ShopProductData.utilRanked) _ShopProductData.utilPct(r).toDouble()], tone: 0),
          _gap(6),
          Row(children: [
            BareStat(value: '${_ShopProductData.utilRanked.isEmpty ? '—' : _ShopProductData.utilRanked.first['name']}', label: 'הכי-מנוצל', inkColor: _acc, mutedColor: _muted),
            BareStat(value: '${_ShopProductData.utilRanked.isEmpty ? '—' : _ShopProductData.utilRanked.last['name']}', label: 'הכי-פחות', inkColor: _warning, mutedColor: _muted),
            BareStat(value: '${_ShopProductData.outbox.length}', label: 'הודעות שנשלחו', inkColor: _ink, mutedColor: _muted),
          ]),
        ]),
      ],
    ];
    ...
    showModalBottomSheet<void>(
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx, scroll) => Padding(
          ...
          child: GlassCard(
            child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
              MediaRow(glyph: ical ? '📆' : '⬇', title: ical ? 'ייצוא iCal (VCALENDAR)' : 'ייצוא CSV', subtitle: ical ? '${rs.length} חדרים · $occN תפיסות השבוע · RFC5545 (buildIcs)' : '${rs.length} חדרים · ${_ShopProductData.csvRows(rs).first.length} עמודות · BOM + חסימת-הזרקה'),
              _gap(10),
              if (!ok)
  ...
  void _pickRoom(List<Map<String, dynamic>> rs, void Function(Map<String, dynamic>) then) {
    showModalBottomSheet<void>(
      ...
      builder: (c2) => Padding(
        padding: const EdgeInsets.all(12),
        child: GlassCard(child: ListView(shrinkWrap: true, padding: const EdgeInsets.all(8), children: [
          const MediaRow(glyph: '🏫', title: 'בחר מוצר', subtitle: 'מהרשימה-הנראית (אחרי חיפוש וסינון)'),
          _gap(6),
          if (rs.isEmpty) const EmptyState(glyph: '🔍', message: 'אין חדרים תואמים'),
          for (final r in rs)
            Row(children: [
              Expanded(child: MediaRow(glyph: _ShopProductData.activeOf(r) ? '🏫' : '⛔', title: '${r['name']}', subtitle: '${r['location']} · ${_ShopProductData.statusOf(r)}')),
