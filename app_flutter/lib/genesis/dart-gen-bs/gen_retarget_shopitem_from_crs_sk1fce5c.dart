...
class _ShopItemScreenState extends State<ShopItemScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(title: 'חוגים ומערכת', subtitle: '${live.length} חוגים חיים · ${_ShopItemData.teachers.length} מורים · ${_ShopItemData.rooms.where((r) => r['active'] == true).length} חדרים', icon: '📚', header: false, children: [ForgeCenteredPageHeader(fields: ['', 'חוגים ומערכת', '${live.length} חוגים חיים · ${_ShopItemData.teachers.length} מורים · ${_ShopItemData.rooms.where((r) => r['active'] == true).length} חדרים']), ...[
        ...
        Align(
          ...
          child: FittedBox(fit: BoxFit.scaleDown, child: ForgeSegmentedPillToggleSelection(bare: true, items: [for (final s in [for (final r in _ShopItemData.roleDefs) r['label'] as String]) [s]], selected: {_role}, onSelect: (i) => setState(() { _role = i; _locks.clear(); }))),
        ),
        _gap(8),
        ...
        if (_loading)
          ...
          ForgeSearchEmptyState(fields: [_ShopItemData.myTeacherId(_role) != null ? 'אין חוגים משובצים למורה זה' : _ShopItemData.myFamilyId(_role) != null ? 'אין חוגים למשפחה — הירשמו מהקטלוג' : 'אין חוגים — צור פריט-חדש או שכפל סמסטר', ''])
  ...
  List<Widget> _tabWaitlist(Map<String, dynamic> c, void Function(void Function()) both) {
    ...
    return [
      ...
      Row(children: [
        ...
        Flexible(child: ForgeSegmentedPillToggleSelection(bare: true, items: [for (final s in const ['פועל', 'כבוי']) [s]], selected: {_ShopItemData.autoPromote ? 0 : 1}, onSelect: (i) => both(() => _ShopItemData.autoPromote = i == 0))),
      ]),
    ];
  }
  ...
  List<Widget> _tabSchedule(Map<String, dynamic> c, void Function(void Function()) both) {
    ...
    return [
      ...
      if (!_ShopItemData.hasSessions(c)) ForgeSearchEmptyState(fields: ['אין מפגשים קבועים — הגדר יום+שעה (מקום-שמור: עורך-מפגשים)', '']),
  ...
  List<Widget> _tabHistory(Map<String, dynamic> c, bool all) {
    ...
    return [
      ...
      if (rows.isEmpty) ForgeSearchEmptyState(fields: [all ? 'אין פעולות עדיין' : 'אין היסטוריה לפריט', '']),
