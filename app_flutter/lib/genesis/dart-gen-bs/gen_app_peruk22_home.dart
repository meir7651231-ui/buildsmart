...
class _GenAppPeruk22HomeScreenState extends State<GenAppPeruk22HomeScreen> {
  ...
  List<Map<String, String>> get _open => appStore.records('app_peruk22_ent1').where((r) => appStore.stageOf('app_peruk22_ent1', r[AppStore.idKey] ?? '') < 4).toList();
  ...
  List<_Item> _items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in _open) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { out.add(_Item('${f.label} · $who', gen_app_peruk22_home_c13.replaceAll('{date}', _iso(d)), rid, f.label, d, f.hard, true)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_Item('${f.label} · $who', off == 0 ? gen_app_peruk22_home_c14 : gen_app_peruk22_home_c15.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false)); break; }
        }
      }
    }
  ...
  List<Widget> _proposals(BuildContext context, DateTime today) {
    ...
    for (final r in _open) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk22_home_c16.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: who, okLabel: gen_app_peruk22_home_c17, noLabel: gen_app_peruk22_home_c18, alwaysLabel: gen_app_peruk22_home_c19,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk22_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk22_home_c20.replaceAll('{n}', n.toString()), source: who, okLabel: gen_app_peruk22_home_c21, noLabel: gen_app_peruk22_home_c22, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); _send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
      if (appStore.stageOf('app_peruk22_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk22_home_c23.replaceAll('{next}', gen_app_peruk22_home_c11), source: who, okLabel: gen_app_peruk22_home_c24, noLabel: gen_app_peruk22_home_c25, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk22_home_c26.replaceAll('{next}', gen_app_peruk22_home_c11), entity: 'app_peruk22_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }
  ...
  void _autopilot() {
    ...
    for (final r in _open) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk22_home_c27.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk22_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk22_home_c28, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk22_home_c29, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  void _act(_Item it, int i, DateTime today) {
    final labels = it.overdue ? [gen_app_peruk22_home_c30, gen_app_peruk22_home_c31, gen_app_peruk22_home_c32] : (it.due == today ? [gen_app_peruk22_home_c33] : [gen_app_peruk22_home_c34, gen_app_peruk22_home_c35]);   // P4 · ביום-ההכרעה אין דחייה
    final a = labels[i.clamp(0, labels.length - 1)];
    if (a == gen_app_peruk22_home_c36) { appStore.advance('app_peruk22_ent1', it.rid, 5); }
    else if (a == gen_app_peruk22_home_c37) { final r = appStore.byId('app_peruk22_ent1', it.rid); if (r != null) { final prev = r[it.field] ?? ''; appStore.update('app_peruk22_ent1', it.rid, {it.field: _iso(it.due.add(const Duration(days: 1)))}); appStore.logAction('auto', gen_app_peruk22_home_c38 + ' · ' + it.title, entity: 'app_peruk22_ent1', rid: it.rid, field: it.field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk22_home_c39 : n <= 1 ? gen_app_peruk22_home_c40 : gen_app_peruk22_home_c41.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk22_home_c42, subtitle: lead, icon: gen_app_peruk22_home_c43, children: [
      DsLoadMeter(count: n, label: gen_app_peruk22_home_c44.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk22_home_c45, gen_app_peruk22_home_c46, gen_app_peruk22_home_c47]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk22_home_c48, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: [gen_app_peruk22_home_c49, gen_app_peruk22_home_c50, gen_app_peruk22_home_c51], onAct: (i) => _act(it, i, today))]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk22_home_c52, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.due == today ? [gen_app_peruk22_home_c53] : [gen_app_peruk22_home_c54, gen_app_peruk22_home_c55], onAct: (i) => _act(it, i, today))]),
      for (final r in open) DsSection(title: (r[gen_app_peruk22_home_c0] ?? '') + ' · ' + const [gen_app_peruk22_home_c6, gen_app_peruk22_home_c7, gen_app_peruk22_home_c8, gen_app_peruk22_home_c9, gen_app_peruk22_home_c10][appStore.stageOf('app_peruk22_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], children: [
        
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => _send(context, r, r[AppStore.idKey] ?? ''), child: ForgeToneButton(items: [[gen_app_peruk22_home_c2]]))), const SizedBox(width: 8), Flexible(child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk22RootScreen(id: r[AppStore.idKey] ?? ''))), child: ForgeToneButton(items: [[gen_app_peruk22_home_c4]])))])),
      ]),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk22_home_c56 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk22_home_c57 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk22_home_c58, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk22_home_c59 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk22_home_c60 + ' ' + gen_app_peruk22_home_c61, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
