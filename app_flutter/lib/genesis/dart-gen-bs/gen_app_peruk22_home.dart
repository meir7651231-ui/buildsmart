...
class GenAppPeruk22HomeScreenToday {
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { final ago = today.difference(d).inDays; out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', gen_app_peruk22_home_c31.replaceAll('{date}', _iso(d)) + ' · ' + (ago == 1 ? gen_app_peruk22_home_c32 : gen_app_peruk22_home_c33.replaceAll('{n}', ago.toString())), rid, f.label, d, f.hard, true, today, tm, rep)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', off == 0 ? '' : gen_app_peruk22_home_c34.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep)); break; }
        }
      }
    }
  ...
  static List<DsTodayItem> undated(DateTime today) {
    ...
    for (final r in open()) {
      ...
      final acts = [gen_app_peruk22_home_c35, gen_app_peruk22_home_c36, gen_app_peruk22_home_c37];
  ...
  static void _setDate(String rid, String field, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    if (a == gen_app_peruk22_home_c38) { appStore.decide('undated:$rid', 'no'); return; }
    final r = appStore.byId('app_peruk22_ent1', rid); if (r == null) return;
    final prev = r[field] ?? '';
    appStore.update('app_peruk22_ent1', rid, {field: _iso(_shift(today.add(Duration(days: a == gen_app_peruk22_home_c39 ? 7 : 1)), false))});   /* «קבע» לא נוחת בשבת */
  ...
  static List<DsTodayItem> stale(DateTime today) {
    ...
    for (final r in open()) {
      ...
      final acts = [gen_app_peruk22_home_c40, gen_app_peruk22_home_c41, gen_app_peruk22_home_c42];
      out.add(DsTodayItem(title: appStore.displayOf('app_peruk22_ent1', rid), sub: gen_app_peruk22_home_c43.replaceAll('{n}', n.toString()), rid: rid, field: '', due: t, hard: false, overdue: false, module: module, actions: acts, act: (i) => _staleAct(rid, today, acts, i)));
    }
  ...
  static void _staleAct(String rid, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    final r = appStore.byId('app_peruk22_ent1', rid); if (r == null) return;
    if (a == gen_app_peruk22_home_c44) { final f = _dates.firstWhere((x) => x.hard, orElse: () => _dates.first); appStore.decide('ign:$rid:${f.label}', ''); appStore.decide('undated:$rid', ''); _setDate(rid, f.label, today, [a], 0); return; }   /* המועד החדש חוזר ל«היום» — ההתעלמות הישנה נמחקת */
    if (a == gen_app_peruk22_home_c45) { final prev = r[AppStore.stageKey] ?? '0'; appStore.update('app_peruk22_ent1', rid, {AppStore.stageKey: '4'}); appStore.logAction('auto', gen_app_peruk22_home_c46.replaceAll('{who}', appStore.displayOf('app_peruk22_ent1', rid)), entity: 'app_peruk22_ent1', rid: rid, field: AppStore.stageKey, prev: prev); return; }
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true}) {   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        out.add(DsApproveCard(question: gen_app_peruk22_home_c47.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk22_home_c48, noLabel: gen_app_peruk22_home_c49, alwaysLabel: gen_app_peruk22_home_c50,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk22_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk22_home_c51.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk22_home_c52, noLabel: gen_app_peruk22_home_c53, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
    if (chain) for (final r in appStore.records('app_peruk22_ent1')) {   // P14 · הצעד-הבא: רשומה שהגיעה לשלב-האחרון (סגורה — לא ב-open) ובלי הכרעה
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_peruk22_ent1', rid);
      if (appStore.stageOf('app_peruk22_ent1', rid) >= 4 && appStore.decision('next:$rid').isEmpty) out.add(DsApproveCard(question: gen_app_peruk22_home_c54.replaceAll('{next}', gen_app_peruk22_home_c11), source: module + ' · ' + who, okLabel: gen_app_peruk22_home_c55, noLabel: gen_app_peruk22_home_c56, onOk: () { appStore.decide('next:$rid', 'ok'); appStore.logAction('next', gen_app_peruk22_home_c57.replaceAll('{next}', gen_app_peruk22_home_c11), entity: 'app_peruk22_ent1', rid: rid, field: 'next:$rid'); }, onNo: () => appStore.decide('next:$rid', 'no')));
    }
    return out;
  }
  ...
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: module + ' · ' + (((r[gen_app_peruk22_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk22_home_c58 : (r[gen_app_peruk22_home_c0] ?? '')), trailing: Text(const [gen_app_peruk22_home_c6, gen_app_peruk22_home_c7, gen_app_peruk22_home_c8, gen_app_peruk22_home_c9, gen_app_peruk22_home_c10][appStore.stageOf('app_peruk22_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), children: [
        
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk22_home_c59, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk22_home_c60, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk22RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk22_home_c61.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk22_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk22HomeScreenState extends State<GenAppPeruk22HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk22_home_c62, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk22_home_c63, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk22_home_c64 : n <= 1 ? gen_app_peruk22_home_c65 : gen_app_peruk22_home_c66.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk22_home_c67, subtitle: lead, icon: gen_app_peruk22_home_c68, children: [
      DsLoadMeter(count: n, label: gen_app_peruk22_home_c69.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk22_home_c70, gen_app_peruk22_home_c71, gen_app_peruk22_home_c72]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk22_home_c73, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk22_home_c74, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk22HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk22_home_c75 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk22_home_c76 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk22_home_c77, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk22_home_c78 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk22_home_c79 + ' ' + gen_app_peruk22_home_c80, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
