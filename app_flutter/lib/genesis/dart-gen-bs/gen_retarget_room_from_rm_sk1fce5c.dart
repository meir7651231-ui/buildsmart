  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(
      ...
      children: [
        ...
        if (_metric != null) ForgeToneBanner(items: [['מסונן למדד: ${RoomFacts.metricDefs.firstWhere((d) => d['key'] == _metric, orElse: () => const {'label': ''})['label']} · ${visible.length} מתוך ${visibleAll.length}']], variants: const <int>[1]),
        ...
        ForgeStripPanelFrame(fields: ['', ''], child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              Expanded(child: ForgeStatPlain(fields: ['🔴 תפוסים-עכשיו', '${_RoomData.busyNowN}'])),
              Expanded(child: ForgeStatPlain(fields: ['🟢 פנויים-עכשיו', '${_RoomData.freeNowN}'])),
              Expanded(child: ForgeStatPlain(fields: ['📊 ניצולת-שבוע', '${_RoomData.utilAvgPct}%'])),
            ...
            Row(children: [
              ...
              Expanded(child: ForgeStatPlain(fields: ['⛔ לא-זמינים', '${_RoomData.unavailableN}'])),
              Expanded(child: ForgeStatPlain(fields: ['⏳ ממתינות-אישור', '${_RoomData.pendingApprovals.length}'])),
              Expanded(child: ForgeStatPlain(fields: ['🧰 ציוד-חסר/תקול', '${_RoomData.brokenEqN}'])),
        ...
        Align(
          alignment: Alignment.centerRight,
          child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: ForgeSegmentedPillToggleSelection(bare: true, items: [for (final s in [for (final r in _RoomData.roleDefs) r['label'] as String]) [s]], selected: {_role}, onSelect: (i) => setState(() => _role = i))),
        ),
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [Builder(builder: (_) { final chips = <(String, bool, VoidCallback)>[for (final b in _RoomData.buildings) (('🏢 $b'), _locks[('building')] == (b), () => setState(() => _locks[('building')] == (b) ? _locks.remove(('building')) : _locks[('building')] = (b))), for (final f in _RoomData.floors) (('🪜 $f'), _locks[('floor')] == (f), () => setState(() => _locks[('floor')] == (f) ? _locks.remove(('floor')) : _locks[('floor')] = (f))), for (final t in _RoomData.types) (('🏷 $t'), _locks[('type')] == (t), () => setState(() => _locks[('type')] == (t) ? _locks.remove(('type')) : _locks[('type')] = (t))), for (final n in _RoomData.capSteps) ('👥 ≥$n', _locks['cap$n'] == '1', () => _setCap(_locks['cap$n'] == '1' ? null : n)), for (final k in _RoomData.eqKeys) ((('🧰 יש $k')), _locks[(('eq:$k'))] == ('1'), () => setState(() => _locks[(('eq:$k'))] == ('1') ? _locks.remove((('eq:$k'))) : _locks[(('eq:$k'))] = ('1'))), ('🟢 פנוי-במשבצת $_slotHour', _freeAtOn, () => _setFreeAt(!_freeAtOn)), ((('🔴 תפוס-עכשיו')), _locks[(('busy'))] == ('1'), () => setState(() => _locks[(('busy'))] == ('1') ? _locks.remove((('busy'))) : _locks[(('busy'))] = ('1'))), ((('🔧 תקלה-פתוחה')), _locks[(('fault'))] == ('1'), () => setState(() => _locks[(('fault'))] == ('1') ? _locks.remove((('fault'))) : _locks[(('fault'))] = ('1'))), ((('🪑 ניצולת<${_RoomData.utilFloor}%')), _locks[(('under'))] == ('1'), () => setState(() => _locks[(('under'))] == ('1') ? _locks.remove((('under'))) : _locks[(('under'))] = ('1'))), ((('♿ נגיש')), _locks[(('access'))] == ('1'), () => setState(() => _locks[(('access'))] == ('1') ? _locks.remove((('access'))) : _locks[(('access'))] = ('1')))]; return ForgeFacetChip(bare: true, items: [for (final ch in chips) [ch.$1]], selected: <int>{for (final (k, ch) in chips.indexed) if (ch.$2) k}, onSelect: (k) => chips[k].$3()); }), if (_locks.isNotEmpty || _q.isNotEmpty) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() { _locks.clear(); _q = ''; }), child: ForgeToneButton(items: [['✖ נקה (${visible.length}/${rooms.length})']], variants: const <int>[3]))]),
        ...
        if (_view == 0) ...[
          ...
          Align(
            ...
            child: SingleChildScrollView(
              ...
              child: ForgeSegmentedPillToggleSelection(bare: true, items: [for (final s in [for (var i = 0; i < letters.length; i++) '${letters[i]} ${_RoomData.weekIsos[i].substring(8)}']) [s]], selected: {_dayIdx}, onSelect: (i) => setState(() => _dayIdx = i)),
            ),
          ),
        ],
        const SizedBox(height: 10),
        ...
        if (_loading)
          ...
          ForgeTitledSection(fields: ['🗓 שבוע-הבניין · ${_RoomData.weekIsos.first} → ${_RoomData.weekIsos.last}', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[_weekGrid(visible, names)]]))
        else ...[
  ...
  List<Widget> _automationCenter() {
    ...
    return [
      ForgeTitledSection(fields: ['🚨 דורש-פעולה · ${_RoomData.actionItems}', '', '', ''], child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [...[
        if (_RoomData.actionItems == 0) ForgeSearchEmptyState(fields: ['אין כפל-תפיסה · אין הזמנות ממתינות · כל שיעור בחדר עם הציוד הנדרש', '']),
    ...
    showModalBottomSheet<void>(
      ...
      builder: (c2) => Padding(
        ...
        child: ForgeStripPanelFrame(fields: ['', ''], child: ListView(shrinkWrap: true, padding: const EdgeInsets.all(8), children: [
          ...
          for (final r in rs)
            Row(children: [
              Expanded(child: ForgeContactTile(fields: ['${r['name']}', '${r['location']} · ${_RoomData.statusOf(r)}'])),
