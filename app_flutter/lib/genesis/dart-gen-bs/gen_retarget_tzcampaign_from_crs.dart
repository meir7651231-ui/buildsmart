...
class _TzCampaignScreenState extends State<TzCampaignScreen> {
    ...
    return DsScaffold(
      ...
      children: [
        ...
        Align(
          ...
          child: FittedBox(fit: BoxFit.scaleDown, child: SegmentedSwitch(items: [for (final r in _TzCampaignData.roleDefs) r['label'] as String], selected: _role, onSelect: (i) => setState(() { _role = i; _locks.clear(); }))),
        ),
        _gap(8),
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [
          ...
          if (_can('crs.new')) SoftButton(label: '➕ מבצע-חדש', tone: 1, onTap: () => _act(() => _TzCampaignData.newCourse(_who), 'נוצר מבצע-חדש (ללא-מורה/ללא-חדר — שבץ בפאנל)')),
          ...
          if (_TzCampaignData.exportOk(_role)) SoftButton(label: '⬇ ייצוא', tone: 0, onTap: () => _openExport(visible)),
          StatusChip(label: 'תפקיד: ${_TzCampaignData.roleName(_role)}${_TzCampaignData.myTeacherId(_role) != null ? ' · החוגים-שלי' : _TzCampaignData.myFamilyId(_role) != null ? ' · המערכת-שלי' : ''}', tone: 0),
        ]),
        _gap(6),
        ...
        Wrap(spacing: 8, runSpacing: 6, children: [
          ...
          for (final st in _TzCampaignData.stateChips) _fchip('state', st[0], '${st[1]} · ${_TzCampaignData.countState(st[0] == 'ended' ? _TzCampaignData.bySemester(_TzCampaignData.allCourses, _sem) : live, st[0])}'),
        ]),
        if (_adv) ...[
          _gap(8),
          // צירי-ממד: תחום (countBy) · מורה · חדר · יום · שעה · שכבה — נעילה-אחת פר-ציר, AND בין צירים
          Wrap(spacing: 8, runSpacing: 6, children: [
            for (final cc in _TzCampaignData.catCounts(live)) if ('${cc[0]}'.isNotEmpty) _fchip('cat', '${cc[0]}', '🗂 ${cc[0]} · ${cc[1]}'),
          ]),
          _gap(6),
          Wrap(spacing: 8, runSpacing: 6, children: [for (final t in _TzCampaignData.teachers) _fchip('teacher', '${t['id']}', '👩‍🏫 ${t['name']}')]),
          _gap(6),
          Wrap(spacing: 8, runSpacing: 6, children: [for (final r in _TzCampaignData.rooms) _fchip('room', '${r['id']}', '🚪 ${r['name']}')]),
          _gap(6),
          Wrap(spacing: 8, runSpacing: 6, children: [
            for (var dd = 0; dd < 6; dd++) _fchip('day', '$dd', '📅 ${dayNames[dd]}'),
            for (final h in _TzCampaignData.gridHours(_TzCampaignData.liveCourses)) _fchip('hour', '${h ~/ 60}', '🕐 ${_TzCampaignData.hm(h)}'),
          ]),
          _gap(6),
        ...
        GradientCard(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            _gap(14),
            Row(children: [
              BareStat(value: '${_TzCampaignData.kpiActive}', label: '📚 פעילים', inkColor: _ink, mutedColor: _muted),
              BareStat(value: '${_TzCampaignData.kpiLessonsWeek}', label: '🗓 שיעורים-השבוע', inkColor: _ink, mutedColor: _muted),
              BareStat(value: '${_TzCampaignData.kpiEnrolled}', label: '🎓 רשומים', inkColor: _ink, mutedColor: _muted),
              BareStat(value: '${_TzCampaignData.kpiOccupancyPct}%', label: '📈 תפוסה-ממוצ׳', inkColor: _TzCampaignData.kpiOccupancyPct >= 80 ? _ok : _acc, mutedColor: _muted),
              BareStat(value: '${_TzCampaignData.kpiFull}', label: '🈵 מלאים', inkColor: _warning, mutedColor: _muted),
            ]),
            _gap(12),
            Row(children: [
              BareStat(value: '${_TzCampaignData.kpiWaiting}', label: '⏳ בהמתנה', inkColor: _TzCampaignData.kpiWaiting > 0 ? _warning : _ok, mutedColor: _muted),
              BareStat(value: '${_TzCampaignData.kpiNoTeacher}', label: '🚫 ללא-מורה', inkColor: _TzCampaignData.kpiNoTeacher > 0 ? _danger : _ok, mutedColor: _muted),
              BareStat(value: _TzCampaignData.kpiBelowMinKnown ? '${_TzCampaignData.kpiBelowMin}' : '—', label: '📉 מתחת-מינ׳', inkColor: _TzCampaignData.kpiBelowMin > 0 ? _danger : _ok, mutedColor: _muted),
              BareStat(value: shekel(_TzCampaignData.kpiDebt.toInt()), label: '💳 חוב-פתוח', inkColor: _TzCampaignData.kpiDebt > 0 ? _warning : _ok, mutedColor: _muted),
            ]),
          ]),
        ),
        _gap(10),
        ...
        if (_loading)
          ...
          EmptyState(glyph: '📚', message: _TzCampaignData.myTeacherId(_role) != null ? 'אין חוגים משובצים למורה זה' : _TzCampaignData.myFamilyId(_role) != null ? 'אין חוגים למשפחה — הירשמו מהקטלוג' : 'אין חוגים — צור מבצע-חדש או שכפל סמסטר')
        else if (visible.isEmpty)
          const Padding(padding: EdgeInsets.only(top: 24), child: EmptyState(glyph: '🔍', message: 'אין חוגים תואמים לחיפוש/סינון'))
        else if (_view == 1)
          DsSection(title: '📋 רשימת-חוגים · ${visible.length} · ${_TzCampaignData.columnDefs.where((c) => _TzCampaignData.colShown(c, visible)).length} עמודות', children: [_table(ranked)])
        else if (_view == 2)
          ..._byTeacher(visible)
        else if (_view == 3)
          ..._byRoom(visible)
        else ...[
          DsSection(title: '📅 מערכת-שעות · ${_week == 0 ? 'השבוע' : 'שבוע הבא'} (${_TzCampaignData.isoOfDay(0, _week)} – ${_TzCampaignData.isoOfDay(5, _week)})', children: [_grid(visible)]),
  ...
  List<Widget> _automations(List<Map<String, dynamic>> live) {
    ...
    return [
      DsSection(title: '🤖 אוטומציות · ${hl.length + rem.length + bm.length + pr.length + noRoomCs.length + noTeacherCs.length + clashRooms.length + demand.length} אותות', children: [
        ...
        Row(children: [
          BareStat(value: '${_TzCampaignData.avgRoomUtilPct}%', label: '🚪 ניצולת-חדרים', inkColor: _TzCampaignData.avgRoomUtilPct < 30 ? _warning : _ok, mutedColor: _muted),
          BareStat(value: _TzCampaignData.avgTeacherLoad.toStringAsFixed(1), label: '👩‍🏫 מפגשים/מורה/שבוע', inkColor: _ink, mutedColor: _muted),
          BareStat(value: '${_TzCampaignData.upcomingHolidayList.length}', label: '🕎 חגים ב-45 ימים', inkColor: _ink, mutedColor: _muted),
        ]),
        _gap(8),
        ...
        if (hl.isNotEmpty) withAction(
          ...
          _can('crs.cancel') ? SoftButton(label: '✖ בטל-אוטו', tone: 3, onTap: () => _act(() => _TzCampaignData.autoCancelHolidays('אוטומציה'), '${hl.length} שיעורי-חג בוטלו אוטומטית — שלח הודעה למשפחות מהפאנל')) : null),
        // מתחת-מינימום X ימים לפני/אחרי פתיחה — התרעה כלכלית
        for (final c in bm)
          AlertBanner(glyph: '📉', tone: 2, message: '${c['name']}: ${_TzCampaignData.enrolled(c)} רשומים מול מינ׳ ${_TzCampaignData.minToOpen(c)} · ${_TzCampaignData.daysToStart(c) >= 0 ? 'מתחיל בעוד ${_TzCampaignData.daysToStart(c)} ימים' : 'התחיל לפני ${-_TzCampaignData.daysToStart(c)} ימים'} — לא-כלכלי'),
        // הצעת-חדר-חלופי: ללא-חדר / התנגשות-חדר ⇒ חדרים פנויים ב-slot
        for (final c in [...noRoomCs, ...clashRooms])
          AlertBanner(glyph: '🚪', tone: 3, message: '${c['name']} — ${_TzCampaignData.noRoom(c) ? 'ללא-חדר' : 'התנגשות-חדר'} · חדר חלופי: ${_TzCampaignData.freeRooms(c).isEmpty ? 'אין חדר פנוי ב-slot' : _TzCampaignData.freeRooms(c).map((r) => '${r['name']} (${r['cap']})').join(' / ')}'),
        // הצעת-מורה-מחליף: ללא-מורה ⇒ מורים פנויים ב-slot (התמחות תואמת קודם)
        for (final c in noTeacherCs)
          AlertBanner(glyph: '👩‍🏫', tone: 3, message: '${c['name']} — ללא-מורה · מורה חלופי: ${_TzCampaignData.freeTeachers(c).isEmpty ? 'אין מורה פנוי ב-slot' : _TzCampaignData.freeTeachers(c).map((t) => '${t['name']}${t['specialty'] == c['cat'] ? ' ✓' : ''}').join(' / ')}'),
        // המתנה-עם-מקום ⇒ העלאה
        for (final c in pr) withAction(
          AlertBanner(glyph: '⏳', tone: 3, message: '${c['name']}: ${_TzCampaignData.waitlist(c).length} ממתינים ויש ${_TzCampaignData.capacity(c) - _TzCampaignData.enrolled(c)} מקומות פנויים'),
          _can('crs.waitlist') ? SoftButton(label: '⬆ העלה', tone: 1, onTap: () => _act(() { while (_TzCampaignData.promoteNext(c, 'אוטומציה') != null) {} }, 'הממתינים הועלו')) : null),
        // תזכורות 48h
        if (rem.isNotEmpty) AlertBanner(glyph: '🔔', tone: 0, message: 'תזכורת ל-48 השעות הקרובות: ${rem.map((r) => '${(r['course'] as Map)['name']} ${_TzCampaignData._iso(r['dt'] as DateTime)} ${_TzCampaignData.hm((r['dt'] as DateTime).hour * 60 + (r['dt'] as DateTime).minute)} (${_TzCampaignData.liveEnrollmentsOf(r['course'] as Map<String, dynamic>).length} משפחות)').join(' · ')} — שלח-הודעה מהפאנל'),
        // תחזית-ביקוש (אות-נוכחי; היסטוריה רב-סמסטרית = מקום-שמור)
        if (demand.isNotEmpty) AlertBanner(glyph: '📈', tone: 1, message: 'ביקוש לסמסטר-הבא: ${demand.map((c) => '${c['name']} (${_TzCampaignData.isFull(c) ? 'מלא' : ''}${_TzCampaignData.waitlist(c).isNotEmpty ? ' +${_TzCampaignData.waitlist(c).length} ממתינים' : ''}${_TzCampaignData.trend(c)['dir'] == 'up' ? ' ↑' : ''})').join(' · ')} ⇒ שקול קבוצה נוספת'),
      ...
      DsSection(title: '🛒 הרשמה-עצמית · ${open.length} חוגים פתוחים (רכז/ת מאשר/ת)', children: [
        ...
        for (final c in open)
          Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(children: [
            Expanded(child: MediaRow(glyph: '📚', title: '${c['name']}', subtitle: '${_TzCampaignData.sessionsLabel(c)} · ${_TzCampaignData.enrolled(c)}/${_TzCampaignData.capacity(c)} · שכבות ${c['gradeMin']}–${c['gradeMax']}')),
            for (final m in (fam['members'] as List))
              Padding(padding: const EdgeInsets.only(left: 4), child: SoftButton(label: '➕ ${m['first']}', tone: _TzCampaignData.fitReason(c, _TzCampaignData.memberOf(m['id'])!) == null ? 1 : 3,
                onTap: () => setState(() => _result(_TzCampaignData.selfEnroll(c, m['id'], _who), 'נרשם/ה — ממתין לאישור')))),
          ])),
      ]),
    ];
  }
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: FittedBox(
          ...
          child: StatusChip(label: '${cancelled ? '✖ ' : ''}${c['name']} ${_TzCampaignData.enrolled(c)}/${_TzCampaignData.capacity(c)}', tone: tone),
        ),
      ),
    );
  }
      ...
      for (final t in _TzCampaignData.teachers)
        () {
          ...
          return DsSection(
            ...
            trailing: StatusChip(label: '${cs.length} חוגים · ${_TzCampaignData.weeklyOf(cs)} מפגשים/שבוע', tone: cs.isEmpty ? 0 : 1),
        ...
        for (final r in _TzCampaignData.rooms)
          () {
            ...
            return DsSection(
              ...
              children: [
                StatRow(label: 'ניצולת שבועית', value: '$weekly מתוך $cap משבצות', fraction: _TzCampaignData.roomUtil(r)),
                _gap(6),
  ...
  Widget _row(Map<String, dynamic> c) {
    ...
    return Padding(
      ...
      child: Row(children: [
        Expanded(child: MediaRow(glyph: '📚', title: '${c['name']}', subtitle: '${t?['name'] ?? '—'} · ${r?['name'] ?? '—'} · ${_TzCampaignData.sessionsLabel(c)} · ${_TzCampaignData.enrolled(c)}/${_TzCampaignData.capacity(c)}')),
        StatusChip(label: _TzCampaignData.statusLabel(c), tone: tone),
        if (_TzCampaignData.waitlist(c).isNotEmpty) ...[const SizedBox(width: 6), StatusChip(label: '⏳ ${_TzCampaignData.waitlist(c).length}', tone: 0)],
            ...
            child: Directionality(
              ...
              child: GlassCard(
                child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                  Row(children: [
                    Expanded(child: MediaRow(glyph: '📚', title: '${c['name']}', subtitle: '${_TzCampaignData.teacherOf(c)?['name'] ?? 'ללא-מורה'} · ${_TzCampaignData.roomOf(c)?['name'] ?? 'ללא-חדר'} · ${_TzCampaignData.sessionsLabel(c)}')),
                    StatusChip(label: _TzCampaignData.statusLabel(c), tone: _TzCampaignData.sev(c) >= 2 ? 2 : _TzCampaignData.sev(c) == 1 ? 3 : 1),
                  ]),
                  _gap(8),
  ...
  List<Widget> _tabOverview(Map<String, dynamic> c, void Function(void Function()) both) {
    ...
    return [
      StatRow(label: 'תפוסה מול קיבולת', value: '${_TzCampaignData.enrolled(c)} מתוך ${_TzCampaignData.capacity(c)}', fraction: _TzCampaignData.occupancy(c)),
      _gap(8),
      Row(children: [
        BareStat(value: '${_TzCampaignData.enrolled(c)}', label: 'רשומים', inkColor: _ink, mutedColor: _muted),
        BareStat(value: '${_TzCampaignData.waitlist(c).length}', label: 'בהמתנה', inkColor: _TzCampaignData.waitlist(c).isEmpty ? _ink : _warning, mutedColor: _muted),
        BareStat(value: _TzCampaignData.minToOpen(c) == null ? '—' : '${_TzCampaignData.minToOpen(c)}', label: 'מינ׳-לפתיחה', inkColor: _TzCampaignData.belowMin(c) ? _danger : _ink, mutedColor: _muted),
        BareStat(value: c['active'] == true ? '${shekel(c['lessonPrice'])}/ש׳' : shekel(c['price']), label: 'מחיר', inkColor: _acc, mutedColor: _muted),
        BareStat(value: _TzCampaignData.trendLabel(c), label: 'מגמת-הרשמה', inkColor: _TzCampaignData.trend(c)['dir'] == 'down' ? _danger : _ok, mutedColor: _muted),
      ]),
      _gap(6),
      MediaRow(glyph: '🚪', title: _TzCampaignData.roomOf(c)?['name'] ?? 'ללא-חדר', subtitle: _TzCampaignData.roomLabel(c)), // roomInfoLabel (מנוע) — טקסט-ארוך ⇒ שורה, לא שבב
      Wrap(spacing: 8, runSpacing: 6, children: [
      ...
      if (_TzCampaignData.noRoom(c) || _TzCampaignData.clashesOf(c).any((k) => k['kind'] == 'room')) ...[
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          if (_TzCampaignData.freeRooms(c).isEmpty) const StatusChip(label: 'אין חדר פנוי', tone: 2),
          for (final r in _TzCampaignData.freeRooms(c))
            if (_can('crs.assignRoom')) SoftButton(label: '🚪 ${r['name']} (${r['cap']})', tone: 1, onTap: () => both(() => _result(_TzCampaignData.assignRoom(c, r['id'], _who), '${r['name']} הוקצה'))) else _chip('🚪 ${r['name']}', 0),
        ]),
      ],
      if (_TzCampaignData.noTeacher(c) || _TzCampaignData.clashesOf(c).any((k) => k['kind'] == 'teacher')) ...[
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          if (_TzCampaignData.freeTeachers(c).isEmpty) const StatusChip(label: 'אין מורה פנוי', tone: 2),
          for (final t in _TzCampaignData.freeTeachers(c))
            if (_can('crs.assignTeacher')) SoftButton(label: '${t['name']}${t['specialty'] == c['cat'] ? ' ✓' : ''}', tone: t['specialty'] == c['cat'] ? 1 : 0, onTap: () => both(() => _result(_TzCampaignData.assignTeacher(c, t['id'], _who), '${t['name']} הוקצה/תה'))) else _chip('${t['name']}', 0),
        ]),
      ],
      if (_TzCampaignData.belowMin(c)) AlertBanner(glyph: '📉', tone: 3, message: 'מתחת-למינימום: ${_TzCampaignData.enrolled(c)} רשומים מול ${_TzCampaignData.minToOpen(c)} (${c['minStudents'] == null ? 'נקודת-איזון: שכר-מורה+חדר ÷ מחיר-לשיעור' : 'מינימום-מוגדר'}) — לא-כלכלי'),
      _h('📅 השיעורים הבאים'),
      ...
      Builder(builder: (_) {
        final acts = <Widget>[
          ...
          if (_can('crs.assignTeacher')) SoftButton(label: '👩‍🏫 הקצה-מורה', tone: _TzCampaignData.noTeacher(c) ? 2 : 0, onTap: () => both(() => _pick = _pick == 'teacher' ? null : 'teacher')),
          if (_can('crs.assignTeacher')) SoftButton(label: '🔄 מורה-מחליף (חד-פעמי)', tone: 0, onTap: () => both(() => _pick = _pick == 'sub' ? null : 'sub')),
          if (_can('crs.assignRoom')) SoftButton(label: '🚪 הקצה-חדר', tone: _TzCampaignData.noRoom(c) ? 2 : 0, onTap: () => both(() => _pick = _pick == 'room' ? null : 'room')),
          if (_can('crs.edit')) SoftButton(label: '✏️ ערוך', tone: 0, onTap: () => both(() => _edit = !_edit)),
          if (_can('crs.duplicate')) SoftButton(label: '📄 שכפל-מבצע', tone: 0, onTap: () => both(() { final cp = _TzCampaignData.duplicate(c, _who); _flash('נוצר ${cp['name']} — יורש slot ⇒ בדוק התנגשות והקצה מחדש', 3); })),
          if (_can('crs.message')) SoftButton(label: '💬 שלח-הודעה', tone: 0, onTap: () => both(() => _pick = _pick == 'message' ? null : 'message')),
          if (_can('crs.end')) SoftButton(label: '🏁 סיים-מבצע', tone: 2, onTap: () => both(() { _TzCampaignData.endCourse(c, _who); _flash('${c['name']} הסתיים — ההרשמות נסגרו', 3); })),
          if (_can('crs.end')) SoftButton(label: '⛔ בטל-מבצע', tone: 2, onTap: () => both(() { _TzCampaignData.cancelCourse(c, _who); _flash('${c['name']} בוטל', 3); })),
        ];
        return acts.isEmpty
      ...
      if (_edit) ...[
        ...
        DsField(label: 'שם-מבצע', hint: 'שם', value: '${c['name']}', onChanged: (v) => both(() => _TzCampaignData.edit(c, 'name', v, _who))),
        DsNumberField(label: 'קיבולת (maxStudents)', value: '${_TzCampaignData.capacity(c)}', onChanged: (v) { final n = int.tryParse(v); if (n != null) both(() => _TzCampaignData.edit(c, 'goal', n, _who)); }),
      ],
    ];
  }
      ...
      return [
        ...
        if (cands.isEmpty) const EmptyState(glyph: '🎓', message: 'כל התלמידים כבר רשומים/ממתינים')
          ...
          for (final m in cands)
            SoftButton(label: '${m['first']} ${m['famName']} · ${m['grade']}', tone: _TzCampaignData.fitReason(c, m) == null ? 0 : 3,
              onTap: () => both(() { _pick = null; _result(p == 'enroll' ? _TzCampaignData.enroll(c, m['id'], _who) : _TzCampaignData.invite(c, m['id'], _who), '${m['first']} שובץ/ה ל-${c['name']}'); })),
        ]),
      ];
    }
    if (p == 'teacher' || p == 'sub') {
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          for (final t in _TzCampaignData.teachers)
            SoftButton(label: '${t['name']} · ${t['specialty']}', tone: t['id'] == c['teacherId'] ? 1 : 0,
              onTap: () => both(() { _pick = null; if (p == 'teacher') { _result(_TzCampaignData.assignTeacher(c, t['id'], _who), '${t['name']} הוקצה/תה ל-${c['name']}'); } else { _TzCampaignData.substitute(c, iso, t['id'], _who); _flash('${t['name']} מחליף/ה ב-$iso', 1); } })),
        ]),
      ];
    }
    if (p == 'room') {
      return [
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          for (final r in _TzCampaignData.rooms.where((r) => r['active'] == true))
            SoftButton(label: '${r['name']} · ${r['cap']} · ${r['slot']} דק׳', tone: r['id'] == c['roomId'] ? 1 : (r['cap'] as int) < _TzCampaignData.capacity(c) ? 3 : 0,
              onTap: () => both(() { _pick = null; _result(_TzCampaignData.assignRoom(c, r['id'], _who), '${r['name']} הוקצה ל-${c['name']}'); })),
        ]),
      ];
    }
    if (p == 'message') {
      ...
      return [
        ...
        Wrap(spacing: 6, runSpacing: 6, children: [
          for (final t in _TzCampaignData.liveCourses.where((t) => t['id'] != c['id']))
            SoftButton(label: '${t['name']} ${_TzCampaignData.enrolled(t)}/${_TzCampaignData.capacity(t)}', tone: _TzCampaignData.isFull(t) ? 3 : 0,
              onTap: () => both(() { _pick = null; _result(_TzCampaignData.move(e, t, _who), 'הועבר/ה ל-${t['name']}'); })),
        ]),
      ];
    }
    return const [];
  ...
  Widget _lessonTile(Map<String, dynamic> c, DateTime dt, void Function(void Function()) both) {
    ...
    return Row(children: [
      Expanded(child: TimelineItem(
        title: '${cancelled ? '✖ מבוטל · ' : ''}${dayNames[dt.weekday % 7]} ${_TzCampaignData.hm(dt.hour * 60 + dt.minute)}${_TzCampaignData.holidayName(dt) != null ? ' · 🕎 ${_TzCampaignData.holidayName(dt)}' : ''}',
        time: iso,
        body: '${subName != null ? '🔄 $subName (מחליף/ה)' : _TzCampaignData.teacherOf(c)?['name'] ?? 'ללא-מורה'} · ${_TzCampaignData.roomOf(c)?['name'] ?? 'ללא-חדר'}',
      )),
      if (_can('crs.cancel')) SoftButton(label: cancelled ? '↩ שחזר' : '✖ בטל', tone: cancelled ? 1 : 2, onTap: () => both(() { _TzCampaignData.cancelSession(c, iso, _who); _flash(cancelled ? 'השיעור $iso שוחזר' : 'השיעור $iso בוטל', cancelled ? 1 : 3); })),
    ]);
  }
  ...
  List<Widget> _tabEnrolled(Map<String, dynamic> c, void Function(void Function()) both) {
    ...
    return [
      ...
      for (final e in es) ...[
        Row(children: [
          Expanded(child: MediaRow(glyph: '🎓', title: _TzCampaignData.memberName(e['memberId']), subtitle: '${_TzCampaignData.enrollStatusLabel(e)} · ${_TzCampaignData.paidLabel(e)}${_TzCampaignData.debtOf(e) > 0 ? ' · חוב ${shekel(_TzCampaignData.debtOf(e).toInt())}' : ''} · נרשם/ה ${e['enrolledAt']}')),
          if (_can('crs.move')) SoftButton(label: '🔁', tone: 0, onTap: () => both(() => _pick = _pick == 'move:${e['id']}' ? null : 'move:${e['id']}')),
          const SizedBox(width: 4),
          if (_can('crs.remove')) SoftButton(label: '➖ הסר', tone: 2, onTap: () => both(() => _result(_TzCampaignData.remove(e, _who), '${_TzCampaignData.memberName(e['memberId'])} הוסר/ה'))),
        ]),
  ...
  List<Widget> _tabWaitlist(Map<String, dynamic> c, void Function(void Function()) both) {
    ...
    return [
      ...
      for (var i = 0; i < w.length; i++)
        Row(children: [
          Expanded(child: MediaRow(glyph: '${i + 1}', title: _TzCampaignData.memberName(w[i]['memberId']), subtitle: 'ממתין/ה מ-${w[i]['enrolledAt']}${_TzCampaignData.clashReason(c, w[i]['memberId']) != null ? ' · ⚠️ התנגשות' : ''}')),
          if (_can('crs.waitlist')) SoftButton(label: '⬆ העלה', tone: _TzCampaignData.isFull(c) ? 3 : 1, onTap: () => both(() => _result(_TzCampaignData.promote(w[i], _who), '${_TzCampaignData.memberName(w[i]['memberId'])} הועלה/תה מההמתנה'))),
          const SizedBox(width: 4),
          if (_can('crs.waitlist')) SoftButton(label: '➖', tone: 2, onTap: () => both(() => _result(_TzCampaignData.remove(w[i], _who), 'הוסר/ה מההמתנה'))),
        ]),
      _gap(6),
      Row(children: [
        ...
        SegmentedSwitch(items: const ['פועל', 'כבוי'], selected: _TzCampaignData.autoPromote ? 0 : 1, onSelect: (i) => both(() => _TzCampaignData.autoPromote = i == 0)),
      ]),
    ];
  }
  ...
  List<Widget> _tabSchedule(Map<String, dynamic> c, void Function(void Function()) both) {
    ...
    return [
      ...
      if (!_TzCampaignData.hasSessions(c)) const EmptyState(glyph: '🗓', message: 'אין מפגשים קבועים — הגדר יום+שעה (מקום-שמור: עורך-מפגשים)'),
      for (final s in ss) if (s['day'] is int) TimelineItem(title: '${dayNames[s['day'] as int]} ${s['time']}', time: '${(s['label'] ?? '') == '' ? 'קבוצה יחידה' : s['label']}', body: _TzCampaignData.roomLabel(c)),
  ...
  List<Widget> _tabAttendance(Map<String, dynamic> c) {
    ...
    return [
      StatRow(label: 'נוכחות-המבצע (נוכח ÷ (נוכח+נעדר))', value: '${(rate * 100).round()}%', fraction: rate),
      _gap(8),
      ...
      for (final e in es)
        () {
          ...
          return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            StatRow(label: _TzCampaignData.memberName(e['memberId']), value: '${sm['presents']}/$tot · החודש ${_TzCampaignData.presentsThisMonth(e)}', fraction: tot == 0 ? 0 : (sm['presents'] as int) / tot),
  ...
  List<Widget> _tabFees(Map<String, dynamic> c) {
    ...
    return [
      ...
      _gap(8),
      for (final e in es)
        MediaRow(glyph: _TzCampaignData.paidStatus(e) == 'paid' ? '✅' : _TzCampaignData.paidStatus(e) == 'partial' ? '🟠' : '🔴', title: _TzCampaignData.memberName(e['memberId']),
          subtitle: '${_TzCampaignData.paidLabel(e)} · שולם ${shekel(paidOf(e).toInt())} מתוך ${shekel(((e['totalDue'] as num?) ?? 0).toInt())}${_TzCampaignData.debtOf(e) > 0 ? ' · חוב ${shekel(_TzCampaignData.debtOf(e).toInt())}' : ''}${'${e['dueDate'] ?? ''}'.isNotEmpty ? ' · לתשלום עד ${e['dueDate']}' : ''}'),
    ];
  }
  ...
  List<Widget> _tabHistory(Map<String, dynamic> c, bool all) {
    ...
    return [
      ...
      if (rows.isEmpty) EmptyState(glyph: all ? '🧾' : '🕓', message: all ? 'אין פעולות עדיין' : 'אין היסטוריה למבצע'),
        ...
        return DraggableScrollableSheet(
          ...
          builder: (ctx, scroll) => Padding(
            ...
            child: Directionality(textDirection: TextDirection.rtl, child: GlassCard(
              child: ListView(controller: scroll, padding: const EdgeInsets.all(6), children: [
                MediaRow(glyph: '⬇', title: 'ייצוא', subtitle: '${cs.length} חוגים · ${_TzCampaignData.csvRows(cs).first.length} עמודות · שער-ייצוא פתוח'),
                _gap(8),
