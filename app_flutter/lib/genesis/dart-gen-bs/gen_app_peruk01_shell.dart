...
class _RootTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    return DsScaffold(title: gen_app_peruk01_shell_c8, subtitle: rs.length.toString() + ' ' + gen_app_peruk01_shell_c9, icon: gen_app_peruk01_shell_c10, header: false, children: [ForgeCenteredPageHeader(fields: ['', gen_app_peruk01_shell_c8, rs.length.toString() + ' ' + gen_app_peruk01_shell_c9]), ...[
      ...
      for (final r in rs) GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk01RootScreen(id: r[AppStore.idKey] ?? ''))), child: ForgeGridHubCard(fields: [(r[gen_app_peruk01_shell_c6] ?? ''), (r[gen_app_peruk01_shell_c7] ?? '')])),
    ]]);
  });
}
