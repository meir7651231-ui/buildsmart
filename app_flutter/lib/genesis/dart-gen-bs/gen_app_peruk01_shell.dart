...
class _RootTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    return DsScaffold(title: gen_app_peruk01_shell_c8, subtitle: rs.length.toString() + ' ' + gen_app_peruk01_shell_c9, icon: gen_app_peruk01_shell_c10, children: [
      ...
      for (final r in rs) DsNavTile(glyph: gen_app_peruk01_shell_c11, title: (r[gen_app_peruk01_shell_c6] ?? ''), sub: (r[gen_app_peruk01_shell_c7] ?? ''), onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk01RootScreen(id: r[AppStore.idKey] ?? '')))),
    ]);
  });
}
