...
class GenAppTasksHomeScreenToday {
  ...
  static String _dayLabel(DateTime d, DateTime today) { final n = _day(d).difference(_day(today)).inDays; if (n == 0) return gen_app_tasks_home_c8; if (n == 1) return gen_app_tasks_home_c9; if (n == -1) return gen_app_tasks_home_c10; final dm = '${d.day}.${d.month}' + (d.year == today.year ? '' : '.${d.year}'); return n.abs() <= 6 ? gen_app_tasks_home_c11.replaceAll('{day}', gen_app_tasks_home_c12.split(',')[d.weekday % 7]) + ' ' + dm : dm; }
  ...
  static void _act(String rid, String field, DateTime due, List<String> acts, int i) {
    ...
    else if (a == gen_app_tasks_home_c26) { final r = appStore.byId('app_tasks_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_tasks_ent1', rid, {field: _iso(_shift(due.add(const Duration(days: 1)), false))}); /* «דחה למחר» לא נוחת בשבת (אותו _shift של תזכורת-רכה) */ appStore.logAction('auto', gen_app_tasks_home_c27 + ' · ' + field, entity: 'app_tasks_ent1', rid: rid, field: field, prev: prev); } }   // נגיעה-ידנית (P5) — נרשמת עם החזר
    else if (a == gen_app_tasks_home_c28) { final r = appStore.byId('app_tasks_ent1', rid); if (r != null) { final prev = r[field] ?? ''; appStore.update('app_tasks_ent1', rid, {field: _iso(_shift(due.add(const Duration(days: 7)), false))}); appStore.logAction('auto', gen_app_tasks_home_c29 + ' · ' + field, entity: 'app_tasks_ent1', rid: rid, field: field, prev: prev); } }   /* «דחה לשבוע» — נגיעה-ידנית עם החזר, לא בשבת */
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { final ago = today.difference(d).inDays; out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', gen_app_tasks_home_c33.replaceAll('{date}', _dayLabel(d, today)) + ' · ' + (ago == 1 ? gen_app_tasks_home_c34 : gen_app_tasks_home_c35.replaceAll('{n}', ago.toString())), rid, f.label, d, f.hard, true, today, tm, rep, ph, mo)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == today.add(Duration(days: dayDelta))) { out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', off == 0 ? '' : gen_app_tasks_home_c36.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep, ph, mo)); break; }
        }
      }
    }
  ...
  static void _setDate(String rid, String field, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    if (a == gen_app_tasks_home_c40) { appStore.decide('undated:$rid', 'no'); appStore.logAction('decide', gen_app_tasks_home_c41.replaceAll('{what}', appStore.displayOf('app_tasks_ent1', rid)), entity: 'app_tasks_ent1', rid: rid, field: 'undated:$rid'); return; }
    final r = appStore.byId('app_tasks_ent1', rid); if (r == null) return;
    final prev = r[field] ?? '';
    appStore.update('app_tasks_ent1', rid, {field: _iso(_shift(today.add(Duration(days: a == gen_app_tasks_home_c42 ? 7 : 1)), false))});   /* «קבע» לא נוחת בשבת */
  ...
  static List<DsTodayItem> stale(DateTime today) {
    ...
    for (final r in open()) {
      ...
      final acts = [gen_app_tasks_home_c43, gen_app_tasks_home_c44, gen_app_tasks_home_c45];
      out.add(DsTodayItem(title: appStore.displayOf('app_tasks_ent1', rid), sub: gen_app_tasks_home_c46.replaceAll('{n}', n.toString()), rid: rid, field: '', due: t, hard: false, overdue: false, module: module, actions: acts, act: (i) => _staleAct(rid, today, acts, i)));
    }
  ...
  static void _staleAct(String rid, DateTime today, List<String> acts, int i) {
    final a = acts[i.clamp(0, acts.length - 1)];
    final r = appStore.byId('app_tasks_ent1', rid); if (r == null) return;
    if (a == gen_app_tasks_home_c47) { final f = _dates.firstWhere((x) => x.hard, orElse: () => _dates.first); appStore.decide('ign:$rid:${f.label}', ''); appStore.decide('undated:$rid', ''); _setDate(rid, f.label, today, [a], 0); return; }   /* המועד החדש חוזר ל«היום» — ההתעלמות הישנה נמחקת */
    if (a == gen_app_tasks_home_c48) { final prev = r[AppStore.stageKey] ?? '0'; appStore.update('app_tasks_ent1', rid, {AppStore.stageKey: '1'}); appStore.logAction('auto', gen_app_tasks_home_c49.replaceAll('{who}', appStore.displayOf('app_tasks_ent1', rid)), entity: 'app_tasks_ent1', rid: rid, field: AppStore.stageKey, prev: prev); return; }
    appStore.decide('stale:$rid', 'no'); appStore.logAction('decide', gen_app_tasks_home_c50.replaceAll('{what}', appStore.displayOf('app_tasks_ent1', rid)), entity: 'app_tasks_ent1', rid: rid, field: 'stale:$rid');
  }
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true, bool rem = true}) {   // rem=false: «בלגן» מרכז את התזכורות לכרטיס אחד (ב׳-לז)   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    final out = <Widget>[];
    final days = _offsets().map((o) => o == 0 ? gen_app_tasks_home_c51 : o == 1 ? gen_app_tasks_home_c52 : gen_app_tasks_home_c53.replaceAll('{n}', o.toString())).join(' · ');   /* ב׳-מב · «3 ימים לפני · יום לפני · ביום» במקום (−3/−1/−0) */
    for (final r in open()) {
      final rid = r[AppStore.idKey] ?? ''; final who = appStore.displayOf('app_tasks_ent1', rid);
      if (rem) for (final f in _dates) {
        ...
        if (appStore.decision(_remKey(rid, f.label)).isNotEmpty) continue;
        out.add(DsApproveCard(question: gen_app_tasks_home_c54.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _dayLabel(d, today)), source: module + ' · ' + who, okLabel: gen_app_tasks_home_c55, noLabel: gen_app_tasks_home_c56, alwaysLabel: gen_app_tasks_home_c57,
  ...
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: module + ' · ' + (((r[gen_app_tasks_home_c0] ?? '')).trim().isEmpty ? gen_app_tasks_home_c58 : (r[gen_app_tasks_home_c0] ?? '')), trailing: Text(const [gen_app_tasks_home_c3, gen_app_tasks_home_c4][appStore.stageOf('app_tasks_ent1', r[AppStore.idKey] ?? '').clamp(0, 1)], style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), children: [
        
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [DsChipButton(label: gen_app_tasks_home_c59, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppTasksRootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_tasks_home_c60.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_tasks_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppTasksHomeScreenState extends State<GenAppTasksHomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_tasks_home_c61, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_tasks_home_c62, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_tasks_home_c63 : n <= 1 ? gen_app_tasks_home_c64 : gen_app_tasks_home_c65.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_tasks_home_c66, subtitle: lead, icon: gen_app_tasks_home_c67, children: [
      DsLoadMeter(count: n, label: gen_app_tasks_home_c68.replaceAll('{n}', n.toString()), stateLabels: [gen_app_tasks_home_c69, gen_app_tasks_home_c70, gen_app_tasks_home_c71]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_tasks_home_c72, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_tasks_home_c73, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppTasksHomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_tasks_home_c74 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_tasks_home_c75 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_tasks_home_c76, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_tasks_home_c77 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_tasks_home_c78 + ' ' + gen_app_tasks_home_c79, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
