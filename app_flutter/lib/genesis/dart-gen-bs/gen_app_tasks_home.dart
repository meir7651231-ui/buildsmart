...
class GenAppTasksHomeScreenToday {
  ...
  static String _dayLabel(DateTime d, DateTime today) { final n = -cockpitDaysSince(_iso(d), _iso(today)).toInt(); if (n == 0) return gen_app_tasks_home_c8; if (n == 1) return gen_app_tasks_home_c9; if (n == -1) return gen_app_tasks_home_c10; final dm = dayMonthOfIso(_iso(d), d.year != today.year); return n.abs() <= 6 ? gen_app_tasks_home_c11.replaceAll('{day}', gen_app_tasks_home_c12.split(',')[weekdayOfIso(_iso(d))]) + ' ' + dm : dm; }   // G34 · דבק: ימים-מאז · יום-בשבוע · יום.חודש — חלקיקים; כאן רק מונחים
  ...
  static List<DsTodayItem> items(DateTime today, {required int dayDelta}) {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        if (dayDelta == 0 && d.isBefore(today)) { final ago = cockpitDaysSince(_iso(d), _iso(today)).toInt(); out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', gen_app_tasks_home_c33.replaceAll('{date}', _dayLabel(d, today)) + (ago <= 1 ? '' : ' · ' + gen_app_tasks_home_c34.replaceAll('{n}', ago.toString()))   /* ב׳-עז · «היה אתמול» כבר אומר הכל — בלי «· אתמול» כפול */, rid, f.label, d, f.hard, true, today, tm, rep, ph, mo)); continue; }
        ...
        for (final off in _offsets()) {
          ...
          if (fire == _plus(today, dayDelta)) { out.add(_mk(_dates.length == 1 ? who : '${f.label} · $who', off == 0 ? '' : gen_app_tasks_home_c35.replaceAll('{n}', off.toString()), rid, f.label, d, f.hard, false, today, off == 0 ? tm : '', rep, ph, mo)); break; }
        }
      }
    }
