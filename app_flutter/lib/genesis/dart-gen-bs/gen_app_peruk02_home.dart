...
class GenAppPeruk02HomeScreenToday {
  ...
  static List<Widget> proposals(BuildContext context, DateTime today, {bool chain = true, bool rem = true}) {   // rem=false: «בלגן» מרכז את התזכורות לכרטיס אחד (ב׳-לז)   // chain=false: «בלגן» מרנדר את כרטיס-הצעד-הבא בעצמו (חוצה-מודולים)
    ...
    for (final r in open()) {
      ...
      if (rem) for (final f in _dates) {
        ...
        if (appStore.decision(_remKey(rid, f.label)).isNotEmpty) continue;
        out.add(DsApproveCard(question: gen_app_peruk02_home_c59.replaceAll('{field}', f.label).replaceAll('{days}', days).replaceAll('{date}', _iso(d)), source: module + ' · ' + who, okLabel: gen_app_peruk02_home_c60, noLabel: gen_app_peruk02_home_c61, alwaysLabel: gen_app_peruk02_home_c62,
      ...
      if (last != null && appStore.decision('fu:$rid:${last['id']}').isEmpty) { final at = DateTime.tryParse(last['at'] ?? ''); final n = at == null ? 0 : today.difference(_day(at)).inDays; if (n >= 3 && (last['prev'] ?? '') == appStore.stageOf('app_peruk02_ent1', rid).toString()) out.add(DsApproveCard(question: gen_app_peruk02_home_c63.replaceAll('{n}', n.toString()), source: module + ' · ' + who, okLabel: gen_app_peruk02_home_c64, noLabel: gen_app_peruk02_home_c65, onOk: () { appStore.decide('fu:$rid:${last['id']}', 'ok'); send(context, r, rid); }, onNo: () => appStore.decide('fu:$rid:${last['id']}', 'no'))); }
    }
  ...
  static Widget card(BuildContext context, Map<String, String> r) => DsSection(title: module + ' · ' + (((r[gen_app_peruk02_home_c0] ?? '')).trim().isEmpty ? gen_app_peruk02_home_c66 : (r[gen_app_peruk02_home_c0] ?? '')), trailing: Text(const [gen_app_peruk02_home_c6, gen_app_peruk02_home_c7, gen_app_peruk02_home_c8, gen_app_peruk02_home_c9, gen_app_peruk02_home_c10][appStore.stageOf('app_peruk02_ent1', r[AppStore.idKey] ?? '').clamp(0, 4)], style: TextStyle(color: DsLook.of(context).muted, fontSize: 13)), children: [
        
        Padding(padding: const EdgeInsets.only(top: 8), child: Row(children: [Expanded(child: DsPrimaryButton(label: gen_app_peruk02_home_c67, onTap: () => send(context, r, r[AppStore.idKey] ?? ''))), const SizedBox(width: 8), DsChipButton(label: gen_app_peruk02_home_c68, onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => GenAppPeruk02RootScreen(id: r[AppStore.idKey] ?? ''))))])),
      ]);
  ...
  static void autopilot() {
    ...
    for (final r in open()) {
      ...
      for (final f in _dates) {
        ...
        appStore.logAction('decide', gen_app_peruk02_home_c69.replaceAll('{field}', f.label).replaceAll('{who}', who), entity: 'app_peruk02_ent1', rid: rid, field: _remKey(rid, f.label));
      }
    }
  }
}
...
class _GenAppPeruk02HomeScreenState extends State<GenAppPeruk02HomeScreen> {
  ...
  Future<void> _digest(String lead, int hardToday) async {
    ...
    try {
      ...
      await n.show(1, gen_app_peruk02_home_c70, lead, const NotificationDetails(android: AndroidNotificationDetails('balagan_digest', 'digest')));
      if (hardToday > 0) await n.show(2, gen_app_peruk02_home_c71, '$hardToday', const NotificationDetails(android: AndroidNotificationDetails('balagan_hard', 'hard')));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: appStore, builder: (context, _) {
    ...
    final lead = n == 0 && open.isEmpty ? gen_app_peruk02_home_c72 : n <= 1 ? gen_app_peruk02_home_c73 : gen_app_peruk02_home_c74.replaceAll('{n}', n.toString());
    ...
    return DsScaffold(title: gen_app_peruk02_home_c75, subtitle: lead, icon: gen_app_peruk02_home_c76, children: [
      DsLoadMeter(count: n, label: gen_app_peruk02_home_c77.replaceAll('{n}', n.toString()), stateLabels: [gen_app_peruk02_home_c78, gen_app_peruk02_home_c79, gen_app_peruk02_home_c80]),
      ...
      if (overdue.isNotEmpty) DsSection(title: gen_app_peruk02_home_c81, tone: 2, children: [for (final it in overdue) DsActionRow(title: it.title, sub: it.sub, tone: 2, actions: it.actions, onAct: it.act)]),   // D6/P6/P7 · באיחור ראשון
      if (todayItems.isNotEmpty) DsSection(title: gen_app_peruk02_home_c82, children: [for (final it in todayItems) DsActionRow(title: it.title, sub: it.sub, actions: it.actions, onAct: it.act)]),
      for (final r in open) GenAppPeruk02HomeScreenToday.card(context, r),
      if (pending.isNotEmpty) DsSection(title: gen_app_peruk02_home_c83 + ' · ' + pending.length.toString(), children: pending),   // D5 · תיבה ≠ היום
      if (did.isNotEmpty) DsSection(title: gen_app_peruk02_home_c84 + ' · ' + did.length.toString(), children: [for (final e in did) DsLogRow(text: e['what'] ?? '', undoLabel: gen_app_peruk02_home_c85, onUndo: () => appStore.undo(e['id'] ?? ''))]),   // T2
      if (tomorrow.isNotEmpty) DsFold(title: gen_app_peruk02_home_c86 + ' (' + tomorrow.length.toString() + ')', details: [for (final it in tomorrow) DsActionRow(title: it.title, sub: it.sub)]),   // D8 · יום-יחיד; מחר מקופל
      if (overdue.isEmpty && todayItems.isEmpty && pending.isEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text(gen_app_peruk02_home_c87 + ' ' + gen_app_peruk02_home_c88, style: TextStyle(color: lk.muted, fontSize: 14))),
    ]);
  });
}
