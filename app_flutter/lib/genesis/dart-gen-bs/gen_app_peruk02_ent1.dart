...
import '../dart-data-bs/auto/gen_app_peruk02_ent1_content.dart';
...
class GenAppPeruk02Ent1Screen extends StatefulWidget {
  const GenAppPeruk02Ent1Screen({this.scopeField, this.scopeId, super.key});

  final String? scopeField;   // G26 · היקף-הורה (ניווט-מקשרים): שדה-הקשר + מזהה ⇒ הרשימה מסוננת לרשומת-ההורה והטופס ממולא-מראש
  final String? scopeId;

  @override
  State<GenAppPeruk02Ent1Screen> createState() => _GenAppPeruk02Ent1ScreenState();
}
...
class _GenAppPeruk02Ent1ScreenState extends State<GenAppPeruk02Ent1Screen> {
  ...
  void _save() {
    ...
    final miss = <String>[];
      if ((_v[0] ?? '').trim().isEmpty) miss.add('חסר ' + gen_app_peruk02_ent1_c9);
      if ((_v[2] ?? '').trim().isEmpty) miss.add('חסר ' + gen_app_peruk02_ent1_c11);
      if ((_v[3] ?? '').trim().isEmpty) miss.add('חסר ' + gen_app_peruk02_ent1_c12);
      if ((_v[4] ?? '').trim().isEmpty) miss.add('חסר ' + gen_app_peruk02_ent1_c13);
    ...
    if (_editId != null) {
      ...
      appStore.add('app_peruk02_ent1', <String, String>{...map, '__stage': '0'});
    }
  ...
  Widget build(BuildContext context) {
    ...
    return DsScaffold(
      ...
      children: [
        AnimatedBuilder(animation: appStore, builder: (context, _) => Padding(padding: const EdgeInsets.only(bottom: 12), child: Row(children: [Expanded(child: DsStat(label: gen_app_peruk02_ent1_c0, value: appStore.count('app_peruk02_ent1').toString(), sub: gen_app_peruk02_ent1_c35, glyph: gen_app_peruk02_ent1_c36)), const SizedBox(width: 10), Expanded(child: DsStat(label: gen_app_peruk02_ent1_c11, value: appStore.sum('app_peruk02_ent1', gen_app_peruk02_ent1_c11).toStringAsFixed(0), sub: gen_app_peruk02_ent1_c31, glyph: gen_app_peruk02_ent1_c32)), const SizedBox(width: 10), Expanded(child: DsStat(label: gen_app_peruk02_ent1_c21, value: appStore.sum('app_peruk02_ent1', gen_app_peruk02_ent1_c21).toStringAsFixed(0), sub: gen_app_peruk02_ent1_c33, glyph: gen_app_peruk02_ent1_c34))]))),
        ...
        DsSection(title: gen_app_peruk02_ent1_c5, children: [
          ForgeDsField(state: (_v[0] ?? '').toString().trim().isEmpty ? ForgeDsFieldState.empty : ForgeDsFieldState.filled, fields: [gen_app_peruk02_ent1_c9, ''], control: DsField(label: gen_app_peruk02_ent1_c9, hint: '', value: _v[0] ?? '', onChanged: (v) => setState(() => _v[0] = v), bare: true)),
          ForgeDsField(state: (_v[1] ?? '').toString().trim().isEmpty ? ForgeDsFieldState.empty : ForgeDsFieldState.filled, fields: [gen_app_peruk02_ent1_c10, ''], control: DsField(label: gen_app_peruk02_ent1_c10, hint: '', value: _v[1] ?? '', onChanged: (v) => setState(() => _v[1] = v), bare: true)),
          ForgeDsNumberField(fields: [gen_app_peruk02_ent1_c11], control: DsNumberField(label: gen_app_peruk02_ent1_c11, value: _v[2] ?? '', onChanged: (v) => setState(() => _v[2] = v), bare: true)),
          ForgeDsField(state: (_v[3] ?? '').toString().trim().isEmpty ? ForgeDsFieldState.empty : ForgeDsFieldState.filled, fields: [gen_app_peruk02_ent1_c12, ''], control: DsField(label: gen_app_peruk02_ent1_c12, hint: '', value: _v[3] ?? '', onChanged: (v) => setState(() => _v[3] = v), bare: true)),
          ForgeDsDateFieldInput(fields: [gen_app_peruk02_ent1_c13], control: DsDateField(label: gen_app_peruk02_ent1_c13, value: _v[4] ?? '', onChanged: (v) => setState(() => _v[4] = v), bare: true)),
          ForgeDsField(state: (_v[5] ?? '').toString().trim().isEmpty ? ForgeDsFieldState.empty : ForgeDsFieldState.filled, fields: [gen_app_peruk02_ent1_c14, ''], control: DsField(label: gen_app_peruk02_ent1_c14, hint: '', value: _v[5] ?? '', onChanged: (v) => setState(() => _v[5] = v), bare: true)),
          ForgeDsField(state: (_v[6] ?? '').toString().trim().isEmpty ? ForgeDsFieldState.empty : ForgeDsFieldState.filled, fields: [gen_app_peruk02_ent1_c15, ''], control: DsField(label: gen_app_peruk02_ent1_c15, hint: '', value: _v[6] ?? '', onChanged: (v) => setState(() => _v[6] = v), bare: true)),
        ...
        DsSection(title: gen_app_peruk02_ent1_c6, trailing: Row(mainAxisSize: MainAxisSize.min, children: [_viewBar(context), const SizedBox(width: 8), _csvBtn(context)]), children: [
          AnimatedBuilder(
            animation: appStore,
            builder: (context, _) {
              final all = (widget.scopeId == null ? appStore.records('app_peruk02_ent1') : appStore.records('app_peruk02_ent1').where((r) => (r[widget.scopeField ?? ''] ?? '') == widget.scopeId).toList());
