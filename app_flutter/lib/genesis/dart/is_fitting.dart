...
//        fittingCats = install_engine.dart:801-806 · fittingTypes = install_engine.dart:811-813.
//        אימות-עוגן: ‏install_engine.dart:816-818 =
//          `bool isFitting(p, fittingCats: fittingCats, fittingTypes: fittingTypes) => fittingCats.contains(p.categoryHe) ||
//           (companyCatalogActive && fittingTypes.contains(p.productType));`
//        — שני סעיפים (הסעיף-השני נוסף ב-main מול ה-snapshot הישן).
// טוהר: פונקציית top-level עצמאית, אפס import פנימי (רק שפה/סטנדרט).
//       ‏fittingCats + fittingTypes = דאטה-קבוע פנימי של האטום (רשימות-מונחים,
//       לא הקשר/זהות/סוד).
//
...
//   הקטגוריה שלו לעולם אינם תואמים ל-fittingCats. התקנים תפקודיים (ברזים/מחלקים/
//   משאבות) אינם כאן.
//
...
/// catalog, whose category names never match [fittingCats]. Gated on
...
bool isFitting(FittingPart p, {bool companyCatalogActive = false, required Set fittingCats, required Set fittingTypes}) =>
    fittingCats.contains(p.categoryHe) ||
    (companyCatalogActive && fittingTypes.contains(p.productType));
