...
/// ProgressRing — seam:fields · 1 חריצים
class ForgeProgressRingFeedback extends StatelessWidget {
  ...
  static const List<String> fieldDemo = <String>["65%"];   // תוכן-העיצוב פר-חריץ — מלמד את המחולל את צורת-החריץ (מספר/טקסט), לא ערך
  ...
  const ForgeProgressRingFeedback({super.key, this.fields, this.child});
  @override
  Widget build(BuildContext context) {
    final skin = DsSeam.skinOf(context);   // מלוא-העיצוב מהחריץ
    final theme = DsSeam.of(context);       // אקצנט (מורף)
    final fonts = DsSeam.fontsOf(context);  // פונט
    final Widget body = _withChild(Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.center, spacing: 9, children: [SizedBox(width: 56, height: 56, child: CustomPaint(painter: _SvgScene([_Op.circle(28, 28, 24, skin.raised2, false, 6), _Op.circle(28, 28, 24, theme.a, false, 6)], 56, 56))), Text(_f(0, "65%"), style: TextStyle(color: skin.mut, fontFamily: fonts.grotesk, fontFamilyFallback: [fonts.he], fontSize: 11))]), child);
    return body;
  }
}
