...
/// DsEnumField — seam:collection · 1 חריצים
class ForgeDsEnumField extends StatelessWidget {
  ...
  const ForgeDsEnumField({super.key, this.fields, this.child, this.control});
  @override
  Widget build(BuildContext context) {
    final skin = DsSeam.skinOf(context);   // מלוא-העיצוב מהחריץ
    final fonts = DsSeam.fontsOf(context);  // פונט
    final Widget body = _withChild(Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, spacing: 6, children: [SizedBox(width: double.infinity, child: Text(_f(0, "Label"), textAlign: TextAlign.right, style: TextStyle(color: skin.mut, fontFamily: fonts.he, fontSize: 11, fontWeight: FontWeight.w600))), Stack(clipBehavior: Clip.none, children: [(control == null ? Directionality(textDirection: TextDirection.rtl, child: SizedBox(width: double.infinity, child: Container(height: 44, constraints: const BoxConstraints(minHeight: 44), padding: const EdgeInsets.fromLTRB(13, 0, 38, 0), decoration: BoxDecoration(color: skin.sunken, border: Border.all(color: skin.hair), borderRadius: BorderRadius.circular(11)), child: Align(alignment: Alignment.centerRight, child: Text("Option", style: TextStyle(color: skin.ink, fontFamily: fonts.he, fontSize: 13)))))) : Padding(padding: const EdgeInsets.fromLTRB(13, 0, 38, 0), child: control!)), Positioned(right: 12, child: SizedBox(width: 16, height: 16, child: CustomPaint(painter: _SvgScene([_Op.path("M6 9l6 6 6-6", skin.mut, false, 1.8)], 24, 24))))])]), child);
    return body;
  }
}
