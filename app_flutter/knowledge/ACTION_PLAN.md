...
> נכון ל-v5.59 · ענף `claude/whats-happening-LyY9G` · 9 commits מקומיים, **לא דחופים**
> (ממתינים ל"תדחוף" מפורש). זהו האינדקס של העבודה הפתוחה — מתעדכן עם כל סשן.

מקרא: 🔴 חוסם / דורש הכרעת משתמש · 🟧 פתוח · 🟩 בוצע (לעיון) · ⛔ חסום על דאטה/שרת.

---

## 0 · 🔴 הכרעה פתוחה — זרימת הניווט לחלוקה (שאלת המשתמש)
...
|---|--------|--------|---------------|
| 1 | חלוקת מים נקיים / שפכים | 🟩 בוצע (v5.59) | `productDivisionSystems` + `nodeHasSystem` + כניסת מחלקה. *תלוי בהכרעת סעיף 0.* |
...
| 6 | autocomplete לחיפוש | 🟧 פתוח | `catalog_screen.dart` search panel — השלמה תוך-כדי-הקלדה מעל `kLipskeyCatalog`/`kCatalogProducts`. תשתית קיימת (`fuzzySearchProducts`). |
...
- 🟧 **הסרת sysOpt כפול מגיליון הפילטרים** — `catalog_screen.dart` filter sheet עדיין
  מכיל "כל המערכות / מים נקיים / שפכים" (מ-ceca667, הגישה שנדחתה). מיותר עכשיו
  שהחלוקה דרך מחלקות. **להסיר אחרי הכרעת סעיף 0** (אם אופציה 2 — אולי נשמר את החיווט).
