...
_Version label: `v6.02` (see `lib/version.g.dart`, generated by `scripts/gen_version.sh`).
...
instead of the last "בקרוב" placeholder). **The manager screen is now COMPLETE — ZERO "בקרוב"
placeholder remains in any tab** (the `_TabPlaceholder` class was removed entirely). A faithful port of
the legacy `renderMgrManage` (@index.html:16645-16890), an accordion (only one section open at a time,
mirroring `mgrManageOpen`) of 5 tools with the verbatim legacy emoji + titles + sub-titles:
- **🗂️ קטגוריות** → the LIVE catalog category list: `ref.watch(managerAnalyticsProvider).catalogCategories`
  (the same TREES-by-`cat` tally the legacy SECTION 3 builds @16716, sorted by count desc), header
  `קטגוריות פעילות (N)` + a `<cat> · <count> מוצרים` row per category + the verbatim hint
  "שינוי שם קטגוריה מעדכן את כל המוצרים שבה.". LIVE — the same map the 📊 dashboard reads.
- **⚙️ הגדרות אפליקציה** → the contractor-app config rows VERBATIM from the legacy constants (SECTION 4
  @16733): תוספת משלוח אקספרס=₪80 (`EXPRESS_FEE` @11961) · מסגרת אשראי לקבלן=₪50,000 (`creditLimit`
  @11963) · שיעור מע״מ=18% (`VAT_RATE` @11941) + the verbatim hint. Display-only (the legacy
  `prompt()`-edit has no backend here).
- **🌳 עץ המוצרים** → an inline summary of the catalog product-tree (the legacy SECTION 1 manages the
  per-product accessory tree via a `prompt()`-driven backend that has no equivalent here): the verbatim
  section purpose + the live tree size (מוצרים בעץ / קטגוריות, from the same analytics map). NO invented edit.
- **🏷️ מותגים ומחירים** → the brands list from `lib/data/brands.dart` (`kBrands`): each brand's
  `emoji name` + tagline + product count (when known); header `מותגים (N)`.
- **🔬 בדיקות רגרסיה** → a `brand` action button that routes to the existing `RegressionPanelScreen`
  (`Navigator.push(RegressionPanelScreen.route())`) — the same target the old manager dial used.
- **LIGHT only** — white `cardLight` accordion cards on `bgLight`, `inkLight`/`mutedLight` text,
  `brand` accents (the intro banner + the regression button). NO dark tokens.
- **Scope** — ONLY the 🛠️ tab body + the route call to `RegressionPanelScreen`. The engine internals,
  the other 3 tabs, the role picker, and the buyer/checkout flow are untouched.
Guard `manager_dashboard_screen_test` grows by 7 M5 tests (intro + 5 tool headers render · 🗂️ shows the
LIVE category list with real counts · ⚙️ shows the verbatim config rows · 🌳 shows the inline tree
summary · 🏷️ lists the kBrands brands · 🔬 routes to `RegressionPanelScreen` · manage tab is
LIGHT/no-dark); the placeholder test now asserts **NO "בקרוב" remains anywhere** (screen complete). All
existing manager + engine tests stay green (screen 30 + dashboard 12 + engine 21).

## 👔 Manager rebuild — M4: 👥 לקוחות tab — live customer list + credit (v6.01)
The 👥 לקוחות tab of `ManagerDashboardScreen` is filled with the manager's **LIVE customer list +
credit**, derived from the SAME shared orders engine (`lib/screens/manager_dashboard_screen.dart` —
the `IndexedStack` index-2 body is now `_CustomersTab` instead of a "בקרוב" placeholder; only 🛠️
ניהול stays a placeholder for M5). A faithful port of the legacy `renderMgrCustomers`
(@index.html:16566-16607) + `mgrCustomerDetail` (@16609-16643):
- **Live customer list** — `ref.watch(managerCustomersProvider)` (the engine's orders grouped by
  buyer `who` via `mgrCustomerList`), so the list is LIVE: a new contractor order placed on the
  engine (by ANY role) **adds or updates a customer card here**. Each `_CustomerCard` (the legacy
  `mc-card`): `👷 name` + `N הזמנות · M אתרים` (M = distinct build-sites per buyer, derived from the
  live orders like the legacy `byName[nm].sites` set) + a status pill, then a **credit-utilisation
  bar** + the line `ניצול אשראי: ₪used / ₪limit (pct%)`. `pct = min(100, round(spend/credit*100))`;
  the credit ceiling is `contractorCredit` (the deterministic hash already in the analytics layer).
  Status (verbatim @16562/16592): **פעיל** (0<pct<90, green) / **⚠️ אשראי גבוה** (pct≥90, amber) /
  לא פעיל (pct=0, grey). A 3-stat `_CustomerSummary` (קבלנים / סך רכש ₪ / ניצול אשראי %) sits on top.
- **Status filter** — a `_CustomerStatusChips` row (`הכל (N)` + a פעיל / אשראי גבוה chip per populated
  status) narrows the list; an emptied filter falls back to הכל. Empty result → the legacy
  "לא נמצאו קבלנים תואמים." line (@16586).
- **Customer-detail bottom sheet** (`mgrCustomerDetail` @16609-16643) on card tap — 👷 + name + a
  status tag, an orders/spend/pct grid, the credit rows (מסגרת אשראי / נוצל / יתרה זמינה / אתרי
  בנייה), and the contractor's own orders (📦 id · ₪sum · stage pill), all off the same live engine.
- **LIGHT only** — white `cardLight` cards on `bgLight`, `inkLight`/`mutedLight` text, `brand` accents;
  green for active / amber for high-credit. NO dark tokens.
- **Scope** — ONLY the 👥 tab body. The engine internals, the other 3 tabs, the role picker, and the
  buyer/checkout flow are untouched; the logic layer (`mgrCustomerList`/`ManagerCustomer`/
  `contractorCredit`) is read, not changed.
Guard `manager_dashboard_screen_test` grows by 6 M4 tests (customers render with credit lines · each
card carries a credit bar · KEYSTONE: placing a new contractor order on the engine ADDS a customer
card live · a >90% contractor shows "⚠️ אשראי גבוה" + the filter narrows to them · the detail sheet
shows limit/used/balance/sites + the contractor's orders · customers tab is LIGHT/no-dark); the
placeholder test now asserts the single remaining "בקרוב" tab (🛠️). All existing manager + engine
tests stay green (screen 35 + dashboard 12 + engine 21).
