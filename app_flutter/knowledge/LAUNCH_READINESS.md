...
5. **שלב-5 (הוכחת-שכפול) — מוכן-להרצה (2026-07-25):** ‏GitHub → Actions → **`clean-two-links`** → Run workflow. שני jobs מאותו commit: `clean` (BuildSmart Clean · קטלוג-v1) ו-`company2` (**BuildMax** · קטלוג-v2 עם +789 מוצרי-Huliot — הבדל גלוי בשם+בקטלוג). כל job מדפיס את **הלינק-החי שלו** ב-Run Summary; הפריסה לערוצי-preview נפרדים — **האתר-הראשי לא-נגוע.** לתעד כאן את 2 הלינקים אחרי הריצה: ▢ clean: ______ ▢ company2: ______
