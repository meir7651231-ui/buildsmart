...
67. **גרסת-Flutter ב-CI חייבת לספק את `environment.sdk` של pubspec — pin של patch ישן = `pub get` נכשל ב-CI בלבד.** `protocol-enforce.yml` קיבע `flutter-version: '3.29.0'` (Dart 3.7.0), אבל `pubspec.yaml` דורש `sdk: ^3.7.2` → "version solving failed" ב-CI, בעוד מקומי (3.29.**3** = Dart 3.7.2) ו-deploy.yml (3.44.0) עוברים. התוצאה: "Protocol Enforcement" אדום על הענף **בלי באג-קוד אמיתי** — בדיקה מקומית ירוקה מסתירה את זה. תוקן: pin → `3.29.3` (תואם מקומי + catalog-qa.yml). כללים: (א) **כל ה-workflows חייבים גרסת-Flutter אחת עקבית** (או ≥ המינימום ש-pubspec דורש) — drift בין pins = red-X מתעתע; (ב) "ירוק מקומי ≠ ירוק ב-CI" כשהסביבות שונות — בדוק `flutter --version` מול ה-pin ב-CI לפני שמכריזים "פרוס"; (ג) שדרוג `environment.sdk` ב-pubspec מחייב סריקת **כל** ה-workflows ל-pin תואם.
