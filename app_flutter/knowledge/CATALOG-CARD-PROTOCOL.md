...
### §14.E — recoverability: "מוסתר-בתצוגה" ≠ "נמחק-מהמודל" (דיווח קטלגן 2026-06-01)

כל **פילטר-תצוגה** שמסתיר token מהעין (כמו `_isNoiseChip`, parens-stripper של
`_chipDisplayLabel`) **חייב** בדיקה התנהגותית שמראה שה-token עדיין **שחזורי
מהמודל** (lossless) — שהשם המלא ניתן לשחזור מהציפ/המודל למרות ההסתרה.
**למה:** §21.A הסתיר "מ\"מ" ב-UI; ה-parser עדיין סיווג אותו (עבר את §14 "אין
leftover"), אבל הוא נעלם מהשחזור — נתפס רק ב-E2E (§21.B). השומרים הקיימים בודקים
"אין leftover", **לא** "אין דליפה בשחזור". הכלל מכליל מעבר לציפים: כל UI שמוסיף
filter על דאטה חייב test שמוודא שמה שהוסתר-מהעין לא נעלם-מהמודל.
...
### §21.A — באגים שתוקנו ב-2026-06-01 (הציפ "בלגן") + יישום-להבא

הכרטיס החיצוני נראה מבולגן לכל קטגוריות הברכיים/תיים. שלושה באגים + הכלל למניעה:

1. **זווית-נבלעת-כגודל** — `ברך 45° פ.פ 160` נתן `[פ.פ, 45°]` והקוטר 160 נעלם:
   ה-`sizeRe` (מתחיל-בספרה) תפס את "45°" כ-size. **תיקון:** `sizeRe` מדלג על
   shape-tokens מוצהרים (`&& !kChipLevel2Shape.contains(t)`).
   **יישום-להבא:** size detection לעולם לא לפני vocabulary של shape.
2. **קוטר-כצורה** — bare `'45'`/`'90'` ב-`kChipLevel2Shape` גנבו קטרים 45/90mm.
   **תיקון:** זוויות תמיד עם ° → רק `'45°','90°'` ב-shape.
   **יישום-להבא:** token דו-משמעי (מספר שגם זווית וגם קוטר) → לסמוך על ה-° כמבחין.
3. **רעש-תצוגה** — סוגריים `(ציפוי כרום)` ויחידת `מ"מ` הוצגו כ-chips.
   **תיקון תצוגה בלבד** ב-`_HierarchyChips`: `_chipDisplayLabel` מסיר סוגריים
   עוטפים, `_isNoiseChip` מסתיר יחידות. nameHe נשאר verbatim (R8), ה-path
   ל-matching נשאר, ו-tap index ממופה חזרה לרמה הנכונה.
4. **ביטוי-רב-מילים-מתפזר** — `לוחית למיקום נקודת מים` → `[מים, למיקום, נקודת]`
   (סדר משובש), כי "מים" קיים ב-L2. **תיקון:** הוספת `'למיקום נקודת מים'`
   כ-compound ב-`_l3Compounds`. **יישום-להבא:** ביטוי תיאורי שמילה ממנו קיימת
   ברמה אחרת → חייב compound, אחרת מתפזר ומאבד סדר.

**§14 שמירה (כולן mutation-verified):**
- "§21 angle fittings keep the diameter as size, angle as shape"
- "§21 multi-word phrase stays one ordered chip (no scatter)"
- "chip hierarchy parser: no leftover tokens, every product has a type"

**מצב סופי:** 774/774 — 0 leftover · 0 ללא-type · 21 סוגים · breadcrumbs קריאים
בכל הקטגוריות.

### §21.B — E2E recoverability: השם המלא חייב להישחזר מהציפ החיצוני בלבד

**הבדיקה (לבקשת המשתמש):** קח שם מלא מהקטלוג של פולירול → `parseChips` →
שחזר אותו רק מ-`[type] + breadcrumb path + material badge` → השווה את
set-המילים למקור. אם משהו חסר, הציפ הוא lossy = המשתמש לא יכול לשחזר את
המוצר מהכרטיס החיצוני.

**למה זו הבדיקה הנכונה:** §21 ו-§21.A מאמתים *נכונות מקומית* (זווית לא נבלעת,
ביטוי לא מתפזר). הם לא מאמתים *שלמות*: שאף token לא נופל בין הכיסאות. בדיקת
ה-E2E היא המבחן ההוליסטי — "האם הכרטיס lossless?".

**הממצא (9/10 בריצה ראשונה):** הכלי `מזוודת ריתוך קטנה 20-63 מ"מ` איבד את
`מ"מ`. הסיבה: היחידה לא התחילה בספרה (לא נתפסה כ-size), ישבה ב-`kChipLevel3Feature`,
ושכבת התצוגה הסתירה אותה כ-"noise" (§21.A bug 3). התוצאה: היחידה נעלמה גם
מה-breadcrumb הנראה וגם מהשחזור → הגודל נקרא "20-63" במקום "20-63 מ"מ".

**התיקון:** `_kChipUnits = {'מ"מ','מ”מ','mm'}` + ענף שמקפל את היחידה *לתוך*
ציפ-הגודל (`l5 = '$l5 $t'`) במקום לזרוק/להציג בנפרד. הוסר `'מ"מ'` מ-L3Feature.
אחרי התיקון: 10/10, ועל כל הקטלוג **774/774 שחזור מלא**.

**יישום-להבא (העיקרון):** parser של ציפים נחשב "שלם" רק אם השם המלא של הקטלוג
ניתן לשחזור מהציפים+badge (lossless). כל token של יחידה/מכמת חייב לנחות במקום
שניתן לשחזר ממנו — לעולם לא נזרק בשקט ולא מוסתר-כ-noise. "noise" בתצוגה ≠
"נמחק מהמודל": אם הסתרת token מהעין, הוא עדיין חייב להופיע בשחזור (מקופל לציפ
אחר), אחרת הכרטיס lossy.

**§14 שמירה (mutation-verified):**
- "§21.B every Polyroll name is fully recoverable from the chips"

### §21.C — תוויות סמנטיות לציפים ולכותרת הבורר (UX: ראשי/משני/אחרון)

**הבעיה (דיווח-משתמש):** *"אני נכנס לבורר בציפ אני לא יודע מה הוא בורר ראשי
ומה משני ומה אחרון — זה בבלגן."* הציפים נראו כולם אותו דבר (pill אפור זהה,
חוץ מציפ-הגודל הכתום), וכותרת הבורר הציגה generic `'בחר ערך:'` בלי לציין
*איזה ערך*. כך לא היה שום אינדיקטור על המיקום של הציפ בהיררכיה.

**הפתרון:** מיפוי `chipIndex → label` סמנטי, עם 5 תוויות קבועות (אחת לכל
רמה ב-§21):

| רמה | תווית | משמעות |
|---|---|---|
| L1 | חיבור | שיטת חיבור (ריתוך / הברגה / ריתוך חשמלי / רקורד) |
| L2 | צורה | זווית / מצרה / שווה / כדורי / מעבר ישר / משושה / וכו' |
| L3 | תכונה | משטח ריסון / ציפוי כרום / ללא ידית / לעבודה בגובה |
| L4 | תבריג | פ.פ / פ.ח / ח.ח / חוץ / פנים / שקע-תקע |
| L5 | מידה | הגודל (תמיד הציפ האחרון = הצר ביותר) |

**איך זה מתבטא ב-UI:**
1. כל pill מקבל **תווית זעירה אפורה (9pt)** מעליו: "חיבור / צורה / ...".
   המשתמש רואה את ההיררכיה במבט אחד — בקריאה RTL, "חיבור" יושב מימין (ראשי),
   "מידה" משמאל (אחרון).
2. **כותרת הבורר** משתמשת באותו label: `'בחר חיבור:'` / `'בחר צורה:'` /
   `'בחר תכונה:'` / `'בחר תבריג:'` / `'בחר מידה:'`. אין יותר generic
   `'בחר ערך'`.

**API:** `ChipPath.levelLabelOf(int pathIndex) → String` ב-`chip_hierarchy.dart`
מחזיר את התווית. ה-UI (`_HierarchyChips`, `_hierarchyPickerTitle`) קורא אותו.

**יישום-להבא:** כל ציפ באב-טיפוס היררכי חייב לשאת **תווית-שם של רמה**, לא רק
ערך — אחרת השכבות הופכות "שטוחות" מבחינת תפיסת המשתמש (chunks ללא מודל).
לפני שמייצרים breadcrumb מציפים מפורסרים, יש להוסיף `levelLabelOf` (או מקבילה)
ולהציג אותה. *הציפ אינו עומד לבד — הוא חי על מערכת היחסים שלו לרמה.*

**§14 שמירה (mutation-verified):**
- "§21.C every visible chip carries a semantic level label" — סורק את כל
  `kPolyrollCatalog`, מאמת שכל ציפ נראה מקבל אחת מ-5 התוויות (אין ריק / אין
  המצאה), ושציפ-הגודל תמיד נקרא "מידה" (העוגן ל-leaf).
...
| p25 ברך 90° תבריג brass | "משטח ריסון" / "חיצוני" / "פנימי" (3 different diagrams) | `nameHe.contains('משטח ריסון' / 'חיצוני')` | `spec_elbow_90_p25_damper.jpg`, `spec_elbow_90_p25_external.jpg`, `spec_elbow_90_p25_internal.jpg` |
| p26 מסעף תבריג brass | "תבריג פנימי" / "תבריג חיצוני" | `nameHe.contains('חיצוני')` | `spec_tee_p26_internal.jpg` / `spec_tee_p26_external.jpg` |
| p27 מתאם עגול תבריג | "פנימי" / "חיצוני" | `nameHe.contains('חיצוני')` | `spec_adapter_round_p27_internal.jpg` / `_external.jpg` |
| p28 מתאם משושה תבריג | "פנימי" / "חיצוני" | `nameHe.contains('חיצוני')` | `spec_adapter_hex_p28_internal.jpg` / `_external.jpg` |
