...
## בית (Finder / מסך הבית) — `lib/screens/finder_screen.dart`

### 1. מזהה ומיקום
מוטמע כ-section `'בית'` בקטלוג (`_CatalogBody`: `active=='בית' ⇒ FinderScreen`) — **נחיתת ברירת המחדל של האפליקציה** (`catalogSectionProvider` ברירת מחדל `'בית'`). אייקון section: `Icons.home_outlined`. מופיע גם ב-`kSearchIndex` (`🏠 בית · קטלוג`).
...
### 3. מבנה ופריסה
שלב 1 — רשימת קבוצות (`_typeList`, שורות WhatsApp: עיגול-אימוג'י 54 + תווית 17 + תיאור בשפה פשוטה (`FinderGroup.desc`) + תג מספר-מוצרים כתום, באותו עיצוב כמו שורות הקטגוריות בקטלוג). שלב 2 (אחרי בחירה) — `_header` עם שביל-פירורים (`קבוצה › תת-סוג › גודל`, לחיצה = צעד אחורה) + שורת `_subBar` (תווית קבועה "סוג") + שורת `_sizeBar` (תווית דינמית: גודל/צבע/דגם/אפשרות) + מונה "נמצאו N מוצרים" + `LipskeyProductsList`.

### 4. נתונים ולוגיקה
- **קבוצות:** `kFinderGroups` — 8 קבוצות בשפה פשוטה + `'אחר'` (catch-all, `cats` ריק ⇐ קטגוריות לא-משויכות). מסודרות לפי חשיבות ללא-טכני (ברזים→אסלות→מקלחת ואמבטיה→ניקוז→צינורות→גינה→מחברים→חבקים→אחר). זרות הדדית. כל קבוצה נושאת `desc` (תיאור חד-שורתי בעברית פשוטה) המוצג בשורת הקבוצה.
...
### 5. בדיקות
מכוסה ב-`test/wiring_test.dart` וגם בכפתור "רגרסיה מלאה" התוך-אפליקטיבי (`test_harness/tests/finder.dart`): קבוצות זרות + catch-all; תיאור לכל קבוצה (`desc`); תת-סוגים מכסים כל קטגוריה עם תוויות ייחודיות; חיפוש סלחני (`catalogProductMatchesQuery`) + דירוג (`searchRelevance`). `test/hard_tests.dart` מאמת בנוסף שנחיתת ברירת המחדל = `'בית'` ושלכל קבוצה יש `desc`.
