...
מסך הקטלוג הוא נקודת הכניסה לגילוי ולעיון במוצרי בנייה: המשתמש מחפש מוצרים/קטגוריות/מסכים, מנווט (drill) בעץ קטגוריות ובעץ-חכם עד למוצר, מסנן לפי facets, ומוסיף לסל דרך גיליון מוצר. בנוסף הוא מארגן את התצוגה לרשימות section מותאמות אישית (כולל מועדפים, חיפושים אחרונים ותכנון חיבור).
