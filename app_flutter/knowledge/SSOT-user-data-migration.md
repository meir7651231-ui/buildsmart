...
| tasks_engine (משימות §6) **cross-party** | `tasks/{taskId}` (`TaskItem.toJson` minus id) | 🟡 **יסוד חי** (Wave T3: `FirebaseTasksRepository` — toDoc/fromDoc מיחזור toJson · seed · sortBy · all() · test 9 · mutation-verify · stage2). **חיווט cross-party = increment 2** (scoped-provider 3-תפקידים · 6-state rules · indexes · bindRemote · UID-plumbing ב-UI) |
