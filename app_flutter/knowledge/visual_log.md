...
## #darkmode-wave19 — 8 files (onboarding/nav_launch/tree_pane/finder/catalog_wheel/home_atoms/updates/order_notif) (2026-08-23)
**גל 19 (8 fixers):** 8 light-surfaces → תמה, 0 דולגו. cardLight/white→`colorScheme.surface` · bgLight→`scaffoldBackgroundColor` · surfaceMid→`surfaceContainerHighest`. tree_pane: `_rowWidget` הושחל context (itemBuilder `(_,i)`→`(context,i)`). nav_launch (service) ו-home_atoms (atom) — context מ-build זמין. `flutter analyze` 0 errors. **אטלס 30→22** (‎-8). מצטבר 659→22 (637 תוקנו).
**החרגה קבועה חדשה שזוהתה:** `edit_safety.dart:132` — `const Color _kRefSurface = BsTokens.cardLight` הוא **קבוע-ייחוס לבדיקת-ניגודיות** (readability SSOT ב-lib/logic/, ללא context, נעול ב-test/studio/safety_test.dart) — לא משטח מרונדר. נשאר קשיח בכוונה (כמו signature_pad canvas).

## #darkmode-wave20 — 8 files (dialogs + word_finder + sudoku_grid) (2026-08-23)
**גל 20 (8 fixers):** 9 light-surfaces → תמה, 0 דולגו. reject_reason_dialog · confirm_dialog · quick_pad_screen · word_finder_home · studio_entry · quick_pad_keyboard · word_keyboard · sudoku_grid(2). white/cardLight→`colorScheme.surface` · surfaceMid→`surfaceContainerHighest`. **sudoku_grid:** שני הלבנים אומתו כ-chrome (אריח-fitting מונח + עיגול-הצעה), לא game-state → הומרו. `flutter analyze` 0 errors. **אטלס 22→13** (‎-9). מצטבר 659→13 (646 תוקנו).
**🏁 שלב ה-fan-out הסתיים.** 13 הנותרים = מקרים-מיוחדים בלבד: 3 החרגות-קבע (signature_pad canvas ×2 · edit_safety reference) · 5 נעולי-golden (bs_keyboard ×4 · smart_chip_strip) · 4 קבצי-תמה (config_theme ×3 · app_theme) · 1 CustomPainter (help_target:464).

## #darkmode-wave21 — golden keyboards + CustomPainter + engine-exclusions → אטלס 0 🏁 (2026-08-23)
**גל 21 (סיום):** 6 light-surfaces אחרונות → תמה + הכרה-במנוע בהחרגות-לגיטימיות.
- **bs_keyboard (4) + smart_chip_strip (1):** מפתחות/תאי-מקלדת `Colors.white`/`cardLight` → `colorScheme.surface`. **golden-PNG:** `kb_golden_test._frame` עבר ל-`AppTheme.light()` (שם surface==לבן) — ה-golden **חודש** (RUN_GOLDENS=1 --update-goldens) ו**אומת ויזואלית** (kb_full.png: מפתחות לבנים · אותיות עבריות קריאות · כפתור-שליחה כתום-מותג · צ'יפ ראשי כתום). המקלדת נראית זהה ב-light ומוכנה ל-dark. הטסט opt-in (RUN_GOLDENS=1) — מדולג בשער, לא חוסם.
- **help_target:464 (`_TailPainter` CustomPainter):** הושחל שדה `Color surface` מה-widget (line 257, `Theme.of(context).colorScheme.surface`) → זנב-הבועה תואם עכשיו לגוף-הבועה (שכבר theme-aware מגל 10) + `shouldRepaint` עודכן.
- **מנוע-האטלס (`tools/atom/decompose/bin/colors.dart`):** נוסף `_isExcludedSurface` — (א) קבצי `lib/theme/` (הגדרות-התמה legitimately מקשיחות את פלטת-האור: `config_theme` ×3 defaults של CfgTheme · `app_theme:43` הענף-הבהיר של `isDark?bgDark:bgLightAlt`); (ב) מרקר `// atlas:ignore` מפורש. **מסומנים:** signature_pad:206,440 (canvas לכידת-חתימה, חייב לבן ל-PNG) · edit_safety:134 (`_kRefSurface` קבוע-ייחוס-ניגודיות, נמדד מולו ולא מרונדר).
- **אימות:** `flutter analyze` (כל lib) 0 errors · signature_pad_test 8/8 · kb goldens 4/4 (RUN_GOLDENS=1).
- **🏁 אטלס: 7→0 LIGHT-SURFACE offenders.** מצטבר: **652 משטחים הומרו ל-theme-aware ב-21 גלים** (659 מקוריים − 7 החרגות-לגיטימיות שזוהו). מצב-כהה שלם: כל משטח שאמור להיכהות — נכהה; כל לבן שנשאר לבן — לגיטימי ומתועד.
