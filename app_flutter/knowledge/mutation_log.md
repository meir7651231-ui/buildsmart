...
## #hr-courier-ledgers — courier attendance/certs/forms → courier{Attendance,Certs,Forms}/{uid} (OFF-safe) (2026-08-13)
- **הנכס:** מאגרי-HR של השליח (נוכחות / ארנק-רישיונות-נהג / 101+מחלה) — התאומים courier-keyed של מאגרי ה-worker. **מיחזור מחלקות** — אותן WorkerAttendanceRepository/WorkerCertsRepository/WorkerFormsRepository (הצורה זהה), רק providers חדשים המגדרים על `role == courier` וכותבים ל-`courier*` collections. **SELF-ONLY** בשרת: השליח מדווח לחנות דרך CHAT → אין store-roster query. אפס-רגרסיה כבוי (3 providers → null OFF → SharedPreferences `bs.courier-*.v1`).
...
## #hr-vacation-cross-party — vacation_requests → vacationRequests/{requestId} (CROSS-PARTY, OFF-safe) (2026-08-13)
- **הנכס:** מאגר-HR תשיעי — **הראשון cross-party**: עובד/שליח מגיש, **מנהל/קבלן-מעסיק מאשר** (status flip). דוק per-request (`vacationRequests/{requestId}`, לא לפי-uid — צד אחר כותב את ההחלטה). `username`==uid (owner key בשרת), `employerId`==uid-קבלן. scopes: worker/courier `where username==uid` (bound 200) · manager הכל (bound 500) · employer roster `where employerId==uid` (bound 500). אפס-רגרסיה כבוי.
- **אימות:** `vacation_requests_repository_test` — OFF-null + submit→create (username/employerId/status=pending) + decide→**merge** (status flip בלי לאבד owner/employer keys) + decodeDocs flatten (שורה פסולה מדולגת). vacation_requests + contractor_vacation_approval + hr_decide_once + stage2 (כל 3 ה-sources עם bound:) ירוקים. analyze 0 (2 info comment_references קיימים-מראש/employerUid).
- **mutation-verify (בוצע בפועל):** גיבוי → מוטציה ב-`loadScoped`: `decodeDocs(...)` → `const []` → RED בשני טסטי submit+decide (`+... -2`) → שחזור (RESTORED-IDENTICAL) → GREEN 4/4.
- **אבטחה (cross-party — הכלל הוא ההגנה):** `vacationRequests/{reqId}` — **create** רק המגיש (`username==auth.uid`, `status=='pending'` — אין הגשה בשם אחר/לא-pending) · **read** מגיש/מעסיק/מנהל בלבד · **update** רק מעסיק (`employerId==auth.uid`)/מנהל, ו-`username`/`employerId` **קפואים** (העובד לא יכול לאשר לעצמו ולא לשנות-בעלות) · **delete** אף פעם מ-client. `deleteAccount.ts`: `purgeOwnedVacationRequests` — מחיקת-doc מלאה לפי `where username==uid` (paginated), כי הדוק לפי reqId ולא uid.

## #hr-courier-clock — courier_clock → courierClock/{uid} (single-doc, self-only, OFF-safe) (2026-08-13)
- **הנכס:** מאגר-HR עשירי — שעון-המשלוחים של השליח (`{orderId:{pickedUpAt,deliveredAt,attempts}}`, מדידת best-effort). ה-writer הוא **free-function** `stampCourierClock` (לא notifier) → הושחל `repo:` דרך 3 נקודות-advance + ענף-שרת ב-`courierClockProvider` (reader). דוק יחיד `courierClock/{uid}={entries,updatedAt}`. אפס-רגרסיה כבוי (repo==null → SharedPreferences).
