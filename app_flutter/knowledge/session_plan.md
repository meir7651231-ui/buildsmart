...
# 👔 Manager unify — control-center + live engine onto the LIVE app (Path B)
Owner: this session
Scope: cherry-pick the COMPLETE manager + shared orders engine onto the live app (contractor
+ worker + store + courier already present). חדש בלבד: `orders_engine.dart` · `manager_dashboard.dart`
· `manager_dashboard_screen.dart` · `manager_dashboard_state.dart` + manager-leaf hunks ב-`bs_dial_widget.dart`/`dial_state.dart`. שמירה על פרוטוקול ה-LIVE (`.githooks` שלו) — שום proto-build protocol.
- `role_picker_sheet.dart`: manager → `ManagerDashboardScreen.route()` לצד worker → `WorkerAppScreen.route()` (שני ה-role-app entries נשמרו).
- DROP של proto-build protocol artifacts: `dx_friction_test.dart`, `stuck_regression_test.dart` (live's restored), `protocol/pins.sha256`, `generate_stuck_regression.sh` (live's restored).
- אימות: `flutter analyze` 0 errors · manager-tests ירוק. contractor+worker+manager מתקמפלים יחד — הבסיס ל-cross-persona wiring הבא.
