...
| E1 | ✅ (v6.x) מזג‑אוויר אמיתי — Open‑Meteo GPS (`services/weather.dart`, בלי מפתח) | agent |
| E2 | שערי מטבע: API + המרה בכל המחירים (הבורר קיים) | agent |
| E3 | ✅ **נבנה+פעיל (23/6)** AI אמיתי — Claude ב‑`functions/claude.ts` (במקום canned) + ~15 ✨ + agentic · דורש `ANTHROPIC_API_KEY` בקונסול | אתה(קונסול)+agent |
...
**5 החוסמים הגדולים (עדכון 23/6):** (1) ✅ זהות‑uid בהזמנות+צ׳אט — **נסגר** (S2) · (2) תשלום — החלטה פתוחה (v1 בלי‑סליקה?) · (3) ✅ AI hub — **נבנה+פעיל** (Claude) · (4) ✅ מצלמה/POD — אמיתי · (5) קטלוג — אינסטלציה (סקופ‑בעלים), הועשר R8. **רוב החוסמים נסגרו; נותרו החלטות‑בעלים + לוגיסטיקת‑השקה.**
