...
> ✅ **אומת end-to-end (קריאה-עצמית של המסך):** `_assemble` קורא `autoFlowFix`→`buildTreeInstallation`/`buildInstallation`(autoCompliance:true)→`plan.criticalOpen`→דיאלוג-אזהרה(🔴 critical-compliance) או `_BomSheet`. auto-tick HW-CLIP/SEALANT (+HW-INSUL כשtemp≥60). `_saveProject`→`savedProjectsProvider.save(name,anchorSkus,tempC,accessories)` (load/rename/delete). זיהוי-manifold דרך `manifoldOutlets(p)>0`. **המסך = UI נאמן מעל המנועים שנקראו — הטענות מדויקות.**
