...
⭐ **קטלוג-אמת: 1,877 מוצרים** ב-`kCatalogProducts` = `[...kLipskeyCatalog, ...kPolyrollCatalog, ...kHuliotCatalog]` (spread ללא-dedup, אומת ע"י `ppr_infra_test`) = **Lipskey 935 + Polyroll-PPR 772 (756 `_ppr`+16 `_acPipe`) + Huliot-SmartLock 170 `_sl`**. ⚠️ **ספירה אומתה שורה-שורה בקוד (935/772/170=1,877).** ⚠️ ה-HW-מים-חמים (`kHotWaterCatalog`, ~133) הוא **רשימה נפרדת** המוזנת ל-`kCompatCatalog` בלבד — **לא** לקטלוג-המאוחד. מחולץ מ-PDFים אמיתיים (Lipskey-Barkan · Polyroll/AQUATEC · Huliot 44-עמ׳) עם **QA דו-שכבתי** (`scripts/catalog_qa.py`, 100+ כללים · verbatim-מ-PDF · override-tables append-only).
...
✅ **תיקון-drift (קריטי, אומת שורה-שורה):** ספירה קודמת של "**1,337 (Lipskey 255)**" הייתה **שגויה** — הוכנסה בסשן קודם בטעות. הקוד האמיתי = **1,877 (Lipskey 935 + Polyroll 772 + Huliot 170)**, התואם כמעט-במדויק את "~1,879" של מסמכי-ה-KB המקוריים. **הקוד קובע; הספירה החדשה אומתה מול גבולות-הרשימות.**
