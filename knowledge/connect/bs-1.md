...
### 38. `scripts/seed/backfill_user_status.js` · js · 82 שורות · §22 = **2**

**1 · מה הוא עושה**
- מיגרציה חד-פעמית שחותמת `status:'active'` על כל `users/{uid}` **קיים** שאין לו שדה status, כדי שחשבונות-ותיקים לא ייקראו כ-`pending` ברגע ש-USER_SYSTEM נדלק  
  *ראיה:* scripts/seed/backfill_user_status.js:2-9, :63
- מנמק את הצורך במדויק: `BsUser.fromWire` מפענח status **חסר** כ-`pending` — ברירת-המחדל הבטוחה של «הכול-באישור», שנכונה להרשמות **חדשות** אך הייתה מסמנת בטעות כל משתמש מהעבר  
  *ראיה:* scripts/seed/backfill_user_status.js:5-9
- **בטוח ואידמפוטנטי**: נוגע רק במסמכים **חסרי** status, לעולם לא דורס pending/active/suspended מפורש, ‏merge:true, וריצה-חוזרת היא no-op  
  *ראיה:* scripts/seed/backfill_user_status.js:11-12, :59-63
- ‏`--dry-run` **אינו נוגע באישורים כלל** — יוצא לפני `require('firebase-admin')`, לא קורא ולא כותב, ורק מדפיס בדיוק מה הריצה האמיתית תעשה  
  *ראיה:* scripts/seed/backfill_user_status.js:17-18, :34-40 (היציאה ב-:39, לפני ה-require ב-:42)
- כתיבה באצוות של ≤400 עם commit ביניים  
  *ראיה:* scripts/seed/backfill_user_status.js:66-72
- מדפיס שלושה מספרים בסיום: נסרקו · כבר-היה-להם-status · הושלמו-ל-active  
  *ראיה:* scripts/seed/backfill_user_status.js:74-77
- אישורים מגיעים מהסביבה בלבד (`GOOGLE_APPLICATION_CREDENTIALS` או application-default) — «owner-provided; never seen here»  
  *ראיה:* scripts/seed/backfill_user_status.js:14-16, :44

**2 · מה הוא לא עושה**
- לא מגדיר תפקידים ולא נוגע ב-claims — רק בשדה `status` במסמך  
  *ראיה:* scripts/seed/backfill_user_status.js:63 — הכתיבה היחידה
- לא ניתן להפוך — אין `--revert`; מסמך שהושלם ל-active לא ניתן להבחנה אחר-כך ממסמך שנוצר active  
  *ראיה:* כל 82 השורות: מסלול-כתיבה יחיד, בלי חותמת-מקור
- לא מסנן לפי תאריך — «קיים» = כל מה שאין לו status ברגע ההרצה. הרצה מאוחרת מדי תאשר גם הרשמות-חדשות שנכשלו בכתיבת ה-status המפורש.  
  *ראיה:* scripts/seed/backfill_user_status.js:49 `db.collection('users').get()` — בלי where
- אינו מעומד-דפים בקריאה — `.get()` יחיד על כל האוסף; הכתיבה מאוצרת, הקריאה לא  
  *ראיה:* scripts/seed/backfill_user_status.js:49 מול :66-72

**3 · מי קורא לו היום**
- **אדם, מהשורה** — `node backfill_user_status.js --project buildsmart-b0b78 [--dry-run]` (:16)
- **-ai-chat-server** — ∅

**4 · איפה שווה לחבר**
- *נקודה:* ‏`machtzev/generator/server.mjs` — חבילת-השרת המחוללת. מדדתי מה היא מכילה: `find server-gen -type f` ⇒ **8 קבצים**: `firestore.rules` · `rules.test.mjs` · `ACTIVATE.md` · `.gitignore` · `package.json` · `firebase.json` · `functions/index.js` · `functions/package.json`. **אין קובץ-זריעה ואין קובץ-מיגרציה.**
- *מה זה נותן:* ‏**מסלול-שינוי-סכמה**, שחסר לגמרי בפלט-המחולל. ‏`ACTIVATE.md` מדריך את הבעלים לפרוס כללים ופונקציות — ואין בו מילה על מה קורה כשהסכמה משתנה והדאטה הקיים כבר לא תואם. ‏backfill_user_status.js הוא התבנית המינימלית: אידמפוטנטי · נוגע רק במה שחסר · לעולם לא דורס מפורש · dry-run **בלי אישורים** · מדפיס שלושה מספרים. הדפוס הנייד: «ברירת-מחדל-בטוחה לקריאה חדשה + מיגרציה חד-פעמית לדאטה ותיק» — שתי ההחלטות ביחד, כי כל אחת לבדה שוברת צד אחר.

**5 · §22 = 2** — §22 היא «אפליקציה עובדת 100%», ואפליקציה שהסכמה שלה השתנתה והדאטה הישן קורא לא-נכון **אינה** עובדת. זו דרישה שתגיע לכל פלט-מחולל בגרסה השנייה, והמחולל היום פולט אפס. לא 3: אינה חוסמת היום (לאפליקציות המחוללות אין עדיין דאטה-ותיק), ואינה באף פריט-HANDOFF. לא 0: `find server-gen -type f` ⇒ 8 קבצים, אף אחד לא זה.

**6 · ראיה — מה הורץ/נקרא**
- `cat -n scripts/seed/backfill_user_status.js (82 שורות, נקראו במלואן)`
- `find server-gen -type f בריפו-המחולל ⇒ 8 קבצים, אין seed/migration`
- `head -30 server-gen/balagan/ACTIVATE.md (צעדי-ההפעלה — אין שינוי-סכמה)`
- `rules_test/approval.test.js:127 («a MISSING users doc is not approval») — הצד השני של אותה החלטה`

### 39. `scripts/seed/upload_seed.js` · js · 79 שורות · §22 = **2**

**1 · מה הוא עושה**
- כותב חבילת-זריעה של Firestore (מקובץ JSON) לפרויקט האמיתי דרך firebase-admin — זריעת-אדמין חד-פעמית שעוקפת כללי-אבטחה  
  *ראיה:* scripts/seed/upload_seed.js:2-4, :41-46, :61
- **אידמפוטנטי**: ממופתח לפי doc-id עם `merge:true` ⇒ ריצה-חוזרת מעדכנת ולעולם לא מכפילה; אצוות ≤400 לכל commit  
  *ראיה:* scripts/seed/upload_seed.js:4-5, :61-70
- צורת-החבילה היא **בדיוק** מה שהאפליקציה קוראת — הופקה ע"י המסרלים המוכחים של Dart (`toDoc` · `verifiedSpecToDoc` · store+inventory); מפתחות-עליונים = שמות-אוספים, כל אחד `{docId: fieldMap}`  
  *ראיה:* scripts/seed/upload_seed.js:7-9, :50-51
- ‏`--dry-run` **טוען עצלות**: `require('firebase-admin')` נמצא בתוך `if (!dryRun)` — «lazy — dry-run needs no install/creds». מדפיס ספירת-מסמכים לכל אוסף בלי רשת ובלי אישורים.  
  *ראיה:* scripts/seed/upload_seed.js:38-46, :52-56
- מתעד שני מסלולי-אימות חלופיים (קובץ service-account או application-default) עם «you provide — I never see the secret»  
  *ראיה:* scripts/seed/upload_seed.js:11-16
- מדפיס לכל אוסף כמה נכתבו, ובסיום סך-הכול, בניסוח שונה ל-dry-run («would write») ולריצה אמיתית («DONE — wrote»)  
  *ראיה:* scripts/seed/upload_seed.js:71-74

**2 · מה הוא לא עושה**
- לא מאמת את החבילה מול סכמה — `JSON.parse` ואז כתיבה; שדה שגוי בקובץ נכתב כמות-שהוא  
  *ראיה:* scripts/seed/upload_seed.js:36 ⇒ :61 — אין ולידציה ביניהם
- לא מוחק — merge בלבד; מסמך שהוסר מהחבילה **נשאר** בפרויקט. אין סנכרון-אמיתי, רק upsert.  
  *ראיה:* scripts/seed/upload_seed.js:61 `{merge:true}`; אין delete בקובץ
- לא מייצר את החבילה — היא מיוצאת מצד ה-Flutter; הקובץ הזה רק מעלה. שתי החבילות בתיקייה (`firestore_seed_full.json` · `firestore_seed_slice.json`) הן קלט, לא פלט שלו.  
  *ראיה:* scripts/seed/upload_seed.js:2-3; `ls scripts/seed/` ⇒ README.md · backfill_user_status.js · firestore_seed_full.json · firestore_seed_slice.json · upload_seed.js
- אינו טרנזקציוני חוצה-אוספים — כשל באמצע משאיר זריעה חלקית, בלי rollback  
  *ראיה:* scripts/seed/upload_seed.js:64-70 + :76-78 (catch שרק מדפיס ויוצא 1)

**3 · מי קורא לו היום**
- **אדם, מהשורה** — `node upload_seed.js <bundle.json> --project buildsmart-b0b78 [--dry-run]` (:13-18)
- **-ai-chat-server** — ∅

**4 · איפה שווה לחבר**
- *נקודה:* ‏`machtzev/generator/server.mjs` (חבילת-השרת) יחד עם `machtzev/generator/app-from-sentences.mjs:243` — שם המחולל מחזיק זריעה **בתוך ה-Dart**: `${cls}.seed()` מחזיר מפה, והבדיקה המחוללת מזריקה שורה ובודקת שעמודת-מקום-שמור «מאירה».
- *מה זה נותן:* **הגשר מזריעה-מוטבעת לזריעה-בענן.** למחולל יש היום שני חצאים שלא נפגשים: (א) `Cls.seed()` — נתוני-הדגמה בתוך המודול המחולל; (ב) `server-gen/balagan/` — חבילת-ענן עם כללים ופונקציות ו**אפס דאטה**. ‏upload_seed.js הוא בדיוק החוליה: קובץ-חבילה שמפתחות-העליון שלו הם שמות-אוספים, ‏upsert אידמפוטנטי, ו-dry-run חסר-אישורים. ⚠️ **סייג של §20:** «לעולם לא לזייף דאטה» — ולכן הערך כאן הוא ה**מנגנון**, ובתנאי שהחבילה נגזרת מהספק של הבעלים ולא מומצאת. ‏upload_seed.js עומד בזה מבנית: הוא **לא מייצר** דאטה, רק מעביר מה שהמסרלים המוכחים ייצאו.

**5 · §22 = 2** — §22 = משפט ⇒ אפליקציה **עובדת**. אפליקציה מחוללת שמופעלת מול ענן ריק אינה עובדת — היא מרונדרת. ‏`ACTIVATE.md` של המחולל מוביל את הבעלים עד פריסת-הכללים ואז נעצר. זו החוליה. לא 3: אינה באף פריט-HANDOFF, והזריעה המוטבעת ב-Dart מספיקה למסלול המקומי שנמדד היום (246/283). לא 0: `find server-gen -type f` ⇒ 8 קבצים, אין מעלה-זריעה.

**6 · ראיה — מה הורץ/נקרא**
- `cat -n scripts/seed/upload_seed.js (79 שורות, נקראו במלואן)`
- `ls scripts/seed/ ⇒ README.md · backfill_user_status.js · firestore_seed_full.json · firestore_seed_slice.json · upload_seed.js`
- `find server-gen -type f בריפו-המחולל ⇒ 8 קבצים`
- `grep -n 'seed' machtzev/generator/app-from-sentences.mjs ⇒ :159 · :236-244 (`${cls}.seed()` · seedSeam)`

### 40. `service-worker.js` · js · 57 שורות · §22 = **1**

**1 · מה הוא עושה**
- ‏Service Worker בשורש-הריפו שנותן תמיכת-offline לפרוטוטיפ-הלגאסי: מטמין את מעטפת-האפליקציה בהתקנה ומגיש אותה מהמטמון כשאין רשת — «construction sites frequently have no signal»  
  *ראיה:* service-worker.js:2-6
- ‏install: פותח `buildsmart-v107`, מבצע `addAll(['./index.html','./manifest.json'])` ואז `skipWaiting()` — גרסה חדשה תופסת מיד  
  *ראיה:* service-worker.js:8-23
- ‏activate: מוחק **כל** מטמון ששמו שונה מהנוכחי ואז `clients.claim()` — ניקוי-גרסאות מלא  
  *ראיה:* service-worker.js:25-37
- ‏fetch: **network-first** עם נפילה-למטמון; תגובות מוצלחות משוכפלות חזרה למטמון («keep the cache fresh»), וב-offline מחזיר את המשאב המטומן או את `./index.html` כמוצא-אחרון  
  *ראיה:* service-worker.js:39-56
- מדלג על כל בקשה שאינה GET  
  *ראיה:* service-worker.js:41
- בולע כשלי-מטמון: `caches.open(...).catch(function(){})` — כשל-כתיבה-למטמון לא מפיל את התגובה  
  *ראיה:* service-worker.js:46-48

**2 · מה הוא לא עושה**
- **אינו ה-SW של האפליקציה החיה `app/`** — שם `VitePWA` מייצר SW משלו מ-workbox עם שלוש אסטרטגיות נפרדות. שני מנגנוני-offline בלתי-תלויים חיים בריפו.  
  *ראיה:* app/vite.config.ts:16-17, :46-74 מול הקובץ הזה
- נרשם **רק** ע"י הפרוטוטיפ-הלגאסי — `navigator.serviceWorker.register('service-worker.js')` ב-`index.html:20363`, שהוא אותו קובץ ש-`app/scripts/extract-catalog.mjs:16` חוצב ממנו את הקטלוג  
  *ראיה:* `grep -rn 'service-worker' --include=*.html --include=*.ts` ⇒ index.html:20357,20363 בלבד
- מטמין **כל** GET מוצלח ללא הבחנה — אין allowlist של מוצא, אין תקרת-גודל, אין פקיעה. בקשת-API עם תשובה משתנה תיטמן אף היא.  
  *ראיה:* service-worker.js:43-49 — אין בדיקת url/type/status
- ‏`CACHE_NAME` מוגדל **ידנית** (v107) — אין חותמת-build; שכחה לעדכן משמעה שמעטפת ישנה נשארת  
  *ראיה:* service-worker.js:8
- מעטפת-האפליקציה היא שני קבצים בלבד — אין CSS/JS מוקדם-מטמון, כי הלגאסי הוא קובץ-HTML יחיד  
  *ראיה:* service-worker.js:9-12

**3 · מי קורא לו היום**
- **`index.html:20363`** — `navigator.serviceWorker.register('service-worker.js')` — הפרוטוטיפ-הלגאסי בשורש
- **`app/`** — ∅ — לא נרשם שם; VitePWA מייצר SW משלו (app/vite.config.ts:16)
- **-ai-chat-server** — ∅ — `grep -rln 'service-worker\|serviceWorker' --include=*.mjs --include=*.js` בריפו-המחולל ⇒ אפס

**4 · איפה שווה לחבר**
- *נקודה:* ‏`gen/site.mjs` — המנוע שהופך פלט-Flutter לאתר-רץ (`flutter build web --release` ⇒ `out/<slug>/site`). מדדתי: `grep -n 'manifest\|workbox\|service' gen/site.mjs` ⇒ **∅**. האתר שהמחולל פולט אינו PWA ואינו עובד offline.
- *מה זה נותן:* ‏**offline לפלט-המחולל**, ב-57 שורות בלי תלויות. אזהרה כנה: זו הגרסה ה**חלשה** משתי הגרסאות בריפו — `app/vite.config.ts:46-74` (‏workbox) עדיפה עליה בכל מובן (‏NetworkFirst למסמכים עם timeout · StaleWhileRevalidate לנכסים · CacheFirst לתמונות עם פקיעה). ⇒ מי שיחבר צריך לקחת את התצורה מ-vite.config, לא את הקובץ הזה. ערכו של `service-worker.js` הוא בהיותו **המימוש הידני חסר-התלויות** — רלוונטי אם המחולל לא ירצה להכניס workbox לשרשרת-הבנייה.

**5 · §22 = 1** — אתר-רץ-בלי-רשת אינו חלק מהרצפה הנמדדת היום (פריט 1 מודד רינדור-מסכים). ובתוך הריפו עצמו יש מקבילה טובה יותר — אבל היא **אינה מחוברת למחולל**, ולכן זה אינו 0 לפי הכלל: התנאי ל-0 הוא «מקבילה **מחוברת** טובה יותר», ו-`app/vite.config.ts` אינו מחובר למחולל (`grep -rn 'vite' --include=*.mjs` בריפו-המחולל ⇒ ∅ במסלול-הבנייה). הניקוד 1 משקף מנוע חי, יחיד-בסוגו מול המחולל, אך שולי ל-§22.

**6 · ראיה — מה הורץ/נקרא**
- `cat -n service-worker.js (57 שורות, נקראו במלואן)`
- `grep -rn 'service-worker' --include=*.html --include=*.js --include=*.ts בריפו buildsmart ⇒ index.html:20357,20363`
- `app/vite.config.ts:16-17 + :46-74 (המקבילה הטובה יותר, לא-מחוברת)`
- `grep -n 'manifest|workbox|service' gen/site.mjs בריפו-המחולל ⇒ ∅`
