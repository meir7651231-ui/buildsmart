...
> v1 = כפתורי טלפון+וואטסאפ (אפס בנייה) · וידאו בתוך‑האפליקציה (Agora) = עתידי, **מוסתר עד שייבנה**. ⚠️ **אין כפתור‑וידאו קיים להסתיר** — נבדק בקוד: שום `tel:`/`wa.me`/UI‑שיחה לא קיים. ה"וידאו" היחיד שגלוי = הגדרות‑placeholder (V2).
| ID | משימה | היכן | DoD | מי | מ' |
|---|---|---|---|---|---|
| V1 | כפתורי שיחה: 📞 חייגן (`tel:`) + 💬 וואטסאפ (`wa.me`) בצ׳אט/הזמנה/איש‑קשר (טלפון מהפרופיל/A7) | `url_launcher` + chat/order UI | לחיצה → חייגן/וואטסאפ למספר הנכון | agent | S |
| V2 | להסתיר עץ‑הגדרות מת "הגדרות שיחות" (דחיסת‑וידאו/צלצול‑שיחה/read‑receipts/חיווי‑הקלדה/פרטיות) עד מימוש — מסיר כל הבטחת‑וידאו שלא עובדת. **לשמור** את מסך‑הצ׳אט עצמו (`search_index:46`) | `search_index.dart:340,436‑453` + מסך‑ההגדרות התואם | אין הגדרת‑וידאו/שיחה מתה גלויה (משלים B5) | agent | S |
