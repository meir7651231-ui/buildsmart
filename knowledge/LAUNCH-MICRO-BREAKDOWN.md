...
> 🟧 **S3 — סטטוס‑מסירה אמיתי בצ׳אט (אומת בקוד 16/6, זיהוי‑בעלים):** ה‑✓✓ **קוסמטי** — `chats_screen.dart:1720‑1722` נצבע לפי טוגל `readReceipts` בלבד, **לא** לפי מסירה‑לשרת; אין שדה‑סטטוס פר‑הודעה. (לכן בצ׳אט‑השבור הופיע ✓✓ בלי סנכרון.) → להוסיף סטטוס אמיתי פר‑הודעה: 🕐 pending (לא אושר) → ✓ נכתב‑לשרת → ✓✓ נמסר/נקרא → ❌ נכשל, מבוסס על תוצאת ה‑background write (שכיום בולע כשל). **פוליש‑כנות ל‑v1** — משתמש‑אמת לא יראה ✓✓ על הודעה שלא נשלחה. גם בסיס ל"באנר אונליין/אופליין".
> 🟥 **S4 — מד‑חיבור חי בבילד‑הבדיקה (בקשת‑בעלים 16/6, חוסם‑בדיקות):** "אני חייב לדעת שהאפליקציה מחוברת לשרת 100% כל הזמן, אחרת אין לי סיכוי לבדוק כלום." כיום: badge (uid/flag) + green "מחובר" = state‑בהתחלה (לא חי); orange "בדוק חיבור" = round‑trip חי על‑פי‑דרישה. → **להוסיף אינדיקטור תמיד‑גלוי, חי, אמין:** 🟢 "מחובר לשרת" / 🔴 "מנותק", מבוסס Firestore real‑time (snapshot metadata `fromCache`/`hasPendingWrites` + `connectivity_plus` לרשת), מתעדכן מיידית בניתוק/חזרה; כשמנותק — אזהרה שפעולות לא יישמרו. **DoD:** כיבוי‑wifi → אדום תוך שניות, החזרה → ירוק. (בילד‑בדיקה; באפליקציה הסופית = באנר‑אופליין ידידותי = S3‑בונוס.)
