...
תוויות-תצוגה verbatim (`taskStatusInfo` @:8048–8054, INSP-0041): **⏳ ממתינה · 🔨 בביצוע · 📸 ממתין-לאישור · ✅ אושר ✓ · ↩️ נדחה — לתקן.**
**⭐ פילטור per-group (3 קבוצות, INSP-0042 — תיקון ל-0041; legacy-filter @8095–8098):** 🔨 **המשימה הנוכחית שלך** = `active`+`rejected` (🔨 בביצוע · ↩️ נדחה—לתקן) · ⏳ **הבאות בתור** = `pending` (⏳ ממתינה) · 📋 **שהגשת** = `review`+`done` (📸 ממתין-לאישור · ✅ אושר ✓). כל קבוצה מסננת לתת-קבוצת-סטטוסים — **לא** כל ה-5 בכל קבוצה.
