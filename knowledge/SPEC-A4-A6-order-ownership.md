...
> ✅ **בוצע — הצי דחף 208f3a9 (13/6):** A4 (claim‑on‑first‑advance + `storeUid`/`courierUid` + orderParticipants) · A5 (scoped listener + שיתוף‑בריכה + indexes) · A6 (דשבורד חנות/שליח = בריכה ∪ שלי) · rules +100 (claim/no‑steal + manager override) · 27/27 emulator + orders_uid_a4_a6_test 22 + suite +2176 · analyze 0 · build web. **מגודר ב‑`useFirebaseBackend`** (דורמנטי בברירת‑מחדל עד F1+דגל). **נותר:** A4' (החלפת מקור‑הזהות seed→Firebase‑uid — תלוי B8/F1; `board_auth.dart` עוד לא נגעו בו) + F1 (שלב 0 למטה).
