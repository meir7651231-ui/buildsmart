...
> ✅ **27/7 — הנחיית registry→טוגלים בוצעה ע"י הצי · מאומת בבייטים+CI.** הצי בנה `swarm-r1` (`e8dd7a96`) + `r2a` (`9d90d05f`) + `r2b` (`4f50c40d`). **r1 מאומת:** מחזר את ה-registry **האמיתי** `kElementRegistry` (**896** רכיבים — גדל מעבר ל-~863) כמקור-הטוגלים (import שורה 44, איטרציה שורה 416); האשף מרנדר **מקובץ מסך→אזור** (`screenLabelHe`/`normalizeScreen`) · **`labelHe`** (שם-עברי) · **אקורדיון עצל** (896 לא נבנים כשסגור) · **שדה-חיפוש** (`elem-search`/'חיפוש רכיב') · `SwitchListTile` פר-רכיב · **ליבה-נעולה** (edit-safety) · +4 טסטים (`org_config`/`org_setup_wizard`/`org_toggle_matrix`/`screen_labels_he`) + `screen_labels_he.dart` (מפת מסך→עברית). **= בדיוק ההנחיה: אותה כמות/שליטה כמו ה-Studio, מוצג יפה כאשף.** **r2a/r2b מאומת:** `CfgVisible` **מסיר את הווידג'ט כולו** (`SizedBox.shrink`; `critical`→`return child` לא-ניתן-להסתרה; גלוי→נתיב זהה-בייטים) · **~37 אתרים מורכבים** עטופים ב-8 מסכים (store 18 · manager 6 · projects 5 · role-inbox 3 · contractor 2 · +3). **חלקי** — נותרו ~50 (profiles/worker-courier/dashboards-2/trade_builder/tasks/dialogs, סומן ע"י הצי). **`default-ON`=זהה-בייטים מאומת** (`kDefaultOrgConfig=OrgConfig()` ריק · absent=on).

> 🟢 **27/7 — CI + חדשות-שער (מתקן דיווח קודם שלי).** על שני ראשים-שהושלמו (`e8dd7a96` + `9d90d05f`) **כל 8 ה-workflows = success**; על הראש-החדש `4f50c40d` — 3 ירוקים (Firebase-web ×3) + 5 `in_progress`, **אפס כשלונות**. 🔺 **תיקון:** 3 השערים שקראתי להם "אדומים-כרוניים" (Protocol Enforcement · Android-package/Play · GitHub-Pages) הם עכשיו **ירוקים** על שני הראשים-שהושלמו — `f91e4f7a` ("fix(gate): restore the zero-tolerance test baseline") תיקן את שער-הפרוטוקול. הדיווח הקודם שלי "3 אדומים קבועים" **לא-מעודכן** לקומיטים האלה. ➕ נחת גם גל p2-w4: `e9631969` עמודת-ח"פ אופציונלית בייבוא-לקוחות (משתמש ב-checksum של `validBusinessId`) · `90db7e3c` תעודת-משלוח (גדור) · `f91e4f7a` כפתור-קבלה + תיקון-שער.

> 🔴 **27/7 — תיקון-נדרש: `wizard-preview.yml` נבנה על פרופיל שגוי (`demo`), לא `clean`.** ה-workflow (`472e7190`) רץ **ירוק** (run #1 = success, ערוץ-Firebase `wizard`) — הצינור עובד — **אבל בנה עם `--dart-define=APP_PROFILE=demo`**, כלומר הלינק מציג **נתוני-דמו** = בדיוק מה שהבעלים סימן. ההנחיה (`DIRECTIVE-arm-wizard-preview.md`) אמרה מפורשות **`clean`** (+"אם הדשבורד חסום ב-clean — חשוף אותו ב-clean"). מאומת: `clean` הוא הפרופיל **המלא-יותר** (`_ux = _bs || _clean || _c2` — דגלי-UX דלוקים; `demo` הכי-רזה) → דשבורד-המנהל נגיש ב-clean. **תיקון = החלף ל-`APP_PROFILE=clean` בשני ה-dart-defines.** עדכנתי הערת-תיקון ב-`DIRECTIVE-arm-wizard-preview.md`.

> 🚀 **27/7 — אושר "צא לדרך": האשף = הסטודיו המלא, בסגנון Maor.** אחרי איטרציה חיה מול הדמייה (`knowledge/wizard-mockup.html`), הבעלים חידד+אישר סקופ **מורחב הרבה מעבר להצג/הסתר**: (1) **כל 5 חלוניות הסטודיו** נחשפות באשף על אותו `element_registry`(~896)+override — 🌳עץ · 🔍מפקח(✎ פר-רכיב: שם/צבע/גודל/משקל/יישור/הצג-הסתר, מאומת מול שדות-override: 895 labelHe·57 color·20 fontSize·11 hidden·weight/align) · 🎨עיצוב · 🔎מצא-החלף(`find_replace_pane`) · 🕘גרסאות(`history_pane`, שחזור לא-הרסני). (2) **סגנון Maor** — סקציות פתוחות-לגלילה (**מתקן את באג "תיבה ריקה"** — הנחיל בנה חיפוש-בלבד) · `descHe` · N/N · טוגל-אב · סמן/נקה-הכל · צ'יפים+חיפוש כמסננים · מונה X/896 · מונחים-שזורים. (3) 🔴 **מודול קבלן חסר** מ-`org_modules.dart` (הפרסונה הראשית!) — הבעלים תפס; נוסף כמודול #1. **הכל-חי בכל-האפליקציה · default זהה-בייטים · ליבה-נעולה · toggle-matrix.** כתבתי `DIRECTIVE-wizard-is-the-studio.md` (5 פרוסות, עצור-לדיווח) + נעלתי הדמייה כספֶק 1:1 + עדכנתי MASTER(צעד 2/2.5). **משגר נחיל.** ➕ **פרוסה-0 (בעלים "ראשון וידחוף") + אבחון-בעלים חד:** ה-edit-mode הקיים (`edit_handle` step9) **לוכד כל tap כעריכה** ואין preview חי לנווט (`studio_screen`=4-פאנלים) → הבעלים **תקוע במסך-הניהול, לא מגיע לקבלן לערוך.** *(מאומת: אין `previewScreen`/screen-picker בקוד.)* פשוט "להדליק edit-mode" לא פותר. **תיקון 2-חלקים:** (1) הדלק edit-mode בתצוגה (שער-בעלים #84 נשמר) + (2) **בורר-מסך/persona מחוץ ל-edit-trap** ב-`studio_top_bar` (נשען על `tree_pane`) → מעבר-חופשי-בין-מסכים+עריכה. **חידוד-בעלים (2 מפלסי-ניווט):** *בתוך*-מסך (פתיחת sheets/דיאלוגים) = **מתג "נווט⇄ערוך"** (edit-כבוי=ניווט רגיל, edit-דלוק=עריכה) · *בין*-בורדים = הבורר. שורש-הבאג: edit תמיד-דלוק ⇒ אי-אפשר לפתוח sheet. **מטרה: האתר החי** (בעלים "עדיף לי") — `kStudio` ON בחי · **שער-בעלים isOwnerEmail #84 STRICT (בלי bypass)** ⇒ רק הבעלים עורך, **כל השאר זהה-בייטים** · כניסת-הסטודיו גדורה ב-owner · אימות-חובה: session לא-בעלים=זהה-בייטים. דחוף לחי; ואז 1-5.

> ✅ **27/7 — הנחיית "האשף=הסטודיו" בוצעה במלואה (s0→s5, "directive complete") · מאומת בבייטים.** **פרוסה-0:** edit-live מזוין על ה-preview + `studio_overlay.dart` — **מתג נווט⇄ערוך** (ברירת-מחדל "נווט"=אפליקציה רגילה → **פותר את מלכודת-הניווט**) + **בורר-בורדים** (ממחזר `showRolePicker`: קבלן/מנהל/ספק/שליח/עובד) + טוגל-הסתר. **גדור-בעלים #84** — `zero_regression_test`: edit inert ללא owner+manager+active (ID: `STUDIO`=`bool.fromEnvironment`, `studio_flags.dart:17`). **פרוסות 1-5:** אקורדיון-Maor + **מודול-קבלן #1** (`kContractorModule` — כסקציית-אשף, **לא** persona-gate כי ה-app-הבסיסי לא נגדר; הקוד מצטט "החור שהבעלים תפס") · מפקח-מלא ✎ (text/color/size/weight) · מונחים-שזורים · מצא-החלף (`FindReplacePane`) · גרסאות (`HistoryPane`). **CI:** slice-0b preview ✅ ירוק; rebuild-כל-5 (`326b0d57`) עדיין `in_progress`. 🔴 **LIVE≠preview:** הבעלים רצה חי — הצי הכין "STUDIO GO-LIVE SEAM" בוורקפלואוז-החיים אבל **דורמנטי** (זהה-בייטים היום). **הזרוע-לחי = הבעלים מגדיר repo-var `STUDIO_DART_DEFINES` += `--dart-define=STUDIO=true`** (GitHub Settings→Variables — פעולת-קונסול של הבעלים; הצי לא נוגע בהגדרות-repo). owner-gate מבטיח: רק הבעלים עורך, כל השאר זהה-בייטים.

> 🟢 **27/7 — הסטודיו/אשף עלה ל-LIVE (הבעלים "עלה בשניהם" — זרע `STUDIO_DART_DEFINES`) · בטיחות מאומתת בקוד+CI.** דיפלוי-החי "Deploy Flutter Web to Firebase Hosting" על `326b0d57` = **success** (לא-בעלים לא נחשף). **לא-בעלים = זהה-בייטים — מאומת ב-3 שערי-בעלים בלתי-תלויים:** `StudioEntryCard` (`studio_entry.dart:25` → `SizedBox.shrink` לכל מי-שאינו-owner-manager, הקוד: "zero change for every other user") · `edit_mode` (owner+manager+active בלבד; `zero_regression_test`) · `edit_handle`/`CfgVisible` (מחזירים `child` ללא-עריכה). ⇒ **רק הבעלים (מחובר) רואה כפתור-סטודיו + מתג נווט⇄ערוך + עורך; כל שאר המשתמשים רואים את האתר כמו אתמול.** **המערכת-הענקית: האשף=הסטודיו — חי, owner-only, על שני הערוצים.** *(נותר אופציונלי: אימות-ריצה של session לא-בעלים על החי — belt-and-suspenders.)*

> 🧩 **27/7 — מאור הופעל: שילוב-מלא (אודיט-אחורה מאומת בקוד).** הבעלים: "מאור גמור, לשלב מלא". חקירה-אחורה (אודיט 14-דפוסים + 5-core מול `app_flutter`): **~60% כבר בפנים והחלק העליון-בדירוג.** ✅8: התראות#1(מחווט)·כרטיס-לקוח+ייבוא+dedup#3·RFM#4·חיפוש-סובל-שגיאות#5·מאתר-קטלוג#6·רופא-נתונים#10·ח"פ-fix C1·טלפון C2. 🟡5: workflow-kernel-**לא-מחובר**#2·credit-בלי-יומן·לוח-DST-בלי-חגים#12·CSV-בלי-export-guard C3·סבילות-בלי-migrate C4. 🔴6: ציר-זמן#7·לוח-משאבים+חסימת-חג#8·מכסה-מראש#9·חזרתיות#11·מספור-רץ#13·דוח-יומי#14. **מאור גדל** (מנועים חדשים, כולם reusable): timer(חיוב-לפי-זמן)·cashbox(POS+מספור-רץ)·bodymap(punch-list אתר)·doncal(heatmap-לקוח)·dedup-merge(מיזוג-כפילות-קיימות); נעול-תחום: מוני-ילדים·§46. מודל-הדאטה של מאור = בדיוק ה-`org_config.dart` שלנו (האשף-החי = עורכו). כתבתי `DIRECTIVE-maor-full-integration.md` (8 פרוסות מדורגות: #1=חבר-workflow הכי-זול·#2=מספור-רץ+קופה·#3=מיזוג·#4=ציר-זמן·#5=timer·#6=credit-יומן·#7 דחוי·#8 הקשחה). additive·default-off·toggle-matrix.

> 🎚️ **27/7 — החלטת-בעלים: טריגר-עריכה-על-המסך → פארק · העריכה עוברת לאשף · "צא לדרך".** אחרי עיצוב חי מול הצילומים: הבעלים ניסה טריגר-דרך-הלוגו (לחיצה-ארוכה) — ותפס ש**אין לו מיקום אוניברסלי** (מסכי ניהול-אתר בלי לוגו קבוע). החלטה: **כבה את chrome-העריכה-על-המסך (`studio_overlay`: נווט⇄ערוך + בורר) — חובה, ראשון, דחוף לחי** (מוקפא עד מיקום-טוב); **האשף נשאר הכניסה-לעריכה.** ואז **ניהול-מסכים באשף** על תשתית **קיימת** שהבעלים כיוון אליה: `hidden_catalog_sections` (הסתר-סקציה, hide/show/toggle+persist, לא-הרסני, "משוחזר מ'ניהול רשימות'") + `home_content_order` (סדר) → מוכללים לכל-מסך: רמה-1 רשימת-מסכים (סדר/הסתר מסך) → חץ/אמצע → רמה-2 עורך-סקציות (סדר+הסתר) + מקלדת-פר-מסך בעורך-המסך + הוסף-הסתר-למנהל-הבית. הדרגתי, מסכים-חשובים קודם. `DIRECTIVE-screen-management-in-wizard.md`. *(עקרונות שהתגבשו: הכלי-לא-עורך-את-עצמו · טריגר-נפרד-מהמקלדת · הזזה-בין-מסכים=פרויקט-נפרד-ענק, לא עכשיו.)*

> ✅ **28/7 — הנחיית ניהול-מסכים בוצעה במלואה (s0→s5c, "directive complete") · מאומת בבייטים+CI.** **s0 (חובה, ראשון):** הטריגר-על-המסך **מוקפא** — `studio_overlay.dart:4` "FROZEN slice-0 · אין כניסה-על-המסך לעריכה, מוקפא עד מיקום-טוב"; הטוגל נווט⇄ערוך + בורר-הבורדים **tree-shaken** (הקובץ התכווץ 245 שורות); **`main.dart` לא-נגע → האשף עדיין mounted.** **s1:** מודל-סקציות-פר-מסך מאוחד (`state/screen_sections.dart` + `config/screen_registry.dart`, סדר+הסתר). **s2:** אשף **"ניהול מסכים" 2-רמות** (רשימת-מסכים → עורך-סקציות). **s3:** מסך-הבית עכשיו **סדר+הסתר** (`home_content_reorder:82`, ההסתר-החסר נוסף; exit/back `critical:true` — לא נעול-משתמש). **s4:** עורך-מקלדת-פר-מסך מתוך עורך-המסך. **s5a/b/c:** קבלן(=home)/מנהל/חנות על מודל-הסקציות. **CI:** דיפלוי-החי "Deploy Flutter Web" על `ff13d137` = **success** (28/7). ➕ תיקוני owner-status (never-pending) · v7.01 · status-chip-מתחת-ללוגו (register-first). **⇒ הטריגר ירד מהחי · האשף = דרך-העריכה · לא-בעלים זהה-בייטים.** *(נותר: הכללת מודל-הסקציות לשאר המסכים — הדרגתי.)*

> ✅ **28/7 — ניהול-מסכים המשך (s6→s10) + בקשת-מאתר-על בוצעה · מאומת בבייטים+CI.** s6: בלוקים-קבועים בבית (תכנון-חיבור+מועדפים)→סקציות-נסתרות. s7: עורך-מקלדת-פר-מסך **מחווט חי** (המקלדת האמיתית מכבדת). s8: **✎ שינוי-שם פר-פריט** בעורך-סקציה+מקלדת. s9: מאתר-על בבית **וגם** במקלדת-הבית, נערך בכל אחד. **s10: מאתר-על פתוח בבית בלי כרטיס-כניסה** — הצי ציטט מילה-במילה את בקשת-הבעלים ("מאתר-על פתוח ללא כרטיס כניסה ואז נפתח"), `smart_home_screen._SuperFinderOpen` מטמיע את **אותו** `CatalogWheelScreen` פתוח-במקום (reuse verbatim). **CI:** דיפלוי-חי על `e9d62f54` = **success**. 🔶 **פתוח:** פתרון-מיקום-קבוע לכפתור-העריכה (הטריגר-על-המסך עדיין מוקפא מ-s0).

> 🖊️ **28/7 — נפתר טריגר-העריכה (אושר מול הדמיה): long-press-מקלדת → ✎ מעל-הסל, בלי באנר.** אחרי הדמיה-אינטראקטיבית של 3 אפשרויות (מחווה/מקלדת/פינה), הבעלים בחר **היברידי:** טריגר = **לחיצה-ארוכה על ה-FAB-מקלדת** (אוניברסלי, על כל מסך → פותר את בעיית-הלוגו) שמדליק/מכבה `editMode`; **בלי באנר**; במקום — **✎-עיפרון פעיל נדלק מעל cart-FAB** כשעריכה דלוקה (owner-only). מדליק חזרה את studio_overlay שהוקפא ב-s0, מעוצב-מחדש. owner-gate #84 STRICT · לא-בעלים זהה-בייטים · על החי. `DIRECTIVE-edit-trigger-keyboard-longpress.md`. *(אגב: באג-קידוד בהדמיה — קובץ בלי charset=utf-8 יצא ג'יבריש בהורדה; תוקן ל-doc-שלם.)*
