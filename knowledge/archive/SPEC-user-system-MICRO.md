...
## ✅ הוכרע (14.7) — כל ההחלטות סגורות · התוכנית פתוחה במלואה
1. **הרשמה: הכל באישור‑admin.** אין הרשמה‑עצמאית‑אוטומטית — כל משתמש חדש (כולל קבלן) נוצר `status=pending` → admin מאשר → `active`. שליטה מלאה על מי‑בפלטפורמה (מחיר: חיכוך‑כניסה — אין signup‑מיידי).
   → **השלכות:** (א) U2 תמיד יוצר משתמש‑ממתין. (ב) **U4.4 (תור‑אישור, דרך `roleRequests`+`reviewRoleRequest` הקיימים) הוא השער האוניברסלי לכל הפרסונות** — לא רק חנות/שליח. (ג) **חדש:** מסך/מצב "ממתין לאישור" עד הפעלה — להוסיף כ‑U1.5 + U2.4.
2. **אורח: רואה קטלוג · הזמנה+פרופיל דורשים הרשמה.**
   → **השלכה:** לגדר "הוסף לסל" / checkout / פרופיל מאחורי הרשמה לאורח (**חדש: U2.5**); עיון‑קטלוג נשאר פתוח (עם ההתחברות‑האנונימית שכבר עלתה ב‑go‑live).
3. **בעלות‑חנות: בעל‑יחיד לכל חנות.** `storeUid` אחד — בלי מודל‑צוות. U3 נשאר פשוט; הרחבה‑לצוות = שלב‑עתידי (לא נשבר).

## השלכות‑הכרעה על המשימות (תוספות)
| ID חדש | משימה | שלב |
|---|---|---|
| U1.5.x | מסך/מצב "ממתין לאישור" — משתמש‑pending חסום מפעולות עד אישור | U1 |
| U2.4.x | כל הרשמה → `status=pending` + יצירת `roleRequest` | U2 |
| U2.5.x | גידור לאורח: "הוסף לסל"/checkout/פרופיל → דורשים הרשמה; קטלוג פתוח | U2 |
| U4.4.x | תור‑אישור אוניברסלי (כל הפרסונות) — admin מאשר→active | U4 |
