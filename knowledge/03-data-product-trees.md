...
⭐ **קטלוג-אמת: 1,877 מוצרים** ב-`kCatalogProducts` = `[...kLipskeyCatalog, ...kPolyrollCatalog, ...kHuliotCatalog]` (spread ללא-dedup, אומת ע"י `ppr_infra_test`) = **Lipskey 935 + Polyroll-PPR 772 (756 `_ppr`+16 `_acPipe`) + Huliot-SmartLock 170 `_sl`**. ⚠️ **ספירה אומתה שורה-שורה בקוד (935/772/170=1,877).** ⚠️ ה-HW-מים-חמים (`kHotWaterCatalog`, ~133) הוא **רשימה נפרדת** המוזנת ל-`kCompatCatalog` בלבד — **לא** לקטלוג-המאוחד. מחולץ מ-PDFים אמיתיים (Lipskey-Barkan · Polyroll/AQUATEC · Huliot 44-עמ׳) עם **QA דו-שכבתי** (`scripts/catalog_qa.py`, 100+ כללים · R8 verbatim-מ-PDF · override-tables append-only).
...
- **`SmartProduct`** (`smart_tree.dart` 2,551ש׳, **82 כרטיסים-מובחרים** — אומת-עצמי שורה-שורה) = `{key, name, emoji, cat, brands[], acc[], diagramTitle, stages[]}` · `SmartBrand{name, tag, price?, rec, sku?, imageAsset?}` (**ה-SKU על ה-brand, לא על המוצר**; ⚠️ **חלק מהכרטיסים עדיין נושאים מותג-גנרי ללא-SKU** — למשל `faucet`: "סטנדרט/כלכלי/פרימיום" — זה ה-gap של 307/365-brands-עם-SKU) · `SmartAcc{name, emoji, why, must, price?, sku?}` · `SmartStage{emoji,label,sub,isFinal,match[]}` (verbatim מ-prototype DIAGRAMS, 3–4/מוצר). getters `recBrand`(firstWhere rec→[0])/`mustCount` · reverse `smartProductForSku`(lazy) · `smartProductByKey`(עלי-catalog) · `kSmartTreeCats`/`smartProductsForCat`.
- ⭐ **`VerifiedSpec`** (`kVerifiedSpecs` map ב-`lipskey_verified_connections.dart` — **887 specs סטטיים** אומת-עצמי, + `registerPolyrollSpecs()` מוסיף ב-runtime ל-Polyroll דרך `putIfAbsent`, נקרא מ-`main.dart`) = מנוע-ההתאמה: `{sku, material, ends[ConnectorEnd{type,size}], pressureRating, maxTempC(ברירת 40), systemOverride}`. enums **EndType**(hdpeCompression/pexPress/copperPress/bspMale/bspFemale/drainOpening) · **WaterSystem**(supply/drainage). `main.dart` מסנתז specs ל-~772 מוצרי-Polyroll באתחול.
- תומכים: `catalog_tree.dart` (1,232ש׳ · **153 `CatalogNode`: 45 פנימיים / ~108 עלים** — 101 עם `lipskeyCategory` + 69 עם `smartKey`; אומת-עצמי — **לא** "~210") · `variant_families.dart` (זיהוי משפחות size/color/model/subtype) · `fuzzy_search.dart` · `brands.dart` (**8 מותגים** ✓) · `lipskey_hotwater`/`lipskey_smart_data`/`polyroll_specs`/`huliot_smartlock_catalog`.
- כל הנתונים **קבועי-Dart** (אין DB/REST). תמונות → CDN (Cloudflare R2) דרך `product_images.dart`.

🔧 **מול אב-הטיפוס:** מוצרי-הקטלוג (`kCatalogProducts`) הם מותגים **אמיתיים** (Lipskey/Polyroll/Huliot) · מוצרי-אינסטלציה אמיתיים (935+772+170) במקום 202 ה-TREES; ה-`pl_`/שלבי-הפרויקט של הפרוטוטייפ **אינם רלוונטיים** (קטלוג-מותג אחר לגמרי). מנוע-ה-`VerifiedSpec` (חיבוריות פיזיקלית) — **חדש לגמרי, אין לו מקבילה בפרוטוטייפ/Preact.** (הערה: ב-`kSmartProducts` חלק מהכרטיסים עדיין נושאים מותג-גנרי "סטנדרט/כלכלי/פרימיום" — ר' למעלה.)
✅ **תיקון-drift (קריטי, אומת שורה-שורה):** ספירה קודמת של "**1,337 (Lipskey 255)**" הייתה **שגויה** — הוכנסה בסשן קודם בטעות. הקוד האמיתי = **1,877 (Lipskey 935 + Polyroll 772 + Huliot 170)**, התואם כמעט-במדויק את "~1,879" של מסמכי-ה-KB המקוריים. **הקוד קובע (R6); הספירה החדשה אומתה מול גבולות-הרשימות.**
