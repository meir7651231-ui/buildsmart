...
> ⭐ **מקור-הלגאסי של ה-BS-FAB (INSP-0021):** role-drawer `מי אתה?` (`index.html:4083–4115`, נפתח מה-welcome-hamburger 3-פסים). header `מי אתה?` + `בחר תפקיד כדי להיכנס` + footer `הדגמה — כל התצוגות חולקות מאגר נתונים אחד`. 5 כפתורי-`<b>` verbatim: קבלן(4090) · **מנהל המערכת**(4095) · **חנות ספק**(4100) · שליח(4105) · עובד(4110). ⭐ **ה-`<small>` subtitles verbatim (`ROLE_DRAWER_SYSTEM`):** 👷 קבלן `הזמנת חומרים, מלאי, משימות` · 👔 מנהל `ניהול מוצרים, חנויות, לקוחות` · 🏪 חנות `הזמנות נכנסות, מלאי החנות` · 🛵 שליח `משלוחים ועדכוני סטטוס` · 🦺 עובד `המשימות שהוקצו לי בשטח` — **deferred ב-Preact** (circle+label בלבד, אין pill שלישי).
> **`enterRole(role)`:** סוגר-מגירה · `appStore.set({role})` (RBAC) · **`auditLog('מעבר תפקיד', role)`** · ניווט ל-`screen-{persona}`. **flows:** קבלן→`screen-login` (tagline `מהשרטוט עד האתר — בלי לשכוח כלום`; profession/prep אופציונלי)→app · מנהל→`screen-manager`→`admTab('m-products')` · חנות→`screen-store-login` (בחירת-חנות; note `🔒 ...זוהי כניסת הדגמה`)→`screen-store` · שליח/עובד→ישר ל-dashboard.
> **תוויות-section verbatim של הפרסונות (BS-dial L2, INSP-0022–0028):** 👔 מנהל `📊לוח בקרה·🚚הזמנות·👥לקוחות·🛠️ניהול` (@4213–4216, `data-at`=m-products/m-orders/m-customers/m-manage) · 🏪 חנות `🏠בית·📥הזמנות·📦מלאי·🧰פורטל` (@4260–4263) · 🛵 שליח `הרכב שלי היום`(18005, emoji 🛵=`HAUL_TYPES[0].ic`@11951=אייקון-הפרסונה)·`📦משלוחים ממתינים לאיסוף`(18019)·`🚚משלוחים פעילים`(7762)·`🧰פורטל השליח`(18039/18043) · 🦺 עובד `🔨המשימה הנוכחית שלך`(8099)·`⏳הבאות בתור`(8101)·`📋שהגשת`(8102 — emoji+label באותה מחרוזת-לגאסי, strictest-verbatim). 👷 **קבלן deferred** — span ה-bottom-tab בלגאסי **חסר emoji-prefix** (אייקוני-SVG נפרדים), לכן אין section verbatim (INSP-0028).

⬆️ **שודרג:** דשבורדי-פרסונה (מסכים-מלאים) → **BS-dial drill-trees**: בחירת-פרסונה ב-L1 → עץ-section ב-dial. שמות: שלמה-הקבלן · מנהל-המערכת · חנות-הסניטריה · שליח·משאית-14 · יוסי-העובד.
...
➖ **הוחסר/placeholder:** `views/` manager(16)/courier(12)/worker(12)/home(11) = מינימליים. הדשבורדים המלאים מהפרוטוטייפ (`md-*`/`mm-*`/`mo-*`/`mc-*` מנהל · ליקוט-חנות מלא · שליח-מעקב · עובד-משימות) — **לא הומרו** (placeholder/חלקי).
...
- ⭐ **טאב "חנות" (`store_screen.dart`, **3,131ש׳** — אומת; טאב-3 ב-`home_shell` IndexedStack) = טאב-ראשי native — אבל ממוקד-קבלן** (לא דשבורד-ספק!): 8 פריטים (סל · הזמנות · השכרת-כלים · פקדונות · החזרה · מכרז-ספקים · גיליונות-בטיחות · השוואת-מחירים) · 4 quick-actions (מועדפים/מועדים/תזמון/שיחה) · sections (הכל/סל/הזמנות/שירותים) · checkout (VAT 18%, אמצעי-תשלום card/bit/אשראי-ספק) · 5 הזמנות-דמו.
