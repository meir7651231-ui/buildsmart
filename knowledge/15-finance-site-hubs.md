...
➖ **מרכז-פיננסים (B) + ניהול-אתר (C) — לא הומרו ל-Preact** במלואם (אין `fin-*`/`sc-*`/`site*` components). קבוצת-ההגדרות "משלוח ותשלום" קיימת כ-rows (דוח 06), אך מרכז-הפיננסים והגאנט/ליקויים/נוכחות/יומן — placeholder/נעדר.
