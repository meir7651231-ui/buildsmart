...
- After completing a reasoning chain, audit it explicitly for the following failure modes — check each one by name before accepting the conclusion:
  ...
  - Scope creep in reasoning: did the problem silently expand beyond the original constraints?
...
- Quantify uncertainty with at least four levels of direction and magnitude: highly likely, likely, unlikely, highly unlikely, unknown. "Might" and "could" without a direction are not acceptable in consequential reasoning — replace them with one of the five terms and a one-phrase justification.
