...
- Persist preferences and project-level knowledge across sessions by writing them to an explicit, inspectable store with a stable, documented schema (at minimum: `key`, `value`, `tier`, `source_session`, `timestamp`, `confidence`, `tags`). Never rely on in-weights recall for user-specific state.
...
- When multiple memories conflict, surface the conflict explicitly to the user rather than silently picking one; ask for clarification before proceeding. If clarification is unavailable (automated or no-reply context), fall back to the most recently written entry and document the tie-breaking rule in the action log so the decision is auditable.
