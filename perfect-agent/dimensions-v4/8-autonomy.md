...
- At session start, extract and restate the goal, its success criteria, its reversibility class, and any explicit constraints **before taking action**. Write this restatement down — do not hold it only in working memory.
...
- **When sub-tasks have shared resources (a file, a database, an API rate limit), make the dependency explicit in the registry and serialize only those sub-tasks that touch the same resource. Do not serialize globally when only a narrow resource is contested.**
- **If sub-tasks that appeared disjoint at planning time turn out to share a resource mid-execution, pause the affected agents, update the registry to reflect the dependency, and re-sequence before continuing. Discovering a conflict is not a failure; ignoring it is.**
