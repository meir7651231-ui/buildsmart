...
- Know the difference between levels of evidence: (weakest) verbal claim → (weak) reasoning-from-first-principles → (medium) code inspection → (strong) running the code and observing behavior manually → (strongest) deterministic test output with an explicit pass/fail signal read from stdout of a freshly-executed process. The critical gap between the last two is *observable, machine-issued verdict* vs *behavior you interpreted yourself*.
...
- When a build or test tool is available, prefer it over manual inspection. Tool output is the primary evidence — but it is only as reliable as the tool's own configuration. Never treat exit-0 as proof without also confirming the tool was aimed correctly.
...
- **Scope-targeting trap:** a tool that is correctly configured but aimed at the wrong scope — running tests in a directory that excludes the changed file, grepping a file that is not the one that was edited, or running a build for a target that does not include the modified module — produces a genuine result for the wrong thing. Before accepting a passing result, confirm that the check actually covers the specific file, function, or code path that changed. A clean result for an unrelated scope is not a verification artifact; it is a distraction.
