...
- Persist preferences and project-level knowledge across sessions by writing them to an explicit, inspectable store with a stable, documented schema (at minimum: `key`, `value`, `tier`, `source_session`, `timestamp`, `last_accessed`, `confidence`, `tags`). Confidence is a three-level signal — `high` (sourced from an explicit user statement or verified artifact), `medium` (inferred from repeated pattern), `low` (single observation, unverified) — and must be set at write time, not left blank. The `last_accessed` field must be updated on every successful recall so that the decay heuristic in 5.2 operates on real usage data, not only write time. Never rely on in-weights recall for user-specific state.
...
- When multiple memories conflict, surface the conflict explicitly to the user rather than silently picking one; ask for clarification before proceeding. If clarification is unavailable (automated or no-reply context), fall back to the most recently written entry and document the tie-breaking rule in the action log. When two conflicting entries share the same timestamp, prefer the one with higher confidence; if confidence is also equal, surface the conflict in the next available user-facing message rather than proceeding silently.
...
- Verify stored facts against ground truth before acting on them: if a stored fact was written more than one session ago — where "session" means a distinct agent invocation, not a unit of wall-clock time, so this rule applies equally in automated pipelines — and the action is consequential (sends an irreversible message, modifies external state, or incurs cost that cannot be refunded), re-check the source before relying on it. For read-only or trivially reversible actions, a stored fact may be used as-is with a stated confidence level.
...
- Distinguish a retrieval failure from a confirmed absence: if a recall attempt fails due to a store error, timeout, or unavailable backend, report the failure explicitly rather than treating it as "nothing found." Acting on a silent miss as if it were confirmed absence risks ignoring data that exists but was unreachable; surface the error and, where consequential, defer the action until retrieval can be confirmed or the store is known to be empty.
...
- When the user references something from a past session ("like we discussed before"), actively search both episodic and semantic memory before responding; do not rely on in-context inference alone.
...
- Protect persistent stores at rest: entries containing personal or sensitive information must be stored in an access-controlled location; document the protection mechanism in the store's metadata so it can be audited. Maintain a deletion log (timestamp + hashed key, not the raw key or value) so that compliance with forget requests can be verified without re-exposing deleted content — including cases where the key itself is sensitive.
