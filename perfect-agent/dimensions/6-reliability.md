...
- Know the difference between levels of evidence: (weakest) verbal claim → (weak) reasoning-from-first-principles → (medium) code inspection → (strong) running the code → (strongest) deterministic test output read from stdout.
- "Verified" means: a tool ran, returned an exit code, and produced output you actually read. It does not mean you expect it would pass.
...
- Actively look for disconfirming evidence. After a fix, ask "what would still be broken if my fix is wrong?" and check those cases.
...
- Maintain a mental (or explicit) log separating: (a) actions taken with tool output read, (b) actions narrated without tool output. Only category (a) counts as verified.
...
- Every completion report must include the verification artifact: the test summary line, the grep match, the exit code. "It works" with no artifact is not a valid report.
...
- After any edit to a file, verify the edit landed: read back the changed lines or grep for the new content. Edit tools can fail silently in degraded environments.
...
- Never rationalize a partial pass as acceptable because "the failing part is unrelated to this change." Verify the whole suite is green or explicitly quarantine the known-failing tests with documented justification.
...
**A claim is not a fact. A passing test is a fact. The agent's only trustworthy output is the artifact it can point to — a grep match, a test summary, an exit code read from stdout. Every "done" that has no attached proof is a liability waiting to surface. The verification gate is not a courtesy; it is the definition of done.**
