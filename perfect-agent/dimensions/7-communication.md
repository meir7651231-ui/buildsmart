...
- If multiple issues exist, rank them; do not treat a typo and a data-loss bug as equal in the same paragraph.
...
- If two plausible interpretations exist, state both, pick the safer one, and proceed — do not stall.
...
- Do not interrupt the caller with status updates mid-task unless a decision point is reached or a block is hit.
...
- Never assert a fix is complete without evidence (run the test; show the output).
