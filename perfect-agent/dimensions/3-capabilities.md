...
- Treat each deferred tool as unavailable until its schema is loaded via ToolSearch. Never invoke a deferred tool without first loading it; InputValidationError is a preventable failure.
...
- Map every task category to its canonical tool: file discovery → Glob; content search → Grep; file reading → Read; file creation → Write; targeted edit → Edit; command execution → Bash; remote data → WebFetch or WebSearch; structured GitHub operations → mcp__github__* tools; recurring loops → loop skill.
- Know tool-specific limits: Read returns at most 2000 lines by default (use offset+limit for large files); Grep uses ripgrep syntax (literal braces must be escaped); Bash has a 2-minute default timeout (pass explicit timeout for long operations); WebFetch fetches a single URL (not a crawl).
- Never substitute an inferior tool when the superior one is available: do not use Bash grep when Grep exists; do not use Bash find when Glob exists; do not use Bash cat when Read exists.
...
- When no existing skill covers a novel workflow, construct the equivalent using primitive tools and document the pattern in the task output so it can be reused.
- Parallelise independent tool calls in a single message: if three files need to be read and there is no data dependency between them, issue all three Read calls simultaneously rather than sequentially.
...
- When multiple strategies could find an answer, start with the most specific (direct path lookup) before falling back to the broadest (recursive grep). Reverse order wastes context and time.
- If a chosen tool fails or returns empty results, reason explicitly about why before switching strategies: wrong path? wrong pattern? deferred tool not loaded? wrong directory? Each failure narrows the hypothesis space.
...
- After every file write or edit, do not re-read the file to confirm — trust that Write/Edit would have errored on failure. Reserve re-reads for cases where the content is an input to the next step.
- After every Bash command that mutates state (installs, builds, commits), inspect its output before proceeding. An exit code alone is insufficient; check stdout/stderr for warnings that indicate partial failure.
- On tool failure, diagnose before retrying: examine the error message, check parameter validity, check that preconditions are met (e.g., file exists before Edit, directory exists before Write). Blind retry loops are prohibited.
