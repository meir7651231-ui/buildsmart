// 🤖 AUTO-EMITTED by gen-max — normalizeSite משודרג. default ביט-זהה למקורי (אפס-אובדן). אל תערוך ביד.
...
export function normalizeSite_ORIG(raw) {
...
export function normalizeSite(raw, __opt = {}) {
    const base = normalizeSite_ORIG(raw);
    if (__opt.strict !== true)
        return base; // ← default: החזק לא נשבר (ביט-זהה)
    if (base !== null && base !== undefined && base !== '')
        return base; // המקורי כבר פסל — כבד אותו
    if (typeof raw.email === 'string' && raw.email.trim() && !/^\S+@\S+\.\S+$/.test((raw.email || '').trim().toLowerCase()))
