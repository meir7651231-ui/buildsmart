// 🤖 AUTO-EMITTED by gen-max — portalHasChannels משודרג. default ביט-זהה למקורי (אפס-אובדן). אל תערוך ביד.
...
export function portalHasChannels_ORIG(config) {
    const c = config.site?.contact ?? {};
    return !!(c.whatsapp || (c.phones && c.phones.length) || c.email);
}
export function portalHasChannels(config, __opt = {}) {
    const base = portalHasChannels_ORIG(config);
    if (__opt.strict !== true)
        return base; // ← default: החזק לא נשבר (ביט-זהה)
    if (base !== null && base !== undefined && base !== '')
        return base; // המקורי כבר פסל — כבד אותו
    if (typeof config.email === 'string' && config.email.trim() && !/^\S+@\S+\.\S+$/.test((config.email || '').trim().toLowerCase()))
