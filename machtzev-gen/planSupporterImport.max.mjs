// 🤖 AUTO-EMITTED by gen-max — planSupporterImport (transform) מועשר. default ביט-זהה. TARGET_fromSource=פייפליין.
function normName(s) {
...
export function planSupporterImport_ORIG(rows, existing) {
...
function parseSupporterGrid(rows) {
    if (!rows.length)
        return [];
...
export function planSupporterImport(rows, existing) { return planSupporterImport_ORIG(rows, existing); }
export function planSupporterImport_fromSource(rows, existing) { return planSupporterImport_ORIG(parseSupporterGrid(rows), existing); }
